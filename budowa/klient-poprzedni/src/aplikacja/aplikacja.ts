...
/**
 * Złożona aplikacja gotowa do uruchomienia: droga do rdzenia, router tras oraz
 * obietnica gotowości Centrum dowodzenia, którą punkt wejścia podaje scenie
 * wejścia.
 */
export interface Aplikacja {
  ...
  router: Router;
  /** Gotowość Centrum dowodzenia, czyli domknięcie pierwszego odczytu strony. */
  stronaGlownaGotowa(): Promise<void>;
}

/**
 * Składa aplikację: powołuje warstwy wspólne i wiąże trzy trasy w jeden
 * przepływ. Żadnego elementu widoku tu nie ma, ponieważ każdą trasę buduje jej
 * własny moduł, a złożenie odpowiada wyłącznie za kolejność powołań.
 */
export function zlozAplikacje(): Aplikacja {
  ...
  // Drogę do rdzenia podaje powłoce złożenie aplikacji.
  ...
  /** Wchodzi do środowiska; router pokazuje trasę pierwszy, bo wtedy powstaje widok. */
