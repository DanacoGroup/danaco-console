...
// bladNieznanegoBytuApp odróżnia „bytu nie ma" od „odczyt się nie powiódł".
// Okno pokazuje wtedy inny komunikat i inaczej podpowiada Operatorowi — tak
// samo jak `bladNieznanejArchitektury` przy architekturze.
...
// wskaznikNapisuApp oddaje wskaźnik na napis; pusty napis daje brak wartości,
// bo pole opcjonalne kontraktu niosące pusty łańcuch mówiłoby „jest, ale nic".
...
// sumaTresciApp liczy sumę kontrolną bajtów wytworu — nazwą pliku w magazynie
// jest właśnie ta suma, a `AppArtifact.checksumSha256` niesie ją do kontraktu.
func sumaTresciApp(bajty []byte) string {
	suma := sha256.Sum256(bajty)
	return hex.EncodeToString(suma[:])
}

// isBrakWierszaApp nazywa „wiersza nie ma" jednym sprawdzeniem. Powtarzane
// `errors.Is(err, dane.ErrBrakWiersza)` w siedmiu miejscach obszaru różniłoby
// się tylko literówką, którą kompilator by przepuścił.
...
// korzenWytworowApp jest korzeniem magazynu liczonym od katalogu danych — ta
// sama para podkatalogów, którą składa `magazynWytworowApp`, więc postać
// odwołania i miejsce zapisu nie mają jak się rozjechać.
const korzenWytworowApp = podkatalogAplikacji + "/" + podkatalogWytworow

// magazynWytworowApp składa magazyn bajtów wytworów modułu nad katalogiem
// danych rdzenia. Katalog nie musi istnieć — powstaje przy pierwszym zapisie.
...
// odwolanieWytworuApp oddaje odwołanie w postaci, którą wolno wypuścić z rdzenia:
// ścieżkę względną magazynu, nie ścieżkę na dysku Operatora. Powód i cena stoją
// w nagłówku `odwolanieMagazynu` — to jedna decyzja dla całego produktu.
func odwolanieWytworuApp(sciezka string) string {
	return odwolanieMagazynu(sciezka, korzenWytworowApp)
}

// bladBrakuMagazynuApp nazywa brak katalogu danych. Moduł ma wtedy odmówić
// z powodem, a nie zameldować wytwór, za którym nie ma ani jednego bajtu —
...
// zapisem, bo zapis jest UPSERT-em i po fakcie nie widać, czy wiersz powstał.
