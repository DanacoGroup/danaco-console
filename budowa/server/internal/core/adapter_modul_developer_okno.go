// Odpowiedzialność pliku: obszar modułu Developer — okno komunikacji, jego
// katalogi robocze, sprawdzenie, czy wskazana ścieżka mieści się w tym obszarze,
// oraz brama trybu uprawnień i znakowanie odmów kodami kontraktu.
//
// Katalogi robocze są listą (session.Ustawienia), więc wszystkie punkty tego
// pliku pracują na liście. Okno bez własnej listy spada na katalog ustalony
// przez rozstrzygacz — brak ustawienia znaczy wartość domyślną, nie odmowę.
//
// Ścieżka spoza obszaru wraca jako odmowa uprawnienia. Code Editor dostaje
// ścieżkę od klienta, więc bez tego sprawdzenia `../../..` czytałby dowolny plik
// maszyny; kod `permission_denied` odróżnia zatrzymanie przez izolację okna od
// awarii odczytu.
package core
...
// oknoDevelopera zwraca okno komunikacji, w którym pracuje moduł.
...
// korzenieOkna zwraca katalogi robocze okna w postaci bezwzględnej. Okno bez
// własnych katalogów dostaje katalog ustalony przez rozstrzygacz; gdy i tego
// nie ma, wynik jest pusty i zatrzyma każdą ścieżkę na sprawdzeniu obszaru.
...
// obszarDevelopera składa obszar izolacji okna z jego pierwszego katalogu
// roboczego — tego, w którym startują procesy okna (session.Ustawienia).
...
// sciezkaWObszarze przekłada wskazanie klienta na ścieżkę bezwzględną i pilnuje,
// żeby leżała wewnątrz któregoś z katalogów roboczych okna.
//
// Wskazanie względne rozstrzyga się po kolei: pierwszy katalog roboczy, w którym
// taka ścieżka istnieje, wygrywa; gdy nie istnieje nigdzie, zostaje pierwszy
// katalog listy. Dzięki temu otwarcie pliku z Project Tree trafia tam, skąd
// wyszło, a zapis nowego pliku ma jednoznaczne miejsce.
...
// wObszarze przepuszcza ścieżkę leżącą w którymś z korzeni i odmawia pozostałym.
...
// wewnatrzKatalogu mówi, czy ścieżka leży w katalogu albo nim jest.
//
// Porównanie idzie po znormalizowanych ścieżkach, a nie przez filepath.Rel:
// Rel na Windows rozróżnia wielkość liter, których system plików nie rozróżnia,
// więc `C:\Praca` wyglądałoby na obszar inny niż `c:\praca` i sprawdzenie dałoby
// się obejść samą zmianą wielkości litery.
...
// znormalizuj sprowadza ścieżkę do postaci porównywalnej na danym systemie.
...
// oczyscKorzen sprowadza katalog roboczy do postaci bezwzględnej i czystej.
// Katalog względny odrzucamy: obszar izolacji liczony względem katalogu procesu
// rdzenia znaczyłby co innego po każdym uruchomieniu serwera.
...
// sprawdzZmianeSystemu jest bramą trybu uprawnień okna dla czynności
// zmieniającej stan systemu: zapisu pliku, czynności repozytorium i uruchomienia
// budowania.
//
// Tryb `plan` znaczy pracę planistyczną bez zmian w systemie, więc wyklucza
// wszystkie trzy. Pozostałe tryby je przepuszczają: komendy modułu Developer
// nie mają w kontrakcie pola inicjatora ani rundy zgody, a żądanie przychodzi
// z okna komunikacji.
...
// bladZadaniaDevelopera znakuje wadę żądania kodem kontraktu.
...
// bladZasobuDevelopera znakuje brak pliku, katalogu albo przebiegu.
...
// bladDostepuDevelopera znakuje zatrzymanie przez granicę obszaru okna.
//
// Kod jest inny niż przy wadzie żądania, bo naprawa jest inna: żądanie samo
// w sobie bywa poprawne, a zatrzymuje je granica katalogu roboczego.
...
// bladWykonaniaDevelopera znakuje niepowodzenie czynności na dysku albo procesie.
