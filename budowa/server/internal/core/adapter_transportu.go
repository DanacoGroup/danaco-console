...
// Obsluz wypełnia interfejs transport.Rdzen. Odpowiedź wraca wynikiem, zdarzenia
// idą rozgłoszeniem do urządzeń konta, a strumień tury do gniazda, które ją zamówiło.
...
// Odlaczono zdejmuje więź rozłączonego urządzenia i jego wpisy w torze
// strumieni. Bez tego obie mapy rosłyby o wpis na każde nawiązanie przez całe
// życie procesu.
...
// transport. Sesja nierozpoznana daje konto domyślne, nie napis pusty: pustego
...
// nazwaKontaTransportu przekłada identyfikator konta z bazy na nazwę konta w
// transporcie — innej nazwy konta transport nie zna. Zero, czyli konto
// nierozpoznane, daje konto domyślne.
...
// kontoAdresata nazywa konto, do którego transport adresuje zdarzenie zamówione
// w tym kontekście: konto rozpoznane z sesji bramki; konto domyślne dla gniazda
// bez rozpoznanej sesji; konto puste — wszystkie połączenia — dla czynności
// własnej rdzenia, która nie przyszła z żadnego gniazda.
...
// zKontemZadania przenosi konto wołającego z kontekstu żądania do kontekstu
// pracy puszczonej w tle. Praca przeżywa żądanie, a jej zdarzenia i zapisy mają
// należeć do konta, które ją zamówiło.
...
// torStrumieni pamięta, które gniazdo zamówiło turę: koperta fragmentu powtarza
// identyfikator żądania, więc identyfikator wystarcza za adres. Wpis żyje od
// zamówienia do fragmentu ostatniego; gniazdo zerwane w trakcie zostawia wpis
// z samym kontem, żeby reszta strumienia poszła do urządzeń tego konta.
...
// dostarcz kieruje fragment do gniazda, które zamówiło turę; fałsz znaczy
// strumień bez wpisu, który idzie zwykłym rozgłoszeniem. Wysyłka nieudana
// zdejmuje ujście z wpisu, a fragment idzie do urządzeń konta zamawiającego.
...
// rozlaczanieSesji zrywa gniazda konta, których sesja bramki przestała nadawać.
// Więź i rozpoznanie ważności wchodzą po złożeniu rdzenia, bo nasłuch powstaje
// przed nim.
...
// RozlaczPoUniewaznieniu wypełnia port RozlaczanieSesji. O sesji rozstrzyga
// wiersz w bazie, nie więź: unieważnienie zaszło gdzie indziej i więź o nim
// nie wie. Ważność nierozstrzygnięta zostawia gniazdo bramce.
...
// zRozlaczaniem przyjmuje zrywanie gniazd dołożone po montażu, bo port składa
// się piętro niżej niż transport — wzorem zWiezia.
type zRozlaczaniem interface {
	przyjmijRozlaczanie(r RozlaczanieSesji)
}

// nasluchTransportu podaje serwer transportu jako nasłuch rdzenia, nadajnik
// zdarzeń rozgłaszanych do urządzeń konta i zrywanie gniazd po unieważnieniu
// sesji. Jest wartością, więc tor i zrywanie stoją za wskaźnikami wspólnymi
// dla wszystkich kopii.
...
// nowyNasluchTransportu składa nasłuch nad serwerem transportu z pustym torem
// strumieni i zrywaniem czekającym na więź rdzenia.
...
// Rozglos wypełnia port Nadajnik rdzenia. Fragment strumienia z wpisem w torze
// idzie do gniazda zamawiającego; pozostałe komunikaty do połączeń konta,
// a przy koncie pustym do wszystkich połączeń rdzenia.
