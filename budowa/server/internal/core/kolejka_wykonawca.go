// Plik jest mostem między pozycją kolejki a realnym wykonaniem: wykonawca uruchamia pracę pozycji, która weszła w stan wykonywana, i zwraca jej wynik, sięgając po ten sam rejestr kanałów co tura okna.
package core
...
// wykonawcaKroku uruchamia realną pracę pozycji kolejki. Zwrócony błąd znaczy niepowodzenie wykonania, brak błędu znaczy pracę zakończoną, którą silnik przesuwa do weryfikacji.
type wykonawcaKroku interface {
	Wykonaj(ctx context.Context, pozycja dane.Pozycja) (string, error)
}

// Odmowy wykonania nazwane raz. Obie znaczą „nie ma czym wykonać kroku", więc silnik zamyka pozycję stanem `bledna`, a nie udanym `do_weryfikacji`.
var (
...
// kanalPozycji rozwiązuje kanał modelu i zasięgi (sesja, okno) dla pozycji, zwracając fałsz, gdy pozycji nie da się przypisać kanału, bo mapowanie okna wykonawcy na kanał żyje w pakiecie sesji.
type kanalPozycji func(ctx context.Context, pozycja dane.Pozycja) (kanal string, zasiegi models.Zasiegi, ok bool)

// wykonawcaModelu wykonuje pozycję turą kanału modelu. Treść zlecenia pozycji staje się treścią zapytania; kanał podaje rozwiązywacz; strumień odpowiedzi idzie wspólnym nadajnikiem zdarzeń.
...
// nowyWykonawcaModelu składa wykonawcę z rejestru kanałów, nadajnika strumienia i rozwiązywacza kanału pozycji.
...
// Wykonaj uruchamia turę kanału dla pozycji i zwraca zebraną treść odpowiedzi oraz wynik. Treść wraca także przy błędzie, bo to, co model zdążył oddać, jest częścią prawdy o nieudanej turze.
func (w wykonawcaModelu) Wykonaj(ctx context.Context, pozycja dane.Pozycja) (string, error) {
	...
	strumien := nowyNadawcaStrumienia(w.nadajnik, kontoAdresata(ctx), idPozycji, zasiegi.Sesja)
