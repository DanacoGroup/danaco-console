...
// rejestrBiegow trzyma ostatni znany stan biegu naprawczego każdego koordynatora i rozgłasza jego zmiany dalej, jako pamiętający pośrednik między pętlą, która stan zna jednorazowo, a stroną pytającą o stan z zewnątrz.
...
// nowyRejestrBiegow zakłada pusty rejestr biegów, gotowy do przyjmowania zdarzeń obiegu i obserwatorów.
func nowyRejestrBiegow() *rejestrBiegow {
	return &rejestrBiegow{stany: map[string]shared.LoopState{}}
}

// Obieg wypełnia interfejs session.ObserwatorObiegu: zapisuje odpis licznika i, gdy różni się od poprzedniego, powiadamia obserwatorów. Ślad obiegu odmówionego i zakończonego usterką dochodzi tą samą drogą.
...
// Obserwuj dokłada odbiorcę zmian stanu biegu, wywoływanego przy każdej zauważonej zmianie stanu koordynatora.
...
// Zachowaj zostawia w rejestrze wyłącznie biegi okien wciąż otwartych i zwraca liczbę śladów wykreślonych. Sprzątanie idzie od strony okien żywych, bo pętla o zamknięciu okna nie mówi. Wykaz pusty niczego nie kasuje.
