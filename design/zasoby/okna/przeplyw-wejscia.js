/* ============================================================================
   DANACO CONSOLE — PRZEPŁYW WEJŚCIA
   ----------------------------------------------------------------------------
   Złożenie jednego prototypu. Wygląd okien stoi w `wejscie.css` (§1–§9),
   rusztowanie sceny w bloku `.pt-*` samego pliku. Tutaj mieści się wyłącznie
   mechanizm:

   1 · przejścia między etapami i stanami — bez przeładowania dokumentu,
   2 · dołączenie ramy aplikacji po wydaniu tokenu (etap 3),
   3 · symulacja cyklu uzgodnienia protokołu (etap 1),
   4 · formularz uwierzytelnienia (etap 2).
   ============================================================================ */
(function () {
  'use strict';
  var D = document;
        ...
        wczytajSkrypt('../../zasoby/powloka.js', function () {
          wczytajSkrypt('../../zasoby/okna-modalne.js', function () {
            wczytajSkrypt('../../zasoby/rama.js', function () {
              if (window.dnToast) {
                window.dnToast('Dostęp do konta potwierdzony',
                  'Okno pracy jest od tej chwili dostępne.', 'sukces');
              }
            });
          });
        });
  ...
  function odtworz(nazwaDocelowa) {
    ...
    odtwarzanie.push(setTimeout(function () {
      ...
      if (nazwaDocelowa === 'w-blad') {
        window.dnToast('Błąd połączenia', 'Serwer Danaco Console nie odpowiedział. Ponowienie nastąpi za 15 sekund.', 'ostrzezenie');
      } else if (nazwaDocelowa === 'w-token') {
        window.dnToast('Urządzenie rozpoznane', 'To urządzenie jest zapisane jako zaufane, więc logowanie zostaje pominięte.', 'sukces');
      } else {
        window.dnToast('Połączenie nawiązane', 'Kolejny krok: rejestracja i logowanie.', 'informacja');
      }
    }, czasCalosci + 200));
  }
  ...
  if (btnPonow) {
    btnPonow.addEventListener('click', function () {
      if (window.dnToast) {
        window.dnToast('Ponawianie połączenia', 'Łączenie zaczyna się od nowa.', 'informacja');
      }
  ...
  wszystkie('[data-odsloniecie]').forEach(function (b) {
    b.addEventListener('click', function () {
      var pole = D.getElementById(b.getAttribute('data-odsloniecie'));
      if (!pole) return;
  ...
  /* Ocena siły hasła — cztery warunki, cztery stopnie. */
  var haslo = D.getElementById('rej-haslo');
  if (haslo) {
    var blok = haslo.closest('.dn-pole').parentNode.querySelector('.au-sila');
    var opis = blok.querySelector('[data-sila-opis]');
    var OPISY = ['Siła hasła oceniana podczas wpisywania', 'Hasło słabe — jeden warunek spełniony',
                 'Hasło dostateczne — dwa warunki spełnione', 'Hasło dobre — trzy warunki spełnione',
                 'Hasło mocne — cztery warunki spełnione'];
    haslo.addEventListener('input', function () {
      ...
      opis.textContent = OPISY[stopien];
    });
  }

  /* Kod potwierdzający — przejście między polami i wklejenie całości. */
  var pola = wszystkie('[data-kod]');
  pola.forEach(function (p, i) {
    p.addEventListener('input', function () {
      p.value = p.value.replace(/[^0-9A-Za-z]/g, '').slice(0, 1).toUpperCase();
      if (p.value && pola[i + 1]) pola[i + 1].focus();
    });
    p.addEventListener('keydown', function (e) {
      if (e.key === 'Backspace' && !p.value && pola[i - 1]) pola[i - 1].focus();
    });
    p.addEventListener('paste', function (e) {
      e.preventDefault();
      var t = (e.clipboardData || window.clipboardData).getData('text').replace(/[^0-9A-Za-z]/g, '');
      pola.forEach(function (q, j) { q.value = (t[j] || '').toUpperCase(); });
      (pola[Math.min(t.length, pola.length - 1)] || p).focus();
    });
  });

  var btnWklej = D.querySelector('[data-wklej-kod]');
  if (btnWklej) {
    btnWklej.addEventListener('click', function () {
      if (!navigator.clipboard || !navigator.clipboard.readText) {
        if (window.dnToast) window.dnToast('Schowek niedostępny', 'Kod należy wpisać ręcznie w sześciu polach.', 'informacja');
        return;
      }
      navigator.clipboard.readText().then(function (t) {
        var czysty = String(t || '').replace(/[^0-9A-Za-z]/g, '');
        pola.forEach(function (q, j) { q.value = (czysty[j] || '').toUpperCase(); });
        if (pola.length) pola[Math.min(czysty.length, pola.length - 1)].focus();
      }).catch(function () {
        if (window.dnToast) window.dnToast('Schowek niedostępny', 'Kod należy wpisać ręcznie w sześciu polach.', 'informacja');
      });
    });
  ...
  /* ── 5 · POSTĘP PRZYGOTOWANIA ŚRODOWISKA (ETAP 3) ──────────────────────
     Szerokość paska bierze się z wartości ogłoszonej czytnikowi ekranu —
     jedna liczba prowadzi obraz i komunikat. */
  var postepPrzygotowania = D.querySelector('[data-postep-przygotowania]');
  if (postepPrzygotowania) {
    var torPrzygotowania = postepPrzygotowania.closest('[role="progressbar"]');
    postepPrzygotowania.style.width =
      (torPrzygotowania ? torPrzygotowania.getAttribute('aria-valuenow') : '0') + '%';
  }
