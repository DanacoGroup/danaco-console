...
(function () {
  ...
  var IKONY_TOASTU = {
    sukces: '<svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><circle cx="12" cy="12" r="10"/><path d="m9 12 2 2 4-4"/></svg>',
    ostrzezenie: '<svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="m21.73 18-8-14a2 2 0 0 0-3.48 0l-8 14A2 2 0 0 0 4 21h16a2 2 0 0 0 1.73-3"/><path d="M12 9v4"/><path d="M12 17h.01"/></svg>',
    blad: '<svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><circle cx="12" cy="12" r="10"/><path d="m15 9-6 6"/><path d="m9 9 6 6"/></svg>',
    informacja: '<svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><circle cx="12" cy="12" r="10"/><path d="M12 16v-4"/><path d="M12 8h.01"/></svg>'
  };
  ...
  D.addEventListener('click', function (e) {
    ...
    /* Kliknięcie musi zawsze dać odpowiedź. Bez tego cisza po kliknięciu znaczy
       raz „nie ma czego pokazać", a raz „coś nie działa" — a Operator nie ma jak
       tych dwóch przypadków rozróżnić. */
    var nazwa = przycisk.getAttribute('data-etykietka') || przycisk.getAttribute('aria-label') || 'Lista';
    if (!m2) {
      toast(nazwa, 'Lista jest niedostępna w tym prototypie.', 'ostrzezenie');
      return;
    }
    if (!m2.querySelectorAll('.sta-menu-poz, [role="menuitem"]').length) {
      toast(nazwa, m2.getAttribute('data-pusty-komunikat') || 'Brak pozycji do pokazania.', 'informacja');
      return;
    }
  ...
  function zbudujPasekPrototypu() {
    ...
    uchwyt.innerHTML =
      '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><circle cx="12" cy="12" r="10"/><path d="M12 16v-4"/><path d="M12 8h.01"/></svg>' +
      '<span>Prototyp</span>';
