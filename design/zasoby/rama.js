...
(function () {
  ...
  /* ── 2 · ZWIJANIE SZYNY SESJI ─────────────────────────────────────────── */

  /* Nazewnictwo: „szyna nawigacji" to pionowy pas środowisk i modułów przy
     krawędzi okna; „szyna sesji" to kolumna sesji wewnątrz przestrzeni
     roboczej. Kontrolka pasa edycji zwija tę drugą. */
  function szynaSesji() {
    return D.querySelector('.pd-szyna, .szyna, .sta-cialo > .dn-boczna, .sta-cialo > .boczna');
  }
  D.addEventListener('click', function (e) {
    var b = e.target.closest('[data-zwin-szyne]');
    if (!b) return;
    var s = szynaSesji();
    if (!s) { toast('Brak szyny sesji', 'Ten widok nie ma kolumny sesji do zwinięcia.', 'informacja'); return; }
    var zwinieta = b.getAttribute('aria-pressed') === 'true';
    s.hidden = !zwinieta;
    b.setAttribute('aria-pressed', zwinieta ? 'false' : 'true');
    b.setAttribute('data-etykietka', zwinieta ? 'Zwiń szynę sesji' : 'Rozwiń szynę sesji');
    b.setAttribute('aria-label', zwinieta ? 'Zwiń szynę sesji' : 'Rozwiń szynę sesji');
  });
