...
- [Wprowadzenie i cel dokumentu](#wprowadzenie-i-cel-dokumentu)
1. [Zasada nadrzędna: konfigurowalność zamiast blokad](#1-zasada-nadrzędna-konfigurowalność-zamiast-blokad)
2. [Cztery rodzaje izolacji](#2-cztery-rodzaje-izolacji)
3. [Zakresy izolacji technicznej](#3-zakresy-izolacji-technicznej)
4. [Zakresy izolacji kontekstu](#4-zakresy-izolacji-kontekstu)
5. [Łączenie wielu zakresów izolacji](#5-łączenie-wielu-zakresów-izolacji)
6. [Poziomy zasięgu reguł izolacji i pierwszeństwo](#6-poziomy-zasięgu-reguł-izolacji-i-pierwszeństwo)
7. [Profile izolacji](#7-profile-izolacji)
8. [Przypisanie punktów izolacji do sesji, roli i projektu](#8-przypisanie-punktów-izolacji-do-sesji-roli-i-projektu)
9. [Współdzielenie historii i pamięci między kartami, modułami i środowiskami](#9-współdzielenie-historii-i-pamięci-między-kartami-modułami-i-środowiskami)
10. [Okno konfiguracji punktów izolacji](#10-okno-konfiguracji-punktów-izolacji)
11. [Warstwy konfiguracji: domyślna i sesji](#11-warstwy-konfiguracji-domyślna-i-sesji)
12. [Stan wyjściowy platformy](#12-stan-wyjściowy-platformy)
13. [Scenariusze konfiguracji punktów izolacji](#13-scenariusze-konfiguracji-punktów-izolacji)
14. [Punkty izolacji warstwy komunikacji operacyjnej](#14-punkty-izolacji-warstwy-komunikacji-operacyjnej)
15. [Zależności i punkty izolacji warstw widoczności](#15-zależności-i-punkty-izolacji-warstw-widoczności)
16. [Słowniczek pojęć](#16-słowniczek-pojęć)
- [Załącznik A. Macierz zakresów izolacji technicznej z objaśnieniami kontekstowymi](#załącznik-a-macierz-zakresów-izolacji-technicznej-z-objaśnieniami-kontekstowymi)
- [Załącznik B. Szablon profilu izolacji](#załącznik-b-szablon-profilu-izolacji)
- [Załącznik C. Tabela poziomów zasięgu i pierwszeństwa](#załącznik-c-tabela-poziomów-zasięgu-i-pierwszeństwa)
- [Załącznik D. Zestawienie punktów izolacji warstwy komunikacji operacyjnej](#załącznik-d-zestawienie-punktów-izolacji-warstwy-komunikacji-operacyjnej)
- [Załącznik E. Zestawienie punktów izolacji warstw widoczności](#załącznik-e-zestawienie-punktów-izolacji-warstw-widoczności)
...
| Okno konfiguracji punktów izolacji nigdy nie wymusza wyboru | Każdy przełącznik w macierzy izolacji (rozdz. 10) można pozostawić w stanie nieustawionym; system przyjmuje wówczas regułę dziedziczoną, a nie regułę restrykcyjną. |
...
|---|---|---|---|
| Historia | Zapis wymiany wiadomości między użytkownikiem a AI w danej karcie sesji (Model danych, rozdz. 4.2) | Każda nowa karta sesji zaczyna z pustą historią | Ten sam zapis rozmowy widoczny jest w kilku kartach, modułach lub środowiskach jednocześnie |
| Pamięć | Zasoby pamięci długoterminowej przypisane do poziomu globalnego, projektu, sesji lub środowiska (Model danych, rozdz. 6) | Pamięć przypisana jednemu zasięgowi nie jest widoczna w innym | Ten sam zasób pamięci zasila kilka zasięgów jednocześnie |
...
Poniższa tabela ilustruje typowe kombinacje, powstające przez łączenie kilku zakresów jednocześnie. Kombinacje te nie są zamkniętym katalogiem — każdy zestaw ustawień dopuszczalny przez macierz izolacji (rozdz. 10) jest prawidłową polityką.
...
Platforma udostępnia siedem poziomów zasięgu, na których ustawia się dowolny z dziewiętnastu zakresów izolacji opisanych w rozdziałach 3, 4, 14 i 15. Poziomy uporządkowane są od najogólniejszego do najbardziej szczegółowego; reguła z poziomu bardziej szczegółowego ma pierwszeństwo przed regułą z poziomu szerszego (Koncepcja, rozdz. 6.5).
...
| 7 | Rola (MultitaskingAI) | Najwyższe | Karta sesji | Profil izolacji przypisany roli Executor 1 |
...
Zestaw ustawień izolacji można zapisać jako nazwany **profil izolacji** i wykorzystywać go wielokrotnie bez ponownej konfiguracji (Koncepcja, rozdz. 6.6; Architektura, rozdz. 12: „Zbiór można zapisać jako profil i przypisać do sesji lub roli"). Profil grupuje sześć elementów:
...
Profil jest niezależny od pojedynczego przypisania — ten sam profil (na przykład „Sesja izolowana — host zdalny") można przypisać jednocześnie do kilku sesji, ról lub projektów, a zmiana profilu odbija się na wszystkich jego przypisaniach. Pełny szablon pól profilu zawiera Załącznik B.
...
## 8. Przypisanie punktów izolacji do sesji, roli i projektu

Konfigurację izolacji można ustanowić na dwa uzupełniające się sposoby: **bezpośrednio**, ustawiając przełączniki macierzy izolacji dla wybranego poziomu zasięgu w oknie konfiguracji punktów izolacji (rozdz. 10), albo **przez przypisanie** wcześniej zapisanego profilu izolacji (rozdz. 7) do konkretnego obiektu platformy. Trzy poziomy zasięgu — karta sesji, rola i projekt — mają, poza centralnym oknem konfiguracji, także własny, kontekstowy punkt przypisania osadzony w miejscu, gdzie dany obiekt jest zarządzany.

| Poziom przypisania | Miejsce dokonania przypisania | Skutek | Odwołanie |
|---|---|---|---|
| Karta sesji | Uproszczone menu kontekstowe karty sesji (Architektura, rozdz. 13) albo panel profilu okna konfiguracji punktów izolacji, warstwa „sesji" | Profil obowiązuje wyłącznie bieżącą kartę; nakłada się na warstwę domyślną, nie modyfikując jej (rozdz. 11) | Koncepcja, rozdz. 8; niniejszy dokument, rozdz. 6, 11 |
...
| Projekt (Workspace) | Atrybut projektu „przypisany profil izolacji" w oknie modułu Workspace | Profil obowiązuje wszystkie karty sesji otwarte w kontekście danego projektu, z zachowaniem pierwszeństwa poziomu karty sesji, gdy ta ma własne ustawienie | Koncepcja, rozdz. 11.2; Model danych, rozdz. 5 |

Pozostałe cztery poziomy zasięgu — globalny, środowisko, moduł i para modułów (rozdz. 6) — konfiguruje się wyłącznie centralnie, z poziomu panelu selektora zasięgu w oknie konfiguracji punktów izolacji, ponieważ nie odpowiadają one pojedynczemu, zarządzalnemu obiektowi platformy, lecz całej klasie obiektów (na przykład wszystkim sesjom otwartym w środowisku CodeStudio).
...
Mechanizm techniczny współdzielenia wykorzystuje strukturę pamięci opisaną w Model danych (rozdz. 6): zasób pamięci ma przypisany poziom (globalna, projekt, sesja, środowisko); ustawienie danej pozycji kontekstu na „współdzielone" dla wybranej pary obiektów oznacza, że oba obiekty odczytują i zapisują ten sam zasób pamięci lub tę samą historię wiadomości, zamiast każdy prowadzić własną.

```
MECHANIZM WSPÓŁDZIELENIA (Model danych, rozdz. 6)
...
|---|---|---|
| Selektor zasięgu | lewy | Lista siedmiu poziomów zasięgu opisanych w rozdziale 6, od najogólniejszego do najbardziej szczegółowego. Wybór poziomu ustala, dla jakiego obiektu (całej platformy, danego środowiska, danego modułu, wskazanej pary modułów, wskazanego projektu, wskazanej karty sesji lub wskazanej roli) obowiązują ustawienia panelu środkowego. Wybór poziomu „para modułów", „projekt", „karta sesji" lub „rola" otwiera dodatkowy selektor wskazujący konkretny obiekt danego poziomu (na przykład parę Studio–Translate albo projekt z modułu Workspace). |
...
> „Każdy element konfiguracji zawiera objaśnienie kontekstowe, opisujące jego działanie i wpływ na aplikację. Treść objaśnienia jest częścią definicji ustawienia."
...
### 10.3. Typowy przebieg konfiguracji
...
1. Użytkownik wybiera w panelu lewym poziom zasięgu, na którym chce ustalić regułę (na przykład „Projekt", a następnie konkretny projekt z listy).
...
| Sesji | Uproszczone menu kontekstowe okna operacyjnego (Architektura, rozdz. 13) albo panel profilu okna konfiguracji punktów izolacji przy wybranej warstwie „sesji" (rozdz. 10.2) | Dla sesji bieżącej; kończy się z zamknięciem karty sesji | Nakłada się na warstwę domyślną bez modyfikacji jej wartości |
...
| Pozostałe zakresy | Odrębne dla każdej roli (m.in. katalog roboczy, model procesu) |
...
Pięć pozycji ustawia się na tych samych siedmiu poziomach zasięgu, co pozostałe zakresy izolacji (rozdz. 6), z zachowaniem pierwszeństwa zasięgu najbardziej szczegółowego.
...
| Pojęcie | Definicja |
|---|---|
| Magistrala kontekstu | Mechanizm przekazywania artefaktu między modułami, respektujący reguły izolacji kontekstu obowiązujące parę modułów i zasięg bieżącej karty — zob. `interfejs-uzytkownika/przeplyw-okien.md`, rozdz. 6a |
...
| Poziom zasięgu | Poziom, na którym obowiązuje reguła izolacji: globalny, środowisko, moduł, para modułów, projekt, karta sesji lub rola; reguły z różnych poziomów rozstrzyga zasada pierwszeństwa zasięgu najbardziej szczegółowego (rozdz. 6). |
...
|---|---|---|
| Nazwa profilu | Nazwa własna profilu | dowolny tekst (np. „Executor zdalny — SSH") |
| Opis | Krótkie uzasadnienie przeznaczenia profilu | dowolny tekst |
| Poziom zasięgu | Poziom obowiązywania reguły | globalny \| środowisko \| moduł \| para modułów \| projekt \| karta sesji \| rola (rozdz. 6) |
...
| Rola | Najwyższe | Okno roli w panelu orkiestracji środowiska MultitaskingAI | Karta sesji |
...
*Koniec dokumentu. Danaco Console — Izolacja i konfigurowalność zależności, wersja 2.0.*
