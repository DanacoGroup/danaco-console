...
| | |
|---|---|
| **Tytuł** | Kontrakty komunikacji |
| **Klasa dokumentu** | Specyfikacja docelowa |
| **Odbiorcy** | deweloper (główny) |
| **Przeznaczenie** | Ustala pełne kontrakty komunikacji między klientem a rdzeniem platformy — kopertę protokołu, wykaz 68 obszarów kontraktu, schematy ładunku poleceń i zdarzeń, kody błędów i diagramy sekwencji dla każdego opisywanego przebiegu |
| **Zakres** | koperta protokołu i etapy nawiązania połączenia, strona główna i jej trzy strefy, wybór środowiska, karty sesji, komponenty własne, piętnaście modułów według macierzy dostępności, oba kanały komunikacji operacyjnej, warstwy widoczności interfejsu, środowisko MultitaskingAI, silnik kolejek, orkiestracja zależności, agenci, pamięć, konfiguracja, punkty izolacji, rozszerzenia, kody błędów kontraktu |
| **Poza zakresem** | pełny model danych encji — [Model danych](model-danych.md); pełny wykaz 68 obszarów z licznikiem komend — [Architektura techniczna](architektura.md) rozdz. 18, niniejszy dokument go nie powiela; wygląd i zachowanie okien — [Elementy okien](../interfejs-uzytkownika/elementy-okien.md) |
| **Dokument nadrzędny** | [Architektura techniczna](architektura.md) |
| **Dokumenty powiązane** | [Koncepcja platformy](koncepcja-platformy.md) · [Model danych](model-danych.md) · [Model konfiguracji](model-konfiguracji.md) · [Izolacja i zależności](izolacja-i-zaleznosci.md) · [Bezpieczeństwo i uwierzytelnianie](bezpieczenstwo-i-uwierzytelnianie.md) · [Integracja modeli](integracja-modeli.md) · [Rozszerzenia](rozszerzenia.md) |
| **Źródła normatywne** | `budowa/shared/contract.json` (1119 komend · 68 obszarów · 552 struktury · 377 wyliczeń · 78 zdarzeń · 8 kodów błędów) |
| **Zasada nadrzędna** | Serwer jest jedynym źródłem prawdy; klient nie utrzymuje stanu niezależnego od potwierdzenia serwera i nie zgaduje pól ładunku poza tymi, które podaje kontrakt |
...
2. [Zasady ogólne komunikacji](#1-zasady-ogólne-komunikacji)
3. [Cykl życia połączenia](#2-cykl-życia-połączenia)
4. [Struktura komunikatu](#3-struktura-komunikatu)
5. [Kategorie komunikatów](#4-kategorie-komunikatów)
   - [4.1 Polecenia (klient → serwer)](#41-polecenia-klient--serwer)
   - [4.2 Zdarzenia (serwer → klient)](#42-zdarzenia-serwer--klient)
   - [4.3 Strumienie (serwer → klient)](#43-strumienie-serwer--klient)
6. [Uwierzytelnianie i tożsamość urządzenia](#5-uwierzytelnianie-i-tożsamość-urządzenia)
7. [Strona główna — kontrakty trzech stref](#6-strona-główna--kontrakty-trzech-stref)
   - [6.1 Strefa 1 — wybór środowiska](#61-strefa-1--wybór-środowiska)
   - [6.2 Strefa 2 — komponenty własne](#62-strefa-2--komponenty-własne)
   - [6.3 Strefa 3 — ustawienia](#63-strefa-3--ustawienia)
8. [Nawigacja i warstwy widoczności](#7-nawigacja-i-warstwy-widoczności)
...
9. [Karty sesji](#8-karty-sesji)
10. [Kanały komunikacji operacyjnej](#9-kanały-komunikacji-operacyjnej)
...
11. [Moduły — kontrakty](#10-moduły--kontrakty)
   - [10.0 Wzorzec wspólny](#100-wzorzec-wspólny)
   - [10.1 Studio](#101-studio)
   - [10.2 Workspace](#102-workspace)
   - [10.3 Automations](#103-automations)
   - [10.4 Browser](#104-browser)
   - [10.5 Research](#105-research)
   - [10.6 Library](#106-library)
   - [10.7 Translate](#107-translate)
   - [10.8 Roundtable](#108-roundtable)
   - [10.9 Design](#109-design)
   - [10.10 Assistant](#1010-assistant)
   - [10.11 Terminal](#1011-terminal)
   - [10.12 Developer](#1012-developer)
   - [10.13 Diagnostics](#1013-diagnostics)
   - [10.14 Apps](#1014-apps)
   - [10.15 Agents](#1015-agents)
12. [MultitaskingAI — panel orkiestracji](#11-multitaskingai--panel-orkiestracji)
   - [11.1 Zespoły](#111-zespoły)
   - [11.2 Role](#112-role)
   - [11.3 Kolejki](#113-kolejki)
   - [11.4 Orkiestracja](#114-orkiestracja)
   - [11.5 Harmonogram i automatyki](#115-harmonogram-i-automatyki)
   - [11.6 Monitor procesu](#116-monitor-procesu)
   - [11.7 Schemat przepływu kontraktu](#117-schemat-przepływu-kontraktu)
13. [Pamięć](#12-pamięć)
14. [Historia i retencja](#13-historia-i-retencja)
15. [Konfiguracja](#14-konfiguracja)
16. [Izolacja — okno konfiguracji punktów izolacji](#15-izolacja--okno-konfiguracji-punktów-izolacji)
   - [15.1 Panel selektora zasięgu](#151-panel-selektora-zasięgu)
   - [15.2 Panel macierzy izolacji](#152-panel-macierzy-izolacji)
   - [15.3 Panel profilu i podglądu](#153-panel-profilu-i-podglądu)
17. [Funkcje globalne](#16-funkcje-globalne)
   - [16.1 Mobile](#161-mobile)
   - [16.2 Always On Display](#162-always-on-display)
18. [Rozszerzenia](#17-rozszerzenia)
19. [Synchronizacja wielourządzeniowa](#18-synchronizacja-wielourządzeniowa)
20. [Obsługa błędów](#19-obsługa-błędów)
21. [Wersjonowanie kontraktu](#20-wersjonowanie-kontraktu)
22. [Kryteria odbioru](#21-kryteria-odbioru)
23. [Załącznik A. Zbiorcza tabela poleceń](#załącznik-a-zbiorcza-tabela-poleceń)
24. [Załącznik B. Zbiorcza tabela zdarzeń](#załącznik-b-zbiorcza-tabela-zdarzeń)
25. [Załącznik C. Przykładowe wymiany komunikatów](#załącznik-c-przykładowe-wymiany-komunikatów)
   - [C.1. Od strony głównej do sesji w module](#c1-od-strony-głównej-do-sesji-w-module)
   - [C.2. Utworzenie komponentu własnego i jego wpięcie w sesję](#c2-utworzenie-komponentu-własnego-i-jego-wpięcie-w-sesję)
   - [C.3. Przypisanie roli w MultitaskingAI i uruchomienie Subagent Network](#c3-przypisanie-roli-w-multitaskingai-i-uruchomienie-subagent-network)
   - [C.4. Od polecenia w Chat Window do zamknięcia zlecenia w Execution Loop Window](#c4-od-polecenia-w-chat-window-do-zamknięcia-zlecenia-w-execution-loop-window)
   - [C.5. Błąd walidacji przy tworzeniu sesji](#c5-błąd-walidacji-przy-tworzeniu-sesji)
26. [Załącznik D. Kody błędów](#załącznik-d-kody-błędów)
27. [Załącznik E. Słowniczek pojęć kontraktu](#załącznik-e-słowniczek-pojęć-kontraktu)
28. [Załącznik F. Schematy ładunku wybranych komunikatów](#załącznik-f-schematy-ładunku-wybranych-komunikatów)
   - [F.1. Uzgodnienie połączenia — `connection.hello` i odpowiedź](#f1-uzgodnienie-połączenia--connectionhello-i-odpowiedź)
   - [F.2. Wejście do modułu — odpowiedź na `workspace.enter`](#f2-wejście-do-modułu--odpowiedź-na-workspaceenter)
   - [F.3. Chat Window — `message.send` i odpowiedź](#f3-chat-window--messagesend-i-odpowiedź)
   - [F.4. Podgląd polityki izolacji — odpowiedź na `isolation.policy.preview`](#f4-podgląd-polityki-izolacji--odpowiedź-na-isolationpolicypreview)
   - [F.5. Sterowanie przebiegiem pętli — `loop.control`](#f5-sterowanie-przebiegiem-pętli--loopcontrol)
   - [F.6. Wywołanie funkcji warstwy 4 — `layer.function.invoke`](#f6-wywołanie-funkcji-warstwy-4--layerfunctioninvoke)
...
| 2. Uzgodnienie i uwierzytelnienie | Klient → Serwer → Klient | `connection.hello` oraz odpowiedź o `type` równym `response` | Klient przedstawia wersję protokołu, identyfikator urządzenia i token dostępu (o ile posiada); serwer potwierdza wersję protokołu oraz zwraca `authenticated`, `gatewayConfigured` i `loginRequired` (rozdz. 5). |
| 3. Powiązanie | Klient → Serwer | `session.bind` albo `home.enter` | Klient zgłasza, czy wraca do aktywnej karty sesji, czy wchodzi na stronę główną; serwer wiąże połączenie ze stanem zgłoszonym przez klienta — kartą sesji albo stroną główną. |
...
Klient                                        Serwer
  ...
  │ ───────────────────────────────────────────►│   i uwierzytelnienie
  │   response (serverVersion, authenticated,   │
  │   gatewayConfigured, loginRequired)         │
  │ ◄───────────────────────────────────────────│
  │                                             │
...
{ "type": "response", "id": "h-1", "status": "ok", "payload": {
    ...
    "authenticated": true, "gatewayConfigured": true,
    "loginRequired": false },
  "timestamp": "2026-08-06T09:00:00Z" }
```
...
Powiadomienie o zmianie stanu. Serwer inicjuje je samodzielnie, niezależnie od tego, czy zmianę wywołało polecenie tego samego urządzenia, polecenie innego urządzenia, proces działający w tle — harmonogram Automations — czy działanie agenta. Zdarzenie niesie stan wystarczający do odświeżenia widoku klienta bez dodatkowego zapytania (Architektura, rozdz. 11).

### 4.3. Strumienie (serwer → klient)

Ciąg fragmentów przekazywanych na żywo. Strumień otwiera odpowiedź `status: "ok"` na polecenie inicjujące — `message.send` — z ładunkiem zawierającym `streamId` równy `id` polecenia; kolejne fragmenty mają `type: "stream.chunk"` i to samo `id`. Ostatni fragment niesie `done: true`. Dwa rodzaje strumieni występują w platformie: strumień odpowiedzi Wykonawcy w Chat Window (rozdz. 9.1) oraz strumień synchronizacji stanu między urządzeniami (rozdz. 18).

---

## 5. Uwierzytelnianie i tożsamość urządzenia

Rozdział realizuje kontrakt komunikacyjny dla mechanizmu opisanego w rozdziale 15 dokumentu Architektury: rejestracja, logowanie, metody dodatkowe oraz wymóg logowania na nasłuchu.
...
Token uzyskany po uwierzytelnieniu jest przedstawiany w polu `token` komunikatu `connection.hello` (rozdz. 2) przy każdym kolejnym nawiązaniu połączenia. Wymóg logowania rozstrzyga nastawa zasięgu `application` o kluczu `gateway.requireLogin`, odczytywana przy każdym powitaniu; jej wartość rdzeń zwraca w polu `loginRequired` odpowiedzi na `connection.hello` (Architektura, rozdz. 15.1). Kontrakt komunikatów pozostaje identyczny niezależnie od wartości tej nastawy — pole niczego nie odmawia, lecz kieruje klienta do okna logowania albo je pomija.

| Pole odpowiedzi na `connection.hello` | Typ | Znaczenie |
|---|---|---|
| `authenticated` | `bool` | Czy to połączenie jest związane z ważną sesją bramki. |
| `gatewayConfigured` | `bool` | Czy sekret bramki jest już ustawiony; fałsz kieruje klienta do okna pierwszej rejestracji. |
| `loginRequired` | `bool` | Czy na tym nasłuchu obowiązuje wymóg logowania, odczytany z nastawy `gateway.requireLogin`. Pole puste oznacza, że rdzeń nastawy nie zna. |
...
| `component.assign` | polecenie | Przypisanie utworzonego komponentu do modułu lub sesji, w której ma zastosowanie — wpięcie automatyki w sesji modułu Developer, zgodnie z rozdz. 7.2 koncepcji. |
...
Warunki błędu rozdziału: `not_found` — wskazany `elementId` albo `functionId` nie istnieje; `permission_denied` — rola użytkownika nie obejmuje funkcji warstwy 4 wskazanej w `layer.function.invoke`; `validation_failed` — wartość `layer` spoza zakresu 1–4 albo brak zarówno `functionId`, jak i `naturalCommand`; `command_not_understood` — treść `naturalCommand` nie została dopasowana do żadnej funkcji; jest to dziewiąty kod zamkniętego zbioru kodów kontraktu (Załącznik D), odrębny od `validation_failed`, aby interfejs odpowiadał na niezrozumiane polecenie inaczej niż na błąd walidacji. Plik `budowa/shared/contract.json` zna osiem kodów — dopisanie dziewiątego jest osobnym zadaniem w kodzie.
...
| `session.open` | polecenie | Otwarcie zapisanej, wcześniej zamkniętej karty sesji, wraz z zapisaną migawką układu okien (Model danych, rozdz. 4.5). |
| `session.focus` | polecenie | Oznaczenie karty sesji jako aktywnej na danym urządzeniu; nie wpływa na stan pozostałych urządzeń. |
| `session.update` **[DO DECYZJI OPERATORA]** — nazwa nie występuje w `budowa/shared/contract.json`; do rozstrzygnięcia, czy komenda ma powstać | polecenie | Zmiana atrybutów karty sesji (tytuł, przypisany projekt, przypisany profil izolacji). |
...
| `session.close` | polecenie | Zamknięcie karty sesji; zapis do powrotu zależny od ustawienia konfiguracyjnego (Model danych, rozdz. 8.2) — proces sesji po stronie serwera trwa dalej niezależnie od zamknięcia karty po stronie klienta (Architektura, rozdz. 7). |
...
| `workspace.context.set` / `workspace.context.get` | polecenie | Context Memory | Zapis i odczyt pamięci kontekstowej projektu (Model danych, rozdz. 9). |
...
| `automation.queue.action` | polecenie | Queue Manager | Akcja na kolejce automatyki — akcje wspólne z silnikiem kolejek, rozdz. 11.3. |
| `automation.orchestrator.define` | polecenie | Orchestrator | Definicja zależności między krokami automatyki — mechanizm wspólny z orkiestracją, rozdz. 11.4. |
...
Powiązanie automatyki ze środowiskiem MultitaskingAI (koncepcja, rozdz. 11.3, 13.6) realizuje `automation.link` (rozdz. 11.5 niniejszego dokumentu) — połączenie nie jest domyślne i wymaga jawnego polecenia.
...
Środowisko MultitaskingAI nie ma bocznej nawigacji modułów (koncepcja, rozdz. 9.4, 10) — `module.list` dla tego środowiska zwraca zamiast wykazu modułów strukturę panelu orkiestracji, złożonego z sześciu sekcji ustalonych w rozdziale 13.8 koncepcji. Poniższe podrozdziały 11.1–11.6 odpowiadają tym sekcjom w tej samej kolejności, podrozdział 11.7 przedstawia schemat przepływu kontraktu, a konfigurację widoczności i kolejności samego panelu niosą dwa polecenia zestawione bezpośrednio niżej.
...
| `subagent.spawn` | polecenie | Uruchomienie przez Executora podagentów Subagent Network — do 15 jednocześnie, z podaną specjalizacją każdego; wcielenia wskazane w Koncepcji platformy, Załącznik B: Agent UI, Agent Backend, Agent API, Agent Security, Agent Database, Agent Testing. |
...
| `orchestration.validate` | polecenie | Weryfikacja spójności zdefiniowanych zależności — wykrycie cyklu w grafie zależności. |
...
| `automation.link.remove` **[DO DECYZJI OPERATORA]** — nazwa nie występuje w `budowa/shared/contract.json`; do rozstrzygnięcia, czy komenda ma powstać | polecenie | Zniesienie powiązania. |
...
Realizuje mechanizm pamięci opisany w rozdziale 9 dokumentu Model danych, działający na poziomach: globalnym, środowiska, modułu, projektu i sesji, zgodnie z hierarchią zasięgów izolacji (rozdz. 15.1).
...
| `retention.set` | polecenie | Ustalenie okresu przechowywania historii na wskazanym poziomie; brak ustawienia oznacza przechowywanie bez limitu (Model danych, rozdz. 8.6). |
...
Realizuje okno konfiguracji oraz warstwową strukturę konfiguracji opisane w rozdziale 13 dokumentu Architektury: warstwa domyślna oraz warstwa sesji, nakładająca się na warstwę domyślną bez jej modyfikacji.
...
| Typ komunikatu | Kierunek | Opis |
|---|---|---|
| `isolation.scope.list` | polecenie | Wykaz ośmiu poziomów zasięgu (koncepcja, rozdz. 6.5) wraz z kolejnością pierwszeństwa: globalny, środowisko, moduł, para modułów, projekt, karta sesji, rola, okno komunikacji. |
...
| Projekt | `project` | ↑ |
| Karta sesji | `session` | ↑ |
| Rola (MultitaskingAI) | `role` | ↑ |
| Okno komunikacji | `window` | Najwyższe — poziom najwęższy |
...
Ośmioma zakresami izolacji technicznej, każdy w stanie `on` albo `off`, są: `workingDirectory` (katalog roboczy sesji), `processEnvironment` (środowisko procesu), `modelDataDirectory` (katalog danych i konfiguracji modelu), `networkAccess` (dostęp sieciowy), `fileAccess` (zakres odczytu i zapisu plików), `accountToken` (konto i token per sesja), `processModel` (model procesu — odrębny lub współdzielony), `executionServer` (serwer wykonania).
...
{ "type": "response", "id": "c-40", "status": "ok", "payload": {
    ...
    "technical": { "workingDirectory": "off", "processEnvironment": "off",
      "modelDataDirectory": "off", "networkAccess": "off", "fileAccess": "off",
...
Pełne opracowanie funkcji wraz z protokołem parowania urządzenia i jego komunikatami (`device.pair.*`, `device.credential.refresh`, `device.revoke`, `device.logout`, `device.changed`) zawiera dokument [Funkcje mobilne](../funkcje-globalne/mobile.md), rozdz. 9.10.
...
| `mobile.queue.sync` **[DO DECYZJI OPERATORA]** — nazwa nie występuje w `budowa/shared/contract.json`; do rozstrzygnięcia, czy komenda ma powstać | polecenie | Przekazanie kolejki działań wykonanych bez połączenia wraz z wersjami stanu do uzgodnienia ([Funkcje mobilne](../funkcje-globalne/mobile.md), rozdz. 7.3). |
| `mobile.notification.action` | polecenie | Działanie wykonane z poziomu powiadomienia wypychanego ([Funkcje mobilne](../funkcje-globalne/mobile.md), rozdz. 8.3). |

### 16.2. Always On Display

Kontrakt realizuje funkcję globalną opisaną w [Always-on Display](../funkcje-globalne/always-on-display.md) (rozdz. 9.5).
...
Realizuje mechanizm rozszerzeń opisany w rozdziale 14 dokumentu Architektury: wtyczki, umiejętności, konektory i serwery MCP, dzielące się na dwa źródła — Danaco Plugin (zestaw wbudowany) i Personal (instalowane przez Operatora).
...
|---|---|
| Nośnik zmiany | Zmiana wykonana na jednym urządzeniu jest przekazywana przez serwer do pozostałych urządzeń powiązanych z tym samym kontem w postaci zdarzenia właściwego zmienionemu obszarowi (rozdz. 4.2): `session.changed`, `memory.changed`, `config.changed`. |
...
| `details` | Obiekt z dodatkowymi danymi diagnostycznymi, obecny wtedy, gdy serwer takie dane przekazuje — wskazanie niepoprawnego pola. |
| `retryable` | `true`, jeżeli ponowienie tego samego polecenia bez zmian ma sens — chwilowa niedostępność kanału modelu. |
...
Kontrakt komunikacji jest wersjonowany niezależnie od wersji koncepcji platformy i od wersji serwera. Numer wersji ma postać `MAJOR.MINOR.PATCH`. Klient i serwer uzgadniają wersję protokołu przy nawiązaniu połączenia, w komunikacie `connection.hello` i w odpowiedzi na niego (rozdz. 2).
...
Kontrakty komunikacji są zrealizowane zgodnie z niniejszym opracowaniem, gdy spełnione są łącznie poniższe warunki. Każdy warunek jest sprawdzalny bez odwołania do intencji autora — wyłącznie przez odczyt kontraktu albo obserwację ruchu na kanale WebSocket.

| Warunek | Sposób sprawdzenia |
|---|---|
| Klient i serwer wymieniają się wyłącznie jednym kanałem WebSocket na połączenie (rozdz. 2, Architektura rozdz. 5) | inspekcja ruchu sieciowego klienta nie ujawnia drugiego równoległego gniazda do tego samego serwera |
| Koperta każdego komunikatu niesie pola `type`, `id` i `timestamp` niezależnie od kierunku (rozdz. 3) | odczyt struktury koperty w `budowa/shared/contract.json`, pole `koperta` |
| Każda z 1119 komend jest przypisana do dokładnie jednego z 68 obszarów wymienionych w Architekturze rozdz. 18, bez obszaru zdefiniowanego powtórnie w niniejszym dokumencie | `grep -oE '"typ": *"[a-zA-Z]+\.' budowa/shared/contract.json` — prefiks przed pierwszą kropką odpowiada nazwie obszaru z tabeli Architektury |
| Błąd operacji nie przerywa połączenia WebSocket ani nie blokuje kolejnych poleceń (rozdz. 19) | po odpowiedzi `status: "error"` klient wysyła kolejne polecenie na tym samym połączeniu bez ponownego `connection.hello` |
| Każdy z 8 kodów błędów wdrożonych w kontrakcie niesie pole `retryable` zgodne z Załącznikiem D; dziewiąty kod `command_not_understood` czeka na wdrożenie w kodzie | odczyt `budowa/shared/contract.json`, pole `kodyBledow`, porównanie z Załącznikiem D wiersz po wierszu |
| Wersja protokołu jest uzgadniana wyłącznie przy `connection.hello` / odpowiedzi na nie, nigdy w trakcie sesji (rozdz. 2, rozdz. 20) | żadna komenda poza etapem nawiązania połączenia nie niesie pola `protocolVersion` jako pola żądania wymaganego |
| Zmiana `MAJOR` kontraktu nie jest wprowadzana bez usunięcia lub oznaczenia `deprecated: true` zmienianego typu komunikatu (rozdz. 20) | wykaz typów komunikatów oznaczonych `deprecated` w bieżącym wydaniu odpowiada różnicy względem poprzedniej wersji `MAJOR` |
| Serwer pozostaje jedynym źródłem prawdy — klient nie utrzymuje stanu niepotwierdzonego odpowiedzią serwera dłużej niż do najbliższego zdarzenia korygującego | po utracie i odzyskaniu połączenia klient odświeża stan komendą właściwą zamiast polegać na buforze lokalnym |

---

## Załącznik A. Zbiorcza tabela poleceń

Zestawienie porządkuje polecenia (klient → serwer) opisane w rozdziałach 2–17 według obszaru funkcjonalnego. Trzy pozycje opatrzono w korpusie oznaczeniem **[DO DECYZJI OPERATORA]**, ponieważ ich nazwy nie występują w `budowa/shared/contract.json`: `session.update` (rozdz. 8), `automation.link.remove` (rozdz. 11.5) i `mobile.queue.sync` (rozdz. 16.1). W zestawieniu poniżej oznaczono je gwiazdką.
...
Kontrakt definiuje dziewięć kodów wspólnych wszystkim obszarom; wykaz poniżej jest dla nich pełny. Pole `kodyBledow` w `budowa/shared/contract.json` zawiera dziś osiem pierwszych pozycji — dopisanie dziewiątego kodu `command_not_understood` jest osobnym zadaniem w kodzie.
...
| `not_authenticated` | Połączenie nie zostało uwierzytelnione, a nastawa `gateway.requireLogin` tego wymaga. | Nie |
| `permission_denied` | Operacja przekracza uprawnienia przypisane kontu, agentowi lub roli. | Nie |
| `conflict` | Operacja koliduje z bieżącym stanem zasobu — usunięcie sesji już usuniętej. | Nie |
...
**Stan wdrożenia kodu `command_not_understood`.** Kod jest dziewiątym kodem kontraktu i wspólnym kodem wszystkich obszarów, nie kodem własnym obszaru — [Architektura techniczna](architektura.md) rozdz. 19 pozostaje jedynym miejscem odniesienia dla wykazu. Pole `kodyBledow` w `budowa/shared/contract.json` zna osiem kodów; dopisanie dziewiątego jest osobnym zadaniem w kodzie. Do chwili wdrożenia sytuacja z rozdz. 7.2 nie ma w kodzie odrębnego kodu odpowiedzi.
...
**Wymóg logowania** — wartość logiczna pola `loginRequired` zwracana w odpowiedzi na `connection.hello`, odczytana z nastawy zasięgu `application` o kluczu `gateway.requireLogin`; rozstrzyga, czy klient pokazuje okno logowania (rozdz. 5, Architektura rozdz. 15).
...
| `token` | warunkowo | Token dostępu uzyskany po uwierzytelnieniu; wymagany, gdy nastawa `gateway.requireLogin` jest włączona (rozdz. 5). |
...
| `authenticated` | zawsze | Czy połączenie jest związane z ważną sesją bramki (rozdz. 5). |
| `gatewayConfigured` | zawsze | Czy sekret bramki jest już ustawiony (rozdz. 5). |
| `loginRequired` | zawsze | Czy na tym nasłuchu obowiązuje wymóg logowania, odczytany z nastawy `gateway.requireLogin` (rozdz. 5). |
...
*Koniec dokumentu. Kontrakty komunikacji — Specyfikacja docelowa, wersja 2.0, 2026-08-20.*
