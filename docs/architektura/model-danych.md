...
- [Załącznik A. Słownik enumeracji](#załącznik-a-słownik-enumeracji)
- [Załącznik B. Pełny wykaz tabel](#załącznik-b-pełny-wykaz-tabel)
- [Załącznik C. Komponenty własne a moduły platformowe](#załącznik-c-komponenty-własne-a-moduły-platformowe)
- [Załącznik D. Diagram powiązań encji](#załącznik-d-diagram-powiązań-encji)
- [Załącznik E. Szablony konfiguracji pól JSON](#załącznik-e-szablony-konfiguracji-pól-json)
- [Załącznik F. Scenariusze danych](#załącznik-f-scenariusze-danych)
...
| Liczba rzeczywista | `REAL` | Stosowana wyjątkowo (np. wagi w regułach warunkowych kolejki). |
...
| Odniesienie polimorficzne | para kolumn `typ_odniesienia TEXT` + `odniesienie_id INTEGER` | Stosowana tam, gdzie jedna kolumna musi wskazywać wiersze różnych tabel (np. poziomy izolacji, cele powiązań). Egzekwowana na poziomie aplikacji, nie ograniczeniem `FOREIGN KEY`. |
...
Zgodnie z architekturą (rozdział 9) dane dostępowe nie są przechowywane w bazie SQLite w postaci jawnej — baza przechowuje wyłącznie skrót lub odwołanie do wartości utrzymywanej poza bazą.
...
│     (siedem poziomów zasięgu)                                    (rozdz. 15)
│
...
| kod | TEXT | Kod modułu, np. `STUDIO`, `WORKSPACE`, `AUTOMATIONS`, `BROWSER`, `RESEARCH`, `LIBRARY`, `TRANSLATE`, `ROUNDTABLE`, `DESIGN`, `ASSISTANT`, `TERMINAL`, `DEVELOPER`, `DIAGNOSTICS`, `APPS`, `AGENTS`. | UNIQUE, CHECK enum |
...
| nazwa | TEXT | Nazwa własna okna (np. „Studio Editor”, „Workflow Builder”). | NOT NULL |
| kolejnosc | INTEGER | Kolejność prezentacji w module. | NOT NULL |
| czy_okno_wspolne | INTEGER | `1` dla Chat Window — okna wspólnego dla wszystkich piętnastu modułów. | CHECK (0,1) |
...
| stan | TEXT | Stan specyficzny dla okna (np. otwarty plik w Code Editor), zapis JSON. | JSON |
...
| etykieta | TEXT | Etykieta widoczna na pozycji; dla strefy 2 zorientowana na działanie twórcze (np. „Utwórz automatykę”). | — |
...
| rodzaj_powiazania | TEXT | Opis powiązania (np. „współdzielenie operacji kontekstowych AI”). | NOT NULL |
...
zaleznosc_orkiestracji ──(polimorficzne)──> model | agent | zadanie | kolejka | automatyka | projekt
```
...
| reguła_czasu | TEXT | Wyrażenie definiujące moment lub cykl uruchomienia. | NOT NULL |
...
| zrodlo_typ | TEXT | `model` \| `agent` \| `zadanie` \| `kolejka` \| `automatyka` \| `projekt`. | CHECK enum |
...
| nazwa | TEXT | Nazwa umiejętności (skilla). | NOT NULL |
| konfiguracja | TEXT | — | JSON |

### 12.3. Rozszerzenie agenta (`agent_rozszerzenie`)

Łączy agenta z pluginem, umiejętnością lub konektorem katalogowanym w `rozszerzenie` (rozdział 14).
...
| id | INTEGER | — | PK |
| agent_id | INTEGER | Agent. | FK → agent.id, ON DELETE CASCADE |
| zakres | TEXT | Zakres uprawnienia (np. dostęp do plików, sieci, konkretnego modułu). | NOT NULL |
...
| nazwa | TEXT | Nazwa podagenta (np. „Agent Backend”, „Agent Security”). | NOT NULL |
...
Limit piętnastu podagentów na wykonawcę jest egzekwowany na poziomie aplikacji (np. wyzwalaczem sprawdzającym liczbę aktywnych wierszy przy wstawieniu) — SQLite nie wyraża tego ograniczenia deklaratywnie w definicji tabeli.
...
| typ_decyzji | TEXT | Rodzaj decyzji (np. planowanie, sterowanie, walidacja). | — |
| tresc | TEXT | — | — |
| znacznik_czasu | TEXT | — | NOT NULL |
...
Tabela słownikowa, siedem stałych wierszy uporządkowanych według pierwszeństwa.

| Pole | Typ | Opis | Klucz |
|---|---|---|---|
| id | INTEGER | — | PK |
| kod | TEXT | `globalny` \| `srodowisko` \| `modul` \| `para_modulow` \| `projekt` \| `karta_sesji` \| `rola`. | UNIQUE, CHECK enum |
| nazwa | TEXT | — | NOT NULL |
| pierwszenstwo | INTEGER | `1` (globalny, warstwa bazowa) do `7` (rola, najwyższe pierwszeństwo). | NOT NULL |

Siedem stałych wierszy słownika, od najogólniejszego do najbardziej szczegółowego:
...
| 7 (najwyższe) | rola | Rola (MultitaskingAI) | Profil izolacji przypisany roli Executor 1. |
...
Realizuje panel selektora zasięgu: przypisanie profilu do konkretnego elementu na jednym z siedmiu poziomów.
...
Element docelowy poziomu zasięgu „para modułów” — na przykład relacja Studio–Translate.
...
| autor | TEXT | `uzytkownik` \| `model`. | CHECK enum |
| data_utworzenia | TEXT | — | NOT NULL |
...
Pojedynczy parametr zachowania, konfigurowalny z okna konfiguracji (architektura, rozdział 13 i 13).

| Pole | Typ | Opis | Klucz |
|---|---|---|---|
| id | INTEGER | Identyfikator ustawienia. | PK |
| klucz | TEXT | Klucz parametru (np. `retencja_historii`, `tryb_pracy_w_tle`). | NOT NULL |
...
Proaktywne doradztwo — sugestie działań, sugestie konfiguracji, wskazania problemów, rekomendacje kolejnych kroków. Reguły wyzwalania sugestii, katalog ich rodzajów i cykl życia opisuje `funkcje-globalne/always-on-display.md` (rozdz. 3–4, 9.4).
...
Pojedyncze zdarzenie zarejestrowane przez centrum powiadomień — jeden mechanizm powiadamiania obowiązujący w całej platformie, opisany jako karta komponentu w `interfejs-uzytkownika/katalog-komponentow.md` (rozdz. 11.6). Encja obsługuje zarówno prezentację zdarzenia w centrum powiadomień, jak i jego dostarczenie na urządzenie mobilne przez funkcję Mobile.
...
Okresy przechowywania są wierszami `ustawienie` (rozdział 17.1) i podlegają regule siedmiu poziomów zasięgu z rozdziału 15.5.
...
| `zaleznosc_orkiestracji` | `zrodlo_typ` / `zrodlo_id`, `cel_typ` / `cel_id` | model, agent, zadanie, kolejka, automatyka, projekt |
...
SQLite nie wspiera kluczy obcych polimorficznych deklaratywnie — integralność tych odniesień jest egzekwowana na poziomie rdzenia serwera (Go), w warstwie rdzenia opisanej w architekturze (rozdział 3), a nie ograniczeniem schematu bazy.
...
|---|---|---|
| Tabele słownikowe | `srodowisko`, `modul`, `okno_operacyjne`, `strefa_glowna`, `poziom_zasiegu`, `sekcja_panelu_orkiestracji`, `warstwa_widocznosci`, `rola_uzytkownika`, `element_interfejsu`, `funkcja_platformy` | Zasiewane przy inicjalizacji bazy wartościami ustalonymi dla platformy; zbiór wierszy stały, natomiast pola konfigurowalne (np. `dostepny`, `widoczna`, `kolejnosc`) w pełni edytowalne przez Operatora. |
...
*Polityka migracji.* Rozróżnienie to determinuje politykę migracji: zmiana zbioru wierszy tabeli słownikowej (np. dodanie nowego środowiska) wymaga migracji schematu, natomiast zmiana wartości pól konfigurowalnych w tych tabelach jest zwykłą operacją zapisu, nie migracją.
...
| `zaleznosc_orkiestracji.zrodlo_typ` / `.cel_typ` | model, agent, zadanie, kolejka, automatyka, projekt |
...
| `poziom_zasiegu.kod` | globalny, srodowisko, modul, para_modulow, projekt, karta_sesji, rola |
...
| `poziom_zasiegu` | 15.1 | Siedem poziomów zasięgu reguł izolacji i konfiguracji. |
...
Model liczy sześćdziesiąt osiem tabel.
...
zaleznosc_orkiestracji ──(odniesienie polimorficzne)──> model / agent / zadanie / kolejka / automatyka / projekt
...
| 2. Przypisanie ról | `rola_multitasking`: `executor_1` (backend), `executor_2` (frontend, `relacja_z_innymi = 'niezalezna'`), `coordinator`, `executor_3_validator` (`wcielenie = 'security_auditor'`, w kolejnym przebiegu `'qa_lead'`) |
...
| 5. Zależność backend–frontend | `zaleznosc_orkiestracji` (`typ_zaleznosci = 'poprzedza'`) |
...
*Koniec dokumentu. Danaco Console — Model danych, wersja 2.0.*
