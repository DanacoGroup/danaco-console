...
## budowa/server/internal/store/migracja_201_apps_architektura_dobudowa.sql
...
## budowa/server/internal/store/migracja_276_agent_zakres_dzialania.sql
Moduły zastosowania eksperta mają własną tabelę zamiast kolumny z listą, ponieważ kontrakt agent.modules.set przyjmuje listę pustą jako brak ograniczenia, a kod modułu pochodzi z rodziny module. Lista trzymana napisem rozdzielonym przecinkiem nie pozwala zapytać, którzy eksperci wolno działają w danym module, bez przeszukiwania napisów, a to pytanie zadaje rdzeń przy każdym nałożeniu eksperta na okno; wiersz na moduł odpowiada na nie indeksem. Brak wierszy dla eksperta znaczy dokładnie to samo, co lista pusta w kontrakcie, czyli brak ograniczenia — nie ma trzeciego stanu i nie ma potrzeby odróżniania braku ustawienia od ustawienia pustego, ponieważ stan wyjściowy platformy jest pełnym dostępem.
Kolumna limit_podagentow siedzi na ekspercie, a nie w osobnej tabeli, ponieważ wartość jest jedna, liczbowa i czytana przy każdym powołaniu podagentów, więc złączenie byłoby kosztem bez zysku. Zero znaczy wyłączenie sieci wieloagentowej, dokładnie jak opisuje pole subagentLimit eksperta. Wartość wyjściowa 15 jest maksimum technicznym platformy przy powoływaniu podagentów.
...
## budowa/server/internal/store/migracja_008_katalog_akcji.sql
Ten sam rejestr akcji zasila narzędzia modelu, więc kod nie zna ani jednej akcji z osobna. Tabela naśladuje wzorzec katalogu kanału modelu: wiersz opisuje byt w całości, kod jest jego trwałym identyfikatorem, kolumna aktywności rozstrzyga widoczność, a kolumna kolejności porządek prezentacji. Akcja należy do jednego z ośmiu poziomów zasięgu; identyfikator poziomu wskazuje poziom, a klucz zasięgu wskazuje konkretny byt tego poziomu, przy czym klucz pusty znaczy każdy byt tego poziomu, tak samo jak w tabeli ustawień. Kolumna komendy niesie nazwę komendy kontraktu wywoływanej przez akcję i nie jest kluczem obcym, bo kontrakt mieszka poza bazą — akcja wskazująca komendę bez obsługiwacza dostaje odpowiedź nieznanej komendy, więc rozjazd katalogu z rdzeniem jest widoczny, a nie wywracający. Warunek dostępności to zamknięty zbiór bytów, których akcja wymaga, żeby dało się ją zaoferować, wyprowadzony z pól obowiązkowych żądania komendy; warunek nie wygasza kontrolki, tylko rozstrzyga, czy akcja trafia do panelu danego kontekstu.

## budowa/server/internal/store/migracja_014_katalog_kont.sql
Rodzaj konta pokrywa się ze słownikami kontraktu wskazującymi tę samą kolumnę: konto programu wiersza poleceń to rodzaj jeden, konto modelu to jeden z dwóch pozostałych rodzajów. Algorytm rotacji mieszka poza bazą; kolumny stanu i wyczerpania są zapisem tego, co pula już rozpoznała, oraz przełącznikiem Operatora, a nie drugą implementacją rotacji — katalog rozstrzyga, które konta wolno brać i w jakiej kolejności, kiedy przejść na następne rozstrzyga pula. Kolumna aktywności mówi, czy konto w ogóle bierze udział w pracy, kolumna stanu — czy jest w tej chwili zdatne; to dwa różne pytania, stąd dwie kolumny. Kolumna odwołania do poświadczenia jest jedynym miejscem danych dostępowych i niesie wyłącznie nazwę wpisu w magazynie sekretów albo ścieżkę profilu — migracja nie dokłada kolumny na sam sekret, a odczyt katalogu w warstwie danych zwraca wyłącznie znacznik, że poświadczenie jest zapisane. Kolumna konta w kanale modelu jest jedynym wiązaniem tych dwóch bytów: jeden kanał wskazuje konto preferowane, jedno konto może obsługiwać wiele kanałów — kanał jest definicją rozmowy z modelem, konto profilem uwierzytelnienia. Usunięcie konta odłącza kanały, ale ich nie kasuje; rotacja bierze konta tego samego rodzaju, więc kanał pracuje dalej także wtedy, gdy jego konto preferowane wyczerpało limit.
## budowa/server/internal/store/migracja_277_agent_izolacja_techniczna.sql
Tabela nie powiela okna konfiguracji punktów izolacji: tamto okno zapisuje regułę na jednym z poziomów zasięgu w tabelach konfiguracja_osi i profil_izolacji, a tutaj zapisuje się wartość wyjściowa właściwa eksperckiemu kontu, obowiązująca wszędzie, gdzie ekspert działa, dopóki nie nadpisze jej reguła z poziomu bardziej szczegółowego. Bez własnego nośnika ustawienie eksperta musiałoby udawać regułę zasięgu, a wtedy nie dałoby się odróżnić ustawienia eksperckiego od ustawienia platformowego.
Brak wiersza znaczy zakres nieodcięty, czyli stan wyjściowy. Tabela nie zakłada ośmiu wierszy przy założeniu eksperta: komplet ośmiu składa odczyt, a baza trzyma wyłącznie to, co rzeczywiście przestawiono.
Wartość wyliczenia zakresu nie jest powtórzona warunkiem CHECK, ponieważ katalog zakresów stoi w kontrakcie, a powtórzenie go dałoby drugie źródło tej samej prawdy, rozjeżdżające się przy każdym rozszerzeniu kontraktu.
...
## budowa/server/internal/store/migracja_032_zaczyn_kanalu_lokalnego.sql
Bez zaczynu rejestr kanałów po instalacji jest pusty, więc pierwsze okno nie ma czym rozmawiać, a użytkownik musiałby sam założyć kanał, zanim zobaczy cokolwiek działającego, nie mając skąd wiedzieć, jakich wartości się od niego oczekuje. Identyfikator modelu zostaje pusty celowo, nie przez niedopatrzenie: nie wiadomo, do których modeli ma dostęp dane konto, a wpisanie nazwy na oko dałoby kanał, który wygląda na skonfigurowany i wywraca się przy pierwszej turze — pusty model znaczy więc model domyślny narzędzia, a wybór modelu pozostaje decyzją Operatora w oknie konfiguracji. Rodzaj kanału wiersza poleceń bez wskazania programu daje program domyślny; konto zostaje puste, bo pula pusta jest stanem dopuszczalnym, a program używa wtedy poświadczeń zastanych w systemie. Zapis pomija konflikt, żeby powtórzenie migracji ani instalacja na istniejącej bazie nie nadpisały niczego, co Operator już ustawił.

## budowa/server/internal/store/migracja_035_przestrzen_robocza.sql
Projekt jest bytem, nie napisem: kolumna kodu projektu w sesji niesie sam napis, bez nazwy, stanu i czasu ostatniej zmiany, a panel projektów ma pokazać dokładnie te trzy rzeczy — tabela projektu daje im miejsce, kolumna kodu wiąże wiersz z napisem, którym posługują się sesje i kontrakt, a kolumna kodu projektu w sesji zostaje nietknięta, bo wiązanie po kodzie zostawia czytelne sesje wiążące się samym napisem. Instrukcji projektu tu celowo nie ma: instrukcje systemowe podlegają dziedziczeniu warstwowemu ośmiu poziomów zasięgu rozstrzyganemu nad tabelą ustawień, więc druga tabela instrukcji znaczyłaby drugi rozstrzygacz — zapis instrukcji workspace idzie więc ustawieniem pod adresem wskazanego poziomu, dokładnie tam, gdzie mieszka reszta konfiguracji warstwowej. Biblioteki projektu też tu celowo nie ma: pliki projektu leżą w katalogu roboczym projektu i odczyt biblioteki czyta ten katalog wprost, bo tabela plików bez komendy zapisu w tym module byłaby pusta i bez pisarza, a repozytorium wiedzy jest bytem osobnego modułu biblioteki. Pochodzenie wpisu pamięci odróżnia ustalenie Operatora od propozycji modelu, bo propozycja czeka na przyjęcie i nie ma prawa wejść do pracy sama. Kolumna kodu eksperta niesie identyfikator kontraktu, więc przypisanie eksperta nie czeka na tabelę modułu agentów.

## budowa/server/internal/store/migracja_037_agenci.sql
Kontrakt niósł osiem komend obszaru agenta i strukturę agenta, ale schemat nie miał ani jednej tabeli, w której ekspert mógłby zamieszkać, choć rejestr okien operacyjnych znał już pięć okien modułu agentów. Wzorcem identyfikatora jest tabela kanału modelu i punktu dostępu: klucz służy powiązaniom wewnątrz bazy, a kod wychodzi na zewnątrz jako pole kontraktu. Kolumna kodu kanału wskazuje ten sam rejestr, z którego korzysta okno rozmowy, ale celowo nie jest kluczem obcym: ekspert bywa zakładany zanim kanał powstanie, a kanał bywa kasowany bez kasowania ekspertów — spójność sprawdza warstwa danych przy zapisie, a kanał nieistniejący znaczy ekspert bez modelu bazowego, nie ekspert niezdatny do pracy. Wersja jest licznikiem, nie archiwum: rośnie przy każdej zmianie tożsamości eksperta, a tabeli archiwum treści tu nie ma, bo kontrakt nie ma komendy, która mogłaby ją odczytać — panel historii wersji składa wykaz ze zdarzeń zmiany widzianych w toku sesji, a przywrócenie idzie zwykłą aktualizacją. Umiejętność jest napisem, nie wierszem katalogu: kontrakt przenosi umiejętności listą identyfikatorów, a katalog dostępnych rozszerzeń nie ma dziś odpowiednich komend, więc tabeli katalogu nie zakładamy, bo nie miałaby czytelnika. Klucz główny na parze agenta i umiejętności nie pozwala przypisać tej samej umiejętności dwa razy, więc powtórzone dodanie jest bezskutkowe zamiast dublować wiersz. Wykaz biblioteki ekspertów idzie po nazwie, a filtr aktywności po kolumnie stanu.
## budowa/server/internal/store/migracja_279_orkiestracja_uklad.sql
Wszystkie trzy tabele wiszą na automatyka(id) i posługują się identyfikatorem zewnętrznym kroku, tym samym napisem, którym posługuje się tabela zależności kroku automatyki. Klucz wiersza kroku nie wchodzi tu celowo: układ zależności przepisuje się w całości, więc kroki bywają zakładane od nowa, a wtedy klucz wiersza jest inny, choć krok pozostaje ten sam z punktu widzenia użytkownika.
Bramka dołączenia jest jedna na krok, ponieważ krok scalający tory ma jedną regułę scalenia i drugiej mieć nie może — stąd klucz pierwotny na parze automatyka i krok, a nie osobny identyfikator. Reguła licznikowa bez liczby torów byłaby bramką, która nigdy się nie otworzy albo otworzy zawsze, zależnie od tego, kto ją później czyta.
Grupa ma identyfikator własny, ponieważ kontrakt przyjmuje identyfikator grupy przy zmianie i pozwala go pominąć przy założeniu. Kroki grupy leżą w tabeli podrzędnej z zachowaną kolejnością — grupa sekwencyjna to grupa, w której kolejność jest treścią, a nie ozdobą.
Kompensacja jest jedna na krok główny, ponieważ krok wycofujący skutki danego kroku jest jeden — dwa wycofania tego samego kroku dałyby stan niemożliwy do rozstrzygnięcia. Zdjęcie kompensacji kasuje wiersz, co odpowiada pustej wartości pola kompensacji w kontrakcie.

## budowa/server/internal/store/migracja_038_agenci_konektory.sql
Kolumna punktu dostępu wskazuje wiersz katalogu punktów, z którego składacz mostu okna buduje wpisy serwerów dla procesu modelu; drugiego rejestru serwerów nie zakładamy. Klucz obcy odłącza konektor przy skasowaniu punktu, lecz nie kasuje eksperta. Kolumna rodzaju przyjmuje dosłownie wartości kontraktu, bo kontrakt nie daje dla tego wyliczenia słownika przekładu bazy. Przy założeniu eksperta warstwa danych wpisuje cztery wiersze przyznania, po jednym na grupę zakresu; brak wiersza znaczy nie rozstrzygnięto, a nie odmowa, więc panel uprawnień pokazuje wtedy wartość wyjściową. Zakres jest napisem opcjonalnym w obrębie grupy: zakres pusty oznacza całą grupę, a para grupy i zakresu jest kluczem, więc uprawnienie węższe nie nadpisuje szerszego i odwrotnie.

## budowa/server/internal/store/migracja_039_automatyki.sql
Wykonaniem kroków zajmuje się jeden silnik kolejek osobny od tych tabel: krok automatyki staje się pozycją kolejki dopiero w chwili uruchomienia, a te tabele opisują wyłącznie definicję, czyli to, co Operator zbudował, zanim cokolwiek ruszyło. Kontrakt niesie zależność dwa razy, polem kroku i osobną strukturą zależności; dwie tabele znaczyłyby dwie prawdy o tym samym łuku grafu, więc prawda jest jedna w tabeli zależności kroku, a pole zależności w kroku powstaje z niej przy odczycie. Cykl dłuższy niż para kroków wykrywa walidacja układu, bo baza nie ma na to więzu.
## budowa/server/internal/store/migracja_280_orkiestracja_spiecie_multitasking.sql
Stan wyjściowy jest rozłączony, więc brak wiersza znaczy brak spięcia i tabela nie zakłada niczego z góry. Spięcie nie jest znacznikiem: silnik kolejek jest w rdzeniu jeden, a środowisko wieloagentowe odróżnia się od pętli sesyjnej tym, czyim koordynatorem kolejka jest prowadzona i jakiego jest rodzaju. Spięcie przestawia właśnie te dwie rzeczy na kolejkach automatyki, a rozłączenie je zdejmuje; wiersz niesie zapis decyzji, a skutek leży w tabeli kolejki.
## budowa/server/internal/store/migracja_283_agent_wersja_pamiec.sql
Migracja poprzednia wypełnia pole poziomów pamięci w wersji podzapytaniem wykonywanym w chwili zakładania migawki, co jest za wcześnie w dwóch przypadkach będących normą: przy zakładaniu eksperta wyzwalacz migawki biegnie zanim wiersze poziomów pamięci wejdą do tabeli, więc migawka zapisuje pustkę zamiast czterech poziomów wyjściowych; a aktualizacja samych poziomów pamięci nie rusza żadnej kolumny tabeli eksperta, więc migawki nie zakłada ani nie odświeża. Skutkiem byłby podgląd wersji niezgodny z tym, co w wersji rzeczywiście stało.
Poprawka idzie wyzwalaczami na tabeli poziomów: każda zmiana składu odświeża poziomy w migawce bieżącej wersji tego eksperta, którą zmiana dotyczy, a wersje wcześniejsze zostają nietknięte, ponieważ opisują stan sprzed zmiany.

## budowa/server/internal/store/migracja_040_harmonogramy_przebiegi.sql
Kontrakt niesie notację cron dwa razy: polem harmonogramu i wyzwalaczem odpowiedniego rodzaju. Kolumna cron jest cyklicznością podstawową harmonogramu, a tabela wyzwalaczy niesie pozostałe zdarzenia wyzwalające oraz cykliczności dodatkowe; najbliższe uruchomienie liczy rdzeń z obu źródeł, więc kolumna następnego uruchomienia jest wynikiem, nie deklaracją Operatora. Przebieg jest zapisem wykonania, nie drugą kolejką: kroki wykonuje jeden silnik kolejek, a tabela przebiegu wiąże uruchomienie automatyki z kolejką, która je realizuje, i przechowuje to, czego kolejka nie wie — którą automatykę wykonuje, ile miała etapów i dlaczego się nie powiodła. Etap bieżący i liczba etapów pochodzą z pozycji kolejki; kolumny są ich utrwaleniem na potrzeby podglądu wykonania po restarcie rdzenia, a nie drugim licznikiem postępu.

## budowa/server/internal/store/migracja_041_terminal.sql
Kontrakt nie ma komendy listującej sesje terminala, więc klient po ponownym połączeniu zna identyfikatory kart wyłącznie z własnej pamięci. Gdyby karta żyła tylko w pamięci procesu rdzenia, restart serwera unieważniłby te identyfikatory i wykonanie polecenia odpowiadałoby brakiem na kartę, którą Operator widzi na ekranie — wiersz karty pozwala rdzeniowi ją odtworzyć. Zmienne środowiska karty celowo nie mają kolumny, bo środowisko procesu bywa nośnikiem poświadczeń, a baza nie jest sejfem: zmienne żyją wyłącznie w pamięci rdzenia i po restarcie karta wraca bez nich. Numer procesu i kod wyjścia karty też nie mają kolumny, bo karta nie jest procesem — kartą jest profil powłoki, a procesem każde wykonane w niej polecenie, więc te pola należą do tabeli procesów i pozostają puste na karcie. Dziennik procesów jest dziennikiem, nie stanem żywym: proces czynny prowadzi rejestr w pamięci rdzenia, bo tylko rdzeń ma uchwyt do drzewa potomstwa i tylko on potrafi proces ubić. Kolumna inicjatora jest kolumną, a nie wnioskiem z okoliczności, bo trzeba odróżnić proces uruchomiony przez Operatora od uruchomionego poleceniem modelu, a po zakończeniu procesu nie ma już skąd tego odczytać.
## budowa/server/internal/store/migracja_290_kondycja_sondy.sql
Rodzina opisuje stan produktu, więc wartość, której nikt nie zmierzył, nie ma prawa się tu znaleźć. Dlatego są dwie tabele, a nie jedna: definicja sondy mówi, co i jak często mierzyć, a wiersz wyniku jest zapisem jednego pomiaru wykonanego o znanej godzinie. Dostępność liczy się wyłącznie z wierszy serii — nie ma kolumny dostępności, którą dałoby się ustawić bez pomiaru.
Ostatni stan i ostatni przebieg w definicji są odbiciem ostatniego wiersza serii, a nie drugim źródłem prawdy: wykaz sond pokazuje je obok definicji, żeby okno nie musiało dociągać serii dla każdej sondy. Zapisuje je wyłącznie przebieg, razem z wierszem wyniku, w jednej transakcji.
Rodzaj sondy i stan wyniku są wartościami kontraktu wprost. Warunku sprawdzającego tu nie ma, ponieważ kontrakt bywa rozszerzany, a wartość spoza wykazu odbija adapter odmową nazwaną wołającemu, a nie awarią schematu.

## budowa/server/internal/store/migracja_291_alert_reguly.sql
Reguła mówi, kiedy produkt ma zawołać; wyzwolenie jest zapisem tego, że zawołał, z wartością zmierzoną w chwili wyzwolenia. Wartość obserwowana leży w wierszu wyzwolenia, a nie w regule, ponieważ reguła trwa, a pomiar dotyczy jednej chwili.
Drogi dostarczenia idą zapisem strukturalnym w jednej kolumnie, ponieważ są wykazem wartości bez własnych atrybutów, tą samą formą co przy kanałach dostarczonych wyzwolenia. Osobna tabela wiązania dawałaby złączenie dla listy trzech napisów.
Wyzwolenie zna sondę i wywołanie modelu wyłącznie kodem, bez klucza obcego, ponieważ usunięcie sondy nie ma prawa wymazać śladu alertu, który realnie się odbył.

## budowa/server/internal/store/migracja_292_schowek_historia.sql
Schowek należy do maszyny operatora i rdzeń go nie czyta. Tabela nie udaje dostępu do cudzego schowka: okno, w którym operator skopiował treść, oddaje ją rdzeniowi wprost, a rdzeń daje jej trwałość, więc historia przestaje ginąć razem z kartą. Wklejenie jest ruchem powrotnym — rdzeń oddaje treść oknu, a wstawia ją okno u siebie.
Odcisk treści jest kluczem powtórzenia: powtórzenie tej samej treści nie mnoży wpisów, tylko podnosi wpis zastany na czoło wykazu. Bez warunku jedyności na odcisku rozstrzygałby to odczyt i zapis, czyli wyścig dwóch okien kopiujących naraz.
Wpis wrażliwy jest wierszem jak każdy inny, różni się polityką, nie schematem: nie wchodzi do eksportu i wygasa wedle zasady retencji. Rdzeń nie szyfruje go osobno, ponieważ baza rdzenia jest tym samym magazynem co reszta stanu, a osobne szyfrowanie jednej kolumny dawałoby poczucie ochrony bez ochrony.

## budowa/server/internal/store/migracja_293_skroty_tekstowe.sql
Skrót rozwija się we wszystkich polach tekstowych platformy, a nie w jednym oknie, dlatego słownik należy do rdzenia — gdyby mieszkał w kliencie, ta sama fraza rozwijałaby się inaczej na dwóch maszynach tego samego operatora.
Para profilu i skrótu jest kluczem: jeden profil asystenta nie może mieć dwóch rozwinięć tego samego skrótu, bo wtedy rozwinięcie rozstrzygałby przypadek kolejności odczytu. Profil pusty, oznaczający skrót wspólny, zapisuje się jako pusty napis, nie jako brak wartości, ponieważ brak wartości nie jest równy sobie, więc warunek jedyności przepuściłby dowolną liczbę duplikatów skrótu bez profilu.
Pola szablonu idą zapisem strukturalnym w jednej kolumnie, ponieważ są wykazem nazw bez własnych atrybutów, a wypełnia je okno przy rozwinięciu.

## budowa/server/internal/store/migracja_042_developer.sql
Bez miejsca, w którym wersja pliku naprawdę powstaje, pole żądania wersjonowania i pole identyfikatora wersji byłyby atrapą — rdzeń oddawałby identyfikator wersji, której nikt nigdzie nie zapisał. Wersja jest migawką treści sprzed zapisu: pozwala Operatorowi wrócić do stanu, który sam nadpisał, i odróżnia zapis wersjonowany od zwykłego nadpisania pliku na dysku. Wersja nie jest commitem i go nie zastępuje — historia repozytorium należy do systemu kontroli wersji, a wersja jest zapisem roboczym edytora, który powstaje przed zatwierdzeniem, często wielokrotnie w obrębie jednej zmiany, i ginie z projektem, a nie z gałęzią. Dziennik budowań jest dziennikiem, nie stanem żywym: przebieg czynny prowadzi rdzeń w pamięci, bo tylko on ma uchwyt do drzewa procesu i tylko on potrafi je przerwać, a tabela niesie ślad po przebiegu — zadanie, kod wyjścia, czasy i ogon dziennika. Kolumna dziennika trzyma ogon przebiegu, nie całość: dziennik na żywo jedzie osobnym zdarzeniem, a kolumna służy wyłącznie temu, żeby przebieg zamknięty przed połączeniem klienta dało się jeszcze pokazać, a przycinanie ogona robi rdzeń, bo baza nie zna rozmiaru.

## budowa/server/internal/store/migracja_043_diagnostyka.sql
Dziennik ma wiersze, a nie bufor w pamięci, żeby podgląd dziennika pokazywał także to, co działo się przed restartem rdzenia, bo bufor pamięci ginie razem z procesem. Czas jest liczbą, nie napisem daty: kontrakt niesie czas wpisu, zakres zapytania i czasy błędu w milisekundach epoki, a kolumna liczbowa przenosi dokładnie wartość kontraktu, bez przekładu w obie strony przy każdym porównaniu zakresu. Inaczej panel błędów pokazywałby wyłącznie ostatnią minutę pracy i gubiłby błąd rzadki, lecz istotny. Kod błędu kontraktu ma własną kolumnę, mimo że struktura kontraktu go osobno nie niesie: kolumna jest źródłem, z którego kod wychodzi do klienta polem kontekstu, dzięki czemu da się po nim także zawęzić zapytanie, czego samo pole złożone by nie dało.
## budowa/server/internal/store/migracja_294_pamiec_konteksty.sql
Kontekst jest zestawem wskazań, nie właścicielem treści: wskazuje poziomy pamięci, wpisy pamięci wprost oraz warstwę promptu systemowego. Dlatego usunięcie kontekstu nie kasuje ani jednego wpisu pamięci, tylko kasuje wskazanie. Poziomy i wpisy idą zapisem strukturalnym w osobnych kolumnach, ponieważ są wykazami identyfikatorów bez własnych atrybutów, a wpis pamięci ma własną tabelę i własne komendy, z którymi kontekst nie ma prawa się rozjechać przez klucz obcy kasujący kaskadowo.
Kontekst czynny karty sesji jest wierszem osobnej tabeli, a nie kolumną w kontekście, ponieważ ten sam kontekst bywa czynny w kilku kartach naraz, a kolumna aktywności w kontekście pozwoliłaby na jedną kartę.
Zasada retencji obowiązuje zapisy kolejne i nie rusza wstecz wpisów zastanych, zgodnie z kontraktem. Schemat tego nie wymusza, to rozstrzygnięcie adaptera, ale kolumna aktualizacji daje granicę, od której zasada obowiązuje, więc wygaszanie ma po czym odróżnić jedno od drugiego.
Trójka złożona z zasięgu, bytu zasięgu i profilu jest kluczem zasady, ponieważ dwie zasady o tym samym zasięgu byłyby dwiema odpowiedziami na jedno pytanie. Byt zasięgu i profil zapisują się pustym napisem, nie brakiem wartości, z tego samego powodu co przy skrótach tekstowych.

## budowa/server/internal/store/migracja_295_nagrania_mowy.sql
Brak, który ta tabela zamyka, jest platformowy, nie okienny: transkrypcja bierze ścieżkę pliku na maszynie silnika, a nagranie z mikrofonu istnieje wyłącznie jako bajty w pamięci karty. Dopóki nie ma dokąd tych bajtów odłożyć, ani dyktowanie w oknie komunikacji, ani mikrofon modułu głosowego nie mają czym dojechać do silnika.
Bajty nie leżą w bazie. Kolumna ścieżki niesie bezwzględną ścieżkę pliku pod katalogiem danych rdzenia, dokładnie taką, jaką przyjmuje transkrypcja i jaką oddaje synteza mowy. Dzięki temu odnośnik nagrania jest jednym rodzajem odnośnika w całym produkcie, a nie dwoma. Nagranie nie opuszcza maszyny rdzenia: droga sieciowa prowadzi tu i z powrotem, nigdzie indziej.
Ten sam wiersz obsługuje odsłuch: wykaz odnośników wydanych przez rdzeń jest tym, co pozwala oddać bajty, nie otwierając przy okazji drogi do czytania dowolnego pliku dysku.
Kolumna wygaśnięcia jest chwilą, po której nagranie przestaje być potrzebne. Wartość pusta znaczy nagranie trwałe, gdy operator włączył zapis nagrań. Wygaszaniem zajmuje się adapter przy kolejnym przyjęciu, osobnego zegara ta tabela nie potrzebuje.

## budowa/server/internal/store/migracja_296_dziennik_asystenta_wyroznienie.sql
Bez tych dwóch kolumn wykaz aktywności pokazywałby wyróżnienie, którego rdzeń nie pamięta: kontrakt niesie pole ważności wpisu, a tabela dziennika asystenta nie ma go gdzie odłożyć. Okno oznaczyłoby wpis, a po odświeżeniu wykazu oznaczenie znikałoby bez śladu.
Kolumny są dwie, ponieważ kontrakt wnosi dwie rzeczy: sam znacznik oraz powód wyróżnienia zapisywany razem z nim. Powód bez znacznika byłby notatką do wpisu, którego nikt nie wyróżnił; znacznik bez powodu jest w porządku, więc notatka zostaje pusta.
Wartość domyślna oznaczająca brak wyróżnienia zapewnia, że wpisy zastane, założone przed tą migracją, nie stają się nagle ważne.

## budowa/server/internal/store/migracja_046_studio.sql
Repozytorium i historia wersji są jednym bytem, nie dwoma: kontrakt mówi o tej samej liście wersji dokumentu trzema drogami różnymi, więc dwie tabele niosłyby dwie prawdy o jednej historii — prawda jest jedna w tabeli wersji dokumentu, a zapis z żądaniem wersjonowania dopisuje do niej wiersz zamiast zakładać własną. Propozycja zmiany nie jest wersją: propozycja i wersja są w kontrakcie dwoma różnymi rzeczami porównywalnymi z tym samym punktem odniesienia, a propozycja jest wynikiem operacji kontekstowej, zanim Operator zdecyduje, czy trafi do repozytorium — commitowanie jej robi zapis dokumentu, nie sama operacja, dlatego ma własną tabelę osobną od wersji. Treść dzieli się na kolumnę treści krótkiej obok kolumny odwołania: treść krótka zostaje w bazie, treść obszerna trafia do pliku, a baza trzyma tylko odwołanie, bo dokument bywa plikiem odczytanym z repozytorium biblioteki. Okno i plik repozytorium są kolumnami tekstowymi bez więzu obcego: okno jest identyfikatorem kontraktu okna modułu, nie wiersza okna komunikacji, a plik repozytorium wskazuje plik modułu biblioteki budowanego równolegle, osobnego modułu — więz obcy do niego wiązałby kolejność migracji, której ta migracja nie kontroluje.
## budowa/server/internal/store/migracja_306_studio_wsad.sql
Wsad ma tabelę, a nie samą odpowiedź, ponieważ kontrakt oddaje identyfikator przebiegu — identyfikator, za którym nic nie stoi, byłby napisem bez znaczenia: operator nie miałby czym zapytać, co się z jego dokumentami stało, a okno przebiegu wykonania nie miałoby czego pokazać. Przebieg jest więc bytem trwałym, tak samo jak przebieg automatyki.
Pozycja jest osobnym wierszem, ponieważ odmowa jednego dokumentu nie ma wstrzymywać pozostałych. Wynik wsadu jest więc wykazem rozstrzygnięć na dokument, nie jednym stanem całości: wiersz na dokument niesie stan i powód odmowy własnymi słowami, a suma tych wierszy daje liczby przyjętych i odrzuconych pozycji.

## budowa/server/internal/store/migracja_047_przegladarka.sql
Migawka nie jest osobnym bytem odwiedzonej strony: druga tabela odwiedzonych stron niosłaby te same fakty, adres, tytuł i czas, drugi raz. Treść obszerna trafia do pliku, baza trzyma odwołanie, bo migawka strony bywa duża — pełny HTML albo długi tekst renderowany; kolumny odwołań niosą odwołanie do pliku, nie treść wprost, a odwołanie do zrzutu ekranu z kontraktu jest już odwołaniem po stronie klienta i zapisuje się bez przekształcenia. Okno jest kolumną tekstową, nie więzem obcym, bo okno tego modułu jest oknem operacyjnym, nie oknem komunikacji. Źródło tego modułu nie jest źródłem modułu badań, choć kontrakt niesie oba pod podobną nazwą: kształty się rozjeżdżają, źródło badania ocenia wiarygodność zasobu badawczego niekoniecznie strony sieciowej, a źródło przeglądania jest odciskiem strony, więc inny kształt i inny cykl życia dają osobną tabelę, nie współdzielenie tabeli źródeł badania. Identyfikator zewnętrzny źródła w notatce jest wartością tekstową, nie więzem obcym, bo jest identyfikatorem opcjonalnym kontraktu.

## budowa/server/internal/store/migracja_050_asystent.sql
Zlecenie asystenta nie jest pozycją silnika kolejek: kolejka obsługuje pętlę sesyjną, jej wiersz wisi na sesji i oknie komunikacji, a stan biegnie po ścieżce pracy wykonawcy nad kartą repozytorium, natomiast zlecenie asystenta wisi na oknie modułu, nie na sesji, ma własną ścieżkę stanu kontraktu oraz pola bez odpowiednika w kolejce, jak para liczb kroku bieżącego wobec łącznego i priorytet sterowalny z panelu akcji. Zlecenie asystenta nie jest też pozycją katalogu akcji: katalog akcji to spis dostępnych czynności, czynny niezależnie od użycia, a zlecenie asystenta jest wykonywaną instancją jednej rozmowy z Operatorem, z historią etapów i wynikiem — katalog odpowiada na pytanie co można zrobić, zlecenie na pytanie co się właśnie dzieje, więc to osobne pytania i osobne tabele. Czas jest liczbą: kolumny czasu niosą milisekundy epoki, a kolumna liczbowa przenosi wartość kontraktu bez przekładu w obie strony. Dziennik działań jest osobnym bytem od zlecenia, nie widokiem na nie: wpis dziennika obejmuje też wpisy własne asystenta bez zlecenia w tle, więc kolumna kodu zlecenia jest opcjonalna, a nie węzłem, przez który dziennik zawsze musiałby przechodzić. Rozpoznawanie mowy nie mieszka w rdzeniu: polecenie głosowe przyjmuje odnośnik do nagrania albo tekst już poprawiony przez Operatora, więc schemat nie zakłada kolumn na krok, którego nikt nie dostarcza, a treść nagrania zostaje plikiem zewnętrznym, w wierszu dziennika ląduje wyłącznie jego odnośnik.

## budowa/server/internal/store/migracja_052_przekazanie_okna.sql
Bez tej migracji więź koordynatora z oknem wykonawczym oraz treść zlecenia, które ją uzasadnia, żyją wyłącznie w pamięci przeglądarki i giną z jej zamknięciem, więc po restarcie nie ma czego pokazać w podglądzie przekazań. Więź koordynator-wykonawca ma już dom w kolumnie okna komunikacji; ta migracja nie zakłada drugiej tabeli na tę samą więź, tylko dokłada samo zlecenie — polecenie i komplet kontekstu — oraz jego powiązanie z pozycją kolejki, która je niesie. Komplet kontekstu jest zapisem strukturalnym, nie sześcioma tabelami: żadne z jego pól nie jest spisem z własną logiką zapytań, to zamrożony migawkowy odczyt tego, co Operator miał pod ręką w chwili przekazania, przenoszony jedną komendą i nieedytowalny później — tabela na każdy rodzaj odwołania dałaby puste powtórzenia tego samego wzorca bez korzyści zapytania, dlatego komplet kontekstu trafia do jednej kolumny zapisu JSON. Wykonanie akcji zostawia trwały ślad, bo inny dziennik akcji już przyjął zasadę przejrzystości zamiast bramy: akcje katalogu potrafią zmieniać stan okna i sesji, a ten sam problem zniknięcia po zamknięciu przeglądarki dotyczy ich efektu tak samo jak więzi koordynatora, więc dziennik odnotowuje każde wykonanie, nie tylko te uznane za istotne — wybór, co pokazać w interfejsie, należy do warstwy prezentacji.
## budowa/server/internal/store/migracja_316_design_wektor.sql
Ścieżka wektorowa jest bytem, a nie polem warstwy, ponieważ warstwa kompozycji opisuje prostokąt: położenie, rozmiar, zasób, a krzywa Béziera nie mieści się w prostokącie, a operacja logiczna na dwóch krzywych potrzebuje obu z osobna. Bez własnego bytu rysunek piórem nie miałby gdzie zostać.
Węzły idą jednym zapisem złożonym, a nie wierszem na węzeł, ponieważ kontrakt nadsyła komplet węzłów przy każdej zmianie: zmiana jednego węzła idzie tą samą drogą co narysowanie ścieżki. Wiersz na węzeł dawałby drugi zapis tego samego kształtu, który trzeba by kasować i wstawiać od nowa przy każdym drgnięciu myszy, a żadne zapytanie nie pyta o pojedynczy węzeł.
Warstwa wskazuje się kodem, a nie kluczem obcym, ponieważ zapis planszy przepisuje komplet warstw kompozycji od nowa, więc klucz wiersza warstwy zmienia się przy każdym zapisie planszy. Klucz obcy do warstwy zrywałby się wtedy przy zwykłym zapisie kompozycji, natomiast identyfikator zewnętrzny warstwy przeżywa ten zapis, ponieważ klient nadsyła go z powrotem.
Członkostwo symbolu leży w osobnej tabeli, ponieważ zapis symbolu podmienia komplet członków przy każdym zapisie, a liczba miejsc, do których zmiana doszła, jest liczbą wierszy naprawdę zapisanych, a nie obietnicą.

## budowa/server/internal/store/migracja_317_design_ramki.sql
Ramka jest bytem obok kompozycji, ponieważ kompozycja jest płótnem, a ramka jest ekranem: to ramka, a nie płótno, wyznacza obszar wydania, a jej rozmiar zmienia się przy sprawdzaniu układu na innym urządzeniu. Bez własnego bytu makieta czterech ekranów obok siebie byłaby jedną powierzchnią bez granic.
Przynależność warstwy do ramki leży osobno, ponieważ zapis planszy przepisuje komplet warstw kompozycji od nowa. Kolumna przynależności dołożona wprost do warstwy ginęłaby przy każdym zapisie planszy, a przesunięcie jednej warstwy wywaliłoby całą makietę z jej ramek. Przynależność wiąże się więc z identyfikatorem zewnętrznym warstwy, który klient nadsyła z powrotem i który ten zapis przeżywa. Warstwa należy najwyżej do jednej ramki, stąd warunek jedyności na samym kodzie warstwy, a nie na parze — warstwa w dwóch ramkach naraz musiałaby przy przeliczeniu przyjąć dwa różne położenia.

## budowa/server/internal/store/migracja_318_design_komponenty.sql
Komponent wisi na oknie, a nie na kompozycji, ponieważ biblioteka interfejsu jest wspólna dla wszystkich plansz stanowiska: przycisk zdefiniowany przy makiecie ekranu logowania ma dać się wstawić na planszę ekranu ustawień. Komponent przypięty do kompozycji byłby biblioteką na jedną planszę, czyli nie byłby biblioteką.
Instancja ma wiersz, ponieważ liczba instancji i liczba miejsc propagacji są liczbami mierzonymi, nie deklarowanymi: bez wiersza na instancję rdzeń mógłby oddać co najwyżej zero albo zmyślenie. Instancja wskazuje warstwę identyfikatorem zewnętrznym z tego samego powodu co przynależność do ramki: zapis planszy przepisuje warstwy od nowa i klucz wiersza warstwy nie przeżywa zapisu planszy.
Warianty idą jednym zapisem złożonym, ponieważ kontrakt nadsyła ich komplet przy każdym zapisie komponentu, a żadne zapytanie nie pyta o pojedynczy wariant w oderwaniu od komponentu.

## budowa/server/internal/store/migracja_319_design_prototyp.sql
Połączenie ma wiersz, a nie pole ramki, ponieważ z jednej ramki wychodzi tyle przejść, ile jest na niej elementów klikalnych, a do jednej ramki wchodzi ich dowolnie wiele; pole przy ramce wyraziłoby jedno wyjście i milczałoby o reszcie.
Ramki wskazuje się kodem, ponieważ kontrakt nazywa ramki identyfikatorami zewnętrznymi i tymi samymi wartościami wraca odczyt grafu. Klucz obcy wymagałby przekładu w obie strony przy każdym odczycie grafu, a graf czyta się w całości, więc przekład byłby robotą bez odbiorcy. Spójność pilnuje adapter: obie ramki muszą leżeć w tej samej kompozycji co połączenie, co jest sprawdzane przed zapisem.
Odczyt grafu oddaje ponadto ramki, do których nie wchodzi żadne połączenie. Liczy je odczyt, nie zapis, ponieważ ramka osierocona dziś bywa jutro ramką początkową, więc utrwalanie tej cechy w kolumnie znaczyłoby przechowywanie wniosku zamiast faktów.

## budowa/server/internal/store/migracja_053_tlumaczenie.sql
Segment nie jest bytem trwałym: kontrakt oddaje zwykłą listę napisów bez identyfikatora i bez pola, po którym dałoby się go zaktualizować pojedynczo, więc tabela segmentów byłaby bytem, którego nikt nie odczytuje po identyfikatorze i nikt nie aktualizuje punktowo. Segmentacja jest funkcją tekstu źródłowego w chwili wywołania: okno tłumaczenia już go trzyma, więc segmentacja dzieli go w locie i zwraca wynik bez zapisu, a liczba segmentów w oknie jest jedyną trwałą pamięcią wyniku ostatniego podziału. Tłumaczenie zwrotne mieszka jako kolumna panelu, nie osobna tabela: kontrakt oddaje jeden tekst na panel, bez historii wielu przebiegów i bez własnego stanu, a panel jest już bytem jeden wiersz na okno i język docelowy, więc tłumaczenie zwrotne to kolejne pole tego samego wiersza. Kolumna tłumaczenia zwrotnego jest nadpisywana przy każdym przebiegu, bo rdzeń nie ma silnika przekładu, więc trzyma wyłącznie to, co realnie dostał. Rdzeń nie tłumaczy i nie rozpoznaje języka: zapis tłumaczenia niesie wyłącznie korektę Operatora, a rozpoznanie źródła nie ma na czym oprzeć się samodzielnie, więc kolumna języka źródłowego zostaje pusta, dopóki Operator albo zapis źródła jej nie poda wprost. Przebieg tłumaczenia zwrotnego bez tekstu panelu nie ma co tłumaczyć z powrotem, więc treść zwrotna zostaje wtedy pusta, a nie zmyślona. Czas jest liczbą milisekund epoki, tak jak w reszcie modułu.

## budowa/server/internal/store/migracja_054_slownik_tlumaczenia.sql
Wystąpienia terminu nie mają tabeli: wystąpienie to fragment tekstu w otoczeniu terminu, odtwarzalny z tekstu okna i treści panelu przeszukanych względem źródła terminu, a osobne wiersze trzeba by unieważniać przy każdej korekcie tłumaczenia panelu, więc warstwa danych liczy je w locie. Import i eksport słownika mają dwie wąskie tabele śladu zamiast jednej wspólnej, bo to różne kierunki i różne kolumny wyniku; liczba przetworzonych terminów jest jedynym faktem o wyniku, którego nie da się odtworzyć bez powtórzenia operacji. Pamięć tłumaczeń jest tabelą gromadzoną z zatwierdzonych par segmentów, nie odczytem po panelach: dopasowanie przybliżenia segmentu źródłowego działa względem wszystkich języków i paneli naraz, a podpowiedź ma się opierać na treści zatwierdzonej, nie na bieżącej treści panelu w trakcie edycji, więc warstwa danych dopisuje wpis dopiero, gdy panel przechodzi w stan gotowości.
## budowa/server/internal/store/migracja_326_design_ikony_gradienty.sql
Katalog ikon otwartoźródłowych nie ma tu tabeli: katalog jest wniesiony do rdzenia jako zasób wkompilowany w binarium, a nie ściągany ani zasiewany do bazy, co daje to samo źródło i tę samą treść na każdym stanowisku bez kroku zasiewu, który mógłby się nie wykonać. Baza trzyma wyłącznie ikony własne, których rdzeń nie ma skąd odtworzyć. Paleta, wynik kontrastu, przeliczenie barwy i podgląd kroju nie mają tabel, ponieważ nie są bytami, tylko rachunkiem z danych żądania — zapisywanie wyniku rachunku dawałoby drugą prawdę o barwie obok samej barwy.
Ikona należy do okna modułu, tak jak zasób i kompozycja. Treść w postaci wektorowej leży w kolumnie, nie w magazynie bloków, ponieważ ikona jest tekstem rzędu kilkuset bajtów, a zestaw symboli składa się z wielu ikon naraz — czytanie ich z osobnych plików byłoby tyloma otwarciami pliku, ile symboli w pakiecie. Pole zestawu odróżnia ikony własne okna od ikon przyniesionych z katalogu rdzenia.
Klucz gradientu jest trójką złożoną z kompozycji, ścieżki i warstwy. Wskazanie pominięte zapisuje się pustym napisem, nie brakiem wartości, ponieważ silnik bazy uznaje dwa braki wartości za różne w warunku jedyności, więc gradient całej kompozycji zakładany dwa razy powstałby dwa razy zamiast nadpisać się raz.

## budowa/server/internal/store/migracja_055_jakosc_i_mowa.sql
Ton panelu nie ma własnej tabeli, bo zmienia jedno pole jednego wiersza panelu i nie ma własnej tożsamości ani cyklu życia odrębnego od panelu. Niezgodności kontroli jakości są wymieniane w całości przy każdej kontroli, nie dopisywane: kontrakt niesie płaski wykaz zastrzeżeń wobec aktualnego stanu panelu bez pola przyrostowego, więc kontrola liczy niezgodności na nowo z treści panelu w chwili wywołania i zastępuje poprzedni wykaz tej samej kontroli — każda kontrola jest więc migawką bieżącą, nie strumieniem zdarzeń. Synteza mowy zostawia ślad trwały, bo odpowiedź syntezy oddaje ścieżkę, po którą kontrakt każe wrócić przy kolejnym zapytaniu o ten sam panel; rdzeń mowy nie syntezuje i nie ma magazynu blobów, więc kolumny na sam dźwięk nie ma. Eksport panelu zostawia ślad z tego samego powodu: zapis żądania eksportu niesie odwołanie do pliku, jeśli je dostał, ale pliku sam nie wytwarza, a kolumna formatu jest własna, bo bez niej ślad nie odpowiada na pytanie, w jakim kształcie panel eksportowano ostatnio.
## budowa/server/internal/store/migracja_336_design_szablony_materialu.sql
Szablon materiału nie jest szablonem promptu: tamten jest gotowym poleceniem dla silnika obrazu, ten jest gotowym układem — rozmiarem materiału i kompletem warstw, z którego zastosowanie szablonu zakłada kompozycję. Wspólna tabela wymagałaby kolumn pustych po obu stronach, ponieważ prompt nie ma szerokości, a układ nie ma tematu.
Warstwy leżą osobno, tak samo jak warstwy kompozycji: szablon bez warstw jest stanem poprawnym, niosącym sam rozmiar materiału, a zapis podmienia komplet warstw naraz. Wskazanie zasobu jest tekstem, nie więzem obcym, z tego samego powodu co w warstwie kompozycji: szablon przeżywa usunięcie zasobu z panelu zasobów, a wyrys pomija warstwę bez bajtów zamiast odmawiać całości.

## budowa/server/internal/store/migracja_337_design_profile_druku.sql
Profil jest bytem okna, nie parametrem jednego wywołania: ta sama drukarnia dostaje od operatora te same nastawy przez cały rok, a przepisywanie ich przy każdym wydaniu jest drogą do pliku wydanego bez spadu. Kontrola przeddrukowa i wydanie czytają stąd jedne nastawy, więc nie ma jak się rozjechać to, wobec czego mierzymy, z tym, co wydajemy.
Nastawy podane wprost w żądaniu nie mają tu wiersza, ponieważ są jednorazowe z zamysłu i nie mają prawa zostać po sobie w oknie.
Rozdzielczość i spad dopuszczają brak wartości: brak wskazania znaczy przyjęcie nastaw domyślnych rdzenia, a zapisane zero znaczyłoby druk bez spadu, co jest innym rozstrzygnięciem i innym wynikiem w drukarni.

## budowa/server/internal/store/migracja_056_wejscie_automations.sql
Boczna nawigacja bierze wykaz pozycji wyłącznie z macierzy środowisko-moduł, więc drogę wejścia do modułu dokłada się wierszem w macierzy, nie literałem w kliencie. Jedno środowisko jest środowiskiem procesów i automatyzacji, drugie traktuje harmonogramy i kolejki uruchomień jako narzędzie pracy nad kodem tak samo jak terminal; trzecie środowisko zostaje bez tego modułu, bo jest środowiskiem pracy z treścią. Zapis pomija konflikt, żeby migracja przechodziła także na bazie, w której wiersz już założono.

## budowa/server/internal/store/migracja_057_sieroty_transportu.sql
Tabela-sierota to tabela, do której nikt nie pisze i z której nikt nie czyta: w całym drzewie nie ma ani jednego zdania SQL nazywającego którąkolwiek z tych dwóch tabel, a schemat, który je trzyma, obiecuje warstwie wyżej nośnik danych, którego nie ma. Tabela połączeń nie dostaje pisarza, tylko odchodzi: telemetria połączeń jest już trwała pod innym adresem w dzienniku rdzenia, kolumna identyfikatora urządzenia jest obowiązkowa, choć połączenie rejestruje się zanim przyjdzie powitanie niosące inne dane, a zapis do bazy wprost z transportu odebrałby pakietowi transportu jego niezależność od rdzenia i trwałości — stan czynnych połączeń prowadzi rejestr w pamięci, który ginie razem z procesem, tak jak ginie samo połączenie. Tabela procesów sesji odchodzi, bo rejestr procesów żyje w pamięci i tam jest prawdziwy: proces okna to proces systemowy objęty uchwytem, ginący razem z rdzeniem, a wiersz, który przeżyłby restart ze stanem pracuje, opisywałby proces nieistniejący — historia, która ma być trwała, ma swoje miejsce w przebiegach automatyk. Żadna tabela nie wskazuje kluczem obcym na te dwie tabele, więc usunięcie nie pociąga kaskad ani nie zostawia wiszących więzów, a obie tabele są puste, więc danych nie tracimy.

## budowa/server/internal/store/migracja_058_motto_srodowiska.sql
Kolumna motta stoi osobno od kolumny opisu, bo to dwa różne zdania o środowisku, drukowane w dwóch różnych miejscach: opis jest jednozdaniowym opisem trybu pracy na karcie wejścia strony głównej, a motto podtytułem w nagłówku kolumny matrycy centrum dowodzenia, łamanym do jednego wiersza obok nazwy — jedna kolumna na oba fakty zmusiłaby jedną ze stron do skracania cudzego zdania w locie. Kolumna dopuszcza brak wartości i nie ma wartości domyślnej: środowisko, dla którego hasła nie ustalono, stoi z wartością pustą, odróżnialną od napisu pustego, który znaczy motta nie ma. Dopasowanie idzie po kodzie, nie po nazwie, bo kod jest jedyną kolumną z warunkiem unikalności na zamkniętym zbiorze czterech wartości, więc zmiana trafia w dokładnie jeden wiersz albo w żaden. Zmiana struktury w bazie tego rodzaju przepisuje sam nagłówek schematu, nie tabelę, więc kolumna dopuszczająca brak wartości nie dotyka ani jednego wiersza danych, a zmiana struktury i wypełnienie treści idą w jednej transakcji.
## budowa/server/internal/store/migracja_338_design_licencje_zasobow.sql
Licencja stoi w tabeli obok, a nie kolumną w tabeli zasobów, ponieważ dotyczy wyłącznie zasobów przyniesionych z zewnątrz, a te są mniejszością wśród zasobów okna. Kolumna w tabeli zasobów byłaby pusta przy każdym zasobie wygenerowanym i wniesionym, a każdy odczyt zasobu ciągnąłby ją bez potrzeby.
Zapis licencji jest częścią wciągnięcia zasobu, nie dodatkiem po nim, ponieważ zasób z katalogu zewnętrznego bez zapisanej licencji jest materiałem, o którym za pół roku nikt nie powie, czy wolno go było użyć w kampanii. Więz obcy z kasowaniem kaskadowym odzwierciedla, że licencja bez zasobu nie opisuje niczego.

## budowa/server/internal/store/migracja_339_design_strony_szablonu.sql
Publikacja wielostronicowa nie ma osobnej rodziny komend, ponieważ wybrano szablon materiału o wielu stronach zamiast nowej rodziny komend albo montażu arkusza. Publikacja jest więc szablonem, który ma strony, i wychodzi tą samą drogą co każdy inny materiał, przez tę samą kontrolę przeddrukową i to samo wydanie. Druga rodzina komend znaczyłaby drugą kontrolę przeddrukową i drugie wydanie, a te dwa musiałyby się potem zgadzać.
Strona jest bytem, a warstwa należy do strony: szablon jednostronicowy zachowuje warstwy w dotychczasowej tabeli warstw szablonu, ponieważ baner nie ma stron. Publikacja dokłada wiersze stron, a warstwa strony wskazuje stronę kluczem obcym; szablon bez ani jednej strony jest szablonem jednostronicowym i jego warstwy leżą tam, gdzie leżały.
Numer strony jest jawną kolumną, nie wynika z klucza wiersza, ponieważ kolejność stron publikacji się zmienia, a klucz wiersza nie. Unikat na parze złożonej z szablonu i numeru zapobiega temu, by dwie strony o tym samym numerze dały przy wydaniu kolejność zależną od porządku odczytu.

## budowa/server/internal/store/migracja_340_okna_warsztatow_designu.sql
Komenda bez okna jest funkcją, której użytkownik nie ma. Moduł Design dostał wiele nowych komend w obszarach wektora, makiety, barwy, ikon, marketingu, druku, fotografii i publikacji, a bez wierszy w katalogu klient postawiłby okna, o których rdzeń nie wie, co zaniżałoby zgłaszany zakres modułu i prowadziło do meldowania kodu poza katalogiem.
Warsztatów jest sześć, nie pięć, ponieważ makieta dostała własne okno: ramki, więzy, komponenty i prototyp są innym rodzajem pracy niż rysowanie i biorą inny materiał. Wciśnięcie ich w warsztat wektora dałoby okno o zbyt wielu czynnościach do przejrzenia.
Warsztat fotografii i warsztat wektora prowadzą pracę na materiale, więc rola wiodąca im nie przysługuje, bo wiodącym modułu jest tablica projektowa, gdzie stoi praca koncepcyjna; nie są też tylko pomocnicze, ponieważ pracują na treści, ale katalog ról nie zna osobnej roli narzędziowej, więc idą jako pomocnicze, wspierając tablicę projektową materiałem, który jej podają. Przeglądarka baz zdjęciowych ma rolę zarządcy, ponieważ jej praca to wybór z wykazu, jak w panelu zasobów. Warsztat druku i warsztat publikacji są pomocnicze, ponieważ wydają to, co już powstało.
Kolejność w module idzie po pozycjach już zajętych przez istniejące okna modułu oraz po panelu tokenów i systemu, który wszedł osobno.

## budowa/server/internal/store/migracja_059_komponenty_strony_glownej.sql
Tabela nie powiela bytu modułowego: nie ma tu kroków automatyki, umiejętności eksperta ani pamięci projektu, bo prawdą o bycie modułowym pozostaje jego własna tabela i jego własne komendy. Rodzina komend komponentu nie składa się jako czysta fasada nad tabelami modułowymi: jedna z wartości rodzaju nie odpowiada żadnej tabeli modułowej wprost, konfiguracja przekazywana adapterowi nie ma kolumny w żadnej tabeli modułowej bez konfliktu znaczeń, przypisanie niesie parę poziomu zasięgu i identyfikatora bytu poziomu, której żadna tabela modułowa nie ma, a identyfikator komponentu trzeba by składać z pary rodzaju i bytu docelowego, sumując niejednorodne zapytania bez wspólnego porządku wyświetlania — stąd własny magazyn. Kolumna bytu docelowego jest tekstem bez więzu obcego, bo wskazuje na cztery różne tabele zależnie od kolumny rodzaju, a baza nie zna więzu warunkowego. Zasięg przypisania idzie zastanym słownikiem poziomów zasięgu, a nie własnym wyliczeniem, bo poziomy zasięgu są jedne na całą platformę; para kolumn poziomu i klucza zasięgu powtarza wzorzec tabeli ustawień i obie zostają puste, dopóki nie padnie przypisanie. Kolumna aktywności celowo nie wchodzi do indeksu wykazu: stałaby między kolumną zawężenia a kolumnami porządku i zepsułaby porządek, więc przełącznik dołączenia nieaktywnych jest odsiewem na odczytanych wierszach.
## budowa/server/internal/store/migracja_346_design_fotografia_czynnosci.sql
Łańcuch nie potrzebuje nowego bytu na zasób, ponieważ wariant zasobu już istnieje: każda obróbka zakłada wariant źródła i oryginał zostaje nietknięty, więc łańcuch edycji da się przejść samymi zasobami. Czego zasoby nie niosą, to czynność, która wariant wytworzyła, i jej nastawy — bez nich odczyt historii pokazywałby wykaz obrazków bez słowa o tym, co je od siebie różni, a zapis nastaw nie miałby skąd wziąć powtarzalnego zestawu.
Nastawy jadą jednym zapisem złożonym, ponieważ różnią się między czynnościami: kadr niesie prostokąt, korekcja barwy dziewięć suwaków, filtr nazwę i siłę. Kolumna na każde pole każdej czynności dałaby tabelę z wieloma kolumnami przeważnie pustymi przy każdym wierszu, a żadne zapytanie nie pyta o pojedynczą nastawę, tylko czyta się je kompletem, tym samym, którym przyszły w żądaniu.
Droga rachunku jest zapisana, nie odtwarzana: część czynności ma wariant neuronowy i wariant rachunkowy, a ich wyniki różnią się jakością. Kolumna wskazująca sposób obliczenia trzyma to, co odpowiedź podała w tym polu — bez niej po czasie nie dałoby się powiedzieć, czy dany wariant wyszedł z kanału modelu, czy z rachunku wkompilowanego, a to jest pierwsze pytanie, gdy wynik zawodzi. Więz obcy z kasowaniem kaskadowym odzwierciedla, że czynność bez zasobu, który z niej powstał, nie opisuje niczego.

## budowa/server/internal/store/migracja_347_design_fotografia_nastawy.sql
Nastawa jest zestawem czynności, nie jedną czynnością: zapisuje się pod nazwą całą drogę obróbki, a wsad puszcza tę samą drogę na wielu zasobach. Czynności leżą więc jednym zapisem złożonym, w kolejności wykonania, ponieważ kolejność jest treścią nastawy, a nie jej ozdobą.
Nastawa należy do okna, nie do zasobu, ponieważ jest sposobem pracy, a nie cechą zdjęcia: zapisuje się raz i stosuje do wszystkiego, co przez to okno przechodzi. Stąd unikat na parze złożonej z okna i nazwy — dwie nastawy o tej samej nazwie w jednym oknie byłyby dwiema odpowiedziami na jedno wskazanie.

## budowa/server/internal/store/migracja_060_profile_izolacji.sql
Wartości jedenastu punktów izolacji leżą w tabeli ustawień pod adresem złożonym z poziomu zasięgu i osi; ta migracja dokłada trzy rzeczy, których w tym układzie nie ma: profil jako nazwany zestaw przełączników zapisywany raz i przypisywany wielokrotnie, przypisanie profilu do poziomu jako odpowiedź na pytanie, z którego profilu pochodzi obowiązująca polityka, oraz wybór warstwy izolacji. Profil nie jest drugim miejscem wartości izolacji: wartość obowiązująca leży wyłącznie w tabeli ustawień i tylko tam czyta ją rozstrzygacz, a profil jest szablonem, którego przypisanie przepisuje przełączniki do tabeli ustawień pod wskazany adres — profil jako drugie źródło prawdy zmuszałby egzekutora do czytania dwóch miejsc. Przełącznik jest wierszem, nie kolumną: tabela niesie parę klucza i wartości dokładnie w postaci, w jakiej czyta je rozstrzygacz, więc przypisanie profilu jest przepisaniem wiersza jeden do jednego, a nowy punkt izolacji wymaga wiersza, nie zmiany schematu. Profil bez kompletu punktów jest poprawny, bo kontrakt oznacza oba zestawy przełączników jako nieobowiązkowe: punkt nieujęty w profilu nie jest przy przypisaniu ruszany. Przypisanie jest jedno na adres, bo pytanie o źródło polityki danego bytu ma dokładnie jedną odpowiedź, a kolejne przypisanie pod ten sam adres zastępuje poprzednie. Warstwy nie ma w adresie zapisu ustawienia i ta migracja jej tam nie wnosi: kolumna warstwy w tabeli ustawień zbudowałaby drugi wymiar adresu, którego rozstrzygacz nie czyta, więc wartość zapisana na warstwie sesyjnej nie doszłaby do egzekutora, a zapis zostałby potwierdzony bez skutku — tabela warstwy izolacji trzyma więc wyłącznie wybór, którą warstwę pokazuje i zapisuje panel izolacji dla danej karty sesji albo okna. Usunięcie profilu zabiera jego przełączniki i przypisania, ale nie cofa wartości już przepisanych do tabeli ustawień, bo te są odtąd wartościami poziomu, nie własnością profilu.

## budowa/server/internal/store/migracja_065_powiazania_kolejki.sql
Sam schemat kolejki znał dotąd jedno wiązanie poza sesją, kolumnę wskazującą jedno okno komunikacji koordynatora, a kontrakt niesie wykaz okien w liczbie mnogiej i trzy dalsze rodzaje bytu, dla których w tabeli kolejki nie było kolumny. Wiązania niesie jedna tabela, a nie cztery kolumny w tabeli kolejki, bo kolumny obsłużyłyby po jednym bycie każdego rodzaju, a kontrakt niesie wykaz — tabela wiążąca obsługuje wszystkie cztery rodzaje jednym kształtem i nie wymaga zmiany schematu przy piątym. Byt jest tekstem, a nie kluczem obcym, bo kontrakt niesie identyfikatory zewnętrzne różnego rodzaju dla każdego bytu, a cztery klucze obce wymagałyby czterech rozwiązań identyfikatora w chwili zapisu i odmawiałyby powiązania z bytem, który nie ma jeszcze wiersza. Kasowanie kolejki zabiera powiązania kaskadą, bo to one wiszą na kolejce, nie odwrotnie, a kolumna okna koordynatora w tabeli kolejki zostaje nietknięta, bo opisuje okno koordynatora pętli, nie okna obsługiwane przez kolejkę.
## budowa/server/internal/store/migracja_363_studio_blokady_fragmentow.sql
Blokada obowiązuje w rdzeniu, nie w oknie, ponieważ model woła komendy rdzenia tak samo jak klient, więc blokada pilnowana wyłącznie przez okno jest pozorna i dałoby się ją ominąć bez wysiłku. Sprawdzenie stoi na drodze każdej komendy zmieniającej dokument, przed dotknięciem treści, i musi mieć skąd czytać zakresy, stąd ten wiersz.
Zasięg jest kolumną, nie założeniem, ponieważ blokada jest skierowana przeciw modelowi, nie przeciw właścicielowi dokumentu: użytkownik zmienia fragment zablokowany bez przeszkód. Gdy ma działać także na użytkownika, jest to osobne, jawne ustawienie blokady, a nie zachowanie domyślne, dlatego zasięg domyślnie stoi na model.
Blokada przechodzi przez wersje i przez szablon: przywrócenie wcześniejszej wersji nie gubi blokad, ponieważ blokada wisi przy dokumencie, nie przy wersji. Szablon niesie swoje blokady do dokumentów z niego zakładanych, ponieważ fragmenty wzorcowe pisma mają zostać wzorcowe, a znacznik pochodzenia z szablonu pozwala rozpoznać, że użytkownik nie założył ich sam.
Przesunięcie zakresu jest odnotowywane, ponieważ zakres blokady liczony jest w znakach: wpis przed blokadą przesuwa ją w prawo, a gdyby zakres pozostał nietknięty, blokada zaczęłaby po pierwszej edycji chronić nie ten fragment, co miała, i to bez śladu. Licznik przesunięć liczy, ile razy rdzeń zakres przeliczył, dając miarę zaufania do zakresu widoczną w wykazie blokad.

## budowa/server/internal/store/migracja_364_studio_dziennik_czynnosci.sql
Rdzeń nie miał zapisu zmiany już wniesionej do treści: zmiana śledzona cofa się pojedynczo, ale zmiana już wniesiona nie cofała się wcale, ponieważ nie było zapisu, że coś się stało. Ten dziennik jest tym zapisem — każda czynność odkłada wiersz, który da się wycofać.
Wiersz niesie stan sprzed i po zamiast samego opisu, ponieważ cofnięcie ma obejmować postać, nie tylko treść: opis słowny nie wystarcza, trzeba mieć stan, do którego się wraca. Niesie wycinek postaci objęty czynnością, nie cały dokument, ponieważ cofnięcie ma działać nie po kolei — migawka całego dokumentu przy cofnięciu czynności ze środka dziennika zabrałaby ze sobą wszystko, co po niej weszło, co byłoby przywróceniem wersji, a nie cofnięciem czynności.
Zależności są osobną tabelą, ponieważ cofnięcie czynności będącej podstawą późniejszej ma odmówić i nazwać zależność, a nie zostawić dokument w stanie niespójnym. Zależność jest relacją wiele do wielu, a wykaz kodów w kolumnie nie pozwoliłby zapytać, co stoi na danej czynności, bez przeszukania wszystkich wierszy.
Kolejność jest osobną kolumną, nie kluczem wiersza, ponieważ ponowienie czynności cofniętej nie zakłada nowego wpisu, tylko przestawia stan zastany. Kolejność musi więc zostać ta sama, co przy pierwszym wykonaniu, żeby dziennik dalej opisywał porządek pracy, a nie porządek zapisu.

## budowa/server/internal/store/migracja_070_katalog_rozszerzen.sql
Pozycja katalogu nie jest ani mostem, ani konektorem eksperta: most opisuje wyłącznie maszynę udostępnioną mostem albo katalog lokalny, a rozszerzenie ma cztery rodzaje, z których część mostem zapisać się nie da; konektor eksperta należy zawsze do jednego eksperta, a katalog stoi poziom wyżej i istnieje niezależnie od eksperta, przy czym konektor ma trzy rodzaje, a rozszerzenie cztery, więc jednego rodzaju konektorem nie zapiszesz. Rozszerzenie rodzaju mostowego wskazuje most, a nie powiela go: drugiego rejestru serwerów platforma nie ma, a więz jest miękki, więc skasowanie punktu odłącza pozycję katalogu, lecz jej nie kasuje. Zawartości kolumny źródła deklarowanego rdzeń nie pobiera i nie uruchamia, bo kontrakt zostawia znaczenie instalacji otwarte, a pobieranie i wykonywanie cudzego kodu zmieniłoby klasę bezpieczeństwa produktu — kolumna trzyma napis podany w żądaniu, żeby go nie zgubić, i nie wychodzi w odpowiedzi, bo kształt odpowiedzi takiego pola nie ma. Identyfikatory są dwa, bo kontrakt ma dwa: jeden jest tożsamością wiersza, drugi kodem pozycji stałym między wydaniami, unikalnym, bo dwie pozycje katalogu o jednym kodzie znaczyłyby, że kod nie jest stały. Odinstalowanie nie kasuje wiersza, inaczej zawężenie wykazu do zainstalowanych nie miałoby czego zawężać: zdejmuje flagi instalacji i włączenia, zostawiając pozycję w katalogu razem z konfiguracją, a ponowna instalacja tym samym kodem ją przywraca.
## budowa/server/internal/store/migracja_367_studio_warsztat_szablonow.sql
Rdzeń oddawał wykaz szablonów fabrycznych i zakładał z niego dokument, ale nie było czym szablonu założyć ani zmienić: tabela szablonu niosła nazwę, opis, format, treść i pola, lecz nie niosła postaci, więc szablon nie mógł przenieść arkusza stylów, nastaw strony, nagłówka, stopki, logo ani tabel, choć to jest wprost wymagane.
Zastosowano dobudowę tabeli, a nie drugą tabelę szablonów, ponieważ szablon fabryczny i szablon użytkownika są tym samym bytem widzianym z dwóch stron, co rozstrzyga kolumna oznaczająca szablon fabryczny. Druga tabela znaczyłaby dwa wykazy szablonów, dwie ścieżki zastosowania szablonu i pytanie, który wygrywa przy tej samej nazwie.
Pola zostają w jednej kolumnie zapisu złożonego, a nie idą do własnej tabeli, ponieważ ta kolumna jest już czytana przez warstwę danych i przez zastosowanie szablonu. Wykaz pól jest krótki, czyta się go cały przy wypełnianiu i nikt nie pyta o jedno pole osobno; własna tabela dawałaby drugie miejsce prawdy o polach.
Blokady wzorcowe szablonu leżą w osobnej tabeli blokad szablonu, ponieważ są zakresami znaków, o które pyta się po zakresie, a nie wykazem czytanym w całości.
Warunku sprawdzającego na kolumnie formatu nie rusza się, ponieważ silnik bazy nie umie zmienić takiego warunku bez przepisania tabeli, a przepisanie tabeli szablonów zabrałoby szablony użytkownika założone wcześniej. Szablon wniesiony z pliku zewnętrznego zapisuje się jako jeden z obsługiwanych formatów wedle tego, czym jego treść jest po odczycie, a to, że przyszedł z pliku szablonu, mówi osobna kolumna źródła pliku.

## budowa/server/internal/store/migracja_071_uwierzytelnienie_bramki.sql
Bramka jest bytem osobnym od zastanych tabel: konto niesie poświadczenie do kanału modelu, a sesja kartę pracy istniejącą niezależnie od tego, czy ktokolwiek się zalogował, a Operator jest jeden i bezimienny, bez konta użytkownika ani adresu e-mail w zakresie. Sekretu w bazie nie ma: kolumna odwołania do sekretu niesie odwołanie do wpisu w sejfie poświadczeń, tym samym, którym jadą konta i punkty dostępu, a w sejfie leży skrót hasła, nie hasło; schemat traktuje odwołanie jak nieprzezroczysty napis, bez kolumny na hasło, PIN ani materiał klucza. Urządzenie jest napisem, nie kluczem obcym, bo kontrakt niesie identyfikator urządzenia nadawany przez klienta i nie wymaga, żeby urządzenie stało wcześniej w katalogu urządzeń wypełnianym przez inny zbiór maszyn niż ten, z którego Operator wchodzi. Kotwica jest najwyżej jedna i pilnuje tego baza, nie warstwa wyżej: kotwicą jest hasło bramki ustawiane raz, ono rozstrzyga o trwałej odmowie powtórzenia rejestracji i jest jedyną metodą, której nie można zdjąć. PIN jest właściwy urządzeniu, więc para urządzenia i rodzaju jest jednoznaczna, a ponowne założenie PIN-u na urządzeniu ma być zmianą PIN-u, nie cichym dołożeniem drugiego. Sesja bramki trzyma skrót tokenu, nie token, bo token jest poświadczeniem na okaziciela i zapisany wprost byłby sekretem w bazie; token powstaje ze źródła losowości kryptograficznej, więc do rozpoznania wystarczy skrót bez soli i bez rozciągania. Unieważnienie jest datą, nie skasowaniem wiersza, bo zmiana hasła musi mieć skąd wziąć liczbę unieważnionych sesji.

## budowa/server/internal/store/migracja_072_macierz_widocznosci.sql
Macierz środowisko-moduł niosła wyłącznie pary widoczne, a para niewidoczna istniała jako brak wiersza — moduł niewidoczny w środowisku i moduł, o którym nie zdecydowano, były w bazie nieodróżnialne. Komplet wierszy czyni z macierzy konfigurację: żeby pokazać moduł w środowisku, wystarczy zmienić jeden bit, a wiersz z widocznością wyłączoną nie jest zakazem i niczego nie blokuje, bo boczna nawigacja bierze tylko pozycje widoczne, więc niewidoczna para daje krótszą listę, a nie pozycję wyszarzoną ani odmowę. Migracja wyłącznie dopisuje brakujące pary, bez żadnej zmiany istniejących, i jest idempotentna także na bazie, w której część par już założono. Jedno ze środowisk zostaje poza macierzą celowo: rodzaj nawigacji środowiska rozstrzyga wyłącznie pustość jego wykazu modułów, więc wiersz w macierzy, nawet niewidoczny, odebrałby tę własność i wymusiłby rozstrzyganie po kodzie środowiska wpisanym w warunek — pełną macierz tworzy więc piętnaście modułów razy trzy środowiska kolumnowe, czterdzieści pięć par. Kolumna kolejności niesie pozycję w bocznej nawigacji, a pozycji niewidocznej w nawigacji nie ma, więc zero jest tu brakiem treści, nie pierwszym miejscem.
## budowa/server/internal/store/migracja_368_studio_pochodzenie_i_wejscie.sql
Pochodzenie jest bytem trwałym, ponieważ fragment wciągnięty ze strony albo z biblioteki niesie zapis, skąd jest: adres, plik, wersja. Bez tego za tydzień nikt nie odtworzy, na czym pismo się opiera; to jest też podstawa pod podobieństwa w panelu redaktora i pod bibliografię. Zakres jest liczony w znakach i przesuwa się razem z treścią, tak samo jak zakres blokady. Fragment usunięty zostawia wiersz o zerowej długości świadomie, ponieważ to, że użytkownik wniósł kiedyś fragment z danego źródła, jest faktem, którego usunięcie tekstu nie unieważnia.
Autozamiana jest zasięgu użytkownika, nie dokumentu, ponieważ skrót zamieniany na znak obowiązuje we wszystkich pismach, a zakładanie go od nowa w każdym dokumencie byłoby pracą bez powodu. Nastawa jest jawna i odwracalna, stąd kolumna czynności, a nie usuwanie wiersza przy wyłączeniu.
Malarz formatów ma wiersz, a nie pamięć procesu, ponieważ kopiuje postać i nanosi ją w innym miejscu, czyli między pobraniem a naniesieniem stoją dwie osobne komendy. Postać trzymana w pamięci procesu przepadłaby przy przeładowaniu rdzenia, a przy pracy modelu przepadłaby jeszcze łatwiej, ponieważ model pobiera postać jednym narzędziem i nanosi ją drugim, być może po kilku innych czynnościach. Wiersz z wygasaniem jest jedynym miejscem, w którym oba wywołania widzą to samo.

## budowa/server/internal/store/migracja_369_studio_tozsamosc_wykonawcy.sql
Rodzaj autora nie wystarcza, ponieważ przy dwóch wykonawcach pracujących naraz nad jednym dokumentem nie da się odróżnić, który z nich co napisał, a przełącznik podświetlający zmiany wykonawców pokazywałby obu jako jednego.
Tożsamość agenta nie jest nową wartością w warunku sprawdzającym, ponieważ w module Agents można założyć dowolnie wielu własnych agentów i później ich powoływać — wyliczenie ich nigdy by nie objęło, a dopisywanie każdego nowego agenta do warunku kazałoby zmieniać schemat przy każdym założonym agencie, czego warunek sprawdzający w bazie nie pozwala zrobić bez przepisania tabeli. Tożsamością jest więc kod agenta w osobnej kolumnie, a rodzaj autora zostaje grubym rozróżnieniem człowiek-wykonawca.
Drugiego pojęcia agenta nie zakłada się, ponieważ platforma identyfikuje wykonawcę dwoma bytami, które już są: agentem głównym i podagentem uruchomionym przez okno. Klucza obcego do tabeli ekspertów tu nie ma celowo: zmiana wniesiona przez agenta ma zostać podpisana także wtedy, gdy ten agent zostanie później usunięty, ponieważ podpis pod pracą ma przetrwać wykonawcę.
Wszystkie kolumny są nieobowiązkowe, ponieważ zmiany, komentarze i adnotacje zapisane przed tą dobudową agenta nie mają wartości w tych kolumnach i mają zostać poprawne. Pusty kod agenta znaczy wykonawcę nienazwanego albo użytkownika, czyli brak wiedzy, a nie twierdzenie.
## budowa/server/internal/store/migracja_371_studio_znaki_specjalne.sql
Migracja nie zakłada nowej tabeli autozamiany znaków, ponieważ ta już stoi wraz z kolumną oznaczającą regułę fabryczną i zasadami wpisanymi wierszami; druga tabela o tej samej roli dałaby dwie prawdy o tym, co wchodzi w miejsce skrótu. Rdzeń czyta więc wykaz z tamtej tabeli, a ta migracja dokłada wyłącznie to, czego tamten wykaz nie niósł.
Znaki ostatnio użyte mają wiersz na znak, nie na użycie, ponieważ pytanie brzmi, czego się używa, a nie ile razy dziś naciśnięto: licznik i czas ostatniego użycia wystarczają, żeby wykaz ułożyć od najbliższego ręce, a jeden wiersz na użycie zasypałby tabelę w godzinę pisania.
Wykaz nie ma przypisania do dokumentu, ponieważ znak użyty wczoraj w jednym piśmie ma być pod ręką również dziś w innym — wykaz przypięty do dokumentu byłby pusty przy każdym nowym piśmie.

## budowa/server/internal/store/migracja_372_katalog_konta_nadawczego.sql
Powód jest jeden i twardy: bez konta nadawczego rejestracja odmawia, a bez rejestracji nie ma jak wejść do produktu. Do tej migracji nastawy nadawcy wchodziły wyłącznie zmiennymi środowiska, więc pomyłka w adresie serwera poczty wymagała zatrzymania rdzenia i wiedzy spoza produktu. Katalog daje drugą drogę wewnątrz okna konfiguracji i nie odbiera pierwszej: zmienne środowiska nadal zasilają start, a zapis w tabeli ustawień je przesłania.
Kategoria jest własna, a nie kategorią bezpieczeństwa, ponieważ konto nadawcze nie jest zgodą ani uwierzytelnianiem, tylko skrzynką, z której platforma pisze listy systemowe — potwierdzenie adresu i drogę odzyskania konta.
To nie jest skrzynka użytkownika: skrzynki użytkownika prowadzi moduł poczty z własnym sejfem poświadczeń, a tu stoi konto nadawcze samej platformy, którym pisze wyłącznie rdzeń.
Nastawy nie wymagają restartu, ponieważ adapter bramki czyta je przy każdym wysłaniu listu tym samym rozstrzygaczem, którym idzie każde inne ustawienie platformy. Dozwolony jest wyłącznie zasięg aplikacji, ponieważ konto nadawcze jest jedno dla całego programu i nie ma czym go zawęzić do okna ani sesji.
## budowa/server/internal/store/migracja_378_dziennik_zmiana_pola.sql
Powód zgłosił wykonawca dziennika Studia: zmiany pól dokumentu odkładały się jako zmiana obiektu, więc nie dało się cofnąć zmiany pola osobno, tylko razem z całą zmianą obiektu. Wyliczenie kontraktu niesie już wartość zmiany pola; bez tej migracji zapis wywracałby się na warunku tabeli, w najgorszym możliwym miejscu, po wykonaniu czynności.
Zastosowano przebudowę tabeli, a nie zmianę w miejscu, ponieważ silnik bazy nie umie poszerzyć warunku sprawdzającego będącego częścią definicji tabeli. Przebudowa idzie wzorem zalecanym przez silnik bazy: nowa tabela, przepisanie wierszy, podmiana nazwy, odtworzenie indeksów.
Wiersze przepisują się wszystkie, wraz z kluczami głównymi, ponieważ tabela zależności czynności wskazuje nimi podstawy czynności; przenumerowanie zerwałoby zależności i cofnięcie przestałoby odmawiać tam, gdzie odmawiać musi. Migracja nie wymaga restartu.

## budowa/server/internal/store/migracja_379_centrum_powiadomien.sql
Encja niesie klasę, wagę, źródło polimorficzne, środowisko prezentacji, sesję pochodzenia, treść, działania, stan, kanał dostarczenia i znacznik czasu.
Nazwa powiadomienia jest w tej bazie zajęta przez kolejkę doręczeń funkcji Mobile: wiersz tamtej tabeli to jedna próba wypchnięcia na urządzenie, z ponowieniami, terminem ważności i dziennikiem doręczeń. To jest inny byt niż rejestr centrum — jeden opisuje zdarzenie, drugie doręczenie tego zdarzenia. Dołożenie nowych kolumn do tamtej tabeli zlepiłoby dwa cykle życia w jeden wiersz, a zmiana jej nazwy ruszyłaby działający silnik kolejki. Rejestr dostaje więc własną tabelę, a kolumna kanału dostarczenia mówi, czy zdarzenie poszło również tamtą drogą.
Stan odłożone ma chwilę powrotu, ponieważ odłożenie bez terminu byłoby cichym skasowaniem: zdarzenie znikałoby z widoku i nie wracałoby. Więz w tabeli nie pozwala zapisać jednego bez drugiego.
Źródło polimorficzne nie ma klucza obcego, ponieważ para pól typu i identyfikatora źródła wskazuje kilka różnych tabel, tak samo jak podobne wskazanie w kolejce doręczeń i w rejestrze sugestii. Więz w tabeli pilnuje przynajmniej tego, żeby nie zapisać połowy wskazania.

## budowa/server/internal/store/migracja_079_wybudzenie_biegu.sql
Wybudzenie to nie pauza: pauza czeka na czas albo na Operatora, wiadomo, kiedy ruszy, a wybudzenie czeka na świat, na reakcję, która może przyjść za godzinę, za trzy dni albo nigdy. Bieg zawieszony musi przeżyć restart rdzenia: bieg trzymany w pamięci procesu ginie z procesem i nie jest wybudzeniem, tylko czekaniem. Tabela oczekiwania już istniała, z terminem, polityką po terminie i powodem wybudzenia; ta migracja nie zakłada drugiego kompletu, tylko rozluźnia nośnik istniejącego. Kolumna przebiegu automatyki nie jest obowiązkowa, bo bieg orkiestracji nie ma przebiegu automatyki: oczekiwanie wisi albo na przebiegu automatyki, albo na biegu orkiestracji, nigdy na obu i nigdy na żadnym. Wyzwalacz uruchamia automatykę od początku, a wybudzenie wznawia konkretny bieg w konkretnym miejscu — to dwa różne byty, więc tabela oczekiwania nie jest drugim wyzwalaczem, a kolumna sygnału została bez warunku właśnie dlatego, że świata nie da się zamknąć w słowniku: kod sygnału ustala ten, kto sygnał wyśle. Na tabeli oczekiwania nie wskazuje żaden klucz obcy, więc przebudowa obejmuje jedną tabelę. Więz dokładnie jednego nośnika liczy wyrażenia logiczne jako zero-jedynkowe, suma równa jeden wyraża dokładnie jeden bez wyzwalacza: oczekiwanie bez nośnika byłoby czekaniem niczyim, z dwoma byłoby czekaniem dwóch biegów na jeden sygnał. Dwa indeksy częściowe stoją zamiast jednego, bo brak wartości nie równa się brakowi wartości i wspólny indeks przepuściłby dublet dla drugiego nośnika.

## budowa/server/internal/store/migracja_080_terminal_bez_nawigacji.sql
Terminal nie jest samodzielnym modułem: jest oknem pomocniczym wewnątrz trzech innych modułów, a klient go trzyma jako panel pomocniczy opisany w tych modułach. Rdzeń dotąd nadal podawał terminal jako pozycję wykazu modułów środowiska programowania, bo wiersz macierzy widoczności miał widoczność włączoną, a wykaz modułów idzie wyłącznie z rdzenia — ta migracja przenosi ten fakt do danych zamiast zostawiać go w warunku wpisanym w kod. Wiersz zostaje w macierzy z widocznością wyłączoną, nie jest usuwany, bo macierz ma być kompletem par, żeby moduł niewidoczny i moduł, o którym nikt nie zdecydował, dały się w bazie odróżnić. Terminal ma wiersze także w dwóch innych środowiskach, oba już niewidoczne; ten krok ich nie dotyka. Kolumna kolejności nie jest pozycją wystawianą kontraktowi wprost, tylko kluczem porządku zapytania: pozostałe siedem par środowiska dostaje numery bez przerwy, a wiersz terminala dostaje zero, będące tu brakiem treści, nie pierwszym miejscem. Moduł terminala zostaje w tabeli modułów aktywny: zostają jego okna operacyjne, jego tabele własne oraz wszystkie jego komendy, bo żadna z nich nie czyta macierzy widoczności. Oba polecenia przypisują wartości bezwzględne, nie przyrosty, zawężone kodami, więc powtórne wykonanie daje ten sam stan.
## budowa/server/internal/store/migracja_390_schowek_postac_fragmentu.sql
Wpis schowka niósł sam tekst, więc wklejenie sposobem zachowania postaci źródła, będącym sposobem domyślnym, nie miało czego zachować i przejmowało postać miejsca wklejenia zamiast postaci źródła — dwa różne sposoby wklejenia robiły dokładnie to samo.
Postać jest kolumną, a nie drugą tabelą, ponieważ należy do tego wpisu schowka i ginie razem z nim; osobna tabela wiązana kluczem obcym dawałaby wiersz postaci bez wpisu po każdym zdjęciu wpisu z historii, czyli to samo kasowanie kaskadowe, tylko dwoma tabelami.
Postać idzie jedną kolumną złożoną, a nie kolumnami na cechy, ponieważ wykaz cech znaku i akapitu rośnie razem z kontraktem, a kolumna na każdą cechę znaczyłaby migrację przy każdym dołożonym polu, choć wpisu schowka nikt nie przeszukuje po pojedynczej cesze postaci.
Kolumna jest nieobowiązkowa, ponieważ wpisy odłożone przed tą dobudową nie mają wartości postaci i mają zostać poprawne — brak postaci znaczy brak wiedzy o postaci źródła, nie twierdzenie o postaci pustej. Kolumna jest ogólna, nie ograniczona do jednego modułu, ponieważ schowek jest jeden, wspólny wszystkim oknom, więc postać odłożona w jednym oknie ma się dać wkleić w tym samym oknie, a wpis odłożony innym oknem zostaje wpisem bez postaci.

## budowa/server/internal/store/migracja_400_akcje_strony_glownej.sql
Wcześniejsza migracja katalogu akcji wzięła komendy nawigacji platformy, sesji, okien, konfiguracji i kanałów oraz pasek promptu każdego modułu, a poza nią zostały dwie strefy Strony głównej: kafle komponentów własnych oraz listwa ustawień z oknem konfiguracji, funkcją Mobile i funkcją Always On Display. Okno konfiguracji ma już pozycję w katalogu, więc z listwy dochodzą dwie funkcje globalne.
Rozwinięcie działań kafla wymienia więcej czynności niż duplikowanie, eksport, import i usunięcie, ale kontrakt ma komendę wyłącznie na usunięcie, więc wchodzi tylko ta pozycja — pozycja katalogu wskazująca komendę, której w rejestrze rdzenia nie ma, byłaby atrapą w oknie. Komenda przypisania komponentu zostaje poza katalogiem świadomie, ponieważ kontrakt zapisuje przy niej, że znaczenie przypisania nie zostało jeszcze rozstrzygnięte, a kontrolka czynności o nierozstrzygniętym skutku jest gorsza od jej braku.
Cztery kafle wołają jedną komendę zakładania komponentu z różnym rodzajem komponentu, a kolumna kodu ma warunek jedyności, więc kod pozycji jest nazwą komendy złączoną z rodzajem komponentu z kontraktu, pełnymi nazwami obu bytów. Warunek dostępności zostaje pusty we wszystkich wierszach, ponieważ żadna z tych komend nie wymaga w żądaniu środowiska, sesji, okna, kolejki ani kanału.

## budowa/server/internal/store/migracja_401_nastawy_wag_stojacych.sql
Wcześniejsza migracja założyła cztery nastawy wskaźnika znaczenia z wartościami domyślnymi opisującymi stan, w którym maszyna nie ma jeszcze żadnych wag. Od tamtej pory wagi modelu stoją już rozłożone na maszynie w katalogu obok rdzenia, a przy dawnych wartościach domyślnych świeże wdrożenie ich nie widziało i przy pierwszym budowaniu wskaźnika sięgało po drugi model do sieci, pobierając ponownie to, co już leżało na dysku.
Wcześniejszej migracji się nie zmienia, ponieważ bazy założone wcześniej mają ją wykonaną i odnotowaną wraz z sumą kontrolną, a zmiana treści tamtego pliku wywróciłaby start rdzenia u każdego, kto go już ma. Zmiana wartości domyślnej jest więc osobnym krokiem.
Krok używa aktualizacji, a nie wstawienia z pominięciem konfliktu, ponieważ wiersze już istnieją, założone wcześniej, a zmienia się w nich wyłącznie wartość domyślna wraz z opisem tłumaczącym tę wartość użytkownikowi. Zapis użytkownika zostaje nietknięty, ponieważ wartość domyślna jest ostatnim ogniwem rozstrzygania nastaw: każdy zapis na dowolnym poziomie zasięgu ma przed nią pierwszeństwo, więc maszyna, na której katalog wag ustawiono ręcznie, dalej pracuje na tym wskazaniu.
Nazwa modelu wchodzi do każdej pozycji wskaźnika i zawęża odczyt przy szukaniu, więc na maszynie, która zdążyła zbudować wskaźnik modelem poprzednim, wiersze tamtego modelu zostają i nie mieszają się z nowymi. Katalog wag jest ścieżką bezwzględną świadomie, ponieważ nastawa opisuje rozmieszczenie wag na maszynie, a nie coś wyliczalnego z katalogu danych rdzenia.

## budowa/server/internal/store/migracja_403_nastawa_wag_mowy.sql
Wcześniejsza migracja założyła katalog wag modelu mowy z wartością pustą, oznaczającą katalog domyślny biblioteki, czyli pamięć podręczną konta uruchamiającego rdzeń. Wagi modelu domyślnego tam właśnie stały, więc transkrypcja działała, ale zależnie od tego, na czyim koncie stoi proces: inne konto ma inny katalog domowy i tych samych wag nie widzi, więc pobierze drugą kopię.
Wagi zostały przeniesione do wspólnego katalogu modeli obok rdzenia, tam gdzie stoją wagi pozostałych zdolności liczących lokalnie, i ta ścieżka jest odtąd wartością domyślną. Wcześniejszej migracji się nie zmienia, ponieważ jej suma kontrolna stoi w rejestrze migracji u każdego, kto rdzeń postawił, a niezgodność sumy wywraca start rdzenia, więc zmiana wartości domyślnej jest osobnym krokiem, wzorem poprzedniej migracji dla modelu osadzeń.
Krok używa aktualizacji, a nie wstawienia z pominięciem konfliktu, ponieważ wiersz istnieje od wcześniejszej migracji, a zmienia się w nim wyłącznie wartość domyślna wraz z podpowiedzią i opisem. Zapis użytkownika zostaje nietknięty, ponieważ wartość domyślna jest ostatnim ogniwem rozstrzygania nastaw. Wskazanie katalogu, w którym wag nie ma, nie jest odmową: pomocnik traktuje go wtedy jak miejsce pobrania, więc krok nie może zepsuć maszyny, na której wag pod tą ścieżką nie rozłożono.

## budowa/server/internal/store/migracja_081_tryb_nakladki_agenta.sql
Kontrakt niósł pole trybu eksperta od dawna, przyjmowane przy założeniu i uaktualnieniu, ale tabela eksperta nie miała pola trybu — pole było przyjmowane i milcząco wyrzucane, a ta kolumna daje mu nośnik. Wartość domyślna dołączenia oznacza, że prompt globalny obowiązuje pierwszy, a instrukcja eksperta dopisuje się do niego jako zakres użytkownika; wartość zastąpienia jest odstępstwem jawnym, oznaczanym przez Operatora świadomie, w którym instrukcja eksperta staje się promptem systemowym, a globalny przestaje obowiązywać dla okien tego eksperta. Wartość domyślna kolumny jest odwrotna niż w tożsamości osi, gdzie pominięcie znaczy zastąpienie, ale to nie jest niekonsekwencja: tożsamość osi jest konfiguracją platformy, modelu albo konta, a ekspert jest warstwą nałożoną na wywołanie, więc byt nakładany nie ma prawa zastępować podkładu domyślnie. Pole należy do eksperta, nie do warstwy, bo tryb dotyczy instrukcji modelu jako całości: albo prompt globalny obowiązuje, albo nie — dotychczasowa kolumna trybu na warstwie była per warstwa, więc ekspert bez warstw nie miał w niej wiersza wcale, a ekspert z trzema warstwami miał trzy kopie jednej prawdy. Warstwa nieczynna albo pusta nie wnosi treści, więc nie wnosi też odstępstwa. Dawna kolumna trybu warstwy zostaje i nie jest kasowana, choć nie ma już właściciela: kontrakt nie pozwala jej ustawić, rdzeń wpisuje w nią stałą, a tryb mieszka odtąd przy ekspercie — jej zdjęcie jest osobnym rozstrzygnięciem i osobną migracją. Każdy ekspert założony wcześniej dostaje wartość dołączenia, bo dołożenie kolumny z wartością domyślną nie dotyka ani jednego wiersza danych, a ekspert bez ani jednej warstwy z treścią nie zastępuje niczego niezależnie od tego pola, bo rdzeń sprawdza to przed rozgałęzieniem, żeby zastąpienie promptu globalnego pustką nie zostawiło modelu bez konstytucji platformy.

## budowa/server/internal/store/migracja_082_library_bez_okna_rozmowy.sql
Okno rozmowy przypina wszystkim modułom reguła zakładająca macierz przypięć okien operacyjnych; ten krok zdejmuje z niej jedno wystąpienie, a definicja okna, przypięcia pozostałych modułów, pozostałe okna biblioteki oraz rodzina komend biblioteki zostają nietknięte. Oba wskazania idą podzapytaniem po kodzie, a nie po kluczu wpisanym z ręki, bo klucze sztuczne nadaje autonumeracja i nie są one tym samym w każdej bazie — gdy któregokolwiek z wierszy nie ma, podzapytanie oddaje brak wartości, porównanie nie jest prawdziwe dla żadnego wiersza i krok kasuje zero wierszy zamiast pomylić się w drugą stronę.

## budowa/server/internal/store/migracja_083_konfiguracja_na_stronie_glownej.sql
Rodzaj modułu orzeka, czym moduł jest, a ta kolumna orzeka, gdzie moduł się nastawia; moduły niosące ją prawdziwie nie mają wspólnego rodzaju, więc kolumna nie jest widokiem rodzaju. Dołożenie kolumny zabrania w bazie wyłącznie klucza głównego i warunku unikalności, a warunek zakresu wartości przyjmuje i egzekwuje; obowiązkowość wymaga wartości domyślnej, bo kolumna dokładana do tabeli z wierszami musi powiedzieć, co znaczy dla wierszy już stojących — moduł bez rozstrzygnięcia na stronie głównej się nie nastawia. Moduł nieobecny w słowniku po prostu nie zostanie trafiony, bo krok nie zakłada wiersza, którego zaczyn nie wniósł.

## budowa/server/internal/store/migracja_084_agent_okna.sql
Kolumna niesie kod eksperta, nie jego klucz wewnętrzny, i nie ma więzu obcego, tak samo jak kolumna kanału eksperta: ekspert bywa kasowany niezależnie od okien, w których pracował, a kasowanie kaskadowe skasowałoby wtedy okna wraz z całą rozmową, podczas gdy zerowanie po cichu przestawiłoby okno na model surowy w środku pracy — kod, którego nikt już nie rozpoznaje, jest faktem czytelnym, bo okno pamięta, kogo wskazano, a warstwa składająca nakładkę rusza z samą osią modelu, gdy eksperta nie znajdzie. Brak wartości znaczy model surowy, a pusty napis byłby drugim sposobem powiedzenia tego samego, więc kolumna jest bez wymogu wartości i bez wartości domyślnej. Wskazanie eksperta niczego nie zamyka i niczego nie sprawdza: nie ma warunku na istnienie eksperta ani reguły, że okno bez eksperta nie może wysłać tury, bo ekspert jest nastawą procesu, nie przepustką.

## budowa/server/internal/store/migracja_085_nastawy_agenta.sql
Ekspert miał dotąd nośnik wyłącznie na prompt; ta migracja daje mu nośnik zaczepów i reguł narzędzi. Kolumna niesie napis JSON w całości, nie rozebrane kolumny, bo kształt tego pliku należy do narzędzia zewnętrznego, nie do platformy — rozebranie go na kolumny znaczyłoby, że rdzeń zna i wersjonuje cudzy format, a każda zmiana po tamtej stronie byłaby migracją tutaj. Pusty napis znaczy, że ekspert nie nakłada własnych ustawień: nie ma tu wartości pustej odróżnianej od braku wartości, bo nie ma czego odróżniać — ekspert bez zaczepów i ekspert, któremu zaczepów nie ustalono, zachowują się identycznie, okno rusza z ustawieniami sesji. Kolumna nie sprawdza treści i nie odmawia jej: napis, który nie jest poprawnym zapisem JSON, jest faktem do pokazania Operatorowi w podglądzie wywołania, nie powodem, żeby zablokować turę. Wszystkie tabele podrzędne eksperta zostają nietknięte, bo przebudowy tabeli nie ma.

## budowa/server/internal/store/migracja_088_hosty_zdalne.sql
Torem jest SSH, nie własny protokół sieciowy: SSH daje semantykę procesu w całości, strumienie wejścia, wyjścia i diagnostyki, kod wyjścia oraz zakończenie po zerwaniu połączenia bez ani jednej linii własnego protokołu, daje też uwierzytelnienie i szyfrowanie, których transport rdzenia nie ma, więc tor nie otwiera nowej drogi wejścia, tylko jedzie usługą, którą host zdalny już wystawia. Proces transportu startuje w tym samym jedynym miejscu uruchamiania procesów, a po stronie zdalnej proces uruchamia istniejącą usługę systemową. Kolumna zgody ma wartość domyślną odmowy: samo wpisanie hosta jeszcze niczego nie otwiera, rdzeń nie zainicjuje połączenia z maszyną, której zgoda nie obejmuje, i nie zapyta interaktywnie o hasło. Klucz publiczny rdzenia musi stać na hoście osobno; ta część zgody zapada po stronie hosta, nie w tej tabeli, bo sekrety nie idą przez bazę. Nazwa hosta odpowiada dosłownie wartości ustawienia wyboru hosta wykonania, tak wybór w oknie komunikacji dochodzi do tego wiersza, a adres pusty znaczy, że adresem sieciowym jest sama nazwa.

## budowa/server/internal/store/migracja_089_przenosiny_plikow.sql
Rozstrzygnięcie jest dwudzielne: pliki zwykłe mają przenosiny pod tą samą zgodą na host co tor procesu, nośnikiem jest bezpieczne kopiowanie po tym samym połączeniu, te same klucze, ta sama zgoda, żaden nowy protokół, a każdy wykonany ruch bajtów zostawia wiersz w tej tabeli jako prowenancję: skąd, dokąd, ile i kiedy — wiersz nie jest kolejką ani zleceniem, zapis następuje po przeniesieniu i niczego nie steruje. Nagrania dźwięku dostają odmowę z dwóch niezależnych powodów: reguła zapisana w kliencie wprost zabrania dźwiękowi opuszczać maszynę Operatora, a potrzeby i tak nie ma, bo silnik mowy jest usługą rdzenia i bierze ścieżkę na maszynie silnika — transkrypcja domyka się przed torem zdalnym, a do procesu na hoście zdalnym jedzie wyłącznie tekst. Identyfikator okna jest identyfikatorem kontraktowym, nie kluczem obcym, bo przenosiny bywają wykonywane poza cyklem życia okna, a prowenancja ma przeżyć okno, którego dotyczyła; napis pusty znaczy ruch poza kontekstem okna.

## budowa/server/internal/store/migracja_092_zamkniecie_tury.sql
Stan odpowiedzi rozstrzyga zdarzenie wyniku programu, nie przebieg strumienia: zdarzenie potrafi przyjść z podtypem sukcesu i jednocześnie znacznikiem błędu, więc brak błędu kanału nie znaczy, że tura zakończyła się poprawnie. Wiersz powstaje po turze i niczym nie steruje, jest zapisem pochodzenia rozstrzygnięcia: jakie zdarzenie zamknęło turę, co niosło i jaki stan wiadomości z niego wyprowadzono. Kod okna i kod wiadomości są identyfikatorami kontraktowymi, nie kluczami obcymi, bo zapis zamknięcia ma przeżyć okno, którego dotyczył, a usunięcie sesji nie wymazuje śladu rozstrzygnięć — tekstu odpowiedzi tu nie ma, mieszka w tabeli wiadomości, tu mieszka wyłącznie zdarzenie.

## budowa/server/internal/store/migracja_093_dziennik_zdarzen.sql
Konfiguracja zaczepów idzie do narzędzia zewnętrznego sekcją ustawień, a przy odpowiednim przełączniku strumień niesie na tym samym torze koperty startu i odpowiedzi zaczepu. Kolumna ładunku niesie surową kopertę ze strumienia, zapis źródłowy, nie przekład; bez niego nie da się odtworzyć, na jakiej podstawie rdzeń uznał zaczep za wykonany. Wiersz niczym nie steruje: zaczep jest narzędziem, dziennik śladem po nim, a odmowy zaczepów idą osobno do diagnostyki jako fakty, tu jako zapis źródłowy tego samego zdarzenia. Kod okna i kod wiadomości są identyfikatorami kontraktowymi, nie kluczami obcymi, bo ślad ma przeżyć byt, którego dotyczy.

## budowa/server/internal/store/migracja_094_przelaczenie_kanalu.sql
Gdy kanał odmawia, tura próbuje kanałem następnym, ale nie po cichu: wskazanie zapasu jest wartością danych wiersza rejestru kanałów, tak samo jak program i adapter, a Operator ustawia je poleceniem aktualizacji kanału bez zmiany kontraktu i bez nowej kolumny. Każde wykonane przełączenie zostawia wiersz: z którego kanału, na który, w której turze i dlaczego; obok wiersza Operator dostaje fragment metadanych konta w strumieniu tury oraz prowenancję drugiego wywołania, więc widzi, czym faktycznie pojechało, z odpowiedzi rdzenia, nie z konfiguracji. Zapis następuje po przełączeniu i niczego nie steruje; identyfikatory kontraktowe zamiast kluczy obcych sprawiają, że ślad ma przeżyć byt, którego dotyczył.

## budowa/server/internal/store/migracja_095_kanal_bielik.sql
Silnik lokalny serwuje interfejs zgodny z powszechnym standardem, więc kanał jedzie generycznym adapterem HTTP bez zaszytego dostawcy: domyślne ścieżki odczytu odpowiedzi są dokładnie ścieżkami tego standardu, więc żaden nowy adapter nie powstaje, nowy silnik to wiersz rejestru, nie gałąź w kodzie. Kanał stoi nieaktywny, bo silnika nie ma w standardowej instalacji: kanał czynny bez silnika byłby przyciskiem-atrapą, okno dałoby się na niego przełączyć, a każda tura kończyłaby się odmową, więc wiersz stoi w rejestrze, a włącza go Operator poleceniem po zainstalowaniu silnika — kanał włączony bez silnika odmawia jawnie błędem, nie udaje odpowiedzi. Identyfikator modelu to znacznik po stronie silnika lokalnego; wpisany jest znacznik bazowy, a Operator wskazuje dokładny wariant poleceniem, bo znacznik zależy od tego, co faktycznie pobrał. Parametr kanału zapasowego jest tu celowo nieobecny: kolejność zapasowa jest nastawą Operatora, nie rozstrzygnięciem schematu.

## budowa/server/internal/store/migracja_097_kosz_sesji.sql
Usunięcie sesji stawia znacznik czasu, a czyszczenie trwałe wykonuje rdzeń przy starcie, wyłącznie dla sesji leżących w koszu dłużej niż termin: utrata danych następuje nadal wyłącznie wskutek usunięcia, kosz i czyszczenie po terminie są dwiema fazami tej samej decyzji, nie nową drogą utraty. Odzysk w oknie terminu przywraca sesję: czyści znacznik i oddaje sesję historii bieżącej wraz z całym zapisem. Sesja w koszu jest niewidoczna, bo wykaz sesji pyta o wiersze bez znacznika, więc sesje z kosza nie wracają w żadnym wykazie ani w wynikach szukania, odnajduje je wyłącznie repozytorium kosza. Kosz nie jest wartością słownika stanu sesji: stan opisuje położenie sesji w historii, nie jej byt, a osobna kolumna pozwala sesji wrócić z kosza dokładnie w tym stanie, w którym ją usunięto. Format znacznika jest datą w formacie tej samej tabeli; brak wartości znaczy sesję żywą.
