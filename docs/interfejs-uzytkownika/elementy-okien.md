...
| | |
|---|---|
| **Tytuł** | Elementy okien przepływu głównego |
| **Klasa dokumentu** | Specyfikacja docelowa |
| **Odbiorcy** | projektant · deweloper |
| **Przeznaczenie** | Ustala pełny inwentarz elementów każdego okna przepływu głównego platformy niemającego własnego odrębnego opracowania — okna startowego, strony głównej i powłok czterech środowisk — wraz z formą, wagą wizualną, stanami, zachowaniem po interakcji oraz warstwą widoczności każdego elementu |
| **Zakres** | okno startowe/logowania, strona główna (trzy strefy), rama okna aplikacji (szyna nawigacji, belka tytułowa, pasek edycji, pasek stanu), karty sesji, boczna nawigacja modułów i panel orkiestracji, obszar roboczy jako kontener, Chat Window, Execution Loop Window, panele pomocnicze, warstwy widoczności elementów interfejsu |
| **Poza zakresem** | okno Konfiguracji i okno Ustawień — [Okno Konfiguracji](konfiguracja.md) · [Okno Ustawień](ustawienia.md); moduł Agents i piętnaście modułów platformy wraz z ich oknami operacyjnymi — opracowania katalogu `moduly/`; wnętrze czterech okien roboczych ról środowiska MultitaskingAI — [Środowisko MultitaskingAI](../srodowiska/multitaskingai.md); pełne widoki funkcji globalnych — [Mobile](../funkcje-globalne/mobile.md) oraz [Always On Display](../funkcje-globalne/always-on-display.md) |
| **Dokument nadrzędny** | [Koncepcja platformy](../architektura/koncepcja-platformy.md) |
| **Dokumenty powiązane** | [Przepływ okien](przeplyw-okien.md) · [Katalog komponentów](katalog-komponentow.md) · [Okno Konfiguracji](konfiguracja.md) · [Okno Ustawień](ustawienia.md) · [Strona główna i nawigacja](strona-glowna-i-nawigacja.md) · [Rama okna](rama-okna.md) · [System wizualny](system-wizualny.md) · [Model konfiguracji](../architektura/model-konfiguracji.md) · [Architektura techniczna](../architektura/architektura.md) · [Bezpieczeństwo i uwierzytelnianie](../architektura/bezpieczenstwo-i-uwierzytelnianie.md) · [Izolacja i zależności](../architektura/izolacja-i-zaleznosci.md) |
| **Prototypy odniesienia** | `design/05-okna/przeplyw/okno-startowe.html` · `design/05-okna/przeplyw/rejestracja-i-logowanie.html` · `design/05-okna/przeplyw/centrum-dowodzenia.html` · `design/05-okna/srodowiska/talkin.html` · `design/05-okna/srodowiska/workspace.html` · `design/05-okna/srodowiska/codestudio.html` · `design/05-okna/srodowiska/multitaskingai.html` |
| **Źródła normatywne** | `budowa/shared/contract.json` (obszary `home`, `environment`, `window`, `session`) · `design/zasoby/zetony/zetony.css` · `design/zasoby/css/komponenty.css`, `rama.css`, `prototyp.css` · `design/zasoby/ikony/manifest.json` |
| **Zasada nadrzędna** | Zero blokad — żaden element interfejsu nie traci klikalności; niegotowość i niekompletność danych wejściowych są sygnalizowane komunikatem obok elementu, nie wyłączeniem kontrolki (rozdz. 6.1) |

Dokument wyszczególnia wszystkie elementy znajdujące się w każdym oknie przepływu głównego platformy Danaco Console, które nie ma własnego odrębnego opracowania: okno startowe/logowania, strona główna (trzy strefy) oraz powłoki czterech środowisk — TalkIn, WorkSpace, CodeStudio, MultitaskingAI (rama okna, karty sesji, boczna nawigacja, obszar roboczy) wraz z obydwoma kanałami komunikacji operacyjnej — Chat Window (Użytkownik ↔ Wykonawca) i Execution Loop Window (Koordynator ↔ Wykonawca). Wszystkie okna rozmieszczone są w układzie pionowym, w kolumnach sąsiadujących poziomo. Dla każdego elementu podano: co to jest, do czego służy, formę i wagę wizualną, stany, zachowanie po interakcji, miejsce występowania, warstwę widoczności (1–4, rozdz. 7) oraz sposób wywołania — tak aby żaden element nie został zamieniony w projekcie na niewłaściwą formę (duży panel tam, gdzie miała być mała ikonka, i odwrotnie).

---

## Spis treści

1. [Zakres dokumentu i mapa okien](#0-zakres-dokumentu-i-mapa-okien)
   - [0.1 Cztery okna/powłoki w zakresie](#01-cztery-oknapowłoki-w-zakresie)
   - [0.2 Poza zakresem — gdzie szukać](#02-poza-zakresem--gdzie-szukać)
2. [Okno startowe / logowania](#1-okno-startowe--logowania)
   - [1.1 Miejsce w przepływie i model jednego konta](#11-miejsce-w-przepływie-i-model-jednego-konta)
   - [1.2 Diagram pełnego przepływu](#12-diagram-pełnego-przepływu)
   - [1.3 Elementy wspólne obu stanów (rejestracja i logowanie)](#13-elementy-wspólne-obu-stanów-rejestracja-i-logowanie)
   - [1.4 Makiety tekstowe](#14-makiety-tekstowe)
   - [1.5 Tabela elementów — pełna](#15-tabela-elementów--pełna)
   - [1.6 Metody dodatkowe uwierzytelniania — tabela odniesienia](#16-metody-dodatkowe-uwierzytelniania--tabela-odniesienia)
3. [Strona główna — Centrum dowodzenia](#2-strona-główna--centrum-dowodzenia)
   - [2.1 Trzy ścieżki działania](#21-trzy-ścieżki-działania)
   - [2.2 Układ całościowy](#22-układ-całościowy)
   - [2.3 Strefa 1 — karty środowisk](#23-strefa-1--karty-środowisk)
   - [2.4 Strefa 2 — kafle komponentów własnych](#24-strefa-2--kafle-komponentów-własnych)
   - [2.5 Strefa 3 — listwa ustawień](#25-strefa-3--listwa-ustawień)
   - [2.6 Przepływ nawigacji ze strony głównej](#26-przepływ-nawigacji-ze-strony-głównej)
4. [Powłoka środowisk modułowych — TalkIn / WorkSpace / CodeStudio](#3-powłoka-środowisk-modułowych--talkin--workspace--codestudio)
   - [3.1 Anatomia powłoki](#31-anatomia-powłoki)
   - [3.2 Rama okna aplikacji](#32-rama-okna-aplikacji)
      - [3.2.1 Belka tytułowa](#321-belka-tytułowa)
      - [3.2.1a Szyna nawigacji](#321a-szyna-nawigacji)
      - [3.2.2 Pasek edycji](#322-pasek-edycji)
      - [3.2.2a Pasek stanu](#322a-pasek-stanu)
      - [3.2.3 Rozwijane menu ramy okna](#323-rozwijane-menu-ramy-okna)
   - [3.3 Karty sesji](#33-karty-sesji)
   - [3.4 Boczna nawigacja modułów](#34-boczna-nawigacja-modułów)
   - [3.5 Obszar roboczy — kontener](#35-obszar-roboczy--kontener)
      - [3.5.1 Chat Window — kanał Użytkownik ↔ Wykonawca](#351-chat-window--kanał-użytkownik--wykonawca)
      - [3.5.2 Execution Loop Window — kanał Koordynator ↔ Wykonawca](#352-execution-loop-window--kanał-koordynator--wykonawca)
      - [3.5.3 Panele pomocnicze — rozszerzenia boczne](#353-panele-pomocnicze--rozszerzenia-boczne)
   - [3.6 Punkty łamania](#36-punkty-łamania)
5. [Powłoka środowiska MultitaskingAI](#4-powłoka-środowiska-multitaskingai)
   - [4.1 Różnice względem powłoki modułowej](#41-różnice-względem-powłoki-modułowej)
   - [4.2 Układ całościowy](#42-układ-całościowy)
   - [4.3 Rama okna i karty sesji — dostosowania](#43-rama-okna-i-karty-sesji--dostosowania)
   - [4.4 Panel orkiestracji](#44-panel-orkiestracji)
   - [4.5 Obszar roboczy MultitaskingAI](#45-obszar-roboczy-multitaskingai)
   - [4.6 MultitaskingAI wobec środowisk modułowych — zestawienie strukturalne](#46-multitaskingai-wobec-środowisk-modułowych--zestawienie-strukturalne)
6. [Tabela zbiorcza dokumentu](#5-tabela-zbiorcza-dokumentu)
7. [Stany wspólne i słowniczek](#6-stany-wspólne-i-słowniczek)
   - [6.1 Słownik stanów interakcji](#61-słownik-stanów-interakcji)
   - [6.2 Słowniczek pojęć własnych dokumentu](#62-słowniczek-pojęć-własnych-dokumentu)
8. [Warstwy widoczności elementów interfejsu](#7-warstwy-widoczności-elementów-interfejsu)
   - [7.1 Zasada nadrzędna](#71-zasada-nadrzędna)
   - [7.2 Cztery warstwy widoczności](#72-cztery-warstwy-widoczności)
   - [7.3 Mechanizmy ukrywania funkcjonalności](#73-mechanizmy-ukrywania-funkcjonalności)
   - [7.4 Zasada jednego kliknięcia](#74-zasada-jednego-kliknięcia)
   - [7.5 Katalog typów elementów warstw 2–4](#75-katalog-typów-elementów-warstw-24)
   - [7.6 Warstwy a makiety](#76-warstwy-a-makiety)
9. [Załącznik A. Katalog ikon wykorzystanych w oknach tego dokumentu](#załącznik-a-katalog-ikon-wykorzystanych-w-oknach-tego-dokumentu)
10. [Załącznik B. Mapa źródeł](#załącznik-b-mapa-źródeł)
...
| Powłoka środowisk modułowych | Rama okna (szyna nawigacji, belka tytułowa, pasek edycji, pasek stanu), karty sesji, boczna nawigacja modułów, obszar roboczy (jako kontener), Chat Window, Execution Loop Window, panele pomocnicze — wspólne dla TalkIn, WorkSpace, CodeStudio | 3 |
| Powłoka środowiska MultitaskingAI | Rama okna (szyna nawigacji, belka tytułowa, pasek edycji, pasek stanu), karty sesji, panel orkiestracji (odpowiednik bocznej nawigacji), obszar roboczy (jako kontener), oba okna komunikacji | 4 |

### 0.2 Poza zakresem — gdzie szukać

| Element poza zakresem | Dlaczego poza zakresem | Gdzie opisany |
|---|---|---|
| Okno konfiguracji (siedemnaście zakresów ustawień oraz okno punktów izolacji) | Ma własne opracowanie | [Okno Konfiguracji](konfiguracja.md) · [Okno Ustawień](ustawienia.md) |
| Moduł Agents — pięć okien operacyjnych (Agent Builder, Model Configuration, Skills Manager, Connectors Manager, Permissions Center) | Ma własne opracowanie | [Moduł Agents](../moduly/agents.md) |
| Piętnaście modułów platformy — ich okna operacyjne (Studio Editor, Code Editor, Workflow Builder, Research Workspace, Library Explorer, Design Board, Voice Console, Terminal Tabs, Diagnostics Center, Product Builder) | Okna operacyjne modułów — dokumentowane osobno dla każdego modułu | [Specyfikacja modułów](../specyfikacje/specyfikacja-modulow.md) oraz opracowania katalogu `moduly/` |
| Wnętrze czterech okien roboczych ról MultitaskingAI (Executor 1, Executor 2, Coordinator, Executor 3 / Validator) — pola, zakładki, konfiguracja roli | Okno operacyjne środowiska — dokumentowane odrębnie | [Środowisko MultitaskingAI](../srodowiska/multitaskingai.md) |
| Pełne widoki funkcji globalnych Mobile i Always On Display | Funkcje globalne o własnym zakresie, poza mechanizmem przełączania środowisk i modułów | [Mobile](../funkcje-globalne/mobile.md) · [Always On Display](../funkcje-globalne/always-on-display.md) |
...
OKNO STARTOWE / LOGOWANIA                                    ◄── rozdział 1 (ten dokument)
        │  uwierzytelnienie urządzenia — wymóg konfigurowalny przez Operatora
        │  (Pomiń, gdy wymóg wyłączony)
        ▼
...
└─ Strefa 3 · listwa ustawień ──────► okno konfiguracji / Mobile / AOD      [poza zakresem]
        │
        ▼
POWŁOKA ŚRODOWISKA                                           ◄── rozdziały 3–4 (ten dokument)
  (TalkIn · WorkSpace · CodeStudio · MultitaskingAI)
├─ rama okna (szyna nawigacji · belka · pasek edycji · pasek stanu)
├─ karty sesji
...
| Godło marki | Sygnet „Delegacja” (`design/zasoby/marka/logo/sygnet.svg`) | Identyfikacja platformy przed uwierzytelnieniem | Pojedynczy element graficzny nad formularzem, rozmiar średni-duży |
...
|---|---|---|---|---|---|---|---|---|
| Pole „Login” | Pole tekstowe jednoliniowe | Wprowadzenie nazwy właściciela konta | `.dn-pole-kontrolka` w `.dn-pole`, etykieta nad polem | Puste (tekst podpowiedzi) · wypełnione · fokus (obrys `--dn-fokus`) · błąd (`.dn-pole-blad`, obrys czerwony + komunikat pod polem) | Wpisywana wartość waliduje się na bieżąco (format); Enter przenosi fokus do pola Hasło | Rejestracja, Logowanie | 1 | Widoczne bez interakcji |
| Pole „Adres e-mail uwierzytelniający” | Pole tekstowe jednoliniowe | Weryfikacja konta, alternatywna metoda logowania, jedyna droga odzyskania konta (rozdz. 4.2, 6.3, 7 Bezpieczeństwo) | `.dn-pole-kontrolka` w `.dn-pole` | Puste · wypełnione · fokus · błąd (format nieprawidłowy) | Format e-mail walidowany na bieżąco | Rejestracja, Odzyskiwanie konta (krok 1) | 1 | Widoczne bez interakcji |
| Pole „Hasło” | Pole hasła z przełącznikiem widoczności | Wprowadzenie/ustawienie hasła dostępowego | `.dn-pole-kontrolka` typ hasło + ikona `oko` (pokaż/ukryj) | Puste · wypełnione · ukryte (domyślnie) · odkryte · fokus · błąd | Kliknięcie ikony `oko` przełącza maskowanie tekstu; hasło nigdy nie trafia do bazy danych w postaci jawnej (przechowywany wyłącznie skrót, poza bazą — rozdz. 10 Bezpieczeństwo) | Rejestracja, Logowanie (metoda bazowa), Odzyskiwanie konta (krok 2, jako „Nowe hasło”) | 1 | Widoczne bez interakcji |
| Przycisk „Utwórz konto” | Przycisk podstawowy formularza rejestracji | Wysłanie danych rejestracyjnych do serwera | `.dn-btn--sygnal` (jedyny CTA widoku, pełna szerokość formularza) | Domyślny · wskazanie kursorem · aktywny (wciśnięcie) · fokus (pierścień `--dn-fokus`) · ładowanie (`.dn-spinner` w miejscu etykiety podczas weryfikacji) | Przycisk zawsze klikalny; gdy wymagane pola nie są wypełnione, kliknięcie pokazuje ostrzeżenie przy brakującym polu zamiast wysyłki; w przeciwnym razie wysyła `auth.register` — sukces → stan „konto niepotwierdzone”; błąd serwera → komunikat pod właściwym polem | Rejestracja | 1 | Widoczny bez interakcji |
| Przycisk „Zaloguj” | Przycisk podstawowy formularza logowania | Wysłanie danych logowania do serwera | `.dn-btn--sygnal`, pełna szerokość formularza | Domyślny · wskazanie kursorem · aktywny · fokus · ładowanie | Przycisk zawsze klikalny; gdy pola są puste, kliknięcie pokazuje ostrzeżenie zamiast wysyłki; w przeciwnym razie wysyła `auth.login` — sukces → token wydany, przejście do strony głównej; błąd → komunikat błędu nad przyciskiem | Logowanie | 1 | Widoczny bez interakcji |
| Link „Masz już konto? Zaloguj się” | Odnośnik tekstowy | Przełączenie widoku formularza z rejestracji na logowanie w obrębie tego samego okna | Tekst-link, krój bazowy, kolor `--dn-sygnal` | Domyślny · wskazanie kursorem (podkreślenie) · fokus | Kliknięcie podmienia zawartość okna na formularz logowania (bez przeładowania okna aplikacji) | Rejestracja | 1 | Widoczny bez interakcji |
| Link „Nie pamiętasz hasła? Odzyskaj dostęp” | Odnośnik tekstowy | Wejście w procedurę odzyskiwania konta | Tekst-link, krój bazowy | Domyślny · wskazanie kursorem · fokus | Kliknięcie podmienia zawartość okna na formularz odzyskiwania (krok 1) | Logowanie | 1 | Widoczny bez interakcji |
| Link „Powrót do logowania” | Odnośnik tekstowy | Wyjście z procedury odzyskiwania bez jej dokończenia | Tekst-link, drugorzędny | Domyślny · wskazanie kursorem · fokus | Kliknięcie wraca do formularza logowania | Odzyskiwanie konta | 1 | Widoczny bez interakcji |
| Przełącznik metody logowania | Grupa segmentowa (przełącznik gęsty) | Wybór aktywnej metody uwierzytelnienia spośród metod włączonych w oknie konfiguracji (rozdz. 6.4 Bezpieczeństwo) | `.dn-zakladki--pigulki` — segmenty: Login i hasło (zawsze) · PIN · Windows Hello · E-mail uwierzytelniający (tylko włączone) | Segment aktywny (tło `--dn-sygnal-tlo`) · pozostałe segmenty nieaktywne · segment niedostępny (metoda wyłączona w konfiguracji — nie jest w ogóle renderowany, zgodnie z zasadą braku twardych blokad: brak metody = mniej segmentów, nie zablokowany segment) | Kliknięcie segmentu podmienia pola formularza pod przełącznikiem na właściwe wybranej metodzie | Logowanie (widoczny tylko, gdy co najmniej jedna metoda dodatkowa jest włączona) | 2 | Ujawniany, gdy w oknie konfiguracji włączona jest co najmniej jedna metoda dodatkowa; wybór segmentu |
| Przycisk „Utwórz konto” / komunikat „Wyślij ponownie” (weryfikacja e-mail) | Przycisk drugorzędny | Ponowne wysłanie wiadomości weryfikacyjnej, gdy pierwsza nie dotarła | `.dn-btn--zarys` | Domyślny · wskazanie kursorem · aktywny · odliczanie informacyjne po wysłaniu (etykieta pokazuje czas do zalecanej ponownej próby — nie blokuje kliknięcia) | Przycisk pozostaje klikalny przez cały czas; kliknięcie zawsze ponawia wysyłkę wiadomości weryfikacyjnej — odliczanie ma charakter wyłącznie informacyjny, nie wstrzymuje akcji | Rejestracja (stan „konto niepotwierdzone”) | 2 | Ujawniany zdarzeniem — przejściem formularza w stan „konto niepotwierdzone” |
| Komunikat stanu „Konto niepotwierdzone” | Blok informacyjny | Poinformowanie, że rejestracja wymaga potwierdzenia adresu e-mail przed pierwszym logowaniem | `.dn-toast--informacja` | Widoczny do chwili potwierdzenia adresu | Zniknięcie po potwierdzeniu (odczytanym przez serwer) — okno przechodzi do stanu „konto utworzone” → wydanie tokenu | Rejestracja (stan pośredni) | 2 | Ujawniany zdarzeniem utworzenia konta |
| Przycisk „Pomiń” | Przycisk drugorzędny/duch | Wejście do platformy bez wypełniania formularza | `.dn-btn--duch` lub `.dn-btn--zarys`, pod przyciskiem głównym, oddzielony separatorem | Domyślny · wskazanie kursorem · aktywny · fokus | Kliknięcie wchodzi bezpośrednio do strony głównej bez wydania tokenu uwierzytelniania (krok „Uwierzytelnienie” cyklu połączenia jest pomijany — rozdz. 8.1 Bezpieczeństwo) | Widoczny, gdy Operator wyłączy wymóg logowania w oknie konfiguracji — domyślnie wyłączony w fazie budowy; ta sama opcja dostępna Operatorowi również w fazie produkcyjnej (rozdz. 8 i 8.1 Bezpieczeństwo) | 2 | Ujawniany ustawieniem „wymóg logowania” w oknie konfiguracji |
| Pole „Nowe hasło” | Pole hasła | Ustawienie hasła zastępującego dotychczasowe po odzyskaniu konta | `.dn-pole-kontrolka` typ hasło + ikona `oko` | Puste · wypełnione · fokus · błąd | Zapis zastępuje dotychczasowy skrót hasła (poza bazą danych); unieważnia tokeny pozostałych urządzeń | Odzyskiwanie konta (krok 2) | 1 | Widoczne bez interakcji |
| Przycisk „Wyślij link odzyskiwania” | Przycisk podstawowy | Zainicjowanie procedury odzyskiwania | `.dn-btn--sygnal` | Domyślny · wskazanie kursorem · aktywny · ładowanie | Przycisk zawsze klikalny; gdy pole e-mail jest puste, kliknięcie pokazuje ostrzeżenie zamiast wysyłki; w przeciwnym razie wysyła `auth.recover` — serwer wysyła wiadomość z drogą potwierdzenia tożsamości | Odzyskiwanie konta (krok 1) | 1 | Widoczny bez interakcji |
| Przycisk „Ustaw nowe hasło” | Przycisk podstawowy | Zatwierdzenie nowego hasła | `.dn-btn--sygnal` | Domyślny · wskazanie kursorem · aktywny · ładowanie | Przycisk zawsze klikalny; gdy pole nowego hasła jest puste lub nie spełnia reguł, kliknięcie pokazuje ostrzeżenie zamiast wysyłki; w przeciwnym razie wysyła `auth.reset` — sukces → powrót do formularza logowania z komunikatem potwierdzającym (`.dn-toast--sukces`) | Odzyskiwanie konta (krok 2) | 1 | Widoczny bez interakcji |
| Komunikat błędu logowania | Blok/tekst inline nad przyciskiem „Zaloguj” | Poinformowanie o nieprawidłowych danych | `.dn-toast--blad` lub tekst `--dn-blad-tekst` z ikoną `blad` | Widoczny wyłącznie po nieudanej próbie | Znika przy kolejnej próbie wysłania formularza | Logowanie | 2 | Ujawniany zdarzeniem nieudanej próby logowania |
| Komunikat potwierdzający zmianę hasła | Dymek powiadomienia | Potwierdzenie skutecznego odzyskania konta | `.dn-toast--sukces`, kreska lewa zielona | Pojawia się na 1 zdarzenie, znika automatycznie | Informuje dodatkowo o wylogowaniu pozostałych urządzeń | Odzyskiwanie konta (po kroku 2) | 2 | Ujawniany zdarzeniem zmiany hasła; znika samoczynnie |
...
```
Stan spoczynkowy                     Stan wskazania kursorem / aktywny
┌──────────────────────┐             ╔══════════════════════╗
│                      │             ║ ▐▐▐ wstęga sygnałowa ║
│       ◈  godło       │             ║       ◈  godło       ║
│                      │             ║                      ║
...
│                      │             ║ cień sygnałowy       ║
└──────────────────────┘             ╚══════════════════════╝
 tło neutralne, bez akcentu           akcent sygnałowy: wstęga górna 3 px
                                      (`--dn-sygnal`), cień `--dn-cien-sygnal`,
...
| Element | Co to jest | Do czego służy | Forma i waga | Stany | Zachowanie po interakcji | Gdzie występuje | Warstwa | Sposób wywołania |
|---|---|---|---|---|---|---|---|---|
| Karta środowiska (całość) | Duży kafel wejścia | Wejście do jednego z czterech środowisk — przez jego przedsionek | `.dn-karta-srodowiska`, jedna z czterech w siatce równej wagi — waga główna strony | Spoczynek (bez akcentu) · wskazanie kursorem (wstęga górna 3 px w `--dn-sygnal`, cień `--dn-cien-sygnal`, uniesienie −2 px) · aktywny/fokus (pierścień sygnałowy 2 px w `--dn-fokus`, klawiatura) | Kliknięcie/Enter otwiera przedsionek wybranego środowiska (krok 1 nawigacji, rozdz. 4.1 Strona główna); przestrzeń robocza otwiera się dopiero po wyborze modułu w przedsionku | Strefa 1 strony głównej — 4 wystąpienia | 1 | Widoczna bez interakcji |
| Godło środowiska | Symbol graficzny właściwy środowisku | Szybka identyfikacja wizualna środowiska | `.dn-karta-srodowiska-godlo`, nad tytułem | Dziedziczy kolor z kontekstu karty (`currentColor`) | Brak własnej interakcji — element wewnątrz karty | W każdej z 4 kart | 1 | Widoczna bez interakcji |
...
└──────────────────────┘
   bez wstęgi sygnałowej i bez cienia sygnałowego w spoczynku —
...
| Element | Co to jest | Do czego służy | Forma i waga | Stany | Zachowanie po interakcji | Gdzie występuje | Warstwa | Sposób wywołania |
|---|---|---|---|---|---|---|---|---|
| Kafel komponentu (całość) | Kafel średniej wagi | Wejście do okna konfiguracji jednego z czterech rodzajów komponentu własnego | `.dn-kafel`, mniejszy niż karta strefy 1, 4 w siatce — waga pośrednia | Spoczynek · wskazanie kursorem (tło, bez wstęgi sygnałowej) · aktywny/fokus (pierścień sygnałowy `--dn-fokus`) | Kliknięcie/Enter otwiera okno nakładkowe (`.dn-modal`) albo okno konfiguracji komponentu własnego — **poza zakresem tego dokumentu** | Strefa 2 strony głównej — 4 wystąpienia | 1 | Widoczny bez interakcji |
| Ikona komponentu | Symbol właściwy rodzajowi komponentu | Identyfikacja wizualna rodzaju wytworu | `.dn-kafel-ikona`, skala mniejsza niż godło środowiska | Dziedziczy kolor z kontekstu | Brak własnej interakcji | W każdym z 4 kafli | 1 | Widoczny bez interakcji |
| Nazwa komponentu | Nazwa modułu źródłowego komponentu | Identyfikacja tekstowa | `.dn-kafel-etykieta`, waga `semibold`, NIE nagłówkowy — odróżnienie od strefy 1 | Stały | Brak własnej interakcji | W każdym z 4 kafli | 1 | Widoczny bez interakcji |
| Etykieta działania | Fraza zorientowana na tworzenie | Komunikacja, że kliknięcie tworzy nowy wytwór, a nie otwiera przestrzeń roboczą | `.dn-kafel-opis`, tekst drugorzędny pod nazwą | Stały | Brak własnej interakcji | W każdym z 4 kafli | 1 | Widoczny bez interakcji |
...
└──────────────────────────────────────────────────────────────────┘
  ...
  bez kroju nagłówkowego · akcent sygnałowy wyłącznie w fokusie
  i przy wskazaniu kursorem pojedynczego elementu ·
  najniższa waga wizualna trzech stref
```
...
| Element | Co to jest | Do czego służy | Forma i waga | Stany | Zachowanie po interakcji | Gdzie występuje | Warstwa | Sposób wywołania |
|---|---|---|---|---|---|---|---|---|
| Pozycja „Okno konfiguracji” | Przycisk z ikoną i etykietą tekstową | Otwarcie pełnego okna konfiguracji platformy (siedemnaście zakresów ustawień) | `.dn-btn-ikona` + etykieta, wzorzec lekki `.dn-narzedzia` | Domyślny · wskazanie kursorem · aktywny · fokus | Kliknięcie otwiera okno konfiguracji — **poza zakresem tego dokumentu** (własny agent) | Strefa 3 strony głównej | 1 | Widoczna bez interakcji |
| Pozycja „Mobile” | Przycisk z ikoną i etykietą tekstową | Aktywacja funkcji globalnej Mobile (dostęp mobilny, monitoring procesów) | `.dn-btn-ikona` + etykieta | Domyślny · wskazanie kursorem · aktywny · fokus | Kliknięcie aktywuje funkcję globalną — nie tworzy nowej przestrzeni roboczej, nie przeładowuje niczego (Koncepcja platformy, rozdz. 5.3) | Strefa 3 strony głównej — także dostępna z szybkiego wyboru szyny nawigacji każdej powłoki środowiska (rozdz. 3.2, 4.3) | 1 | Widoczna bez interakcji |
| Pozycja „Always On Display” | Przycisk z ikoną i etykietą tekstową | Aktywacja globalnego agenta towarzyszącego (pływający avatar) | `.dn-btn-ikona` + etykieta, ikona `dzwonek`/awatar | Domyślny · wskazanie kursorem · aktywny · fokus | Kliknięcie przywołuje pływający avatar — dostępny wszędzie, nie tworzy przestrzeni roboczej | Strefa 3 strony głównej — także dostępna z szybkiego wyboru szyny nawigacji każdej powłoki środowiska (rozdz. 3.2, 4.3) | 1 | Widoczna bez interakcji |
...
┌───────────────────────────────────────────────────────────────────────┐
│ BELKA TYTUŁOWA                                                 (3.2)  │
│ ◈ Danaco Console       tytuł bieżącego widoku          ─   □   ✕      │
├──────┬────────────────────────────────────────────────────────────────┤
│SZYNA │ PASEK EDYCJI                                            (3.2)  │
│(3.2) │ 🔍 ⛶ 📋 │ ← →  ścieżka   [ szukaj ] │ ⌂ ⟳ ⇤ │ ⚙ ◐                │
│  ☰   ├────────────────────────────────────────────────────────────────┤
│ ──── │ KARTY SESJI                                             (3.3)  │
│ ▣ 🗨  │ [● Studio ✕] [ Research ✕] [+]                                 │
│ ▣ 📁  ├──────────┬─────────────┬──────────────────────┬────────────────┤
│  · ▪ │ BOCZNA   │ CHAT WINDOW │ OBSZAR ROBOCZY (3.5) │ PANEL          │
│  · ▪ │ NAWIGACJA│ Użytkownik ↔│                      │ POMOCNICZY     │
│  · ▪ │ MODUŁÓW  │ Wykonawca   │  okna operacyjne     │ (rozszerzenie  │
│ ▣ ⌨  │ (3.4)    │   (3.5.1)   │  modułu wybranego    │  boczne,       │
│ ▣ ⚡  │          │             │  w szynie            │  3.5.3)        │
│ ──── │          │ EXECUTION   │                      │                │
│  ◉   │          │ LOOP WINDOW │                      │                │
│      │          │   (3.5.2)   │                      │                │
...
### 3.2 Rama okna aplikacji

Rama okna to **cztery pasy obecne w każdym oknie platformy**, niezależnie od
środowiska, modułu i widoku: belka tytułowa, szyna nawigacji, pasek edycji
i pasek stanu. Rama nie należy do żadnego środowiska ani modułu — należy do
aplikacji. Jej skład jest niezmienny; zmienne są wyłącznie tytuł widoku,
karty otwartych sesji i wskazania stanu.

> **Pełna specyfikacja ramy wraz z zawartością wszystkich menu rozwijanych:**
> [Rama okna](rama-okna.md).

| Pas | Wymiar | Rola |
|---|---:|---|
| Szyna nawigacji | szer. 48 px | menu aplikacji, cztery środowiska, moduły środowiska rozwiniętego, profil |
| Belka tytułowa | wys. 48 px | tożsamość produktu, tytuł widoku, sterowanie oknem systemowym |
| Pasek edycji | wys. 48 px | czynności okna i treści, wyszukiwanie, karty otwartych sesji, widok, ustawienia |
| Pasek stanu | wys. 28 px | operator, położenie, sesje czynne, motyw, obciążenie zasobów |

**Szyna biegnie od samej góry okna**, a belka tytułowa zaczyna się na prawo od
niej. Szerokość szyny równa się wysokości belki, więc menu aplikacji zajmuje
kwadrat w narożniku okna.

#### 3.2.1 Belka tytułowa

Pas atramentowy o wysokości 48 px, przylegający do górnej krawędzi okna
systemowego i zaczynający się na prawo od szyny nawigacji. **Jedyny dom godła
produktu** — znak nie występuje w żadnym innym pasie ramy. Belka pełni funkcję uchwytu przeciągania okna natywnego; obszar
sterowania oknem jest z przeciągania wyłączony.

| Element | Co to jest | Do czego służy | Forma i waga | Stany | Zachowanie po interakcji | Warstwa |
|---|---|---|---|---|---|---|
| Znak i nazwa produktu | Sygnet marki oraz napis „Danaco Console” | Identyfikacja produktu i punkt orientacji w oknie systemowym | Blok przy lewej krawędzi, krój nagłówkowy w stopniu małym | Statyczny | Brak interakcji własnej | 1 |
| Tytuł bieżącego widoku | Nazwa pracy w toku wraz z miejscem, w którym się toczy — na przykład „Raport końcowy — redakcja — WorkSpace › Studio” | Rozpoznanie okna na pasku zadań systemu i wśród wielu otwartych okien aplikacji | Napis wyśrodkowany, stopień najmniejszy, barwa drugorzędna ramy; przycinany wielokropkiem przy ciasnym oknie | Statyczny; treść zmienia się wraz z widokiem | Brak interakcji własnej | 1 |
| Minimalizacja okna | Kontrolka sterowania oknem systemowym | Zejście okna do paska zadań systemu | Przycisk 46 × 48 px bez obrysu, ikona kreski | Domyślny · wskazanie kursorem (tło rozjaśnione) · fokus | Okno schodzi do paska zadań; stan pracy pozostaje nietknięty | 1 |
| Maksymalizacja i przywrócenie | Kontrolka dwustanowa | Wypełnienie ekranu i powrót do rozmiaru poprzedniego | Jak wyżej, ikona prostokąta | Domyślny · wskazanie kursorem · fokus | Okno wypełnia ekran; ponowne naciśnięcie przywraca rozmiar | 1 |
| Zamknięcie okna | Kontrolka sterowania oknem systemowym | Zamknięcie okna aplikacji | Jak wyżej, ikona krzyżyka; wskazanie kursorem daje tło barwy błędu | Domyślny · wskazanie kursorem (tło czerwone) · fokus | Okno zostaje zamknięte; sesje pracujące w tle trwają dalej po stronie serwera i wracają przy kolejnym uruchomieniu | 1 |

#### 3.2.1a Szyna nawigacji

Pionowy pas o szerokości 48 px przy lewej krawędzi, biegnący **od samej góry
okna** aż do paska stanu. Niesie atrament ramy — ten sam co belka tytułowa,
z którą łączy się bez kreski w jedną bryłę. Prowadzi **całą nawigację między
środowiskami i modułami**.

| Odcinek | Zawartość |
|---|---|
| Głowa | Menu aplikacji (hamburger) — kwadrat w narożniku okna |
| Strefa 1 · praca | Nowa sesja · Historia sesji · Nowy projekt |
| Strefa 2 · środowiska | Cztery środowiska; pod środowiskiem rozwiniętym — jego moduły |
| Strefa 3 · szybki wybór | Assistant · Agents · Automations · Workspace · Mobile · Always on Display oraz pozycja „Dodaj skrót” |
| Stopka | Ustawienia i konfiguracja · motyw jasny i ciemny · profil użytkownika |

**Strefa pracy** zaczyna pracę albo do niej wraca. „Nowa sesja” i „Nowy projekt”
wywołane spoza środowiska otwierają menu żądające wskazania środowiska — sesja
ani projekt nie istnieją poza środowiskiem. Wskazanie środowiska w „Nowej sesji”
otwiera **przedsionek** (moduł wybiera się kaflem); wskazanie w „Nowym projekcie”
otwiera od razu **Workspace** tego środowiska w trybie zakładania projektu.
„Historia sesji” otwiera **nakładkę okna ustawień i konfiguracji** z wybraną
sekcją „Historia sesji” — rejestr wszystkich sesji konta w trzech zbiorach:
czynne, zakończone, archiwalne. Rejestr nie zabiera całego okna: widok, z którego
został wywołany, pozostaje pod nakładką nietknięty. Pełny opis obu okien — [Rama okna](rama-okna.md) rozdz. 2a.4.

Trzy strefy rozdziela delikatna kreska. Głowa i stopka stoją nieruchomo,
strefy przewijają się między nimi, gdy okno jest za niskie na komplet pozycji.

**Szybki wybór** zbiera komponenty własne i okna platformowe, po które Operator
sięga niezależnie od środowiska. Grupa odcina się odstępem, nie kreską; pozycja
„Dodaj skrót” ma obrys przerywany, bo jest miejscem czekającym na wypełnienie.

**Jeden byt — jeden rysunek.** Godła środowisk w szynie to te same rysunki, co
na kartach środowisk strony głównej; ikony modułów — te same, co na kaflach
modułów w przedsionku.

**Hierarchia niesiona kształtem.** Środowisko mierzy 40 × 40 px, moduł
34 × 34 px i stoi wcięty. W spoczynku żadna pozycja nie ma ramki: ramka jest
znakiem stanu i nosi ją wyłącznie środowisko rozwinięte. Kreska ikony ma pełną
jasność ramy — na atramencie kreska osłabiona gaśnie. Naciśnięcie środowiska rozwija jego
moduły w dół, przesuwając środowiska następne niżej; rozwinięte jest najwyżej
jedno środowisko naraz.

**Dwa różne wskazania.** Środowisko rozwinięte dostaje ramkę i rozjaśnione
tło. Moduł bieżący dostaje wypełnienie sygnałowe. Wypełnienie należy do miejsca
pracy, obramowanie — do stanu listy.

**Etykieta pozycji** ujawniana wskazaniem kursorem i fokusem wysuwa się w prawo
i jest dwuczłonowa: nazwa oraz jedno zdanie o przeznaczeniu.

#### 3.2.2 Pasek edycji

**Belka nakładana** o wysokości 48 px pod belką tytułową, na prawo od szyny:
leży na tle okna z odstępem, ma zaokrąglone narożniki i **nie ma obrysu** —
od otoczenia oddziela ją wyłącznie różnica odcienia. Układ pasa jest
przeglądarkowy: sterowanie po lewej, za nim pole wyszukiwania, dalej pasmo
kart otwartych sesji, a rodziny widoku i aplikacji zamykają pas od prawej.
Skupia czynności wykonywane na oknie i na treści, niezależnie od tego, jaki
moduł jest otwarty. Kontrolki są pogrupowane w **rodziny**, a rodziny
rozdzielone kreską pionową — kreska nie występuje między pojedynczymi ikonami
wewnątrz rodziny.

Każda kontrolka pasa niesie **etykietę ujawnianą wskazaniem kursorem i fokusem**
oraz **mikroreakcję ruchu**: przy wskazaniu kursorem ikona unosi się o 1 px w czasie
`--dn-czas-1`. Przy `prefers-reduced-motion` uniesienie ustaje; zmiana tła
pozostaje.
...
ikonową, i nie należy do żadnej rodziny.

**Skład pasa — lewa strona:**

| Rodzina | Kontrolka | Do czego służy | Rozwijane menu |
|---|---|---|---|
| Układ | Zwiń szynę sesji | Oddanie szerokości treści roboczej | nie — kontrolka dwustanowa |
| Nawigacja historii | Wstecz | Powrót do widoku poprzedniego w tej karcie sesji | nie |
| Nawigacja historii | Do przodu | Przejście do widoku, z którego nastąpił powrót | nie |
| Widok | Centrum dowodzenia | Powrót na stronę główną; karty sesji trwają w tle | nie |
| Widok | Odśwież widok | Ponowne pobranie danych okien roboczych ze źródła | nie |
| Treść i przechwytywanie | Zrzut ekranu | Przechwycenie obrazu okna, obszaru albo okna roboczego | **tak** — rozdz. 3.2.3 |
| Treść i przechwytywanie | Schowek | Dostęp do historii schowka i jego ustawień | **tak** — rozdz. 3.2.3 |

**Skład pasa — środek:** pole wyszukiwania (szerokość do 360 px), a za nim
**pasmo kart otwartych sesji** zajmujące resztę szerokości pasa. Ścieżki
położenia pasek nie niesie — miejsce pracy nazywa tytuł widoku w belce
tytułowej oraz nagłówek okna roboczego.

**Skład pasa — prawa strona:** karty otwartych sesji, za nimi dwie rodziny
czynności niezwiązanych z bieżącą treścią — „kontekst i praca w tle” (kolejka
zadań, magistrala kontekstu, izolacja kontekstu) oraz „widok i powiadomienia”
(powiadomienia, tryb skupienia, pełny ekran) — dalej rozwinięcie **„Więcej”**
(`⋯`) z wyszukiwaniem w widoku, dokumentacją platformy i skrótami klawiszowymi,
a na samym końcu pasa przycisk **„Dostosuj”**. Pozycje zwinięte pod rozwinięciem
zachowują swoje skróty klawiszowe. Otwiera on okno konfiguracji wstążki: skład i kolejność
składników, postać kontrolek (same ikony albo ikony z etykietami), sposób
wyświetlania kart sesji oraz formę i gęstość samego pasa. Ustawienie należy do
konta i obowiązuje na wszystkich urządzeniach. Pełny opis okna — [Rama okna](rama-okna.md)
rozdz. 3c.
...
w głowie, trzy pozostałe w stopce. Wyboru środowiska i modułu pasek nie
prowadzi. Wskaźnik sesji czynnych stoi w pasku stanu oraz przy każdej sesji
w szynie sesji.

#### 3.2.2a Pasek stanu

Pas o wysokości 28 px przy dolnej krawędzi okna. Wyłącznie odczyt, bez
czynności: operator i adres konta, położenie (środowisko i moduł), liczba
sesji czynnych, motyw bieżący oraz obciążenie procesora i pamięci. Miary
zasobów odczytuje powłoka natywna aplikacji; w prototypie są przykładowe.

#### 3.2.3 Rozwijane menu ramy okna

Pięć kontrolek ramy otwiera menu rozwijane — trzy w pasku edycji, dwie w szynie nawigacji (menu aplikacji i profil użytkownika). Menu są **jednopoziomowe**,
podzielone nagłówkami sekcji, a pozycje niosą ikonę, nazwę czynności
i — gdzie czynność ma skrót — jego zapis.

| Menu | Sekcje | Uwagi |
|---|---|---|
| **Menu aplikacji** | Plik · Edycja · Widok · Przejdź · Narzędzia · Pomoc | Najszersze menu platformy. Obejmuje czynności na sesji i oknie (nowa sesja, nowe okno aplikacji, zapis i eksport sesji), czynności na treści (cofnij, ponów, schowek, znajdź), przełączniki widoku (szyna, pas kart, tryb skupienia, pełny ekran, powiększenie), przejścia (Centrum dowodzenia, przedsionek środowiska, karty sesji, paleta poleceń), narzędzia platformy (menedżer sesji, magistrala kontekstu, kolejka zadań, punkty izolacji) oraz pomoc |
| **Zrzut ekranu** | Wykonaj zrzut · Po wykonaniu · Ustawienia | Pięć rodzajów zrzutu (całe okno, zaznaczony obszar, wskazane okno robocze, widok przewijany w całości, zrzut z opóźnieniem). Sekcja „Po wykonaniu” jest wyborem jednokrotnym: do schowka, do pliku, do modułu Library, do Chat Window jako załącznik. Ustawienia obejmują otwarcie edytora adnotacji, automatyczne ukrycie danych wrażliwych, katalog docelowy i wzorzec nazwy oraz **dwa pola przypisania skrótu klawiszowego** |
| **Schowek** | Schowek · Ostatnie pozycje · Ustawienia | Otwarcie schowka, wklejenie ostatniej pozycji, wklejenie jako zwykły tekst, historia, wyczyszczenie. Ustawienia obejmują liczbę przechowywanych pozycji, pomijanie treści z pól haseł, współdzielenie między urządzeniami, czyszczenie przy zamknięciu oraz **dwa pola przypisania skrótu**. Przytrzymanie kombinacji wklejania otwiera listę pozycji; zwolnienie klawiszy wstawia pozycję wskazaną |
| **Ustawienia i konfiguracja** | Aplikacja · Środowisko i sesja · Rozszerzenia · Utrzymanie | Rozdziela trzy zakresy konfiguracji platformy (aplikacja, środowisko, sesja) zgodnie z modelem konfiguracji oraz prowadzi do punktów izolacji, uprawnień, konektorów i utrzymania |
| **Profil użytkownika** | Zalogowany operator · konto · status obecności · wyjście | Otwiera dane konta, plan i rozliczenia, urządzenia i sesje logowania, uwierzytelnianie dwuskładnikowe, powiadomienia; wybór statusu obecności (dostępny, zajęty, niewidoczny) oraz wylogowanie |

**Przypisanie skrótu klawiszowego.** Pole skrótu przyjmuje kombinację
naciśniętą przez Operatora. Kombinacja już zajęta nie zostaje przypisana —
w jej miejsce pojawia się komunikat wskazujący czynność, która ją zajmuje.
Klawisz `Escape` przywraca zapis poprzedni. Zgodnie z zasadą zero blokad pole
pozostaje aktywne również wtedy, gdy kombinacja jest zajęta.
...
| Wskaźnik stanu | Mała kropka/plakietka stanu | Sygnalizacja aktywnej pracy w tle (proces automatyki, aktywna rola MultitaskingAI itp.) | `.dn-kropka` (kolor semantyczny wg statusu) | Brak aktywności (niewidoczny) · aktywny (kropka widoczna, kolor wg statusu) | Powiązany z funkcją Mobile — monitoring procesów; kliknięcie otwiera podgląd statusu procesu | Każda karta sesji z aktywnym procesem w tle | 1 | Widoczny bez interakcji, gdy proces w tle jest aktywny |
| Kontrolka zamknięcia („✕”) | Mała ikona | Zamknięcie karty sesji | `.dn-btn-ikona` mały, ikona `zamknij`, w prawym rogu karty | Domyślny · wskazanie kursorem · aktywny · fokus | Kliknięcie zamyka widok karty w bieżącym oknie; los procesu sesji leżącego u podstaw jest funkcją modelu sesji i procesów (Architektura, rozdz. 5), poza zakresem tego dokumentu | Każda karta sesji | 2 | Ujawniana wskazaniem kursorem na karcie; kliknięcie zamyka kartę |
| Kontrolka nowej karty („+”) | Przycisk ikonowy | Otwarcie nowej, pustej karty sesji w bieżącym środowisku | `.dn-btn-ikona`, ikona `plus`, na końcu paska kart | Domyślny · wskazanie kursorem · aktywny · fokus | Kliknięcie tworzy nową kartę z odrębnym układem, historią i kontekstem (stan wyjściowy izolacji, konfigurowalny); boczna nawigacja / panel orkiestracji aktywna, obszar roboczy w stanie pustym (rozdz. 3.5) | Koniec paska kart sesji, wszystkie 4 środowiska | 1 | Widoczna bez interakcji |
| Obszar przełączania karty | Cała powierzchnia karty poza kontrolką zamknięcia | Uczynienie karty aktywną | Cała karta jako obszar klikalny | Aktywna (podświetlona, na pierwszym planie) · w tle (przygaszona) · wskazanie kursorem | Kliknięcie przełącza główny obszar roboczy na układ, historię i kontekst zapisane w tej karcie; boczna nawigacja/panel orkiestracji podświetla właściwy moduł/sekcję | Każda karta sesji | 1 | Widoczny bez interakcji |
...
```
  ...
  │   Workspace  │
  │   Browser    │
  │ ▸ Research   │  ← stan wybrany (podświetlenie, tło `--dn-sygnal-tlo`)
  │   Library    │
  │   Translate  │
  │   Roundtable │
  │   Assistant  │
...
│                    (ikona pustego stanu)                    │
│                                                             │
│           Wybierz moduł z bocznej nawigacji,                │
│                  aby rozpocząć pracę                        │
│                                                             │
│                                                             │
└─────────────────────────────────────────────────────────────┘
...
| Element | Co to jest | Do czego służy | Forma i waga | Stany | Zachowanie po interakcji | Gdzie występuje | Warstwa | Sposób wywołania |
|---|---|---|---|---|---|---|---|---|
| Historia rozmowy | Przewijana lista wiadomości | Prezentacja dotychczasowego przebiegu rozmowy w bieżącym module | Obszar przewijany, treść krojem bazowym; cytowania kodu krojem mono (`--dn-ff-mono`) | Pusta (nowa karta/moduł) · z treścią · przewijana | Przewijanie w górę pokazuje starsze wiadomości; domyślnie odrębna per karta sesji, współdzielenie konfigurowalne (poza zakresem) | Chat Window w każdym module, wszystkie 4 środowiska | 1 | Widoczna bez interakcji |
| Pole wprowadzania poleceń | Pole tekstowe wieloliniowe | Wpisanie polecenia/pytania do AI | `.dn-pole-kontrolka`, w stopce kolumny Chat Window, pełna szerokość kolumny | Puste (tekst podpowiedzi) · wypełnione · fokus | Pole pozostaje edytowalne również podczas generowania odpowiedzi (można przygotować kolejne polecenie w tym czasie); Enter/kliknięcie „Wyślij” wysyła treść — treść trafia do historii jako wiadomość użytkownika | Chat Window, wszystkie moduły | 1 | Widoczne bez interakcji |
| Przycisk „Wyślij” | Ikona strzałki/samolotu | Wysłanie wpisanego polecenia | `.dn-btn-ikona`, ikona `wyslij`, przy polu poleceń | Domyślny · wskazanie kursorem · aktywny | Przycisk zawsze klikalny; przy pustym polu kliknięcie pokazuje krótkie ostrzeżenie zamiast wysyłki; w przeciwnym razie wysyła treść pola do AI i czyści pole | Chat Window, wszystkie moduły | 1 | Widoczny bez interakcji |
| Przycisk „Załącznik” | Ikona spinacza | Dołączenie pliku do polecenia | `.dn-btn-ikona`, ikona `spinacz` | Domyślny · wskazanie kursorem · aktywny | Kliknięcie otwiera wybór pliku; załącznik pojawia się jako podgląd nad polem poleceń | Chat Window, wszystkie moduły | 1 | Widoczny bez interakcji |
| Kontrolka „Odpowiedz/cytuj” | Mała ikona przy wiadomości | Odniesienie się do konkretnej wcześniejszej wiadomości | `.dn-btn-ikona`, ikona `odpowiedz`, widoczna przy wskazaniu kursorem wiadomości | Ukryta domyślnie · widoczna na wskazanie kursorem wiadomości | Kliknięcie wstawia cytat do pola poleceń | Chat Window, wszystkie moduły | 2 | Ujawniana wskazaniem kursorem na wiadomości |
...
| Element zbiorczy „Operacje ▼” | Jeden przycisk zwinięty zastępujący zestaw przycisków akcji | Grupowanie logiczne akcji Chat Window (zastosuj do zaznaczenia, przekaż do modułu, zapisz wynik, ponów, wyczyść kontekst) | Przycisk `.dn-btn--zarys` ze znacznikiem `▼`, przy polu poleceń | Zwinięty (spoczynek) · rozwinięty (lista akcji) · fokus | Kliknięcie rozwija pełną listę akcji; wybór akcji zwija element samoczynnie | Chat Window, wszystkie moduły | 3 | Kliknięcie przycisku „Operacje ▼” — jedno kliknięcie do każdej akcji |
| Menu kebab wiadomości (⋮) | Menu rozwijane pojedynczej wiadomości | Dostęp do wariantów operacji na wiadomości: kopiuj, cytuj, rozwiń kontekst, ponów odpowiedź, zgłoś wynik | Ikona `⋮` w narożniku bloku wiadomości | Ukryte w spoczynku · widoczne na wskazanie kursorem/fokus wiadomości · otwarte | Kliknięcie otwiera menu; wybór pozycji zamyka menu | Chat Window, wszystkie moduły | 3 | Kliknięcie ikony ⋮ w narożniku wiadomości |
| Pasek znaczników kontekstowych | Lekkie znaczniki środowiska, repozytorium, projektu, modelu i wykonawcy | Prezentacja i zmiana kontekstu pracy bez otwierania okna konfiguracji, np. `[Danaco Console] [Ubuntu] [Worktree] [Fable 5] [Ultra]` | Rząd plakietek `.dn-plakietka` nad polem poleceń | Spoczynek (zwinięte znaczniki) · znacznik aktywny · selektor otwarty | Kliknięcie znacznika otwiera właściwy selektor (model, wykonawca, środowisko, tryb pracy, poziom wysiłku); po wyborze selektor zwija się samoczynnie | Chat Window, wszystkie moduły | 2 | Widoczne w spoczynku jako znaczniki zwinięte; kliknięcie znacznika otwiera selektor |
| Kontrolki zatwierdzenia i przerwania | Para przycisków sterujących przebiegiem | Zatwierdzenie działania zaproponowanego przez Wykonawcę oraz przerwanie działania w toku | `.dn-btn--sygnal` (zatwierdź) i `.dn-btn--zarys` (przerwij), w bloku wiadomości wymagającej decyzji | Ukryte, gdy decyzja nie jest wymagana · widoczne przy oczekiwaniu na decyzję · widoczne w trakcie generowania (przerwij) | Kliknięcie „Zatwierdź” uruchamia działanie; kliknięcie „Przerwij” zatrzymuje strumień i zapisuje stan częściowy w historii | Chat Window, wszystkie moduły | 2 | Ujawniane zdarzeniem: oczekiwaniem na decyzję albo generowaniem odpowiedzi |
...
Execution Loop Window jest drugim kanałem komunikacji operacyjnej platformy i elementem pierwszoplanowym architektury każdego środowiska. Prezentuje komunikację między Koordynatorem — komponentem orkiestrującym platformy — a Wykonawcą. Odpowiada za prowadzenie pętli wykonawczej, koordynację zadań, nadzór nad realizacją, orkiestrację działań oraz kontrolę realizacji procesów. Okno otwierane jest jako **kolumna sąsiadująca z Chat Window** — obok niego, nie pod nim — i zachowuje pełną wysokość przypisanej mu przestrzeni. Nagłówek okna należy do warstwy 1, jego rozwinięcia sterujące — do warstw 2 i 3 (rozdz. 7).
...
| Blok bieżącego zlecenia | Nazwa zlecenia wraz z jego dekompozycją na zadania | Prezentacja tego, co Koordynator zlecił Wykonawcy i jak zostało rozłożone na zadania | Lista zagnieżdżona pozycji `.dn-karta--klikalna` ze wskaźnikami stanu | Brak zlecenia · zlecenie aktywne · zlecenie zakończone | Kliknięcie zadania rozwija jego szczegóły w panelu popover | Kolumna komunikacji, wszystkie 4 środowiska | 1 | Widoczny bez interakcji |
| Kolejka i stan zadań | Lista zadań ze stanem wykonania | Podgląd kolejności realizacji i bieżącego stanu każdego zadania | `.dn-kropka` semantyczna + tekst zadania | W kolejce · w toku · zakończone · odrzucone | Kliknięcie zadania przenosi kontekst wymiany komunikatów do tego zadania | Kolumna komunikacji, wszystkie 4 środowiska | 1 | Widoczna bez interakcji |
...
| Wynik kontroli jakości | Blok statusu weryfikacji zadania | Prezentacja rozstrzygnięcia kontroli jakości i decyzji o ponowieniu | `.dn-plakietka` (`--sukces`/`--blad`/`--informacja` wg stanu) + licznik ponowień | Zgodne · niezgodne (ponowienie) · oczekuje | Kliknięcie plakietki otwiera panel popover ze szczegółami niezgodności | Kolumna komunikacji, wszystkie 4 środowiska | 2 | Widoczny jako plakietka; kliknięcie otwiera panel popover ze szczegółami |
...
| Element zbiorczy „Sterowanie ▼” | Jeden przycisk zwinięty grupujący akcje sterowania przebiegiem | Wstrzymanie, wznowienie, przerwanie oraz korekta zlecenia | Przycisk `.dn-btn--zarys` ze znacznikiem `▼`, stopka kolumny | Zwinięty · rozwinięty · fokus | Kliknięcie rozwija listę akcji sterujących; wybór akcji zwija element samoczynnie | Kolumna komunikacji, wszystkie 4 środowiska | 3 | Kliknięcie przycisku „Sterowanie ▼” |
| Kontrolka otwarcia i zamknięcia okna pętli | Przełącznik kolumny Execution Loop Window | Otwarcie kolumny pętli wykonawczej i jej zamknięcie | Ikona w nagłówku Chat Window oraz w belce okna roboczego | Zamknięte (kolumna nieobecna) · otwarte | Kliknięcie otwiera kolumnę sąsiadującą; ponowny kliknięcie zamyka ją, a kolumna znika całkowicie z przestrzeni roboczej | Nagłówek Chat Window i belka okna roboczego, wszystkie 4 środowiska | 2 | Kliknięcie ikony w nagłówku Chat Window albo w belce okna roboczego |
...
| Element | Co to jest | Do czego służy | Forma i waga | Stany | Zachowanie po interakcji | Gdzie występuje | Warstwa | Sposób wywołania |
|---|---|---|---|---|---|---|---|---|
| Panel wysuwany | Kolumna boczna otwierana z prawej krawędzi obszaru roboczego | Pomieszczenie funkcji potrzebnych chwilowo: szczegółów elementu, historii wersji, listy narzędzi | Kolumna boczna o regulowanej szerokości, pełna wysokość obszaru roboczego | Zamknięty (nieobecny) · otwarty · szerokość regulowana | Kliknięcie wyzwalacza otwiera kolumnę; zamknięcie usuwa ją z przestrzeni roboczej bez pozostawiania śladu | Prawa strona obszaru roboczego, wszystkie 4 środowiska | 3 | Kliknięcie ikony lub znacznika kontekstowego |
| Panel popover | Mały panel zakotwiczony przy elemencie wywołującym | Prezentacja szczegółów i ustawień szybkich bez zmiany układu kolumn | Panel unoszony `.dn-karta` (pływająca, zakotwiczona przy wyzwalaczu), szerokość dopasowana do treści | Zamknięty · otwarty · fokus wewnątrz | Kliknięcie poza panelem zamyka go; wybór pozycji zamyka panel i stosuje ustawienie | Przy znacznikach kontekstowych, plakietkach stanu i pozycjach list | 3 | Kliknięcie elementu kotwiczącego |
| Menu aplikacji (☰) | Głowa szyny nawigacji | Otwarcie menu aplikacji (Plik, Edycja, Widok, Przejdź, Narzędzia, Dla programistów, Pomoc) z podmenu rozwijanymi w bok | Ikona `menu` w kwadracie narożnika okna | Zwinięte · rozwinięte | Kliknięcie rozwija menu; podmenu otwiera się w prawo na wskazanie kursorem i fokus | Szyna nawigacji, wszystkie 4 środowiska | 3 | Kliknięcie ikony ☰ |
| Menu kebab powłoki (⋮) | Menu rozwijane w belce okna roboczego | Dostęp do zestawu akcji powłoki: konfiguracja sesji, motyw, Mobile, Always On Display, tryb administracyjny | Ikona `⋮` w belce okna roboczego | Zwinięte · rozwinięte · pozycja podświetlona | Kliknięcie rozwija listę akcji; wybór pozycji zwija menu | Belka okna roboczego, wszystkie 4 środowiska | 3 | Kliknięcie ikony ⋮ |

### 3.6 Punkty łamania

Cztery progi szerokości okna niosą żetony `--dn-bp-w1`–`--dn-bp-w4` (`design/zasoby/zetony/zetony.css`); zachowanie poniżej podaje przełamania rzeczywiście zaimplementowane regułami `@media` w `design/zasoby/rama.css`. Powyżej `--dn-bp-w3` układ nie zmienia się dalej — szerokie biurko (`--dn-bp-w4`) mieści oba okna komunikacji operacyjnej obok siebie bez kompresji kolumn.

**Tabela 13c — Punkty łamania ramy okna i powłoki**

| Próg szerokości | Żeton | Zachowanie układu |
|---|---|---|
| ≤ 640 px | `--dn-bp-w1` | Widok mobilny — poza zakresem tego dokumentu (funkcja globalna Mobile — [Mobile](../funkcje-globalne/mobile.md)); rama okna aplikacji nie renderuje się w tej szerokości na urządzeniu klasy komputerowej |
| ≤ 900 px | — (próg pomocniczy, bez żetonu `--dn-bp-*`) | Siatka dwukolumnowa okna „Dostosuj wstążkę” (rozdz. 3.2.2) zwija się do jednej kolumny |
| ≤ 960 px | `--dn-bp-w2` | Belka tytułowa ukrywa tytuł bieżącego widoku (`.dn-belka-tytul`); pasek edycji ukrywa rodzinę „kontekst i praca w tle” (`.dn-narzedzia-grupa--prawa`) i rodzinę drugorzędną; szyna nawigacji zwija pozycję środowiska z 40 × 40 px do 36 × 36 px (`.dn-szyna-poz`); pasek stanu ukrywa pozycje drugorzędne (`.dn-stan-poz--drugorzedna`); boczna nawigacja modułów (rozdz. 3.4) i panel orkiestracji (rozdz. 4.4) przechodzą pod menu hamburger (☰, warstwa 3) |
| ≤ 1200 px | `--dn-bp-w2`–`--dn-bp-w3` (próg pomocniczy wewnątrz przedziału) | Przycisk „Dostosuj” traci etykietę tekstową i pozostaje samą ikoną; rodzina oznaczona `--proba-1200` w pasku edycji znika |
| ≤ 1280 px | `--dn-bp-w3` | Próg biurka — pełny kokpit; poniżej tego progu pole wyszukiwania paska edycji zwęża podstawę do 340 px, a rodzina oznaczona `--proba-1400` znika |
| > 1280 px | `--dn-bp-w3`–`--dn-bp-w4` | Pełny układ ramy i powłoki bez kompresji; od `--dn-bp-w4` (1600 px) mieszczą się jednocześnie Chat Window i Execution Loop Window jako dwie kolumny pełnej wysokości bez zwężania kolumny obszaru roboczego modułu |
...
| Rama okna | Belka tytułowa `.dn-belka`, szyna nawigacji `.dn-szyna-nawigacji`, pasek edycji `.dn-narzedzia` i pasek stanu `.dn-stan` (rozdz. 3.2) | Ten sam wzorzec wspólny — bez zmian (rozdz. 4.2) |
...
║   i automatyki ║ ──────────── ║   poza zakresem tego    ║               ║
║   Monitor      ║ EXECUTION    ║   dokumentu ]           ║               ║
║   procesu      ║ LOOP WINDOW  ║                         ║               ║
║                ║ Koordynator  ║                         ║               ║
║                ║ ↔ Wykonawca  ║                         ║               ║
╚════════════════╩══════════════╩═════════════════════════╩═══════════════╝
...
### 4.3 Rama okna i karty sesji — dostosowania

**Tabela 15 — Różnice elementów w MultitaskingAI**

| Element | Zachowanie w powłoce modułowej | Zachowanie w powłoce MultitaskingAI |
|---|---|---|
| Rama okna | Belka tytułowa, szyna nawigacji, pasek edycji i pasek stanu w pełnym składzie | Identyczny zestaw i mechanika — bez zmian (rozdz. 3.2) |
...
| Sekcja „Zespoły” | Pozycja listy | Otwarcie widoku zapisanych konfiguracji zespołu (presety ról, powiązań, kolejek) | `.dn-karta--klikalna` w układzie pionowym, ikona `gwiazdka`/`plik` | Domyślny · wskazanie kursorem · wybrany · fokus | Kliknięcie pokazuje w obszarze roboczym listę zapisanych zespołów z akcjami zapisu/wczytania/duplikowania | Panel orkiestracji | 1 | Widoczna bez interakcji |
| Sekcja „Role” | Pozycja listy | Otwarcie czterech okien roboczych ról i przypisania agentów | `.dn-karta--klikalna` w układzie pionowym, ikona `uzytkownik` | Domyślny · wskazanie kursorem · wybrany (domyślnie aktywna w przykładzie) · fokus | Kliknięcie pokazuje w obszarze roboczym 4 karty ról: Executor 1, Executor 2, Coordinator, Executor 3 / Validator (rozdz. 4.5) | Panel orkiestracji | 1 | Widoczna bez interakcji |
| Sekcja „Kolejki” | Pozycja listy | Otwarcie definicji kolejek (globalne, lokalne, modeli, agentów, projektów) i ich akcji | `.dn-karta--klikalna` w układzie pionowym, ikona `filtr`/`strzalka-prawo` | Domyślny · wskazanie kursorem · wybrany · fokus | Kliknięcie pokazuje tabelę kolejek z akcjami enqueue/dequeue/retry jako przyciski ikonowe | Panel orkiestracji | 1 | Widoczna bez interakcji |
| Sekcja „Orkiestracja” | Pozycja listy | Otwarcie zależności między modelami, agentami, zadaniami, kolejkami, automatyzacjami, projektami | `.dn-karta--klikalna` w układzie pionowym, ikona `link-zewnetrzny` | Domyślny · wskazanie kursorem · wybrany · fokus | Kliknięcie pokazuje listę zależności ze statusem zgodności (plakietka) | Panel orkiestracji | 1 | Widoczna bez interakcji |
| Sekcja „Harmonogram i automatyki” | Pozycja listy | Otwarcie harmonogramu pracy ciągłej i wpiętych automatyk z modułu Automations | `.dn-karta--klikalna` w układzie pionowym, ikona `zegar`/`kalendarz` | Domyślny · wskazanie kursorem · wybrany · fokus | Kliknięcie pokazuje listę wpiętych automatyk i harmonogram — wzorzec Scheduler (moduł Automations, poza zakresem) | Panel orkiestracji | 1 | Widoczna bez interakcji |
| Sekcja „Monitor procesu” | Pozycja listy | Otwarcie podglądu przebiegu pętli, statusów przebiegów, hierarchii decyzji | `.dn-karta--klikalna` w układzie pionowym, ikona `oko` | Domyślny · wskazanie kursorem · wybrany · fokus | Kliknięcie pokazuje tabelę statusów procesów z plakietkami stanu; miejsce nadzoru funkcji Always On Display | Panel orkiestracji | 1 | Widoczna bez interakcji |
...
│    poza zakresem tego dokumentu ]                           │
└─────────────────────────────────────────────────────────────┘
   `.dn-tabela` + `.dn-plakietka` (`--sukces`/`--blad`/`--informacja` wg stanu)
   — wzorzec monitora, jak w oknach modułu Automations (Execution Monitor,
   Process Monitor)
```
...
| Element | Co to jest | Do czego służy | Forma i waga | Stany | Zachowanie po interakcji | Gdzie występuje | Warstwa | Sposób wywołania |
|---|---|---|---|---|---|---|---|---|
| Obszar roboczy (kontener) | Główna powierzchnia powłoki MultitaskingAI | Pomieszczenie zawartości wybranej sekcji panelu orkiestracji | Kolumna dominująca układu, między kolumną komunikacji a kolumnami paneli pomocniczych, pod kartami sesji | Pusty (nowy proces, brak skonfigurowanych ról) · przeładowanie (zmiana sekcji) · wypełniony | Kliknięcie sekcji panelu zamienia całą zawartość kontenera | Każda karta sesji środowiska MultitaskingAI | 1 | Widoczny bez interakcji |
| Karta roli (w sekcji „Role”) | Kafel reprezentujący jedną z czterech ról | Podgląd skrócony roli (przypisany agent/model, status) i wejście w jej okno robocze | `.dn-karta` z `.dn-awatar` (wariant `--kwadrat` dla agenta przypisanego, kołowy dla modelu bazowego) | Bez przypisanego wykonawcy · z przypisanym agentem/modelem · aktywny (praca w toku) · nieaktywny | Kliknięcie otwiera pełne okno robocze roli (Executor 1, Executor 2, Coordinator, Executor 3 / Validator) — **poza zakresem tego dokumentu** | Obszar roboczy, sekcja „Role” — dokładnie 4 karty | 1 | Widoczna bez interakcji |
| Rozwinięcie Subagent Network | Zagnieżdżona lista podagentów pod kartą wykonawcy | Podgląd do 15 podagentów uruchamianych przez rolę Executor | Zagnieżdżone pozycje `.dn-karta--klikalna` pod kartą roli, nie osobna sekcja panelu | Zwinięte · rozwinięte · pojedynczy podagent aktywny/zakończony | Kliknięcie roli z uruchomionym Subagent Network rozwija listę podagentów | Karta roli Executor 1 / Executor 2, gdy mechanizm aktywny | 3 | Kliknięcie karty roli z uruchomionym mechanizmem rozwija listę podagentów |
| Zawartość innych sekcji (Zespoły, Kolejki, Orkiestracja, Harmonogram, Monitor) | Widok listy/tabeli właściwy sekcji | Prezentacja danych organizacyjnych warstwy orkiestracji | `.dn-tabela` albo pozycja listy `.dn-karta--klikalna`, zależnie od sekcji (Tabela 16) | Pusty stan (brak konfiguracji) · z zawartością | Zależne od sekcji — szczegóły managerów (Queue Manager, Orchestrator, Scheduler, Execution Monitor) poza zakresem, patrz dokument modułu Automations | Obszar roboczy, sekcje inne niż „Role” | 1 | Widoczna bez interakcji |
...
┌──────────────────────────┐               ┌──────────────────────────┐
│ rama okna                 │               │ rama okna                 │  ← identyczny
├──────────────────────────┤               ├──────────────────────────┤
...
| Powłoka modułowa — rama okna (szyna nawigacji, belka, pasek edycji, pasek stanu) | Elementy ramy — rozdz. 3.2 | 3.2 | 1 | — |
...
| Powłoka MultitaskingAI — różnice oraz rama okna/karty | Tabela różnic (Tabela 14) + tabela dostosowań (Tabela 15) | 4.1, 4.3 | 2 | — |
...
| Wskazanie kursorem | Subtelna zmiana tła albo uniesienie 1–2 px dla elementów akcentowanych sygnałowo | Kursor nad elementem interaktywnym |
| Aktywny (wciśnięcie) | Przesunięcie 1 px w dół (`translateY(1px)`) | Moment kliknięcia/naciśnięcia |
| Fokus | Pierścień sygnałowy 2 px (`--dn-fokus`) + odsunięcie 2 px | Nawigacja klawiaturą (`:focus-visible`) — obowiązkowy dla wszystkich kontrolek interaktywnych |
| Niegotowy / niekompletny (bez blokady interakcji) | Element pozostaje w pełni klikalny — bez wygaszenia, bez zmiany kursora, bez odebrania reakcji na wskazanie kursorem; niekompletność sygnalizowana opisowo obok elementu albo komunikatem/ostrzeżeniem po kliknięciu | Warunek wykonania niespełniony w bieżącym kontekście (np. puste pole wymagane) — kliknięcie wywołuje komunikat zamiast wykonania akcji; element nigdy nie traci klikalności |
| Zaznaczony / wybrany / aktywny (stan trwały) | Akcent sygnałowy: podkreślenie zakładki, obrys karty, tło `--dn-sygnal-tlo` | Pozycja bocznej nawigacji, zakładka, segment przełącznika aktualnie wybrane |
...
| Błąd | Kreska/obrys czerwony (`--dn-blad-tekst`/`--dn-blad-tlo`/`--dn-blad-obrys`), komunikat z ikoną `blad` | Walidacja nieudana, dane odrzucone przez serwer |
...
| Magistrala kontekstu | Mechanizm przekazywania artefaktu między modułami — kolumna boczna z referencjami do odłożonych artefaktów; opis rozstrzygający w [Przepływ okien](przeplyw-okien.md), rozdz. 6a | 3.2 |
| Centrum powiadomień | Jedyny mechanizm powiadamiania platformy — kolumna boczna z rejestrem zdarzeń, wywoływana plakietką powiadomień w pasku kontekstu; karta komponentu w [Katalog komponentów](katalog-komponentow.md), rozdz. 11.6 | 3.2 |
| Paleta poleceń | Jedyna klawiaturowa nakładka platformy, wywoływana skrótem `Ctrl/Cmd + K`; karta komponentu w [Katalog komponentów](katalog-komponentow.md), rozdz. 15.8 | 3.2 |
...
| Rama okna | Cztery pasy obecne w każdym oknie platformy — szyna nawigacji (menu aplikacji, środowiska i moduły, szybki wybór, ustawienia, motyw i profil w stopce), belka tytułowa (znak i nazwa produktu, tytuł widoku, sterowanie oknem systemowym), pasek edycji (czynności okna i treści, pole wyszukiwania z filtrem zakresu, pasmo kart otwartych sesji) oraz pasek stanu (operator, położenie, sesje czynne, motyw, obciążenie zasobów) | 3.2, 4.3 |
...
| Znacznik kontekstowy | Lekka plakietka opisująca jeden wymiar kontekstu pracy (środowisko, repozytorium, projekt, model, wykonawca); kliknięcie otwiera właściwy selektor | 7.3, 7.5 |
| Element zbiorczy z rozwinięciem | Jeden przycisk zwinięty (`Operacje ▼`, `Sterowanie ▼`) zastępujący zestaw przycisków akcji | 7.3, 7.5 |
| Uproszczone menu kontekstowe | Skrócony dostęp ze stopki szyny nawigacji (menu „Ustawienia i konfiguracja”) albo z okna operacyjnego do ustawień warstwy sesji, bez opuszczania trwającej pracy | 3.2 |
...
|---|---|---|---|---|---|---|
| Element zbiorczy z rozwinięciem (`Operacje ▼`) | Jeden przycisk zwinięty zastępujący zestaw przycisków akcji | Grupowanie logiczne akcji jednorodnego przeznaczenia | Przycisk `.dn-btn--zarys` ze znacznikiem `▼`, waga pojedynczego przycisku | Zwinięty · rozwinięty · fokus | 3 | Kliknięcie elementu; wybór akcji zwija element samoczynnie |
| Menu kebab (⋮) | Menu rozwijane przypisane do pojedynczego elementu lub okna | Dostęp do wariantów operacji na konkretnym elemencie | Ikona `⋮` w narożniku elementu, waga ikony | Ukryte w spoczynku · widoczne na wskazanie kursorem/fokus · otwarte | 3 | Kliknięcie ikony; kliknięcie poza menu zamyka je |
| Menu hamburger (☰) | Zwinięty wyzwalacz nawigacji | Otwarcie kolumny nawigacji, gdy szerokość okna jej nie mieści | Ikona `menu` w pasku edycji | Zwinięte · rozwinięte | 3 | Kliknięcie ikony; wybór pozycji zwija menu |
| Panel popover | Mały panel zakotwiczony przy elemencie wywołującym | Prezentacja szczegółów i ustawień szybkich bez zmiany układu kolumn | Panel unoszony, szerokość dopasowana do treści | Zamknięty · otwarty · fokus wewnątrz | 3 | Kliknięcie elementu kotwiczącego; kliknięcie poza panelem zamyka go |
| Panel wysuwany | Kolumna boczna otwierana z prawej krawędzi obszaru roboczego | Pomieszczenie funkcji potrzebnych chwilowo | Kolumna boczna o regulowanej szerokości, pełna wysokość obszaru roboczego | Zamknięty (nieobecny) · otwarty | 3 | Kliknięcie ikony lub znacznika; zamknięcie usuwa kolumnę z przestrzeni roboczej |
| Znacznik kontekstowy | Lekka plakietka opisująca jeden wymiar kontekstu pracy | Prezentacja i zmiana środowiska, repozytorium, projektu, modelu, wykonawcy, trybu pracy, poziomu wysiłku | Plakietka `.dn-plakietka` w pasku kontekstu | Spoczynek · aktywny · selektor otwarty | 2 | Kliknięcie znacznika otwiera selektor; po wyborze selektor zwija się samoczynnie |
...
|---|---|
| `dom` | Powrót do strony głównej / Centrum dowodzenia — kontrolka „Centrum dowodzenia” na pasku edycji (3.2, 4.3) |
| `ustawienia` | Otwarcie okna konfiguracji z listwy strony głównej (2.5); menu „Ustawienia i konfiguracja” w stopce szyny nawigacji (3.2) |
| `szukaj` | Pole wyszukiwania w pasku edycji — z filtrem zakresu środowisk (3.2) |
...
| `dzwonek` | Always On Display — szybki wybór szyny nawigacji (3.2, 4.3), listwa strony głównej (2.5) |
...
| `slonce` / `ksiezyc` | Przełącznik motywu jasny/ciemny — stopka szyny nawigacji (3.2) |
| `blad` / `ostrzezenie` / `info` | Komunikaty stanu formularza logowania/rejestracji (1.5) |
| `⋮` (kebab) | Menu kebab wiadomości Chat Window (3.5.1) i menu kebab powłoki w belce okna roboczego (3.2, 3.5.3) |
| `menu` | Menu hamburger — otwarcie bocznej nawigacji / panelu orkiestracji na wąskich ekranach (3.4, 4.4, 7.3) |

Pełny zbiór ikon platformy (82 pozycje, format SVG): `design/zasoby/ikony/manifest.json` i `design/zasoby/ikony/svg/`.
...
| [Środowisko MultitaskingAI](../srodowiska/multitaskingai.md) | 6 (Panel orkiestracji — boczna nawigacja) |
| architektura/izolacja-i-zaleznosci.md | 6 (poziomy zasięgu — kontekst dla karty sesji i roli) |
| tokens.css | Tokeny ramy okna (`--dn-wym-szyna`, `--dn-wym-belka`, `--dn-wym-pasek`, `--dn-wym-stan`) |
| components.css | `.dn-szyna-nawigacji`, `.dn-belka`, `.dn-belka-marka`, `.dn-narzedzia`, `.dn-narzedzia-szukaj`, `.dn-stan`, `.dn-zakladki--pigulki`, `.dn-awatar` |

---

*Koniec dokumentu. Elementy okien przepływu głównego — Specyfikacja docelowa, wersja 2.0, 2026-08-20.*
