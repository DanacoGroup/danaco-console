...
| | |
|---|---|
| **Tytuł** | Okno Konfiguracji |
| **Klasa dokumentu** | Specyfikacja docelowa |
| **Odbiorcy** | projektant · deweloper |
| **Przeznaczenie** | Ustala projekt okna Konfiguracji — panelu programującego nakładkę na model bazowy, pełnego zestawu pól sterujących wywołaniem modelu, konfiguracji obu kanałów komunikacji operacyjnej, konfiguracji warstw widoczności oraz Panelu prowenancji wywołania |
| **Zakres** | warstwowa konfiguracja nakładki (Konstytucja → Profil/Rola → Ekspertyza zadaniowa), pełny zestaw pól sterujących wywołaniem modelu, profile ekspertów, dane dostępowe i zmienne środowiskowe, zależności i izolacja jako ustawienia konfiguracyjne, uwierzytelnianie jako ustawienie konfiguracyjne, konfiguracja Chat Window i Execution Loop Window, konfiguracja warstw widoczności, Panel prowenancji, import i eksport konfiguracji, makieta, katalog elementów, stany, komunikacja klient–serwer |
| **Poza zakresem** | tożsamość Operatora, metody uwierzytelniania i urządzenia — [Okno Ustawień](ustawienia.md); pełna specyfikacja pozostałych jedenastu zakresów okna Konfiguracji (Aplikacja, Procesy, Akcje, Rozszerzenia, Integracje, Historia, Pamięć, Izolacja, Karty sesji, Komponenty własne, Always On Display) — [Model konfiguracji](../architektura/model-konfiguracji.md); okno konfiguracji punktów izolacji — [Izolacja i konfigurowalność zależności](../architektura/izolacja-i-zaleznosci.md); wymiary, tokeny kolorów i typografii, ostateczny układ graficzny — prace wykonawcze |
| **Dokument nadrzędny** | [Elementy okien](elementy-okien.md) |
| **Dokumenty powiązane** | [Elementy okien](elementy-okien.md) · [Katalog komponentów](katalog-komponentow.md) · [Przepływ okien](przeplyw-okien.md) · [Okno Ustawień](ustawienia.md) · [Model konfiguracji](../architektura/model-konfiguracji.md) · [Architektura techniczna](../architektura/architektura.md) · [Integracja modeli](../architektura/integracja-modeli.md) · [Specyfikacja agentów](../specyfikacje/specyfikacja-agentow.md) · [Izolacja i konfigurowalność zależności](../architektura/izolacja-i-zaleznosci.md) · [Bezpieczeństwo i uwierzytelnianie](../architektura/bezpieczenstwo-i-uwierzytelnianie.md) · [System wizualny](system-wizualny.md) · [Rozszerzenia](../architektura/rozszerzenia.md) |
| **Prototypy odniesienia** | `design/05-okna/platformowe/konfiguracja.html` |
| **Źródła normatywne** | `budowa/shared/contract.json` (obszary `config`, `settings`, `panel`, `isolation`, `provenance`, `model`, `role`, `agent`) · `design/zasoby/zetony/zetony.css` · `design/zasoby/css/komponenty.css`, `rama.css`, `prototyp.css` |
| **Zasada nadrzędna** | Żadna pozycja konfiguracji nie wprowadza twardej blokady — brak ustawienia oznacza dziedziczenie albo wartość domyślną, przyciski pozostają aktywne, walidacja ma charakter wyłącznie ostrzegawczy, dane dostępowe są domyślnie jawne, a uwierzytelnianie i izolacja są ustawieniami konfiguracyjnymi, nie wymogami |
...
1. [Wprowadzenie i pozycja dokumentu](#wprowadzenie-i-pozycja-dokumentu)
   - [Zakres dokumentu](#zakres-dokumentu)
2. [Okno Konfiguracji jako serce produktu](#1-okno-konfiguracji-jako-serce-produktu)
   - [1.1 Dlaczego serce produktu](#11-dlaczego-serce-produktu)
   - [1.2 Siedemnaście zakresów okna Konfiguracji — struktura](#12-siedemnaście-zakresów-okna-konfiguracji--struktura)
   - [1.3 Nakładka i wywołanie modelu — scalony obszar roboczy](#13-nakładka-i-wywołanie-modelu--scalony-obszar-roboczy)
3. [Warstwowa konfiguracja nakładki: Konstytucja → Profil/Rola → Ekspertyza zadaniowa](#2-warstwowa-konfiguracja-nakładki-konstytucja--profilrola--ekspertyza-zadaniowa)
   - [2.1 Trzy warstwy nakładki](#21-trzy-warstwy-nakładki)
   - [2.2 Gdzie każda warstwa się ustala](#22-gdzie-każda-warstwa-się-ustala)
   - [2.3 Kompozycja nakładki w jeden prompt systemowy](#23-kompozycja-nakładki-w-jeden-prompt-systemowy)
   - [2.4 Precedencja i dziedziczenie](#24-precedencja-i-dziedziczenie)
   - [2.5 Relacja do ogólnej warstwowości platformy](#25-relacja-do-ogólnej-warstwowości-platformy)
   - [2.6 Diagram warstw nakładki](#26-diagram-warstw-nakładki)
   - [2.7 Pola treści nakładki](#27-pola-treści-nakładki)
   - [2.8 Sterowanie zasięgiem pola](#28-sterowanie-zasięgiem-pola)
4. [Pełny zestaw pól sterujących wywołaniem modelu](#3-pełny-zestaw-pól-sterujących-wywołaniem-modelu)
   - [3.0 Zestawienie ogólne](#30-zestawienie-ogólne)
   - [3.1 System prompt](#31-system-prompt)
   - [3.2 Harness](#32-harness)
   - [3.3 Hooki](#33-hooki)
   - [3.4 Narzędzia dozwolone i zabronione](#34-narzędzia-dozwolone-i-zabronione)
   - [3.5 Model](#35-model)
   - [3.6 Effort i parametry próbkowania](#36-effort-i-parametry-próbkowania)
   - [3.7 Kanał i tryb wywołania](#37-kanał-i-tryb-wywołania)
   - [3.8 Pula kont i rotacja — kanał Code CLI](#38-pula-kont-i-rotacja--kanał-code-cli)
   - [3.9 Objaśnienia kontekstowe — tabela zbiorcza](#39-objaśnienia-kontekstowe--tabela-zbiorcza)
5. [Profile ekspertów](#4-profile-ekspertów)
   - [4.1 Profil eksperta jako byt konfiguracji](#41-profil-eksperta-jako-byt-konfiguracji)
   - [4.2 Wersjonowanie profilu eksperta](#42-wersjonowanie-profilu-eksperta)
6. [Dane dostępowe i zmienne środowiskowe](#5-dane-dostępowe-i-zmienne-środowiskowe)
7. [Zależności i izolacja jako ustawienia konfiguracyjne](#6-zależności-i-izolacja-jako-ustawienia-konfiguracyjne)
   - [6.1 Relacja do okna konfiguracji punktów izolacji](#61-relacja-do-okna-konfiguracji-punktów-izolacji)
   - [6.2 Relacja do zakresu Integracje](#62-relacja-do-zakresu-integracje)
   - [6.3 Relacja do Rozszerzeń i Permissions Center](#63-relacja-do-rozszerzeń-i-permissions-center)
   - [6.4 Stan wyjściowy](#64-stan-wyjściowy)
8. [Uwierzytelnianie jako ustawienie konfiguracyjne](#7-uwierzytelnianie-jako-ustawienie-konfiguracyjne)
9. [Konfiguracja kanałów komunikacji: Chat Window i Execution Loop Window](#8-konfiguracja-kanałów-komunikacji-chat-window-i-execution-loop-window)
   - [8.1 Konfiguracja Chat Window](#81-konfiguracja-chat-window)
   - [8.2 Konfiguracja Execution Loop Window](#82-konfiguracja-execution-loop-window)
   - [8.3 Relacja obu kanałów do nakładki](#83-relacja-obu-kanałów-do-nakładki)
10. [Konfiguracja warstw widoczności](#9-konfiguracja-warstw-widoczności)
   - [9.1 Cztery warstwy widoczności](#91-cztery-warstwy-widoczności)
   - [9.2 Przypisanie funkcji do warstw](#92-przypisanie-funkcji-do-warstw)
   - [9.3 Konfiguracja warstw według roli użytkownika](#93-konfiguracja-warstw-według-roli-użytkownika)
   - [9.4 Wyszukiwarka funkcji](#94-wyszukiwarka-funkcji)
   - [9.5 Skróty klawiszowe](#95-skróty-klawiszowe)
   - [9.6 Tryb administracyjny](#96-tryb-administracyjny)
   - [9.7 Ergonomia i personalizacja okna](#97-ergonomia-i-personalizacja-okna)
11. [Panel prowenancji — „co poszło do modelu”](#10-panel-prowenancji--co-poszło-do-modelu)
   - [10.1 Cel i pozycjonowanie](#101-cel-i-pozycjonowanie)
   - [10.2 Bloki zawartości](#102-bloki-zawartości)
   - [10.3 Tryby podglądu i narzędzia analizy](#103-tryby-podglądu-i-narzędzia-analizy)
   - [10.4 Diagram przepływu danych](#104-diagram-przepływu-danych)
12. [Import i eksport konfiguracji](#11-import-i-eksport-konfiguracji)
13. [Makieta tekstowa okna](#12-makieta-tekstowa-okna)
   - [12.1 Pozycja okna Konfiguracji w układzie obszaru roboczego](#121-pozycja-okna-konfiguracji-w-układzie-obszaru-roboczego)
   - [12.2 Powłoka okna — nawigacja zakresów i zawartość](#122-powłoka-okna--nawigacja-zakresów-i-zawartość)
   - [12.3 Widok szczegółowy — Nakładka i wywołanie modelu](#123-widok-szczegółowy--nakładka-i-wywołanie-modelu)
   - [12.4 Widok — Kanały komunikacji](#124-widok--kanały-komunikacji)
   - [12.5 Widok — Warstwy widoczności](#125-widok--warstwy-widoczności)
   - [12.6 Widok — Pula kont Code CLI](#126-widok--pula-kont-code-cli)
   - [12.7 Widok — Panel prowenancji rozwinięty](#127-widok--panel-prowenancji-rozwinięty)
14. [Katalog elementów interfejsu](#13-katalog-elementów-interfejsu)
   - [13.1 Warstwy widoczności w oknie](#131-warstwy-widoczności-w-oknie)
15. [Stany](#14-stany)
   - [14.1 Stany okna (globalne)](#141-stany-okna-globalne)
   - [14.2 Stany pojedynczego pola / ustawienia](#142-stany-pojedynczego-pola--ustawienia)
   - [14.3 Stany Panelu prowenancji](#143-stany-panelu-prowenancji)
   - [14.4 Diagram przejść stanów (pole warstwowe)](#144-diagram-przejść-stanów-pole-warstwowe)
16. [Zachowanie i komunikacja klient–serwer](#15-zachowanie-i-komunikacja-klientserwer)
   - [15.1 Polecenia obsługujące Panel prowenancji i profile ekspertów](#151-polecenia-obsługujące-panel-prowenancji-i-profile-ekspertów)
   - [15.2 Sekwencja podglądu na żywo](#152-sekwencja-podglądu-na-żywo)
   - [15.3 Spójność z zasadą jednego źródła prawdy](#153-spójność-z-zasadą-jednego-źródła-prawdy)
17. [Zgodność z zasadami nadrzędnymi platformy](#16-zgodność-z-zasadami-nadrzędnymi-platformy)
18. [Scenariusze użycia](#17-scenariusze-użycia)
   - [17.1 Ustanowienie Konstytucji i weryfikacja jej przez hash](#171-ustanowienie-konstytucji-i-weryfikacja-jej-przez-hash)
   - [17.2 Rola Executor 1 z kanałem Code CLI i pulą kont dla dwóch spraw](#172-rola-executor-1-z-kanałem-code-cli-i-pulą-kont-dla-dwóch-spraw)
   - [17.3 Audyt nietypowej odpowiedzi przez Panel prowenancji](#173-audyt-nietypowej-odpowiedzi-przez-panel-prowenancji)
   - [17.4 Przełączenie trybu wywołania z API na Agent SDK](#174-przełączenie-trybu-wywołania-z-api-na-agent-sdk)
   - [17.5 Przełączenie profilu eksperta i weryfikacja wersji](#175-przełączenie-profilu-eksperta-i-weryfikacja-wersji)
   - [17.6 Konfiguracja pętli wykonawczej dla zlecenia wieloetapowego](#176-konfiguracja-pętli-wykonawczej-dla-zlecenia-wieloetapowego)
   - [17.7 Dostosowanie warstw widoczności dla roli użytkownika](#177-dostosowanie-warstw-widoczności-dla-roli-użytkownika)
19. [Słowniczek pojęć](#18-słowniczek-pojęć)
20. [Załącznik A. Zbiorczy szablon konfiguracji nakładki](#załącznik-a-zbiorczy-szablon-konfiguracji-nakładki)
21. [Załącznik B. Katalog ikon zastosowanych w oknie](#załącznik-b-katalog-ikon-zastosowanych-w-oknie)
22. [Załącznik C. Mapowanie pól na komunikaty WebSocket](#załącznik-c-mapowanie-pól-na-komunikaty-websocket)

---

## Wprowadzenie i pozycja dokumentu

Okno Konfiguracji jest, obok okna operacyjnego, jednym z dwóch okien realizujących interfejs użytkownika platformy (Architektura, rozdz. 13) i jedynym miejscem, z którego konfiguruje się wszystkie zależności i zachowania platformy (Koncepcja platformy, rozdz. 6 i rozdz. 14, zasada 2). Siedemnaście zakresów ustawień tego okna oraz jego warstwowość (globalna → środowisko → projekt → sesja) ustala Model konfiguracji; niniejszy dokument osadza w nich obszar roboczy, który programuje model przy każdym jego wywołaniu, wraz z konfiguracją obu kanałów komunikacji operacyjnej i konfiguracją warstw widoczności interfejsu.
...
| Model konfiguracji | Struktura okna Konfiguracji, siedemnaście zakresów, warstwowość ogólna, mechanizm objaśnień kontekstowych, wzorzec okna trzykolumnowego (Izolacja) | rozdz. 3–6 |
...
| Pozostałe zakresy okna Konfiguracji (Aplikacja, Procesy, Akcje, Rozszerzenia, Integracje, Historia, Pamięć, Izolacja, Karty sesji, Komponenty własne, Always On Display) | Przywołane w zakresie relacji do nakładki (rozdz. 6) | Pełna specyfikacja pozostaje przedmiotem Modelu konfiguracji |

Odbiorcami dokumentu są projektant, dla którego rozdziały 12–14 rozstrzygają co, gdzie i w jakiej formie znajduje się na ekranie, oraz deweloper, dla którego rozdziały 2–11 i 15 oraz załączniki rozstrzygają, jaki mechanizm i jaki kontrakt komunikacji zostaje zbudowany.
...
### 1.2 Siedemnaście zakresów okna Konfiguracji — struktura

Model konfiguracji (rozdz. 5) ustala siedemnaście zakresów ustawień okna Konfiguracji.
...
| 5.14 | Chat Window | Kanał Użytkownik ↔ Wykonawca: zapamiętane położenie paska rozdzielającego kolumny, zachowanie strumienia, zatwierdzanie i przerywanie działań, zakres pamięci |
| 5.15 | Execution Loop Window | Kanał Koordynator ↔ Wykonawca: równoległość zadań, polityka ponowień, progi kontroli jakości, zakres autonomii, punkty zatwierdzeń |
| 5.16 | Warstwy widoczności funkcji | Przypisanie funkcji do warstw 1–4, profile warstw wg roli użytkownika, wyszukiwarka funkcji, skróty klawiszowe, tryb administracyjny |
| 5.17 | Always On Display | Obecność i tryb funkcji globalnej, klasy zdarzeń wyzwalających sugestie, progi i częstotliwość, reguły wyciszania, tor głosowy |

### 1.3 Nakładka i wywołanie modelu — scalony obszar roboczy

Trzy spośród siedemnastu zakresów — 5.4, 5.5 i 5.6 — opisują wspólnie jedną rzecz: to, czym model jest i jak zostaje osiągnięty w chwili wywołania. Rozdzielenie ich na trzy osobne pozycje nawigacji jest poprawne porządkowo, lecz operacyjnie niewygodne — Operator programujący zachowanie konkretnego agenta lub roli ustala wszystkie trzy jednocześnie i widzi wynik ich złożenia. Model konfiguracji rozwiązuje analogiczny problem dla zakresu „Izolacja” (5.11), wydzielając dla niego własne, trzykolumnowe okno (rozdz. 6 Modelu konfiguracji) — ze względu na własną złożoność, a nie dlatego, że przestaje być jednym z siedemnastu zakresów.

Ten sam zabieg obejmuje zakresy 5.4–5.6, scalając je w jeden obszar roboczy: **Nakładka i wywołanie modelu**. Zabieg nie tworzy osiemnastego zakresu ustawień ponad siedemnaście ustalonych — jest ich operacyjnym rozwinięciem, analogicznym do rozdziału 6 Modelu konfiguracji.
...
**Przebieg operacji „Przywróć wersję N”.** Kroki wykonywane w kolejności, bez
pominięć.

| Krok | Czynność Operatora | Zachowanie okna |
|---|---|---|
| 1 | Otwarcie historii wersji profilu z selektora profilu eksperta | Lista wersji od najnowszej: numer, data, urządzenie autora, notatka zmiany, hash wersji |
| 2 | Wskazanie wersji N i wybór „Przywróć wersję N” | Otwiera się Okno nakładkowe porównania: wersja bieżąca po lewej, wersja N po prawej, pole po polu (rozdz. 4.2, wiersz „Porównanie wersji”) |
| 3 | Zatwierdzenie przywrócenia | Powstaje **nowa wersja** o treści wersji N; wersja bieżąca nie znika z historii — przywrócenie nigdy nie usuwa wersji |
| 4 | Potwierdzenie serwera | Dymek powiadomienia sukcesu z numerem nowej wersji i jej hashem; Panel prowenancji odświeża hash wersji eksperta |

**Przebieg operacji „Utwórz wariant z wersji N”.**

| Krok | Czynność Operatora | Zachowanie okna |
|---|---|---|
| 1 | Wskazanie wersji N i wybór „Utwórz wariant z wersji N” | Pole nazwy wariantu z podpowiedzią złożoną z nazwy profilu i przyrostka wariantu |
| 2 | Nadanie nazwy i zatwierdzenie | Powstaje odrębny profil eksperta o własnej historii wersji, rozpoczętej wersją pierwszą o treści wersji N |
| 3 | Potwierdzenie serwera | Nowy profil pojawia się w selektorze profilu; profil źródłowy pozostaje niezmieniony i nadal aktywny |

**Przypadki brzegowe.**

| Sytuacja | Rozstrzygnięcie |
|---|---|
| Kolizja wersji — inne urządzenie zapisało nową wersję profilu, gdy Operator miał otwarte porównanie | Zatwierdzenie zwraca kod `conflict`; okno prezentuje Komunikat blokowy wariantu `--ostrzezenie` z numerem wersji zapisanej w międzyczasie i dwoma działaniami: „Odśwież porównanie” oraz „Przywróć mimo to” — drugie tworzy wersję kolejną po wersji zapisanej w międzyczasie, nie nadpisuje jej |
| Profil aktywny w trwającej sesji — przywrócenie albo przełączenie wersji profilu, którego używa sesja z wywołaniem w toku | Wywołanie w toku dokańcza się na wersji, na której się rozpoczęło; nowa wersja obowiązuje od kolejnego wywołania. Karta sesji prezentuje znacznik kontekstowy „profil: wersja N → N+1 od następnego wywołania” do chwili pierwszego wywołania na wersji nowej |
| Profil aktywny w wielu sesjach równolegle | Zmiana obejmuje wszystkie sesje korzystające z profilu, każdą od jej kolejnego wywołania; okno wymienia liczbę sesji objętych zmianą przed zatwierdzeniem |
| Usunięcie wersji, do której odwołuje się wpis Panelu prowenancji | Wersja nie jest usuwana — historia wersji jest wyłącznie przyrostowa; wpis prowenancji zachowuje hash wersji i pozostaje weryfikowalny |
| Wariant utworzony z wersji profilu następnie zmienionego | Wariant jest bytem odrębnym od chwili utworzenia; późniejsze zmiany profilu źródłowego nie przenoszą się do wariantu |

Nazwy komend kontraktu obsługujących te operacje nie są ustalone — rozdz. 15.1
oznacza je `[DO DECYZJI OPERATORA]`; powyższy przebieg opisuje zachowanie okna,
nie przesądzając postaci kontraktu.
...
| Zapamiętane położenie paska | Szerokość lewej kolumny obszaru roboczego nie jest nastawą wpisywaną w oknie — Operator ustawia ją przesunięciem paska rozdzielającego kolumny, a klucz `chat.column.width` przechowuje ustawione położenie paska między sesjami | zapamiętane położenie paska; wartość procentowa jako domyślne położenie startowe | globalna, środowisko, sesja |
...
| Zapamiętane położenie paska | Szerokość kolumny okna pętli wykonawczej ustawiana wyłącznie przesunięciem paska rozdzielającego; klucz `loop.column.width` przechowuje ustawione położenie paska między sesjami, bez suwaka ani pola nastawy w oknie | zapamiętane położenie paska; wartość procentowa jako domyślne położenie startowe | globalna, środowisko, sesja |
...
| 3 | Rozwinięcia kontekstowe | Zestawy akcji, ustawienia szybkie, warianty operacji | Menu kebab (⋮), menu hamburger (☰), menu kontekstowe, panel popover, panel wysuwany, lista rozwijana |
...
Niniejsza sekcja jest miejscem rozstrzygającym dla trybu administracyjnego całej platformy: ustala jedną drogę wejścia, jeden model uprawnień, postać znacznika stanu oraz przejścia stanu. [Model konfiguracji](../architektura/model-konfiguracji.md) (rozdz. 7.4) podaje klucze konfiguracji trybu, [Bezpieczeństwo i uwierzytelnianie](../architektura/bezpieczenstwo-i-uwierzytelnianie.md) (rozdz. 14.2) — model uprawnień i rejestrację w dzienniku audytu.

**Droga wejścia.** Tryb administracyjny wywołuje się wyłącznie z palety poleceń (`Ctrl/Cmd + K`, [Katalog komponentów](katalog-komponentow.md), rozdz. 15.8), pozycją „Tryb administracyjny”. Okno konfiguracji nie zawiera przełącznika trybu — prezentuje znacznik stanu trybu oraz rejestr wejść. Pozycja palety jest widoczna wyłącznie dla roli, której konfiguracja obejmuje warstwę 4.

**Model uprawnień.**

| Element modelu | Treść |
|---|---|
| Warunek widoczności pozycji | Rola użytkownika obejmująca warstwę 4 (klucz proponowany `visibility.layer.role` — poza rejestrem `KnownSessionConfigKeys` bieżącego kontraktu, `[DO DECYZJI OPERATORA]`) |
| Warunek wejścia | Potwierdzenie uprawnień — uwierzytelnienie właściciela konta metodą ustawioną w oknie Ustawień ([Okno Ustawień](ustawienia.md), rozdz. 3) |
| Zakres ujawnianych funkcji | Wartość klucza proponowanego `visibility.admin.scope` (poza rejestrem kontraktu, `[DO DECYZJI OPERATORA]`): narzędzia diagnostyczne niskiego poziomu, konfiguracja ról, operacje na procesach sesji |
| Zasięg trybu | Sesja, urządzenie albo rola użytkownika — wartość klucza proponowanego `visibility.admin.mode` (poza rejestrem kontraktu, `[DO DECYZJI OPERATORA]`) |
| Czas trwania | Tryb wygasa po okresie bezczynności właściwym poświadczeniu urządzenia oraz przy rozłączeniu urządzenia |
| Rejestracja | Wejście, wyjście i wygaśnięcie trybu oraz każda zmiana konfiguracji uprawnień odnotowywane są w dzienniku audytu ([Bezpieczeństwo i uwierzytelnianie](../architektura/bezpieczenstwo-i-uwierzytelnianie.md), rozdz. 13.5) |
...
|---|---|---|
| Znacznik stanu trybu (`.dn-plakietka--sygnal` z ikoną `⛨`) | 1 w czasie działania trybu; nieobecny poza trybem | Widoczny bez interakcji od chwili wejścia w tryb |
...
| Forma | Blok tekstowy formatowany w kroju maszynowym `--dn-ff-mono` | Blok tekstowy formatowany w kroju maszynowym `--dn-ff-mono`, pięć sekcji |
...
```
SETTINGS — wywołanie: rola Executor 1, zespół „Sprawa X”      (krój --dn-ff-mono)
...
WYWOŁANIE MODELU (dowolny kanał — rozdz. 3.7; Chat Window albo Execution Loop Window)
   ...
   ├── rozstrzygnięcie pól wywołania (rozdz. 3) ─────► bloki „settings”
   │                                                   i „warstwy i pochodzenie”
   ├── zbudowanie parametrów technicznych adaptera ─► blok „argv”
   │   (Integracja modeli, rozdz. 4)
...
│ zaznaczenie:         │ [ tylko do odczytu ]                         │   │ blok --dn-ff-mono — │  │
│  tło akcentu         │ Edytuj w Agent Builder →                     │   │ zawartość bloku,    │  │
│                      │                                              │   │ dane dostępowe      │  │
...
│ Kroki pośrednie[ zwinięte ▼ ]      │ Poniżej progu        [ ponów ▼ ]   │   │ --dn-ff-mono    │ │
...
│ Historia i pamięć        [ ⋮ ]     │ Prezentacja pętli    [ pełna ▼ ]   │                       │
│                                    │                                    │ [ Podgląd na żywo ]   │
└────────────────────────────────────┴────────────────────────────────────┴───────────────────────┘
```
...
|---|---|---|---|---|---|---|---|
| 1 | Pozycja nawigacji zakresów | Wejście do jednego z zakresów okna Konfiguracji | 1 | Widoczna bez interakcji | Pozycja listy pionowej (`.dn-karta--klikalna`), pełna szerokość kolumny lewej | domyślny, wskazanie kursorem, zaznaczony | Kliknięcie ładuje odpowiedni zakres w kolumnie zawartości |
...
| 3 | Selektor warstwy nakładki | Wybór warstwy zasięgu, dla której edytowane są pola kolumny środkowej | 1 | Widoczny bez interakcji | Lista pionowa pozycji (`.dn-karta--klikalna`), kolumna lewa okna 12.3 | domyślny, wskazanie kursorem, zaznaczony | Kliknięcie przełącza zawartość kolumny środkowej i prawej na wybraną warstwę |
| 4 | Edytor tekstu Konstytucji | Pole edycji pełnej treści Konstytucji dla wybranego zasięgu | 1 | Widoczny bez interakcji | Duże pole tekstowe wieloliniowe (`textarea.dn-pole-kontrolka`), pełna szerokość sekcji | domyślny, edycja, zapisywanie, zapisano, błąd zapisu, pusty | Wpisywanie tekstu; przycisk „Zapisz” zatwierdza zmianę i przelicza hash |
| 5 | Przycisk „Zapisz” (Konstytucja) | Zatwierdzenie edytowanej treści Konstytucji | 1 | Widoczny bez interakcji | Przycisk drugorzędny (`.dn-btn--zarys`), mały | domyślny, wskazanie kursorem, wciśnięty, ładowanie, błąd | Zawsze aktywny; wysyła `config.set`; kliknięcie bez zmian nie wywołuje ruchu sieciowego; po potwierdzeniu serwera dymek powiadomienia sukcesu i odświeżenie hasha |
| 6 | Wyświetlacz hasha Konstytucji | Skrócony skrót SHA-256 treści obowiązującej Konstytucji | 1 | Widoczny bez interakcji | Etykieta monospaced z przyciskiem ikonowym kopiowania | domyślny, po kopiowaniu, pusty | Kliknięcie ikony kopiuje pełną wartość do schowka, pokazuje dymek powiadomienia „Skopiowano” |
| 7 | Selektor profilu eksperta | Wybór aktywnego profilu eksperta dla wybranej warstwy | 2 | Menu progresywne `Ekspert ▼` w kolumnie środkowej i w pasku kontekstu | Lista rozwijana (`.dn-pole-kontrolka`) z etykietą wersji | domyślny, otwarta, wybrany, brak aktywnego | Wybór natychmiast aktywuje profil i odświeża pola nakładki oraz Panel prowenancji |
| 8 | Menu profilu eksperta | Operacje na profilu: wersje, przywrócenie, wariant, porównanie wersji, galeria, harmonogram, eksport | 3 | Menu kebab (⋮) przy selektorze eksperta | Menu kontekstowe (`.dn-karta` + `.dn-karta--klikalna`, pływające) | domyślny, otwarte | Wybór pozycji otwiera właściwy widok operacji |
...
| 10 | Skrót „Edytuj w Agent Builder →” / „Edytuj w sekcji Role →” | Przejście do właściwego edytora danej warstwy | 1 | Widoczny bez interakcji | Łącze tekstowe akcentowane, małe | domyślny, wskazanie kursorem, fokus | Kliknięcie nawiguje do wskazanego okna modułu Agents lub panelu orkiestracji |
...
| 13 | Pole „Negatyw”, „Styl i ton”, „Słowniczek”, „Format wyjścia”, „Język odpowiedzi”, „Budżet” | Pola treści nakładki (rozdz. 2.7) | 2 | Zwinięta sekcja „Pola treści nakładki” rozwijana jednym kliknięciem | Pola tekstowe i listy rozwijane (`.dn-pole-kontrolka`, `.dn-pole-kontrolka`) | domyślny, edycja, dziedziczony, ustawiony jawnie | Zmiana wysyła `config.set`; wartość i warstwa widoczne w plakietce pola |
...
| 15 | Podgląd łańcucha dziedziczenia | Rozwinięcie pokazujące wszystkie warstwy pola z zaznaczeniem warstwy zwycięskiej | 3 | Rozwinięcie kontekstowe (popover) z plakietki warstwy | Panel popover (`.dn-karta` pływająca, zakotwiczona przy wyzwalaczu) | domyślny, ładowanie | Zawiera akcje „Przypnij / Odepnij / Przywróć dziedziczenie” |
| 16 | Pole „Harness” | Podgląd i edycja parametrów pętli agentowej właściwej wybranemu kanałowi | 2 | Widoczne w sekcji „Pola wywołania” | Pole tekstowe lub lista rozwijana (`.dn-pole-kontrolka`) zależna od kanału | domyślny, edytowalny, tylko do odczytu (Code CLI), zapisywanie | Zmiana wysyła `config.set`; dla Code CLI pole pozostaje klikalne i pokazuje wartość tylko do odczytu z objaśnieniem `[?]` |
| 17 | Lista hooków | Zestaw podłączonych punktów zaczepienia wraz z przypisaną automatyką | 2 | Widoczna w sekcji „Pola wywołania” | Lista pozycji (`.dn-karta--klikalna`), każda z etykietą punktu i akcji | domyślny, pusty, edycja pozycji | Kliknięcie pozycji otwiera edycję przypisanej automatyki; ikona usunięcia odłącza hook |
| 18 | Przycisk „+ dodaj punkt zaczepienia” | Dodanie nowego hooka | 2 | Widoczny przy liście hooków | Przycisk drugorzędny (`.dn-btn--zarys`), mały, z ikoną plusa | domyślny, wskazanie kursorem, wciśnięty | Otwiera formularz wyboru punktu cyklu i wskazania automatyki |
...
| 20 | Pole „Narzędzia dozwolone i zabronione” | Wybór zestawu rozszerzeń i akcji dostępnych modelowi | 2 | Widoczne w sekcji „Pola wywołania” | Dwie listy wielokrotnego wyboru (`.dn-pole-kontrolka` wariant multi) albo etykieta „pełny dostęp” tylko do odczytu | domyślny (pełny dostęp), zawężony, tylko do odczytu, ładowanie rejestru | Zaznaczenie pozycji aktualizuje zestaw; dla wywołań z agentem pole pokazuje wartość dziedziczoną ze skrótem „Edytuj w Permissions Center →” |
| 21 | Grupy narzędzi | Nazwane zestawy uprawnień zaznaczane jednym kliknięciem | 3 | Lista rozwijana przy polu narzędzi | Lista rozwijana (`.dn-pole-kontrolka`) | domyślny, wybrany | Wybór grupy podmienia zawartość obu list narzędzi |
| 22 | Podgląd efektywnego zestawu narzędzi | Wynikowa lista narzędzi po złożeniu list i dziedziczenia | 3 | Rozwinięcie kontekstowe przy polu narzędzi | Blok treści technicznej w kroju maszynowym `--dn-ff-mono`, przewijalny | domyślny, ładowanie | Tylko do odczytu; odświeża się przy każdej zmianie list |
| 23 | Pole „Model” | Wybór modelu bazowego z listy właściwej kanałowi albo wpisanie nazwy wprost | 2 | Widoczne w sekcji „Pola wywołania” | Lista rozwijana z polem wpisu (`.dn-pole-kontrolka`) | domyślny, otwarta, wybrany, ładowanie listy, lista niedostępna | Wybór natychmiast aktualizuje ustawienie i odświeża Panel prowenancji |
| 24 | Pole „Model zapasowy” | Wskazanie modelu i kanału uruchamianego przy niedostępności podstawowego | 3 | Rozwinięcie kontekstowe przy polu „Model” | Lista rozwijana (`.dn-pole-kontrolka`) | domyślny, wybrany, pusty | Wybór zapisuje ustawienie dla warstw rola i agent |
| 25 | Przełącznik i pole „Effort” | Włączenie nadpisania wartości effort na tej warstwie oraz wybór poziomu | 2 | Widoczne w sekcji „Pola wywołania” | Przełącznik (`.dn-suwak`) + lista rozwijana (`.dn-pole-kontrolka`) w jednym wierszu | wyłączony (dziedziczy), włączony z wartością, ładowanie | Włączenie przełącznika odsłania listę poziomów wraz z szacunkiem czasu i kosztu |
...
| 27 | Pole „Kanał” | Wybór trybu wywołania: Code CLI, Agent SDK, API | 2 | Menu progresywne `Kanał ▼` w sekcji „Pola wywołania” | Lista rozwijana (`.dn-pole-kontrolka`) | domyślny, otwarta, wybrany | Wybór natychmiast przełącza zestaw pól zależnych (harness, model, pula kont) |
...
| 31 | Przycisk „zarządzaj…” (Pula kont) | Otwarcie widoku zarządzania pulą kont Code CLI | 2 | Widoczny w podsekcji Code CLI | Łącze tekstowe, małe | domyślny, wskazanie kursorem | Otwiera widok „Pula kont Code CLI” (12.6) |
| 32 | Pole „Strategia przydziału” | Wybór strategii rozdziału wywołań między konta puli | 2 | Widoczne w podsekcji Code CLI | Lista rozwijana (`.dn-pole-kontrolka`) | domyślny, otwarta, wybrany | Wybór natychmiast aktualizuje ustawienie |
| 33 | Wiersz konta puli | Pojedyncze konto: etykieta, stan, obciążenie, limit, katalog profilu | 2 | Widoczny w widoku puli kont | Pozycja listy (`.dn-karta--klikalna`) z plakietką stanu (`.dn-plakietka--sygnal`) | dostępne, zajęte, błąd uwierzytelnienia, limit bliski wyczerpania | Menu (⋮) wiersza zawiera „Testuj połączenie”, „Ustaw jako aktywne”, „Usuń”; „Popraw” przy błędzie otwiera ponowne uwierzytelnienie; usunięcie odwracalne akcją „Cofnij” |
| 34 | Przycisk „+ Dodaj konto” | Dodanie nowego konta do puli | 2 | Widoczny w widoku puli kont | Przycisk drugorzędny (`.dn-btn--zarys`) | domyślny, wskazanie kursorem | Otwiera formularz uwierzytelnienia nowego konta narzędzia Code CLI |
...
| 36 | Rejestr kluczy i danych dostępowych | Widok wszystkich odwołań do danych dostępowych z wartością jawną i przełącznikiem widoczności | 3 | Pozycja zakresu „Aplikacja”, rozwinięcie kontekstowe | Tabela pozycji (`.dn-karta--klikalna`) | domyślny, wartość ukryta, wartość jawna, błąd | Przełącznik „Pokaż / Ukryj” zmienia widoczność wartości; pozycja wskazuje miejsca użycia klucza |
...
| 38 | Skrót „Otwórz okno konfiguracji punktów izolacji →” | Przejście do trzykolumnowego okna izolacji, z zaznaczonym poziomem zasięgu | 2 | Widoczny na końcu kolumny środkowej | Przycisk drugorzędny (`.dn-btn--zarys`), pełna szerokość sekcji | domyślny, wskazanie kursorem | Kliknięcie otwiera okno izolacji (Model konfiguracji, rozdz. 6) |
| 39 | Ikona objaśnienia kontekstowego `[?]` | Wywołanie treści objaśniającej działanie i wpływ ustawienia | 2 | Wskazanie kursorem albo fokus klawiaturą | Mała ikona (`svg`, token `--dn-wym-ikona-sm`, 16 px) w dymku (`.dn-tooltip`) | domyślny, dymek widoczny | Prezentuje treść z rozdz. 3.9 i z definicji ustawienia |
...
| 41 | Blok „argv” | Dosłowny zapis parametrów technicznych wywołania, z danymi dostępowymi domyślnie jawnymi | 2 | Nagłówek sekcji `▸` w Panelu prowenancji | Blok treści technicznej w kroju maszynowym `--dn-ff-mono`, przewijalny | domyślny, ładowanie, pusty | Rozwijany i zwijany nagłówkiem sekcji; zawartość tylko do odczytu, z przyciskiem kopiowania |
| 42 | Blok „system prompt” | Pełny, skomponowany tekst nakładki z oznaczonymi granicami warstw i sekcją Koordynatora | 2 | Nagłówek sekcji `▸` w Panelu prowenancji | Blok treści technicznej w kroju maszynowym `--dn-ff-mono`, przewijalny, z wyróżnieniem nagłówków sekcji | domyślny, ładowanie, pusty | Rozwijany i zwijany; kopiowanie całości albo pojedynczej warstwy |
| 43 | Blok „settings” | Wynikowe wartości pól wywołania po dziedziczeniu, z oznaczeniem poziomu każdej wartości | 2 | Nagłówek sekcji `▸` w Panelu prowenancji | Blok treści technicznej w kroju maszynowym `--dn-ff-mono` | domyślny, ładowanie | Rozwijany i zwijany; tylko do odczytu |
| 44 | Blok „warstwy i pochodzenie” | Warstwa zwycięska i wartość dziedziczona dla każdego pola | 3 | Nagłówek sekcji `▸` w Panelu prowenancji | Blok treści technicznej w kroju maszynowym `--dn-ff-mono` | domyślny, ładowanie | Tylko do odczytu; kliknięcie pozycji przenosi do pola w kolumnie środkowej |
| 45 | Blok „hash konstytucji i wersji eksperta” | Skrócona i pełna wartość skrótów kryptograficznych | 2 | Nagłówek sekcji `▸` w Panelu prowenancji | Blok treści technicznej w kroju maszynowym `--dn-ff-mono` dwuwierszowy + przyciski kopiowania | domyślny, pusty | Kopiowanie pełnej wartości do schowka |
| 46 | Przycisk „Podgląd na żywo” | Przeliczenie i pokazanie Panelu prowenancji dla stanu bieżącego, bez wysyłania wywołania | 1 | Widoczny bez interakcji w Panelu prowenancji | Przycisk główny (`.dn-btn--atrament`) | domyślny, wskazanie kursorem, wciśnięty, ładowanie | Przelicza i odświeża bloki kolumny prawej |
...
| 53 | Profil warstw roli | Nazwany zestaw przypisań warstw dla roli użytkownika | 3 | Kolumna lewa zakresu „Warstwy widoczności” | Lista pozycji (`.dn-karta--klikalna`) | domyślny, zaznaczony, dziedziczony z globalnej | Wybór profilu ładuje macierz przypisań tej roli |
...
| 59 | Powiadomienie potwierdzenia | Krótki komunikat potwierdzający zapis, skopiowanie lub błąd | 1 | Pojawia się po akcji | Powiadomienie (`.dn-toast`), warianty sukces i błąd | pojawienie się, zanikanie | Nieinteraktywny, znika automatycznie lub po kliknięciu „x” |
...
|---|---|---|
| 1 | Nawigacja zakresów, selektor warstwy nakładki, edytor Konstytucji z przyciskiem zapisu i hashem, podglądy Profil/Rola i Ekspertyza zadaniowa, nagłówek i przycisk „Podgląd na żywo” Panelu prowenancji, powiadomienie potwierdzenia | Widoczne bez interakcji |
| 2 | Selektor profilu eksperta, pola treści nakładki, pola wywołania (harness, hooki, narzędzia, model, effort, kanał, pula kont), przełącznik zapisu archiwalnego, bloki Panelu prowenancji, skrót do okna izolacji, objaśnienia `[?]` | Znacznik kontekstowy, menu progresywne `▼`, nagłówek sekcji `▸`, wskazanie kursorem |
...
| Edycja pola | Fokus na polu edytowalnym | Pole aktywne (obrys `--dn-fokus`); pozostałe elementy bez zmian |
...
| Zapisano | Otrzymanie `config.changed` z potwierdzeniem | Dymek powiadomienia sukcesu; wartości bieżące i hash odświeżone |
| Błąd zapisu | Serwer odrzucił zmianę albo utracono połączenie w trakcie zapisu | Dymek powiadomienia błędu z treścią przyczyny; pole wraca do ostatniej znanej wartości, zawartość wpisana przez Operatora pozostaje w polu do ponowienia |
| Zmiana z innego urządzenia | Odebranie `config.changed` niewywołanego lokalnie | Widok odświeża się na żywo; nienachalny dymek powiadomienia „Zaktualizowano z innego urządzenia” — bez blokowania bieżącej edycji innego pola |
...
| Wskazanie kursorem | Subtelna zmiana tła `--dn-hover` | Wszystkie elementy interaktywne |
| Fokus | Pierścień sygnałowy 2 px, wyzwalany przez `:focus-visible` | Wszystkie kontrolki interaktywne, obowiązkowo |
| Aktywny / zaznaczony | Akcent sygnałowy — tło `--dn-sygnal-tlo`, tekst `--dn-sygnal` | Pozycja nawigacji, pozycja selektora warstwy, zakładka |
| Kontekstowo nieadekwatny | Etykieta lub komunikat pomocniczy obok pola (`.dn-pole-opis`) albo po interakcji (dymek powiadomienia); pole pozostaje w pełni klikalne | Pole, którego wartość nie ma zastosowania w bieżącym kontekście — Harness dla Code CLI pokazuje podgląd tylko do odczytu, z objaśnieniem `[?]` |
...
Zmiany dokonane w oknie Konfiguracji przekazywane są tym samym kanałem WebSocket, który obsługuje pozostałe polecenia i zdarzenia platformy (Model konfiguracji, rozdz. 9; Architektura, rozdz. 11).
...
| Pola wywołania (harness, hooki, narzędzia, model, effort, parametry próbkowania, kanał) | `model.channel.set`, `config.set` (Integracja modeli, rozdz. 8.2) | `config.changed` |
| Pula kont Code CLI | `config.set` (atrybut puli w encji kanału modelu) | `config.changed` |
| Profile ekspertów — zapis, wersja, przywrócenie, wariant | brak komend kontraktu dla wersjonowania profilu — [DO DECYZJI OPERATORA] | — |
...
| Panel prowenancji — podgląd na żywo | `config.explain.get`, `config.effective.get` | — (odpowiedź niesiona wynikiem komendy) |
| Panel prowenancji — oś czasu, porównanie, eksport | `provenance.call.list`, `provenance.call.get`, `provenance.call.replay`, `provenance.trace.export` | — (odpowiedź niesiona wynikiem komendy); brak zdarzenia push dla nowego wpisu |
...
Kontrakt (`budowa/shared/contract.json`) niesie komendy obszaru `config` pokrywające rdzeń Panelu prowenancji (podgląd na żywo) oraz obszaru `provenance` pokrywające oś czasu, porównanie i eksport wpisów rzeczywiście wykonanych wywołań. Wersjonowanie profilu eksperta (rozdz. 4.2) nie ma pokrycia w bieżącym kontrakcie i jest oznaczone `[DO DECYZJI OPERATORA]`.

| Polecenie / zdarzenie | Kierunek | Funkcja |
|---|---|---|
| `config.explain.get` | klient → serwer, odpowiedź synchroniczna | Zwraca prowenancję wywołania modelu: `argv`, `systemPrompt`, `settings`, `constitutionHash` — pola źródłowe bloków „argv”, „system prompt”, „settings” i „hash konstytucji” (rozdz. 10.2). Parametry: `windowId` albo `sessionId` |
| `config.effective.get` | klient → serwer, odpowiedź synchroniczna | Zwraca konfigurację sesji obowiązującą po rozstrzygnięciu warstw wraz z pochodzeniem obszarów — źródło bloku „warstwy i pochodzenie” (rozdz. 10.2) i widoku „Podgląd na żywo” (rozdz. 10.3) |
| `config.session.get` / `config.session.set` | klient → serwer | Odczyt i zapis pól wywołania (rozdz. 3) na wskazanym poziomie zasięgu, bez rozstrzygania dziedziczenia |
| `config.reset` | klient → serwer | Przywraca wartość domyślną wskazanego klucza na wskazanym poziomie zasięgu — realizuje akcję „Przywróć dziedziczenie” (rozdz. 2.8) |
| `config.capabilities.get` | klient → serwer, odpowiedź synchroniczna | Zwraca, dla każdego pola konfiguracji sesji, czy adapter wybranego kanału je obsługuje (`supported`/`partial`/`unsupported`) — źródło stanu „tylko do odczytu” pól zależnych od kanału (rozdz. 3.2, 3.7) |
| `config.window.open` | klient → serwer | Otwiera okno Konfiguracji na wskazanym zakresie i poziomie zasięgu — realizuje skróty „Otwórz okno konfiguracji punktów izolacji →” (rozdz. 6.1) i wejścia z uproszczonego menu kontekstowego (rozdz. 10.3) |
| `config.changed` | serwer → klient (zdarzenie) | Rozgłasza zmianę konfiguracji do wszystkich połączonych urządzeń — zdarzenie wspólne dla wszystkich zapisów tego rozdziału |
| `provenance.call.list` | klient → serwer, odpowiedź synchroniczna | Rejestr wywołań kanału modelu zawężony filtrami (zasięg czasu, sesja, okno, kanał, model, konto, status, próg kosztu) — źródło Osi czasu prowenancji i filtra (rozdz. 10.3) |
| `provenance.call.get` | klient → serwer, odpowiedź synchroniczna | Zwraca jedno wywołanie wraz z drzewem odcinków oraz treścią promptu i odpowiedzi — źródło widoku „Zapis archiwalny — wybrany wpis” (rozdz. 10.3, 14.3) |
| `provenance.call.replay` | klient → serwer, odpowiedź synchroniczna | Powtarza wywołanie i zestawia jego odpowiedź z pierwowzorem — źródło „Porównania dwóch wywołań” (rozdz. 10.3) |
| `provenance.call.rate` | klient → serwer | Zapisuje ocenę trafności odpowiedzi nadaną przez Operatora |
| `provenance.trace.export` | klient → serwer, odpowiedź synchroniczna | Wydaje ślady wywołań we wskazanym formacie maszynowym — źródło „Eksportu prowenancji” (rozdz. 10.3) |
| **[DO DECYZJI OPERATORA]** — nazwa komendy nieustalona | — | Włączenie i wyłączenie automatycznego zapisu archiwalnego jako ustawienie warstwowe (rozdz. 10.3). Obszar `provenance` nie niesie dedykowanej komendy przełącznika; rozstrzygnięcia wymaga, czy przełącznik zapisuje się kluczem `config.session.set`, czy zapis wywołań jest bezwarunkowy po stronie rdzenia, a przełącznik steruje wyłącznie jego widocznością w interfejsie |
| **[DO DECYZJI OPERATORA]** — nazwy komend nieustalone | — | Wersjonowanie profilu eksperta: zapis wersji, przywrócenie, wariant, porównanie wersji (rozdz. 4.2). Profil eksperta nie istnieje jako byt kontraktu; rozstrzygnięcia wymaga, czy zyskuje własny obszar (analogicznie do `isolation.profile.save`, `isolation.profile.list`, `isolation.profile.load`, `isolation.profile.assign`, `isolation.profile.delete`), czy jest realizowany kluczami `config.session.set` |
...
Urządzenie A                    Serwer                          Urządzenie A
    ...
    │  ◄─── argv · systemPrompt · settings · constitutionHash ──────── │
    │       (wynik komendy — rozdz. 10.2)                              │
```

### 15.3 Spójność z zasadą jednego źródła prawdy

Serwer pozostaje jedynym źródłem prawdy (Architektura, rozdz. 1.3, zasada 6) również dla nakładki, profili ekspertów, konfiguracji kanałów komunikacji, warstw widoczności i Panelu prowenancji: każda zmiana dokonana na jednym urządzeniu jest rozgłaszana zdarzeniem `config.changed` do wszystkich pozostałych połączonych urządzeń — według sekwencji ustalonej w rozdziale 9 Modelu konfiguracji.
...
NAKŁADKA I WYWOŁANIE MODELU — szablon konfiguracji
  ...
  pola_wywolania:
    ...
    hooki:                   [ { punkt: <punkt_cyklu>, akcja: <automatyka>,
                                 kolejność: <n>, włączony: true|false }, … ]
    narzedzia:
  ...
  dane_dostepowe:
    ...
    zmienne_srodowiskowe:     [ { klucz: <KLUCZ>, wartość: <wartość | odwołanie>,
                                  warstwa: <poziom> }, … ]
...
Zestaw ikon zgodny z pełnym katalogiem 82 ikon platformy (`design/zasoby/ikony/manifest.json`); okno nie wprowadza nowych ikon.
...
| `info` | Ikona objaśnienia kontekstowego `[?]` przy każdym polu |
...
| Profil eksperta (zapis, wersja, wariant) | brak komend kontraktu — `[DO DECYZJI OPERATORA]` (rozdz. 15.1) | — |
...
| Model, model zapasowy | `model.channel.set` | `agent.changed` |
| Effort, parametry próbkowania | `config.set` | `config.changed` |
| Kanał / tryb wywołania, parametry kanału | `model.channel.set` | `agent.changed` |
...
| Oś czasu i wybór wpisu (Panel prowenancji) | `provenance.call.list`, `provenance.call.get` | — |
| Podgląd na żywo (Panel prowenancji) | `config.explain.get`, `config.effective.get` | — (odpowiedź niesiona wynikiem komendy) |
| Porównanie wywołań, eksport prowenancji | `provenance.call.replay`, `provenance.trace.export` | — |
| Import, eksport, migawki konfiguracji | `config.get`, `config.set` | `config.changed` |

---

*Koniec dokumentu. Okno Konfiguracji — Specyfikacja docelowa, wersja 2.0, 2026-08-20.*
