...
| | |
|---|---|
| **Odbiorcy** | Designer (forma, rozmieszczenie i waga wizualna elementów) oraz Deweloper (budowa, zachowanie, kontrakt komunikacji) |
| **Opracowanie** | Danaco Console |
| **Źródło** | Koncepcja platformy; Model konfiguracji 1.1; Bezpieczeństwo i uwierzytelnianie 1.1; Strona główna i nawigacja 1.1; System wizualny; Integracja modeli; funkcje-globalne/mobile.md |
...
- [Wprowadzenie i mapa źródeł](#wprowadzenie-i-mapa-źródeł)
1. [Miejsce Ustawień w architekturze platformy](#1-miejsce-ustawień-w-architekturze-platformy)
2. [Struktura okna, nawigacja wewnętrzna i warstwy widoczności](#2-struktura-okna-i-nawigacja-wewnętrzna)
3. [Konto Operatora](#3-konto-operatora)
4. [Uwierzytelnianie](#4-uwierzytelnianie)
5. [Wygląd: motyw i język](#5-wygląd-motyw-i-język)
6. [Urządzenia: lista, parowanie Mobile, synchronizacja](#6-urządzenia-lista-parowanie-mobile-synchronizacja)
7. [Powiadomienia](#7-powiadomienia)
8. [Zarządzanie kontami modeli](#8-zarządzanie-kontami-modeli)
9. [Okna komunikacji operacyjnej i ich ustawienia](#9-okna-komunikacji-operacyjnej-i-ich-ustawienia)
10. [Warstwy widoczności — ustawienia według roli użytkownika](#10-warstwy-widoczności--ustawienia-według-roli-użytkownika)
11. [Always On Display](#11-always-on-display)
12. [Makieta całościowa okna Ustawień](#12-makieta-całościowa-okna-ustawień)
13. [Katalog elementów interfejsu](#13-katalog-elementów-interfejsu)
14. [Stany — zestawienie zbiorcze](#14-stany--zestawienie-zbiorcze)
15. [Komunikacja — polecenia i zdarzenia](#15-komunikacja--polecenia-i-zdarzenia)
...
- [Załącznik A. Scenariusze użycia](#załącznik-a-scenariusze-użycia)
- [Załącznik B. Szablony konfiguracji](#załącznik-b-szablony-konfiguracji)
...
| Parowanie urządzenia mobilnego | funkcje-globalne/mobile.md | rozdz. 9 | Przejęte przez odesłanie — pełny protokół pozostaje w dokumencie funkcji globalnej; tu opisany interfejs parowania i zarządzania sparowanymi urządzeniami |
...
Model konfiguracji (rozdz. 5) wylicza trzynaście zakresów ustawień platformy. Pierwszy z nich — „Aplikacja” — ma status osobnego okna, dostępnego bezpośrednio z listwy ustawień strony głównej. Pozostałe dwanaście zakresów pozostaje w oknie Konfiguracji, opisanym odrębnym opracowaniem. Rozgraniczenie nie jest podziałem technicznym, lecz podziałem według rodzaju decyzji: Ustawienia gromadzą to, co dotyczy Operatora i jego tożsamości w platformie; Konfiguracja gromadzi to, co dotyczy zachowania samej platformy podczas pracy.
...
Schemat 2 — Układ kolumnowy okna Ustawień (podział lewa–prawa)
 ...
 ═════════════════════════════════════════════════════════════════════════════════
  Chat Window      │ Execution Loop ▼│ Sekcje ☰            │ ZAWARTOŚĆ SEKCJI                      │ Panel pomocniczy ▼
  Użytkownik ↔     │ Koordynator ↔   │                     │                                       │ (rozszerzenie
  Wykonawca        │ Wykonawca       │ ◈ Konto             │ [ nazwa ustawienia ]            [?] ⋮ │   boczne)
...
| Urządzenia | Wykaz urządzeń połączonych, parowanie Mobile, stan synchronizacji | `karta-przegladarki` (urządzenie mobilne) | 2 | Pozycja selektora sekcji `☰` | 6 |
| Powiadomienia | Przełącznik główny, klasy zdarzeń, kanał dostarczenia | `dzwonek` | 2 | Pozycja selektora sekcji `☰` | 7 |
| Konta modeli | Wykaz kont modeli w kanale CLI, w tym kont Claude Code, katalogi profili | `kod` | 2 | Pozycja selektora sekcji `☰` | 8 |
| Okna komunikacji | Ustawienia Chat Window i Execution Loop Window — dwóch kanałów komunikacji operacyjnej | `czat` | 2 | Pozycja selektora sekcji `☰`; polecenie języka naturalnego w Chat Window | 9 |
...
Każda pozycja ustawienia w oknie Ustawień jest opatrzona objaśnieniem kontekstowym `[?]`, zgodnie z zasadą jednolitą dla całego okna Konfiguracji i przyjętą tu wprost (Model konfiguracji, rozdz. 3.3; Architektura, rozdz. 13). Nośnikiem jest komponent `.dn-tooltip` (System wizualny, rozdz. 8.11), wyzwalany najechaniem lub fokusem klawiatury.
...
Mechanizmy ukrywania funkcjonalności zastosowane w oknie: menu progresywne (`Sekcje ☰`, `Operacje ▼`), panele wysuwane (modale, panele pomocnicze jako rozszerzenia boczne), grupowanie logiczne akcji (menu wiersza `⋮`) oraz znaczniki kontekstowe paska kontekstu, których kliknięcie otwiera odpowiedni selektor.
...
Makieta 1 — Sekcja „Konto”
 ...
 ═════════════════════════════════════════════════════════════════════════════════
  Pasek kontekstu:  [Danaco Console] [Ubuntu] [Worktree] [Fable 5] [Ultra]
 ═════════════════════════════════════════════════════════════════════════════════
  Chat Window      │ Execution Loop ▼│ Sekcje ☰            │ Konto Operatora                       │ Panel pomocniczy ▼
  Użytkownik ↔     │ Koordynator ↔   │                     │ ┌───────┐                             │ (rozszerzenie
  Wykonawca        │ Wykonawca       │ ◈ Konto             │ │ awatar│  Zmień awatar               │   boczne)
                   │                 │   Uwierzytelnianie  │ └───────┘                             │
  [ polecenie… ]   │ [ pętla: 0 zad.]│   Wygląd i język    │                                       │
                   │                 │   Urządzenia        │ Login                       [?] ⋮     │
...
Operator zalogowany, otwiera sekcję „Konto” → „Zmień hasło”
        │
        ▼
Modal: podanie nowego hasła (dwukrotnie); pole hasła bieżącego widoczne,
...
poza bazą (rozdz. 10 Bezpieczeństwa); zawsze sprawdza zgodność nowego hasła z powtórzeniem
        │
        ├── dane poprawne ──► zapis nowego skrótu hasła ──► toast potwierdzenia
        │                                                    (zdarzenie auth.changed, rozdz. 15.2)
        └── ostrzeżenie (hasło bieżące niezgodne / nowe hasła się różnią / pole puste)
              ──► komunikat w modalu, formularz pozostaje otwarty do poprawy i ponowienia
```
...
Makieta 2 — Sekcja „Uwierzytelnianie”
 ...
 ═════════════════════════════════════════════════════════════════════════════════
  Pasek kontekstu:  [Danaco Console] [Ubuntu] [Worktree] [Fable 5] [Ultra]
 ═════════════════════════════════════════════════════════════════════════════════
  Chat Window      │ Execution Loop ▼│ Sekcje ☰            │ Uwierzytelnianie                      │ Panel pomocniczy ▼
  Użytkownik ↔     │ Koordynator ↔   │                     │                                       │ (rozszerzenie
  Wykonawca        │ Wykonawca       │   Konto             │ Hasło                       [?] ⋮     │   boczne)
...
Przełącznik wymogu logowania **nie jest** pozycją tej sekcji ani żadnej innej sekcji Ustawień — jest ustawieniem okna Konfiguracji, zgodnie z rozgraniczeniem przyjętym w rozdz. 1.1 (Ustawienia: tożsamość Operatora; Konfiguracja: zachowanie platformy). Pozostaje jednak w pełni dostępny i zmienialny przez Operatora (zasada zerowych blokad, rozdz. 1.3) — nie jest parametrem poza jego zasięgiem. Sekcja Uwierzytelnianie prezentuje informacyjną plakietkę stanu (`.dn-plakietka--info`), tak aby Operator wiedział, czy okno logowania obecnie wymusza uwierzytelnienie, wraz z odnośnikiem do pozycji w Konfiguracji, gdzie może tę wartość zmienić. Operator zmienia tę wartość również poleceniem języka naturalnego skierowanym do Wykonawcy w Chat Window (rozdz. 9.1).
...
Makieta 3 — Sekcja „Wygląd i język”
 ...
 ═════════════════════════════════════════════════════════════════════════════════
  Pasek kontekstu:  [Danaco Console] [Ubuntu] [Worktree] [Fable 5] [Ultra]
 ═════════════════════════════════════════════════════════════════════════════════
  Chat Window      │ Execution Loop ▼│ Sekcje ☰            │ Wygląd i język                        │ Panel pomocniczy ▼
  Użytkownik ↔     │ Koordynator ↔   │                     │                                       │ (rozszerzenie
  Wykonawca        │ Wykonawca       │   Konto             │ Motyw wizualny              [?] ⋮     │   boczne)
                   │                 │   Uwierzytelnianie  │ ┌────────┬─────────┬────────┐         │
...
Przycisk „Sparuj urządzenie” uruchamia protokół parowania opisany w całości w dokumencie funkcji globalnej (`funkcje-globalne/mobile.md`, rozdz. 9). Parowanie jest drogą wejścia urządzenia przenośnego do funkcji Mobile, zakładane w obecności Operatora przy maszynie już uwierzytelnionej, i wydaje nowemu urządzeniu poświadczenie odrębne od pozostałych — odwołanie jednego urządzenia nie odcina pozostałych. Niniejszy rozdział dokumentuje interfejs parowania po stronie okna Ustawień: modal kodu, potwierdzenie zgłoszenia i miejsce obu kroków w sekcji Urządzenia.
...
Sekcja „Urządzenia” → [ Sparuj urządzenie ]        (device.pair.start)
        │
        ▼
Modal kodu parowania: ciąg znaków + kod graficzny + licznik ważności
        │
        ▼
Operator odczytuje kod aplikacją na urządzeniu przenośnym  (device.pair.confirm)
        │
        ▼
Modal potwierdzenia zgłoszenia: typ, nazwa wstępna, czas zgłoszenia
        │
        ├── [ Potwierdź ] ──► wydanie poświadczenia (device.pair.approve)
        │                      nowy wiersz w tabeli urządzeń
        │                      toast „Sparowano: <nazwa urządzenia>”
        │                      (zdarzenie device.changed, rozdz. 14.2)
        │
        ├── [ Odrzuć ] ──────► kod unieważniony (device.pair.reject)
        │
        └── czas ważności upłynął ──► toast błędu, kod unieważniony,
                                      działanie „Wydaj nowy kod”
```

| Element interfejsu parowania | Co to jest | Do czego służy | Stany | Warstwa | Sposób wywołania |
|---|---|---|---|---|---|
| Modal kodu parowania | Modal `.dn-modal` z kodem w dwóch postaciach | Przekazanie kodu urządzeniu przenośnemu | Generowanie · kod aktywny · kod wygasły · kod unieważniony | 3 | Przycisk „Sparuj urządzenie” |
| Kod w postaci znakowej | Ciąg znaków do wpisania ręcznego | Parowanie urządzenia bez odczytu kamerą | Domyślny · skopiowany | 3 | Widoczny w modalu |
| Kod graficzny | Kod do odczytu kamerą urządzenia przenośnego | Parowanie bez wpisywania znaków | Domyślny · wygasły | 3 | Widoczny w modalu |
| Licznik ważności kodu | Wskaźnik pozostałego czasu ważności | Sygnał, ile czasu pozostaje na odczyt kodu | Odliczanie · wyzerowany | 3 | Widoczny w modalu |
| Działanie „Unieważnij kod” | Przycisk pomocniczy modalu | Zamknięcie parowania przed jego użyciem | Domyślny · ładowanie | 3 | Widoczne w modalu |
| Działanie „Wydaj nowy kod” | Przycisk pomocniczy modalu | Powtórzenie kroku wydania kodu po jego wygaśnięciu | Domyślny · ładowanie | 3 | Widoczne po wygaśnięciu kodu |
| Modal potwierdzenia zgłoszenia | Modal z danymi urządzenia zgłaszającego | Rozstrzygnięcie, czy urządzenie wchodzi do rejestru | Oczekuje · potwierdzone · odrzucone | 3 | Zdarzenie zgłoszenia urządzenia |
| Pole nazwy urządzenia w potwierdzeniu | Pole `.dn-input` z nazwą wstępną | Nadanie nazwy rozpoznawalnej w rejestrze | Domyślne · zmienione | 3 | Widoczne w modalu potwierdzenia |
| Komunikat kontekstowy błędu parowania | Wiersz komunikatu w modalu | Wskazanie przyczyny: kod niepoprawny, wygasły, użyty, zgłoszenie odrzucone | Ukryty · widoczny | 3 | Pojawia się przy warunku błędu (`funkcje-globalne/mobile.md`, rozdz. 9.11) |
...
| Odświeżenie poświadczenia | Wydanie poświadczenia o nowym terminie ważności (`device.credential.refresh`) | 4 | Polecenie języka naturalnego w Chat Window; wyszukiwarka funkcji |

Postępowanie przy utracie urządzenia — rozpoznanie wiersza po nazwie, typie i dacie ostatniego połączenia, odwołanie poświadczenia i zapis w dzienniku audytu — opisuje `funkcje-globalne/mobile.md`, rozdz. 9.9.
...
| Element | Forma | Treść |
|---|---|---|
| Wskaźnik stanu synchronizacji | `.dn-plakietka--stan` z `.dn-kropka` | „Synchronizacja aktywna” (kropka `--sukces`) / „Brak połączenia — zmiany zostaną zsynchronizowane po powrocie łączności” (kropka `--ostrz`) |
...
Makieta 4 — Sekcja „Urządzenia”
 ...
 ═════════════════════════════════════════════════════════════════════════════════
  Pasek kontekstu:  [Danaco Console] [Ubuntu] [Worktree] [Fable 5] [Ultra]
 ═════════════════════════════════════════════════════════════════════════════════
  Chat Window      │ Execution Loop ▼│ Sekcje ☰            │ Urządzenia      ● Synchronizacja      │ Panel pomocniczy ▼
  Użytkownik ↔     │ Koordynator ↔   │                     │                   aktywna             │ (rozszerzenie
  Wykonawca        │ Wykonawca       │   Konto             │                                       │   boczne)
                   │                 │   Uwierzytelnianie  │ ┌────────────────────────────────┐    │
...
Sekcja steruje jednym mechanizmem powiadamiania obowiązującym w całej platformie — centrum powiadomień, którego taksonomię klas zdarzeń, postać wizualną, umiejscowienie w układzie pionowym, warstwę widoczności, sposób wywołania, stany i działania opisuje karta komponentu w `interfejs-uzytkownika/katalog-komponentow.md` (rozdz. 11.6). Encję zdarzenia opisuje `architektura/model-danych.md` (rozdz. 18.4).

| Ustawienie | Opis | Warstwa konfiguracji | Wartość domyślna | Warstwa widoczności | Sposób wywołania |
|---|---|---|---|---|---|
| Powiadomienia | Zakres klas zdarzeń i kanał dostarczenia powiadomień centrum powiadomień, w tym zdarzeń nadzorowanych przez funkcję Mobile (Koncepcja platformy, rozdz. 12.1; funkcje-globalne/mobile.md, rozdz. 8) | globalna, sesja | powiadomienia aktywne | 2 | Sekcja „Powiadomienia” w selektorze sekcji `☰` |
...
Klasy zdarzeń odpowiadają taksonomii centrum powiadomień (`interfejs-uzytkownika/katalog-komponentow.md`, rozdz. 11.6) i zdarzeniom zdefiniowanym w kontraktach komunikacji poszczególnych zakresów platformy — sekcja nie wprowadza nowych zdarzeń, lecz udostępnia Operatorowi wybór, które z nich mają skutkować powiadomieniem w centrum powiadomień i na urządzeniu mobilnym.
...
| System | `auth.changed`, `device.changed` (Bezpieczeństwo i uwierzytelnianie, rozdz. 12.2); `config.changed`, `memory.changed`, `history.changed` (Model konfiguracji, rozdz. 8); wiadomości i zdolności telefonu (funkcje-globalne/mobile.md, rozdz. 8.1) | aktywne | 2 | Tabela klas zdarzeń w sekcji „Powiadomienia” |
...
|---|---|---|---|
| Centrum powiadomień | Pozycja w kolumnie centrum powiadomień oraz Toast (`.dn-toast`, System wizualny, rozdz. 8.10) w chwili wystąpienia zdarzenia | 2 | Kolumna kanału w tabeli klas zdarzeń |
| Mobile — powiadomienie wypychane | Dostarczane dwoma kanałami o różnej poufności: kanałem własnym przy aplikacji na pierwszym planie oraz kanałem pośrednika poza nią — pełny mechanizm w funkcje-globalne/mobile.md, rozdz. 8.5 | 2 | Kolumna kanału w tabeli klas zdarzeń |
...
Makieta 5 — Sekcja „Powiadomienia”
 ...
 ═════════════════════════════════════════════════════════════════════════════════
  Pasek kontekstu:  [Danaco Console] [Ubuntu] [Worktree] [Fable 5] [Ultra]
 ═════════════════════════════════════════════════════════════════════════════════
  Chat Window      │ Execution Loop ▼│ Sekcje ☰            │ Powiadomienia                         │ Panel pomocniczy ▼
  Użytkownik ↔     │ Koordynator ↔   │                     │                                       │ (rozszerzenie
  Wykonawca        │ Wykonawca       │   Konto             │ Powiadomienia aktywne       [?] ⋮     │   boczne)
                   │                 │   Uwierzytelnianie  │                           ( ● wł.)    │
  [ polecenie… ]   │ [ pętla: 0 zad.]│   Wygląd i język    │                                       │
...
┌──────────────────────────────┐                  ┌──────────────────────────────┐
│ Katalog kont Claude Code      │   konto wybrane  │ Kanał modelu przypisany do   │
...
|---|---|---|---|---|
| Wykaz kont modeli | Nazwane, zapisane konta wiersza poleceń (w tym konta Claude Code) wraz z katalogiem profilu i stanem | pusty do chwili dodania pierwszego konta | 2 | Sekcja „Konta modeli” w selektorze sekcji `☰` |
...
Konto B otrzymuje plakietkę „aktywne”
        │
        ▼
Zdarzenie model.account.changed rozgłoszone do wszystkich urządzeń (rozdz. 15.2)
        │
        ▼
...
Makieta 7 — Sekcja „Konta modeli”
 ...
 ═════════════════════════════════════════════════════════════════════════════════
  Pasek kontekstu:  [Danaco Console] [Ubuntu] [Worktree] [Fable 5] [Ultra]
 ═════════════════════════════════════════════════════════════════════════════════
  Chat Window      │ Execution Loop ▼│ Sekcje ☰            │ Konta modeli                          │ Panel pomocniczy ▼
  Użytkownik ↔     │ Koordynator ↔   │                     │                                       │ (rozszerzenie
  Wykonawca        │ Wykonawca       │   Konto             │ ┌────────────────────────────────┐    │   boczne)
...
Przed dodaniem pierwszego konta sekcja prezentuje `.dn-pusty-stan` (System wizualny, rozdz. 8.12): ikona `kod`, tytuł „Brak skonfigurowanych kont modeli”, opis „Dodaj konto Claude Code lub innego narzędzia wiersza poleceń, aby platforma mogła się nim posługiwać jako kanałem CLI”, przycisk „Dodaj konto modelu”.
...
| Ustawienie | Opis | Wartość domyślna | Warstwa | Sposób wywołania |
|---|---|---|---|---|
| Szerokość kolumny Chat Window | Szerokość lewej kolumny obszaru roboczego; regulacji podlega wyłącznie szerokość, położenie kolumny pozostaje stałe | wartość systemu wizualnego | 2 | Sekcja „Okna komunikacji” w selektorze sekcji `☰`; uchwyt rozdzielający kolumny |
...
| Szerokość kolumny Execution Loop Window | Szerokość kolumny sąsiadującej | wartość systemu wizualnego | 2 | Sekcja „Okna komunikacji” w selektorze sekcji `☰`; uchwyt rozdzielający kolumny |
...
```
Makieta 9 — Sekcja „Okna komunikacji”

 ═════════════════════════════════════════════════════════════════════════════════
  Pasek kontekstu:  [Danaco Console] [Ubuntu] [Worktree] [Fable 5] [Ultra]
 ═════════════════════════════════════════════════════════════════════════════════
  Chat Window      │ Execution Loop ▼│ Sekcje ☰            │ Okna komunikacji                      │ Panel pomocniczy ▼
  Użytkownik ↔     │ Koordynator ↔   │                     │                                       │ (rozszerzenie
  Wykonawca        │ Wykonawca       │   Konto             │ Chat Window                  [?] ⋮    │   boczne)
                   │                 │   Uwierzytelnianie  │ Użytkownik ↔ Wykonawca                │
  [ polecenie… ]   │ [ pętla: 0 zad.]│   Wygląd i język    │ szerokość kolumny  [ ——●—— ]          │
                   │                 │   Urządzenia        │ gęstość strumienia [ zwarta ▾ ]       │
                   │                 │   Powiadomienia     │ potwierdzanie działań ( ● wł.)        │
                   │                 │   Konta modeli      │ sterowanie ustawieniami ( ● wł.)      │
                   │                 │ ◈ Okna komunikacji  │                                       │
...
Zmiana przypisania obowiązuje natychmiast na wszystkich urządzeniach Operatora (`ui.layer.set`, `ui.layer.changed`, rozdz. 15.2). Przypisanie nie usuwa funkcji z platformy — zmienia wyłącznie sposób jej wywołania; każda funkcja pozostaje osiągalna jednym kliknięciem, jednym skrótem klawiszowym albo jednym poleceniem języka naturalnego w Chat Window (zasada jednego kliknięcia, rozdz. 2.5).

### 10.2. Makieta sekcji Warstwy widoczności

```
Makieta 10 — Sekcja „Warstwy widoczności” (tryb administracyjny)

 ═════════════════════════════════════════════════════════════════════════════════
  Pasek kontekstu:  [Danaco Console] [Ubuntu] [Worktree] [Fable 5] [Ultra]
 ═════════════════════════════════════════════════════════════════════════════════
  Chat Window      │ Execution Loop ▼│ Sekcje ☰            │ Warstwy widoczności          [?] ⋮    │ Panel pomocniczy ▼
  Użytkownik ↔     │ Koordynator ↔   │                     │ warstwa 4 — tryb administracyjny      │ (rozszerzenie
  Wykonawca        │ Wykonawca       │   Konto             │                                       │   boczne)
...
Sekcja udostępnia ustawienia funkcji globalnej Always On Display — agenta towarzyszącego obecnego ponad wszystkimi środowiskami, modułami, projektami i sesjami. Pełną dokumentację funkcji, w tym reguły wyzwalania sugestii, katalog ich rodzajów oraz rozgraniczenie toru głosowego wobec modułu Assistant, zawiera `funkcje-globalne/always-on-display.md`. Pełny zakres ustawień funkcji obejmuje okno Konfiguracji (Model konfiguracji, rozdz. 5.17); niniejsza sekcja udostępnia pozycje o zastosowaniu codziennym.
...
|---|---|---|---|---|
| Obecność awatara | Przełącznik widoczności pływającego awatara; ukrycie nie przerywa działania funkcji w tle — dostęp pozostaje przez ikonę paska górnego, skrót klawiszowy i polecenie języka naturalnego | awatar widoczny | 2 | Sekcja „Always On Display” w selektorze sekcji `☰` |
...
```
Makieta 11 — Sekcja „Always On Display”

 ═════════════════════════════════════════════════════════════════════════════════
  Pasek kontekstu:  [Danaco Console] [Ubuntu] [Worktree] [Fable 5] [Ultra]
 ═════════════════════════════════════════════════════════════════════════════════
  Chat Window      │ Execution Loop ▼│ Sekcje ☰            │ Always On Display              ⋮      │ Panel pomocniczy ▼
  Użytkownik ↔     │ Koordynator ↔   │                     │                                       │ (rozszerzenie
  Wykonawca        │ Wykonawca       │   Konto             │ Obecność awatara            [?]       │   boczne)
                   │                 │   Uwierzytelnianie  │                           ( ● wł.)    │
  [ polecenie… ]   │ [ pętla: 0 zad.]│   Wygląd i język    │                                       │
...
```
Makieta 8 — Okno Ustawień, widok całościowy (stan spoczynku, sekcja domyślna: Konto)

 ═════════════════════════════════════════════════════════════════════════════════
  Pasek kontekstu:  [Danaco Console] [Ubuntu] [Worktree] [Fable 5] [Ultra]
 ═════════════════════════════════════════════════════════════════════════════════
  Chat Window      │ Execution Loop ▼│ Sekcje ☰            │ USTAWIENIA                        ✕   │ Panel pomocniczy ▼
  Użytkownik ↔     │ Koordynator ↔   │                     │                                       │ (rozszerzenie
  Wykonawca        │ Wykonawca       │ ◈ Konto             │ Konto Operatora                       │   boczne)
                   │                 │   Uwierzytelnianie  │ ┌───────┐                             │
...
Okno otwiera się jako pełnoekranowy widok analogiczny do Konfiguracji — nie jako `.dn-modal` — ponieważ liczba i objętość sekcji (w szczególności tabele Urządzeń i Kont modeli) przekracza zakres wygodny dla nakładki modalnej; zasada zgodna z klasyfikacją okna Konfiguracji punktów izolacji jako okna globalnego, nie modala (Specyfikacja okien operacyjnych, rozdz. 8).
...
|---|---|---|---|---|---|---|---|---|
| Selektor sekcji (panel rozwinięty) | Pionowa lista dziewięciu pozycji, komponent `.dn-karta--pozycja` | Wybór sekcji ustawień do wyświetlenia w panelu zawartości | Panel boczny, szerokość stała, waga niska (tło neutralne) | domyślny · wybrany (podświetlenie `--dn-akcent-tlo`) · najechanie | Kliknięcie pozycji zamienia zawartość panelu prawego bez przeładowania okna | Całe okno (rozdz. 2.1) | 1 | Widoczny bez interakcji; selektor `☰` w stanie zwiniętym na wąskich szerokościach |
...
| Plakietka stanu wymogu logowania | Informacyjny blok, `.dn-alert--info` | Poinformowanie Operatora, czy okno logowania aktualnie wymusza uwierzytelnienie | Mały blok informacyjny z kreską lewą, tylko do odczytu | wymóg logowania włączony · wymóg logowania wyłączony | Brak — element wyłącznie informacyjny, bez interakcji | Sekcja Uwierzytelnianie (4.2, 4.3) | 1 | Widoczna bez interakcji w sekcji Uwierzytelnianie |
...
| Lista wyboru języka | Pole wyboru, `.dn-select` | Wybór języka interfejsu | Pole formularza z rozwijaną listą, waga mała | domyślny (Polski) · rozwinięty · wybrany inny język | Wybór natychmiast przełącza język całego interfejsu | Sekcja Wygląd i język (5.2, 5.3) | 2 | Sekcja „Wygląd i język” w selektorze sekcji `☰` |
...
| Przycisk „Odłącz” (wiersz urządzenia) | Przycisk ikonowy w wierszu tabeli, `.dn-btn-ikona` (36×36 px) | Unieważnienie tokenu dostępu wskazanego urządzenia | Mała ikona w kolumnie akcji | domyślny · najechanie · ładowanie | Wykonuje `device.revoke` od razu; toast „Odłączono: <nazwa urządzenia>” z dostępną przez kilka sekund akcją „Cofnij”; modal potwierdzenia przed odłączeniem jest ustawieniem konfiguracyjnym sekcji Uwierzytelnianie (rozdz. 4), domyślnie wyłączonym | Sekcja Urządzenia (6.1) | 3 | Menu akcji wiersza `⋮` w tabeli urządzeń |
| Wskaźnik stanu synchronizacji | Plakietka ze wskaźnikiem, `.dn-plakietka--stan` + `.dn-kropka` | Informowanie o stanie połączenia synchronizacji na żywo | Mała plakietka w nagłówku sekcji | aktywna (`--sukces`) · przerwana (`--ostrz`) | Brak interakcji — wyłącznie odczyt stanu | Sekcja Urządzenia (6.3, 6.4) | 1 | Widoczny bez interakcji w nagłówku sekcji Urządzenia |
| Przycisk „Sparuj urządzenie” | Przycisk główny akcji, `.dn-btn--zloty` | Uruchomienie procedury parowania nowego urządzenia mobilnego | Duży przycisk, jedyny CTA sekcji | domyślny · ładowanie (generowanie kodu) · aktywny kod wyświetlony | Otwiera modal z kodem parowania i kodem graficznym (protokół: funkcje-globalne/mobile.md, rozdz. 9) | Sekcja Urządzenia (6.2, 6.4) | 2 | Sekcja „Urządzenia”; polecenie języka naturalnego w Chat Window |
| Przełącznik główny powiadomień | Toggle, `.dn-suwak` | Włączenie/wyłączenie wszystkich powiadomień jednocześnie | Mały suwak inline, umieszczony na szczycie sekcji | włączony · wyłączony | Wyłączenie wygasza (nie kasuje) ustawienia poszczególnych kategorii poniżej | Sekcja Powiadomienia (7.4, 7.5) | 2 | Sekcja „Powiadomienia” w selektorze sekcji `☰` |
| Tabela kategorii powiadomień | Tabela z polami wyboru, `.dn-tabela` + `.dn-check` w komórkach | Wybór, które kategorie zdarzeń i którym kanałem mają być dostarczane | Duża tabela, kolumny: kategoria, w aplikacji, e-mail, Mobile | wypełniona · przygaszona wizualnie (gdy przełącznik główny nieaktywny), w pełni klikalna | Zaznaczenie/odznaczenie checkboxa zapisuje natychmiast, bez zbiorczego zapisu — zmiany dokonane przy wyłączonym przełączniku głównym obowiązują po jego ponownym włączeniu | Sekcja Powiadomienia (7.2, 7.4) | 2 | Sekcja „Powiadomienia” w selektorze sekcji `☰` |
| Przełącznik obecności awatara Always On Display | Toggle, `.dn-suwak` | Włączenie i ukrycie pływającego awatara funkcji globalnej | Mały suwak inline | włączony · wyłączony · ładowanie (podczas zapisu) | Ukrycie awatara nie przerywa działania funkcji — dostęp pozostaje przez ikonę paska górnego, skrót klawiszowy i polecenie języka naturalnego | Sekcja Always On Display (11.2, 11.3) | 2 | Sekcja „Always On Display” w selektorze sekcji `☰` |
...
| Tabela kont modeli | Tabela danych, `.dn-tabela` | Prezentacja skonfigurowanych kont wiersza poleceń, w tym kont Claude Code | Duża tabela pełnej szerokości panelu | ładowanie · wypełniona · pusta (`.dn-pusty-stan`) · wiersz w błędzie uwierzytelnienia | Menu akcji wiersza (ikona `wiecej`): „Ustaw jako aktywne”, „Testuj połączenie”, „Edytuj”, „Usuń” | Sekcja Konta modeli (8.2, 8.5, 8.6) | 2 | Sekcja „Konta modeli” w selektorze sekcji `☰` |
| Plakietka stanu konta modelu | Plakietka, `.dn-plakietka--stan` | Sygnalizacja, czy dane konto jest aktywne, skonfigurowane czy w błędzie | Mała pigułka w komórce tabeli | aktywne (`--zloto`) · skonfigurowane (`--info`) · błąd uwierzytelnienia (`--blad`) | Brak interakcji bezpośredniej — stan wynikowy testu połączenia lub przełączenia | Sekcja Konta modeli (8.2, 8.5) | 2 | Widoczna w wierszu tabeli kont modeli |
| Przycisk „Dodaj konto modelu” | Przycisk główny akcji, `.dn-btn--zloty` | Otwarcie formularza dodania nowego konta modelu | Duży przycisk, jedyny CTA sekcji | domyślny · ładowanie (po zatwierdzeniu formularza) | Otwiera modal opisany w rozdz. 8.3 | Sekcja Konta modeli (8.3, 8.5, 8.6) | 2 | Sekcja „Konta modeli”; polecenie języka naturalnego w Chat Window |
| Modal „Dodaj konto modelu” | Okno nakładkowe, `.dn-nakladka` + `.dn-modal` | Wprowadzenie danych nowego konta: nazwa, katalog profilu, dane dostępowe | Duży panel nakładkowy, wyśrodkowany, cztery pola | domyślny · ostrzeżenie pola (katalog zajęty, dane dostępowe puste) · test w toku · test powiódł się/nie powiódł się · zapisywanie | Przycisk „Zapisz konto” jest zawsze aktywny; zapis tworzy encję konta modelu z odwołaniem do danych dostępowych poza bazą, ostrzeżenie przy polu nie blokuje zatwierdzenia; modal zamyka się i pojawia toast | Sekcja Konta modeli (8.3) | 3 | Rozwinięcie kontekstowe po użyciu działania „Dodaj konto modelu” |
| Przycisk „Testuj połączenie” | Przycisk drugorzędny, `.dn-btn--zarys` | Sprawdzenie poprawności danych dostępowych bez zamykania formularza | Mały przycisk tekstowy wewnątrz modala | domyślny · ładowanie · sukces (`.dn-plakietka--sukces`) · błąd (`.dn-plakietka--blad`) | Wywołuje próbne połączenie kanałem CLI; wynik wyświetla się obok przycisku | Modal dodania/edycji konta modelu (8.3) | 3 | Rozwinięcie kontekstowe wewnątrz modala konta modelu |
| Chat Window | Główne okno komunikacji Użytkownik ↔ Wykonawca, komponent `.dn-czat` | Przyjmowanie poleceń w języku naturalnym, prezentacja strumienia odpowiedzi i wyników, zatwierdzanie i przerywanie działań, wyjaśnianie wyniku i kontekstu; podstawowy mechanizm sterowania wszystkimi procesami platformy | Lewa kolumna, stała, pełna wysokość obszaru roboczego | gotowe · oczekiwanie na odpowiedź Wykonawcy · strumieniowanie odpowiedzi · oczekiwanie na zatwierdzenie · błąd kanału | Polecenie Użytkownika uruchamia zadanie; zatwierdzenie i przerwanie sterują jego przebiegiem; wynik pozostaje w strumieniu rozmowy | Każdy kontekst pracy, w tym otwarte okno Ustawień (rozdz. 2.1, 9.1) | 1 | Widoczne bez interakcji |
| Execution Loop Window | Okno pętli wykonawczej Koordynator ↔ Wykonawca, komponent `.dn-petla` | Prezentacja zlecenia i jego dekompozycji na zadania, kolejki i stanu zadań, wymiany komunikatów sterujących, wyników kontroli jakości i decyzji o ponowieniu oraz sterowanie przebiegiem pętli | Kolumna sąsiadująca z Chat Window, otwierana, pełna wysokość obszaru roboczego | zwinięte · pętla bezczynna · pętla w toku · wstrzymana · ponowienie zadania · przerwana · błąd | Wstrzymanie, wznowienie, przerwanie i korekta zlecenia zmieniają przebieg pętli; każde zdarzenie pętli trafia do strumienia Chat Window | Każdy kontekst pracy, w tym otwarte okno Ustawień (rozdz. 2.1, 9.2) | 2 | Wyzwalacz `▼` „Execution Loop” w kolumnie sąsiadującej; polecenie języka naturalnego w Chat Window |
| Pasek kontekstu | Zestaw znaczników kontekstowych, komponent `.dn-znacznik` | Prezentacja bieżącego środowiska, repozytorium, projektu, modelu i wykonawcy, na przykład `[Danaco Console] [Ubuntu] [Worktree] [Fable 5] [Ultra]` | Wąski pas znaczników w kolumnie obszaru roboczego, waga niska | gotowe · znacznik aktywny · selektor rozwinięty | Kliknięcie znacznika otwiera odpowiedni selektor; wybór zwija selektor samoczynnie | Każdy kontekst pracy (rozdz. 2.5) | 1 | Widoczny bez interakcji; kliknięcie znacznika otwiera selektor warstwy 2 |
...
| Objaśnienie kontekstowe `[?]` | Tooltip, `.dn-tooltip` | Wyjaśnienie działania danego ustawienia i jego wpływu na aplikację | Mała ikona/znak przy etykiecie ustawienia | ukryty · widoczny (najechanie lub fokus) | Najechanie lub fokus klawiaturą wyświetla dymek z treścią; brak zmiany stanu ustawienia | Każda pozycja ustawienia w każdej sekcji (rozdz. 2.3) | 2 | Najechanie lub fokus klawiatury na znaku `[?]` przy etykiecie ustawienia |

---

## 14. Stany — zestawienie zbiorcze

Rozdział 12 dokumentuje stany każdego elementu z osobna. Poniższe zestawienie porządkuje stany na poziomie okna i poszczególnych sekcji — sytuacje złożone, w których stan wynika z więcej niż jednego elementu jednocześnie.
...
| Błąd połączenia | Utrata kanału WebSocket podczas pracy w oknie | `.dn-alert--blad` na szczycie panelu zawartości: „Brak połączenia z serwerem — zmiany nie będą zapisywane do czasu przywrócenia łączności”; pola pozostają edytowalne lokalnie, zapis wstrzymany |
...
|---|---|---|---|
| Urządzenia (6) | Nie występuje — co najmniej urządzenie bieżące jest zawsze obecne | `.dn-alert--blad`: „Nie udało się pobrać wykazu urządzeń” + przycisk „Ponów” | Tabela `.dn-tabela` z co najmniej jednym wierszem |
| Konta modeli (8) | `.dn-pusty-stan`: „Brak skonfigurowanych kont modeli” + CTA „Dodaj konto modelu” | `.dn-alert--blad`: „Nie udało się pobrać kont modeli” + przycisk „Ponów” | Tabela `.dn-tabela` z co najmniej jednym wierszem |
| Powiadomienia (7) | Nie dotyczy — kategorie są stałym katalogiem platformy, nie listą dynamiczną | — | Tabela kategorii zawsze w pełni wypełniona |

### 14.3. Stany formularzy i modali

| Modal / formularz | Stan początkowy | Stan ostrzeżenia walidacji | Stan sukcesu |
|---|---|---|---|
| Zmiana hasła (3.4) | Pola nowego hasła puste (pole hasła bieżącego widoczne tylko, gdy ustawienie jest włączone — rozdz. 4.1); przycisk zatwierdzenia aktywny od otwarcia modala | „Hasło bieżące nieprawidłowe” / „Nowe hasła nie są identyczne” / „Uzupełnij nowe hasło” pod właściwym polem, `.dn-pole--ostrz` — ostrzeżenie, zapis można ponowić od razu | Zamknięcie modala + toast „Hasło zmienione” |
| Dodanie konta modelu (8.3) | Cztery pola puste; przycisk zatwierdzenia aktywny od otwarcia modala | „Katalog profilu już wykorzystany przez inne konto” / „Dane dostępowe odrzucone przez dostawcę” / „Uzupełnij pola wymagane” pod właściwym polem, `.dn-pole--ostrz` — ostrzeżenie, zapis można ponowić od razu | Zamknięcie modala + toast „Konto modelu dodane” + nowy wiersz w tabeli |
| Parowanie urządzenia (6.2) | Kod parowania / kod QR wyświetlony, oczekiwanie | „Czas parowania wygasł” — kod wygasły, przycisk „Wygeneruj nowy kod” | Zamknięcie modala + toast „Sparowano: <nazwa urządzenia>” + nowy wiersz w tabeli urządzeń |
...
| Aktywny/włączony | Tor w gradiencie złotym, wskaźnik po prawej | Ustawienie aktywne |
...
| Błąd | Tor z obrysem `--blad`, `.dn-tooltip` z komunikatem przy najechaniu | Zapis się nie powiódł — przełącznik wraca do stanu sprzed próby zmiany |
...
|---|---|---|---|
| Uwierzytelnianie — metody dodatkowe | `auth.method.set` | `auth.changed` | Bezpieczeństwo i uwierzytelnianie, rozdz. 12 |
...
Kontrakt komunikacji obejmuje polecenia właściwe Kontu Operatora edytowanemu z Ustawień, kategorię `model.account.*`, kategorię `comm.window.*` dla obu kanałów komunikacji operacyjnej oraz kategorię `ui.layer.*` dla warstw widoczności — w tej samej konwencji nazewniczej i strukturze koperty komunikatu co pozostałe kategorie platformy.
...
| `auth.password.set` | klient → serwer | Zmiana hasła przez Operatora już zalogowanego (rozdz. 3.4); odrębne od `auth.reset`, właściwego wyłącznie odzyskiwaniu konta |
| `model.account.list` | klient → serwer | Pobranie wykazu skonfigurowanych kont modeli (rozdz. 8.2) |
| `model.account.add` | klient → serwer | Dodanie nowego konta modelu wraz z danymi dostępowymi (rozdz. 8.3) |
| `model.account.test` | klient → serwer | Próbne połączenie kanałem CLI z wykorzystaniem danych dostępowych wskazanego konta (rozdz. 8.3) |
| `model.account.set-active` | klient → serwer | Ustawienie wskazanego konta jako aktywnego (rozdz. 8.4) |
| `model.account.remove` | klient → serwer | Usunięcie konta modelu i jego danych dostępowych z magazynu poza bazą |
| `model.account.changed` | serwer → klient | Rozgłoszenie zmiany wykazu kont modeli lub zmiany konta aktywnego do wszystkich urządzeń (rozdz. 8.4) |
| `comm.window.set` | klient → serwer | Zmiana ustawienia Chat Window albo Execution Loop Window: szerokość kolumny, gęstość strumienia, zakres wyświetlanych komunikatów sterujących, progi potwierdzeń (rozdz. 9.3) |
| `comm.window.changed` | serwer → klient | Rozgłoszenie zmiany ustawień okien komunikacji operacyjnej do wszystkich urządzeń Operatora (rozdz. 9.3) |
| `ui.layer.set` | klient → serwer | Przypisanie warstw widoczności do roli użytkownika (rozdz. 10.1) |
| `ui.layer.changed` | serwer → klient | Rozgłoszenie zmiany przypisania warstw widoczności do roli (rozdz. 10.1) |
...
| `aod.status.get` | klient → serwer | Odczyt stanu aktywacji i widoczności funkcji globalnej po zmianie ustawienia (`funkcje-globalne/always-on-display.md`, rozdz. 9.5) |
...
| **Konto modelu** | Nazwany, zapisany zestaw danych dostępowych kanału CLI (w szczególności konta Claude Code) wraz z katalogiem profilu, zarządzany w sekcji Konta modeli (rozdz. 8) |
...
metoda_uwierzytelniania:
  ...
  wyłączalna:        tak                              # wszystkie metody konfigurowalne, hasło włącznie
...
```
# Szablon — kategoria powiadomień (rozdz. 7)
kategoria_powiadomien:
  nazwa:               <np. "Zdarzenia MultitaskingAI">
...
| 6. Urządzenia | Bezpieczeństwo i uwierzytelnianie; funkcje-globalne/mobile.md; Architektura | 3, 11.2, 13; 9; 13 |
...
*Koniec dokumentu. Danaco Console — Okno Ustawień (poziom aplikacji), wersja 2.0.*
