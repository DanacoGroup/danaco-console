...
| **Przeznaczenie** | Ustala interfejs modułu Agents: komplet okien operacyjnych, katalog elementów każdego okna, diagram cyklu tworzenia eksperta, stany oraz punkty sterowania z okna konfiguracji |
| **Zakres** | Siedem okien operacyjnych modułu Agents — Chat Window, Execution Loop Window, Agent Builder, Model Configuration, Skills Manager, Connectors Manager, Permissions Center — przy czym Chat Window (Użytkownik ↔ Wykonawca) i Execution Loop Window (Koordynator ↔ Wykonawca) są kolumnami wspólnymi wszystkim pozostałym widokom modułu; komendy kontraktu obszaru `agent` |
| **Poza zakresem** | Model agenta jako komponentu własnego — definicja, siedem komponentów definicji, cykl życia, model danych i kontrakty komunikacji poza oknem modułu — [Specyfikacja agentów](../specyfikacje/specyfikacja-agentow.md) |
| **Dokument nadrzędny** | [Specyfikacja modułów](../specyfikacje/specyfikacja-modulow.md) |
| **Dokumenty powiązane** | [Specyfikacja agentów](../specyfikacje/specyfikacja-agentow.md) · [Koncepcja platformy](../architektura/koncepcja-platformy.md) · [Specyfikacja okien operacyjnych](../specyfikacje/specyfikacja-okien-operacyjnych.md) · [Strona główna i nawigacja](../interfejs-uzytkownika/strona-glowna-i-nawigacja.md) · [System wizualny](../interfejs-uzytkownika/system-wizualny.md) · [Model konfiguracji](../architektura/model-konfiguracji.md) · [MultitaskingAI](../srodowiska/multitaskingai.md) · [Izolacja i zależności](../architektura/izolacja-i-zaleznosci.md) · [Rozszerzenia](../architektura/rozszerzenia.md) · [Bezpieczeństwo i uwierzytelnianie](../architektura/bezpieczenstwo-i-uwierzytelnianie.md) |
| **Prototypy odniesienia** | `design/05-okna/moduly/agents.html` |
| **Źródła normatywne** | `budowa/shared/contract.json` (obszar `agent`) · `design/zasoby/zetony/zetony.css` · `design/zasoby/css/komponenty.css`, `rama.css`, `prototyp.css` |
...
Moduł Agents udostępnia siedem okien operacyjnych (Koncepcja platformy, rozdz. 11.15; Specyfikacja okien operacyjnych, rozdz. 6.15): **Chat Window**, **Execution Loop Window**, **Agent Builder**, **Model Configuration**, **Skills Manager**, **Connectors Manager**, **Permissions Center**. Niniejszy dokument projektuje wszystkie siedem na poziomie gotowym do wykonania — makieta, katalog elementów, stany — z Permissions Center rozumianym wyłącznie jako **konfiguracja zakresu możliwości eksperta**, nigdy jako mechanizm blokujący (rozdz. 9 niniejszego dokumentu; podstawa: Specyfikacja agentów, rozdz. 6).
...
**Rozgraniczenie wobec dokumentu systemowego.** Model agentów — definicja agenta jako komponentu własnego, siedem komponentów definicji, role agenta w środowiskach, cykl życia agenta, pełny przebieg pętli wykonawczej, model danych i kontrakty komunikacji agenta — opisuje [Specyfikacja agentów](../specyfikacje/specyfikacja-agentow.md). Niniejszy dokument opisuje wyłącznie **interfejs modułu Agents**: okna, ich makiety, elementy, warstwy widoczności, stany oraz punkty sterowania z okna konfiguracji. Ustalenia modelowe nie są tu powtarzane, lecz przywoływane odesłaniem do właściwego rozdziału dokumentu systemowego.
...
1. [Agents jako fabryka ekspertów — pozycja w platformie](#1-agents-jako-fabryka-ekspertów--pozycja-w-platformie)
   - [1.1 Łańcuch wartości](#11-łańcuch-wartości)
   - [1.2 Agent jako komponent własny — pozycja wśród czterech rodzajów](#12-agent-jako-komponent-własny--pozycja-wśród-czterech-rodzajów)
   - [1.3 Gdzie ekspert działa — punkty styku interfejsu](#13-gdzie-ekspert-działa--punkty-styku-interfejsu)
...
2. [Mapa okien modułu Agents](#2-mapa-okien-modułu-agents)
   - [2.1 Komplet okien operacyjnych modułu i ich relacje](#21-komplet-okien-operacyjnych-modułu-i-ich-relacje)
   - [2.2 Legenda ikon okien (System wizualny, 47-ikonowy zestaw — Załącznik C)](#22-legenda-ikon-okien-system-wizualny-47-ikonowy-zestaw--załącznik-c)
   - [2.3 Makieta powłoki modułu](#23-makieta-powłoki-modułu)
   - [2.4 Chat Window i Execution Loop Window — dwa kanały komunikacji operacyjnej](#24-chat-window-i-execution-loop-window--dwa-kanały-komunikacji-operacyjnej)
...
4. [Stany](#4-stany)
   - [4.1 Stany eksperta jako komponentu własnego](#41-stany-eksperta-jako-komponentu-własnego)
   - [4.2 Stany interfejsu — reguła wspólna (System wizualny, rozdz. 6) i jej zastosowanie w module Agents](#42-stany-interfejsu--reguła-wspólna-system-wizualny-rozdz-6-i-jej-zastosowanie-w-module-agents)
5. [Agent Builder — okno nadrzędne](#5-agent-builder--okno-nadrzędne)
   - [5.1 Cel, zawartość ogólna i pozycja w przepływie](#51-cel-zawartość-ogólna-i-pozycja-w-przepływie)
   - [5.2 Widok „Biblioteka ekspertów”](#52-widok-biblioteka-ekspertów)
   - [5.3 Widok „Edytor” — pasek zakładek i zakładka Tożsamość](#53-widok-edytor--pasek-zakładek-i-zakładka-tożsamość)
   - [5.4 Panel „Historia wersji”](#54-panel-historia-wersji)
   - [5.5 Stany szczegółowe okna Agent Builder](#55-stany-szczegółowe-okna-agent-builder)
6. [Model Configuration](#6-model-configuration)
   - [6.1 Cel i zawartość](#61-cel-i-zawartość)
   - [6.2 Makieta](#62-makieta)
   - [6.3 Katalog elementów](#63-katalog-elementów)
   - [6.4 Stany](#64-stany)
7. [Skills Manager](#7-skills-manager)
   - [7.1 Cel i zawartość](#71-cel-i-zawartość)
   - [7.2 Makieta](#72-makieta)
   - [7.3 Katalog elementów](#73-katalog-elementów)
   - [7.4 Stany](#74-stany)
8. [Connectors Manager](#8-connectors-manager)
   - [8.1 Cel i zawartość](#81-cel-i-zawartość)
   - [8.2 Makieta](#82-makieta)
   - [8.3 Katalog elementów](#83-katalog-elementów)
   - [8.4 Stany](#84-stany)
9. [Permissions Center](#9-permissions-center)
   - [9.1 Cel — konfiguracja możliwości, nie kontrola dostępu](#91-cel--konfiguracja-możliwości-nie-kontrola-dostępu)
   - [9.2 Cztery grupy zakresu](#92-cztery-grupy-zakresu)
   - [9.3 Makieta](#93-makieta)
   - [9.4 Katalog elementów](#94-katalog-elementów)
   - [9.5 Relacja do okna konfiguracji punktów izolacji](#95-relacja-do-okna-konfiguracji-punktów-izolacji)
   - [9.6 Stan wyjściowy — pełny dostęp operacyjny](#96-stan-wyjściowy--pełny-dostęp-operacyjny)
   - [9.7 Stany szczegółowe okna](#97-stany-szczegółowe-okna)
   - [9.8 Przebieg rozstrzygania kolizji między poziomami zasięgu](#98-przebieg-rozstrzygania-kolizji-między-poziomami-zasięgu)
   - [9.9 Zawężenie zakresu w trakcie trwającego zadania](#99-zawężenie-zakresu-w-trakcie-trwającego-zadania)
10. [Od eksperta do Wykonawcy — zastosowanie operacyjne](#10-od-eksperta-do-wykonawcy--zastosowanie-operacyjne)
   - [10.1 Wykorzystanie w TalkIn, WorkSpace i CodeStudio](#101-wykorzystanie-w-talkin-workspace-i-codestudio)
   - [10.2 Przypisanie do projektu — Agent Manager](#102-przypisanie-do-projektu--agent-manager)
   - [10.3 Przypisanie do roli — sekcja Role panelu orkiestracji](#103-przypisanie-do-roli--sekcja-role-panelu-orkiestracji)
   - [10.4 Udział Wykonawców w pętli wykonawczej — odwzorowanie w interfejsie](#104-udział-wykonawców-w-pętli-wykonawczej--odwzorowanie-w-interfejsie)
11. [Katalog komponentów i ikon systemu wizualnego w module Agents](#11-katalog-komponentów-i-ikon-systemu-wizualnego-w-module-agents)
   - [11.1 Komponenty `.dn-*` użyte w module Agents](#111-komponenty-dn--użyte-w-module-agents)
   - [11.2 Ikony (z zestawu 47 — System wizualny, Załącznik C)](#112-ikony-z-zestawu-47--system-wizualny-załącznik-c)
12. [Słowniczek pojęć](#12-słowniczek-pojęć)
13. [Punkty sterowania z okna konfiguracji](#13-punkty-sterowania-z-okna-konfiguracji)
14. [Komendy kontraktu obszaru `agent`](#14-komendy-kontraktu-obszaru-agent)
   - [14.1 Obszar `agent` — 30 komend](#141-obszar-agent--30-komend)
15. [Kryteria odbioru](#15-kryteria-odbioru)
16. [Załącznik — szablon definicji eksperta jako formularz interfejsu](#16-załącznik--szablon-definicji-eksperta-jako-formularz-interfejsu)
...
Diagram — łańcuch wartości modułu Agents
  ...
  (inteligencja)                 wybór i kanał połączenia → Model Configuration
        ...
        │   + wtyczki, konektory, serwery MCP            ─────  Connectors Manager
...
Zakres i charakter wykorzystania agenta w każdym środowisku ustala [Specyfikacja agentów](../specyfikacje/specyfikacja-agentow.md), rozdz. 7–9. Poniższa tabela wskazuje wyłącznie miejsce interfejsu, z którego przypisanie następuje.
...
| MultitaskingAI | Sekcja Role panelu orkiestracji ([MultitaskingAI](../srodowiska/multitaskingai.md), rozdz. 6.2) | 10.3 |
...
| 5 | Skills Manager | Rejestr / panel narzędziowy | Zakładka edytora w kolumnie dominującej | 2 | Zakładka `Umiejętności` paska zakładek edytora, polecenie języka naturalnego w Chat Window | Podłączanie umiejętności do definicji eksperta |
| 6 | Connectors Manager | Rejestr / panel narzędziowy | Zakładka edytora w kolumnie dominującej | 2 | Zakładka `Konektory` paska zakładek edytora, polecenie języka naturalnego w Chat Window | Podłączanie wtyczek, konektorów i serwerów MCP do definicji eksperta |
...
Schemat — okna modułu Agents i ich wzajemne relacje
   ...
   model bazowy          umiejętności        wtyczki ·              zakres możliwości
   + kanał połączenia                         konektory ·            eksperta
...
Makieta — powłoka modułu Agents w środowisku (TalkIn / WorkSpace / CodeStudio), stan spoczynku
 ...
 ══════════════════════════════════════════════════════════════════════════════
  belka + wstążka   Danaco Console · [tytuł widoku]   [ szukaj ]   ⚙ 👤
 ══════════════════════════════════════════════════════════════════════════════
  ...
  Browser     │ │ rozmowy          │ │ 🧑Tożsamość│🚀Model│⭐Umiejętności│ ⋮
...
Układ jest wyłącznie pionowy — kolumny sąsiadują poziomo. Chat Window zajmuje lewą kolumnę, stałą, o pełnej wysokości obszaru roboczego; Execution Loop Window otwiera się jako kolumna sąsiadująca; obszar roboczy modułu Agents jest kolumną dominującą; panele pomocnicze otwierają się jako rozszerzenia boczne po prawej stronie obszaru roboczego.

**Szerokość okna centralnego — reguła platformowa.** Szerokość okna centralnego, mieszczącego okna robocze i okna komunikacji, nie jest nastawą własną żadnego z tych okien. Wynika z dwóch rzeczy: ze stanu dwóch okien sąsiednich — czy lewe okno nawigacyjne jest otwarte i czy prawe okno pomocnicze jest rozwinięte — oraz z ręcznego przesunięcia paska przez Operatora, który ustala indywidualną szerokość każdego z trzech okien głównych. Podział samego okna centralnego zależy wyłącznie od liczby otwartych okien komunikacji:

| Liczba okien komunikacji | Zachowanie okna centralnego |
|---|---|
| jedno | Okno stoi na środku okna centralnego; okno centralne nie dzieli się |
| dwa | Okno centralne dzieli się na dwa pola o automatycznie równej szerokości |
| trzy i więcej | Okno centralne nie dzieli się dalej — wchodzi przełącznik widoku, widoczne są najwyżej dwa okna naraz, pozostałe podglądy Operator przeklikuje przełącznikiem |
...
| Akcje kontekstowe w odpowiedzi | `.dn-btn--zarys` małe, osadzone w treści odpowiedzi Wykonawcy | Skrót do wykonania sugerowanej czynności bez opuszczania rozmowy | 1 | Widoczne bez interakcji | Małe przyciski inline | Domyślny · wskazanie kursorem | Wykonuje akcję i potwierdza wynik w strumieniu | Wewnątrz odpowiedzi Wykonawcy |
...
| Pole wpisu | `.dn-pole-kontrolka` pełnej szerokości kolumny | Wpisanie polecenia w języku naturalnym | 1 | Widoczne bez interakcji | Duże pole tekstowe, dół kolumny | Domyślny · fokus · wypełnione | Enter lub „Wyślij” wysyła polecenie | Dolna część lewej kolumny |
...
| Przycisk „Wyślij” | `.dn-btn--sygnal` mały | Wysyła treść pola wpisu | 1 | Widoczny bez interakcji | Mały, przy polu wpisu | Zawsze w pełni klikalny — pole puste przy kliknięciu sygnalizuje komunikatem „wpisz polecenie”, kontrolka nie jest blokowana | Wysyła treść i czyści pole; strumień aktualizuje się na żywo | Obok pola wpisu |
...
| Błąd połączenia | Utrata WebSocket | `.dn-toast--blad` w kolumnie; pole wpisu pozostaje aktywne — treść wysłana w tym stanie jest kolejkowana do ponowienia po przywróceniu połączenia |
...
Diagram — cykl tworzenia i życia eksperta
  ...
  START            STREFA 2 STRONY GŁÓWNEJ, kafel „Agents”
                   ...
                   ║ 5. Wtyczki/konektory/MCP Connectors    ║
...
|---|---|---|---|
| Szkic | Definicja rozpoczęta w Agent Builder, jeszcze nigdy niezapisana | Kliknięcie „＋ Nowy ekspert” | Nagłówek edytora — plakietka `.dn-plakietka--rola.dn-plakietka--ostrzezenie` „SZKIC” |
| Niezapisane zmiany | Zmiana dokonana na zapisanej już definicji, jeszcze niezapisana jako nowa wersja | Edycja dowolnego pola w dowolnej zakładce | Nagłówek edytora — kropka `.dn-kropka--ostrzezenie` przy nazwie |
...
| Wskazanie kursorem | `--dn-hover` albo uniesienie `translateY` dla elementów akcentowanych | Karta eksperta unosi się `-1px`; wiersz rejestru w Skills/Connectors Manager podświetla tło |
| Fokus (klawiatura) | Pierścień `--dn-fokus` 2 px, wyłącznie `:focus-visible` | Każde pole formularza, każdy przełącznik macierzy uprawnień |
| Aktywny / zaznaczony | Akcent sygnału: obrys, podkreślenie, tło `--dn-sygnal-tlo` | Zakładka bieżąca edytora; pozycja wybrana w selektorze grup Permissions Center |
...
| Ostrzeżenie | `.dn-kropka--ostrzezenie` / obramowanie bursztynowe pola — sygnalizuje problem, nigdy nie blokuje akcji | Nazwa zduplikowana w zasięgu widoczności; pole nazwy puste w chwili zapisu (uzupełniane nazwą roboczą) |
| Błąd | `.dn-toast--blad` / `.dn-pole-blad` — wyłącznie awarie techniczne, nigdy wynik walidacji formy | Utrata połączenia przy zapisie; nieprawidłowy adres serwera MCP |
...
| Typologia komponentowa | Okno kreatora — System wizualny, rozdz. 10.6: `.dn-zakladki` lub kroki formularza, `.dn-karta` boczna z podsumowaniem, `.dn-btn--sygnal` jako jedyne CTA finalizujące |
...
| Przycisk „＋ Nowy ekspert” | Przycisk pierwszoplanowy `.dn-btn--sygnal` z ikoną `plus` | Rozpoczyna tworzenie nowej definicji eksperta | 1 | Widoczny bez interakcji | Duży przycisk, jedyny CTA sygnałowy widoku — zasada oszczędnego akcentu (System wizualny, rozdz. 8.1) | Domyślny · wskazanie kursorem (uniesienie 2 px) · aktywny (wciśnięcie) · fokus (pierścień `--dn-fokus`) | Otwiera widok Edytor z pustym formularzem w stanie „Szkic” | Prawy górny róg widoku Biblioteka |
| Pole wyszukiwania | Pole tekstowe `.dn-pole-kontrolka` z ikoną `szukaj` | Filtruje listę po nazwie lub opisie przeznaczenia | 2 | Kliknięcie ikony `szukaj` w nagłówku widoku; po użyciu pole zwija się samoczynnie | Pole średnie, pełna szerokość paska filtrów przy braku innych filtrów | Domyślny · fokus · wypełnione (z przyciskiem `zamknij` czyszczącym) | Filtruje siatkę kart na żywo, bez przeładowania widoku | Pasek filtrów widoku Biblioteka |
| Filtr „Środowisko” | Rozwijana lista `.dn-pole-kontrolka` (wielokrotny wybór) | Zawęża do ekspertów wykorzystywanych w TalkIn / WorkSpace / CodeStudio / MultitaskingAI | 3 | Grupa `Filtry ▼` — lista rozwijana | Mała kontrolka w pasku filtrów | Domyślny · rozwinięty · zaznaczone pozycje (plakietka liczby) | Zawęża siatkę kart | Pasek filtrów |
| Filtr „Widoczność” | `.dn-pole-kontrolka`: Globalny / Projektowy | Zawęża wg zasięgu widoczności (Specyfikacja agentów, rozdz. 5.2) | 3 | Grupa `Filtry ▼` — lista rozwijana | Mała kontrolka | Jak wyżej | Zawęża siatkę kart | Pasek filtrów |
| Filtr „Stan” | `.dn-pole-kontrolka`: Aktywny / Szkic / Zarchiwizowany | Zawęża wg stanu eksperta (rozdz. 4.1) | 3 | Grupa `Filtry ▼` — lista rozwijana | Mała kontrolka | Jak wyżej | Zawęża siatkę kart | Pasek filtrów |
| Karta eksperta | `.dn-karta` w układzie siatki | Reprezentuje jedną zapisaną definicję eksperta; punkt wejścia do edycji | 1 | Widoczna bez interakcji | Karta średnia, siatka 3–4 kolumn responsywnie | Domyślny · wskazanie kursorem (`--interaktywna`, podświetlenie tła) · zaznaczona (przy akcji zbiorczej, jeśli skonfigurowana) | Kliknięcie otwiera widok Edytor z załadowaną definicją (wersja aktywna) | Główny obszar widoku Biblioteka |
...
| Plakietka wersji | `.dn-plakietka` mała, np. „w.3” | Numer bieżącej wersji aktywnej | 1 | Widoczna bez interakcji | Bardzo mała, pigułka | Domyślny | Wskazanie kursorem pokazuje dymek podpowiedzi z datą ostatniego zapisu | Karta eksperta |
...
| Plakietka stanu | `.dn-plakietka--sukces` / `.dn-plakietka--ostrzezenie` / `.dn-plakietka` z `.dn-kropka` (sukces/ostrzeżenie/neutralna) | Aktywny / Szkic / Zarchiwizowany | 1 | Widoczna bez interakcji | Mała, pigułka z kropką | sukces (aktywny) · ostrzeżenie (szkic) · neutralny (zarchiwizowany) | — | Karta eksperta |
| Licznik przypisań | Tekst „N przypisań” / „1 rola” | Sygnalizuje wykorzystanie w Agent Manager lub sekcji Role | 1 | Widoczny bez interakcji | Bardzo mała waga | Domyślny · zero (tekst „bez przypisań”, `--dn-tekst-3`) | Wskazanie kursorem pokazuje dymek podpowiedzi z listą projektów/ról | Karta eksperta |
| Menu akcji karty | Trzy przyciski tekstowe/`.dn-btn-ikona`: Edytuj · Duplikuj · Więcej (`wiecej`) | Szybkie akcje bez otwierania pełnego edytora | 3 | Menu kebab (⋮) na karcie | Mały pasek na stopce karty | Domyślny · wskazanie kursorem | „Edytuj” → widok Edytor; „Duplikuj” → nowy szkic wypełniony tą definicją; „Więcej” rozwija: Archiwizuj (`archiwum`), Usuń (`kosz`, wariant `.dn-btn--niebezpieczny` — sygnalizuje wagę, nie blokuje) | Stopka karty eksperta |
...
Makieta — Agent Builder, widok: Edytor, zakładka Tożsamość (stan spoczynku)
 ...
 ══════════════════════════════════════════════════════════════════════════════
  ...
  Wykonawca        │ Wykonawca        │ 🧑Tożsamość│🚀Model│⭐Umiejętności│ PODSUMO-
 ──────────────────┼──────────────────┤ 🔗Konektory │🛡Uprawnienia   │ WANIE
  strumień rozmowy │ zlecenie i       │ Nazwa       [ .dn-pole-kontrolka ]   │ DEFINICJI
                   │ dekompozycja     │ Opis        [ .dn-pole-kontrolka ]│ Model: —
                   │ kolejka zadań    │ Moduły  ☐TalkIn ☐WorkSpace  │ Kanał: —
                   │ kontrola jakości │         ☐CodeStudio         │ Umiejętn.: 0
...
| Element | Co to jest | Do czego służy | Warstwa | Sposób wywołania | Forma i waga | Stany | Zachowanie po interakcji | Gdzie występuje |
|---|---|---|---|---|---|---|---|---|
| Odnośnik „← Biblioteka” | Link tekstowy z ikoną `strzalka-lewo` | Powrót do widoku Biblioteka | 1 | Widoczny bez interakcji | Mały, w lewym rogu nagłówka | Domyślny · wskazanie kursorem | Powrót; przy niezapisanych zmianach — nieblokujący dymek powiadomienia „zmiany zachowane jako szkic” (nie okno nakładkowe zatrzymujące) | Nagłówek edytora |
| Pole nazwy (inline, w nagłówku) | `.dn-pole-kontrolka` bez obramowania w stanie spoczynku, edytowalne po kliknięciu | Szybka zmiana nazwy bez wchodzenia w zakładkę Tożsamość | 1 | Widoczny bez interakcji | Duża waga — krój nagłówkowy przy odczycie, `.dn-pole-kontrolka` przy edycji | Domyślny (odczyt) · edycja (fokus) · ostrzeżenie (nazwa pusta lub zduplikowana — obramowanie bursztynowe, nieblokujące) | Zapis nazwy przy utracie fokusu; nazwa pusta → uzupełniana nazwą roboczą „Nowy ekspert”; duplikat oznacza pole z komunikatem ostrzegawczym — zapis i pozostałe pola pozostają w pełni dostępne | Nagłówek edytora, zsynchronizowane z polem „Nazwa eksperta” w zakładce Tożsamość |
| Plakietka stanu | `.dn-plakietka--rola` łączona z `.dn-plakietka--ostrzezenie` / `.dn-plakietka--sukces` / `.dn-plakietka--rola` samodzielna | SZKIC / AKTYWNY / ZARCHIWIZOWANY (rozdz. 4.1) | 1 | Widoczna bez interakcji | Mała pigułka obok nazwy | Wg stanu encji | — | Nagłówek edytora |
...
| Przycisk „Anuluj” | `.dn-btn--duch` | Odrzuca niezapisane zmiany bieżącej sesji edycji | 1 | Widoczny bez interakcji | Mały, obok „Zapisz eksperta” | Domyślny · wskazanie kursorem | Przywraca ostatnią zapisaną wersję do formularza; nie usuwa historii | Stopka edytora |
| Przycisk „Zapisz eksperta” | `.dn-btn--sygnal` | Zapisuje bieżący stan formularza jako nową wersję | 1 | Widoczny bez interakcji | Duży, jedyny CTA sygnałowy widoku Edytor | Domyślny — zawsze w pełni klikalny, również z nazwą pustą · ładowanie (`.dn-spinner` w przycisku) · błąd (dymek powiadomienia, wyłącznie awaria techniczna) | Sukces: nazwa pusta uzupełniana automatycznie nazwą roboczą „Nowy ekspert” z ostrzeżeniem; nowa wersja, plakietka „zapisano”, karta w Bibliotece aktualizuje się na żywo (`agent.changed`) | Stopka edytora, widoczna na wszystkich pięciu zakładkach |
...
| Wiersz wersji | `.dn-listwa-pozycja` | Reprezentuje jeden zapisany stan definicji | 3 | Widoczny po otwarciu panelu Historia wersji | Mały wiersz listy | Aktywna (`.dn-kropka--sukces` wypełniona) · archiwalna (`.dn-kropka` pusta) | — | Panel Historia wersji |
...
| Zapisano | Potwierdzenie serwera | Dymek powiadomienia sukcesu, wskaźnik zapisu aktualizuje znacznik czasu, nowa pozycja w Historii wersji |
| Błąd zapisu | Utrata połączenia lub odrzucenie przez serwer | Dymek powiadomienia błędu z treścią przyczyny, formularz pozostaje edytowalny z wprowadzonymi danymi |
...
Makieta — zakładka Model Configuration (stan spoczynku)
 ...
 ══════════════════════════════════════════════════════════════════════════════
  💬 CHAT WINDOW   │ ⟳ EXECUTION LOOP │ 🧑Tożsamość │🚀Model Configuration │⭐Umiejętności
...
| Błąd testu | Nieosiągalny kanał | `.dn-toast--blad` z treścią przyczyny, możliwość zapisania eksperta mimo błędu testu (test jest narzędziem pomocniczym, nie bramką) |
...
Connectors Manager podłącza do definicji eksperta wtyczki, konektory i serwery MCP — rozszerzenia otwierające ekspertowi dostęp poza platformę (Specyfikacja agentów, rozdz. 5.5; Rozszerzenia, rozdz. 1 i 6). Podobnie jak w Skills Manager, okno rozstrzyga wyłącznie **czy rozszerzenie jest podłączone do definicji eksperta** — zakres, w jakim ekspert może je faktycznie wykorzystać (odczyt, zapis, wywołanie akcji), rozstrzyga Permissions Center (rozdz. 9).
...
Schemat — rodzaje rozszerzeń i ich okna w module Agents
  ...
  │
  ├── Umiejętność ───────────────────────────────────► Skills Manager (rozdz. 7)
  ...
  z formułą zadania „umiejętności / wtyczki / konektory / MCP”. Rozszerzenia
  (rozdz. 6) grupują wtyczkę koncepcyjnie z umiejętnościami na poziomie kontraktu
...
Makieta — zakładka Connectors Manager (stan spoczynku)
 ...
 ══════════════════════════════════════════════════════════════════════════════
  💬 CHAT WINDOW   │ ⟳ EXECUTION LOOP │ 🧑Tożsamość │🚀Model │⭐Umiejętności
...
| Element | Co to jest | Do czego służy | Warstwa | Sposób wywołania | Forma i waga | Stany | Zachowanie po interakcji | Gdzie występuje |
|---|---|---|---|---|---|---|---|---|
| Pole wyszukiwania | `.dn-pole-kontrolka` z ikoną `szukaj` | Filtruje rejestr po nazwie | 2 | Kliknięcie ikony `szukaj` w nagłówku widoku; po użyciu pole zwija się samoczynnie | Średnie pole | Domyślny · fokus · wypełnione | Filtruje listę na żywo | Górny pasek |
...
| Filtr „Źródło” | `.dn-pole-kontrolka`: Danaco Plugin / Personal | Zawęża wg pochodzenia | 3 | Grupa `Filtry ▼` — lista rozwijana | Mała kontrolka | Domyślny · rozwinięty | Filtruje listę | Górny pasek |
| Wiersz rozszerzenia | `.dn-listwa-pozycja` z ikoną `link-zewnetrzny` | Reprezentuje jedną zarejestrowaną wtyczkę, konektor lub serwer MCP | 1 | Widoczny bez interakcji | Średni wiersz listy | Podłączony / Niepodłączony (adnotacja w wierszu) | Kliknięcie nazwy rozwija panel konfiguracji instancji | Lista główna |
| Plakietka rodzaju | `.dn-plakietka--informacja`: „Wtyczka” / „Konektor” / „Serwer MCP” | Rozróżnia rodzaj rozszerzenia w wierszu | 1 | Widoczna bez interakcji | Bardzo mała pigułka | Domyślny | — | Wiersz rozszerzenia |
| Przełącznik „Podłączony temu ekspertowi” | `.dn-przelacznik` w wierszu | Decyduje, czy TEN ekspert ma dostęp do danego rozszerzenia | 1 | Widoczny bez interakcji | Mały przełącznik | Domyślny (wyłączony) · włączony — zawsze w pełni klikalny, również gdy rozszerzenie jest wyłączone w rejestrze globalnym | Włączenie podłącza rozszerzenie do definicji eksperta (jeśli wciąż wyłączone w rejestrze globalnym — zapisane jako „podłączone, oczekuje aktywacji w rejestrze”, zaczyna działać automatycznie po włączeniu go tam); zakres wykorzystania pozostaje pełny dopóki nieskonfigurowany inaczej w Permissions Center (rozdz. 9.6) | Każdy wiersz |
| Panel konfiguracji instancji (dla konektora/serwera MCP) | Rozwijany fragment wiersza z polami `.dn-pole-kontrolka` (adres serwera, dane dostępowe — domyślnie jawne, z maskowaniem jednym kliknięciem, jak w Model Configuration, rozdz. 6.3) | Parametry połączenia właściwe temu egzemplarzowi rozszerzenia (Rozszerzenia, rozdz. 4) | 3 | Kliknięcie nazwy rozszerzenia — panel rozwija się w miejscu | Średni, rozwijany | Zwinięty · rozwinięty · ostrzeżenie (adres wygląda na nieprawidłowy — sygnał, nie blokada zapisu) | Edycja zapisuje się w atrybucie Konfiguracja | Pod wierszem po kliknięciu |
| Baner „Zakres wykorzystania → Permissions Center” | `.dn-toast--informacja` | Jednoznacznie oddziela decyzję „czy podłączone” (tu) od decyzji „w jakim zakresie” (Permissions Center) | 1 | Widoczny bez interakcji | Pełna szerokość, pod listą | Stały | Kliknięcie „→” przełącza na zakładkę Permissions Center | Dół listy, nad stopką |
| Przycisk „Podłącz nową wtyczkę / konektor / serwer MCP Personal…” | `.dn-btn--zarys` z ikoną `plus` | Skrót do instalacji nowego rozszerzenia typu Personal | 3 | Grupa `Operacje ▼` w stopce listy | Mały przycisk | Domyślny · wskazanie kursorem | Otwiera przepływ instalacji w oknie konfiguracji platformy | Stopka zakładki |
...
|---|---|---|
| Dostęp do rozszerzeń | Które z podłączonych w Connectors Manager wtyczek, konektorów i serwerów MCP ekspert może rzeczywiście wykorzystać oraz w jakim zakresie (odczyt, zapis, wywołanie akcji) | Connectors Manager (rozdz. 8) |
...
Makieta — zakładka Permissions Center (stan spoczynku; funkcje warstwy 4)
 ...
 ══════════════════════════════════════════════════════════════════════════════
  💬 CHAT WINDOW   │ ⟳ EXECUTION LOOP │ 🧑Tożsamość │🚀Model │⭐Umiejętności │🔗Konektory
...
| Element | Co to jest | Do czego służy | Warstwa | Sposób wywołania | Forma i waga | Stany | Zachowanie po interakcji | Gdzie występuje |
|---|---|---|---|---|---|---|---|---|
| Baner „Stan wyjściowy: pełny dostęp” | `.dn-toast--informacja`, pełna szerokość, ikona `tarcza` | Ustala ramę odbioru całego okna — zakres, nie blokada (rozdz. 9.1) | 4 | Tryb administracyjny lub konfiguracja roli; polecenie języka naturalnego w Chat Window, skrót klawiszowy albo wyszukiwarka funkcji. Użytkownik podstawowy nie widzi tego elementu | Duży, nieprzewijalny pasek w nagłówku zakładki | Stały, niezależny od grupy wybranej po lewej | — (statyczny, tylko przycisk „Resetuj” jest interaktywny) | Góra zakładki Permissions Center |
| Przycisk „Resetuj do pełnego dostępu” | `.dn-btn--zarys` z ikoną `odswiez` | Jednym kliknięciem przywraca stan wyjściowy we wszystkich czterech grupach | 4 | Tryb administracyjny lub konfiguracja roli; polecenie języka naturalnego w Chat Window, skrót klawiszowy albo wyszukiwarka funkcji. Użytkownik podstawowy nie widzi tego elementu | Mały przycisk w prawym rogu banera | Domyślny — zawsze w pełni klikalny, również gdy żadne zawężenie nie zostało wprowadzone | Przywraca natychmiast pełny dostęp we wszystkich grupach, bez okna nakładkowego potwierdzenia; `.dn-toast--sukces` „przywrócono pełny dostęp” z akcją „Cofnij” (przywraca poprzednią konfigurację zawężeń) — przy braku uprzednich zawężeń dymek powiadomienia „już pełny dostęp, brak zmian” | Baner górny |
| Selektor grup | Lista `.dn-listwa-pozycja` bez akcji w wierszu, cztery pozycje | Wybór jednej z czterech grup zakresu do edycji | 4 | Tryb administracyjny lub konfiguracja roli; polecenie języka naturalnego w Chat Window, skrót klawiszowy albo wyszukiwarka funkcji. Użytkownik podstawowy nie widzi tego elementu | Mała lista, lewa kolumna | Pozycja wybrana (tło `--dn-sygnal-tlo`, tekst `--dn-sygnal`) · pozostałe (domyślny) | Kliknięcie przełącza zawartość prawej kolumny | Lewa kolumna okna |
...
```
Schemat — Permissions Center wobec ośmiu poziomów zasięgu izolacji
...
(wartość wyjściowa
 właściwa temu ekspertowi)
        ...
        │            zasięg: Projekt ─────────────────────┤
        │            zasięg: Karta sesji ─────────────────┤
        │            zasięg: Rola (MultitaskingAI) ───────┤
        └───────────► zasięg: Okno komunikacji ───────────┤  (najwyższe)
                                                          ▼
                                              POLITYKA EFEKTYWNA
                                     (ten sam panel `.dn-karta` w typografii `--dn-ff-mono`, wynik po
                                      uwzględnieniu dziedziczenia)
```

Konsekwencja projektowa: gdy ekspert pełni rolę w MultitaskingAI i tej roli przypisano własny profil izolacji (poziom zasięgu „Rola”, wyższe pierwszeństwo od poziomów szerszych, ustępujące jedynie poziomowi okna komunikacji — Koncepcja platformy rozdz. 6.5), profil roli nadpisuje ustawienia z Permissions Center tego eksperta **wyłącznie w kontekście tej roli** — poza nią ekspert zachowuje ustawienia zapisane we własnym Permissions Center (Specyfikacja agentów, rozdz. 8.5).
...
### 9.8 Przebieg rozstrzygania kolizji między poziomami zasięgu

Zakres tego samego eksperta bywa ustawiony na więcej niż jednym poziomie zasięgu naraz.
Gdy dwa poziomy nakładają na ten sam zakres sprzeczne wartości, o wyniku decyduje
**pierwszeństwo poziomu najbardziej szczegółowego** — kolejność ośmiu poziomów rosnącego
pierwszeństwa: `global`, `environment`, `module`, `modulePair`, `project`, `session`,
`role`, `window` (Izolacja i zależności, rozdz. 6; Model danych, rozdz. 15.5). Poziom
o wyższym pierwszeństwie wygrywa; brak ustawienia na danym poziomie oznacza przejście do
poziomu szerszego, aż do warstwy bazowej `global` (zasada zero blokad, rozdz. 9.6).

Przykład kolizji dwóch poziomów — zakres techniczny „dostęp sieciowy” eksperta w roli
wykonawczej MultitaskingAI:

```
Kolizja dwóch poziomów zasięgu na jednym zakresie „dostęp sieciowy”

  Poziom KARTA SESJI (session, 6.)         Poziom ROLA (role, 7.)
  dostęp sieciowy = WYŁĄCZONY              dostęp sieciowy = WŁĄCZONY
          │                                        │
          └───────────────┬────────────────────────┘
                          ▼
        rozstrzyganie: pierwszeństwo poziomu szczegółowszego
        role (7.) ma wyższe pierwszeństwo niż session (6.)
                          │
                          ▼
        WYNIK w kontekście tej roli: dostęp sieciowy WŁĄCZONY
        (isolation.policy.preview → policy)
```

Przebieg pracy:

1. Ekspert ma zakres „dostęp sieciowy” zapisany na poziomie karty sesji jako wyłączony
   (`isolation.technical.set` z `scope` = `session`) oraz na poziomie roli jako włączony
   (`isolation.technical.set` z `scope` = `role`).
2. Przy uruchomieniu eksperta w kontekście tej roli platforma rozstrzyga wartość zakresu
   komendą `isolation.policy.preview` (`scope`, `scopeId`, `layer`), która zwraca `policy`
   obowiązującą po rozstrzygnięciu ośmiu poziomów, bez zapisu.
3. Ponieważ poziom `role` (7.) ma wyższe pierwszeństwo niż `session` (6.), wartość z roli
   wygrywa — w kontekście tej roli dostęp sieciowy jest włączony. Poza tą rolą, gdzie
   poziom `role` nie jest ustawiony, wynik dziedziczy z poziomu karty sesji (wyłączony).
4. Panel „Polityka efektywna dla tego eksperta” (rozdz. 9.4) prezentuje wynik rozstrzygnięcia
   z zaznaczeniem, z którego poziomu pochodzi wartość każdego zakresu.

Przypadki brzegowe:

- **Kolizja na poziomie okna komunikacji.** Ustawienie zakresu na poziomie `window`
  (8., najwyższe pierwszeństwo) wygrywa z każdym innym poziomem — dwa okna jednej karty
  sesji mogą mieć różne wartości tego samego zakresu (rozdz. 9.5; przykład: Execution Loop
  Window wobec Chat Window).
- **Brak ustawienia na obu poziomach kolizji.** Gdy żaden z dwóch poziomów nie ma wartości,
  rozstrzyganie schodzi do poziomu szerszego, aż do `global` — nie powstaje blokada, lecz
  wartość odziedziczona (stan wyjściowy: pełny dostęp operacyjny, rozdz. 9.6).
- **Profil roli nadpisujący Permissions Center.** Profil izolacji przypisany roli
  (poziom `role`) nadpisuje wartość wyjściową z Permissions Center eksperta wyłącznie
  w kontekście tej roli; poza nią obowiązuje wartość z Permissions Center (rozdz. 9.5).

### 9.9 Zawężenie zakresu w trakcie trwającego zadania

Operator może zawęzić zakres możliwości eksperta, gdy zadanie tego eksperta już trwa.
Permissions Center jest **konfiguracją zakresu, nie mechanizmem blokującym** (rozdz. 9.1)
— zawężenie zapisane w trakcie zadania nie przerywa go ani nie ubija. Zmiana wchodzi
w rozstrzyganie przy następnym punkcie oceny zakresu, zgodnie z zasadą zero blokad.

```
Oś czasu — zawężenie zakresu podczas trwającego zadania

  zadanie w toku ──────────────────────────────────────────────►  zadanie kończy się
  (Execution Loop Window: kolejka zadań, przebieg pętli)
          │                          │
          │        Operator zawęża zakres w trakcie:
          │        agent.permission.set (granted = false, scope)
          │                          │
          ▼                          ▼
  bieżąca akcja Wykonawcy       zapisana nowa wartość zakresu
  biegnie do końca              (bez przerwania, bez okna blokady)
                                     │
                                     ▼
                          następny punkt oceny zakresu:
                          isolation.policy.preview → policy
                                     │
                          brak nadania na tym poziomie →
                          dziedziczenie z poziomu szerszego,
                          aż do global (zero blokad)
```

Przebieg pracy:

1. Ekspert realizuje zadanie; Execution Loop Window prowadzi kolejkę i przebieg pętli.
2. Operator zawęża zakres komendą `agent.permission.set` (`agentId`, `group`,
   `granted` = false, `scope`) — na przykład odbiera dostęp do jednego modułu.
3. Bieżąca akcja Wykonawcy biegnie do końca — Permissions Center nie jest bramką, więc
   zapis zawężenia nie wstrzymuje ani nie ubija zadania w toku (rozdz. 9.1, 9.6).
4. Zawężona wartość wchodzi w rozstrzyganie przy następnym punkcie oceny zakresu (kolejna
   akcja Wykonawcy albo kolejne zadanie kolejki), obliczanym przez `isolation.policy.preview`.
5. Panel „Polityka efektywna dla tego eksperta” aktualizuje się na żywo (rozdz. 9.4);
   plakietka „zawężone” pojawia się przy nazwie grupy w selektorze (rozdz. 9.7).

Przypadki brzegowe:

- **Następna akcja potrzebuje odebranej możliwości.** Odebranie nadania na jednym poziomie
  nie tworzy twardej blokady — rozstrzyganie schodzi do poziomu szerszego (zasada zero
  blokad). Dopiero gdy żaden szerszy poziom nie nadaje tej możliwości, akcja nie zostaje
  wykonana, a Wykonawca melduje to w Execution Loop Window jako wynik zadania, nie jako
  awarię platformy.
- **Cofnięcie zawężenia w trakcie.** Operator cofa zawężenie komendą `agent.permission.set`
  (`granted` = true) lub przyciskiem „Resetuj do pełnego dostępu” (rozdz. 9.4); wartość
  wraca przy następnym punkcie oceny, zadanie w toku pozostaje nieprzerwane.
- **Zawężenie na poziomie roli podczas zadania roli.** Zawężenie zapisane na poziomie
  `role` obowiązuje wyłącznie kontekst tej roli i wchodzi przy następnym zadaniu tej roli;
  zadania tego eksperta w innych rolach i oknach pozostają bez zmian (rozdz. 9.5).
...
| Rozszerzenia — wtyczki, konektory, serwery MCP | Connectors Manager (rozdz. 8) |
| Pamięć | Agent Builder → zakładka Tożsamość (rozdz. 5.3) |
| Zakres możliwości, w tym Subagent Network | Permissions Center (rozdz. 9) |

W środowisku MultitaskingAI role Executor 1, Executor 2 i Executor 3 / Validator są rolami Wykonawców, a rola Coordinator jest miejscem, w którym Koordynator platformy działa na definicji wskazanej w sekcji Role (rozdz. 10.3; [MultitaskingAI](../srodowiska/multitaskingai.md), rozdz. 3).
...
|---|---|---|
| `.dn-btn` | `--sygnal`, `--zarys`, `--duch`, `--niebezpieczny`, `--sm` | CTA finalizujące (Zapisz), akcje drugorzędne (Anuluj, Testuj połączenie, Resetuj), akcja niebezpieczna (Usuń eksperta) |
| `.dn-btn-ikona` | — | Menu akcji karty eksperta, przyciski w wierszach rejestrów |
| `.dn-pole` / `.dn-pole-etykieta` / `.dn-pole-kontrolka` | `.dn-pole-blad`, `.dn-pole-opis` | Wszystkie pola formularza pięciu zakładek edytora |
| `.dn-przelacznik` | — | Przypisanie umiejętności/rozszerzeń, macierz izolacji technicznej, Subagent Network |
...
| `.dn-karta` | `--klikalna`, `--wybrana` | Karty ekspertów w Bibliotece, karty modeli w Model Configuration, wiersze rejestrów oraz panel „Polityka efektywna dla tego eksperta” (typografia `--dn-ff-mono`) |
| `.dn-plakietka` | `--rola`, `--sukces`, `--ostrzezenie`, `--blad`, `--informacja`, `--sygnal` | Stan eksperta, wersja, widoczność, rodzaj rozszerzenia |
...
| `.dn-toast` | `--sukces`, `--informacja`, `--blad` | Potwierdzenie zapisu, błąd zapisu, błąd instalacji rozszerzenia, baner Permissions Center, komunikaty błędów kanału i adresu |
| `.dn-tooltip` | — | Wszystkie objaśnienia kontekstowe `[?]` |
| `.dn-pusty-stan` | — | Pusta Biblioteka, pusty rejestr Skills Manager i Connectors Manager |
...
| `link-zewnetrzny` | Ikona okna Connectors Manager; wiersze wtyczek, konektorów i serwerów MCP |
...
| `kosz` | Usunięcie eksperta (wariant `.dn-btn--niebezpieczny`) |
...
|---|---|---|---|
| **Okna modułu** | Zakładka otwierana przy wejściu do edytora | Tożsamość, Model, Umiejętności, Konektory, Uprawnienia | Tożsamość |
| | Widok wejściowy okna Agent Builder | Biblioteka ekspertów albo ostatnio edytowany ekspert | Biblioteka ekspertów |
| | Otwarcie Execution Loop Window | Okno rozwinięte przy wejściu do modułu albo zwinięte do znacznika | Rozwinięte |
| | Zachowanie okna centralnego przy dwóch oknach komunikacji | Podział na dwa pola o równej szerokości albo przełącznik widoku z jednym polem widocznym | Podział na dwa pola o równej szerokości |
...
| **Potwierdzenia i cofanie** | Potwierdzenia akcji nieodwracalnych | Okno nakładkowe przed usunięciem eksperta albo wersji definicji | Wyłączone (akcja od razu + „Cofnij”) |

---

## 14. Komendy kontraktu obszaru `agent`
...
### 14.1 Obszar `agent` — 30 komend

| Komenda | Przeznaczenie | Pola żądania | Pola wyniku |
|---|---|---|---|
| `agent.archive` | Odkłada eksperta poza wykaz czynnych bez utraty definicji i historii | `agentId:string` (wym) | `agentId:string` (wym)<br>`archived:bool` (wym)<br>`agent:Agent` (opc) |
| `agent.archive.list` | Zwraca ekspertów odłożonych do archiwum | `offset:int` (opc)<br>`limit:int` (opc) | `agents:Agent[]` (wym)<br>`total:int` (wym) |
| `agent.assignment.list` | Zwraca przypisania eksperta: projekty i role, w których jest wykorzystywany. `workspace.agent.assign` zapisuje przynależność, ale żadna komenda nie odczytuje jej od strony eksperta, więc biblioteka nie ma skąd wziąć liczby | `agentId:string` (opc)<br>`kind:AgentAssignmentKind` (opc) | `assignments:AgentAssignment[]` (wym)<br>`total:int` (wym) |
| `agent.connector.add` | Podłącza ekspertowi konektor, wtyczkę albo serwer MCP | `agentId:string` (wym)<br>`name:string` (wym)<br>`kind:AgentConnectorKind` (wym)<br>`accessPointId:string` (opc)<br>`config:json` (opc) | `connector:AgentConnector` (wym) |
| `agent.connector.configure` | Zapisuje konfigurację instancji konektora eksperta: adres serwera i parametry połączenia właściwe temu egzemplarzowi rozszerzenia | `agentId:string` (wym)<br>`connectorId:string` (wym)<br>`accessPointId:string` (opc)<br>`config:json` (opc)<br>`enabled:bool` (opc) | `connector:AgentConnector` (wym) |
| `agent.connector.list` | Oddaje konektory eksperta wraz z ich definicją; `Agent.connectorIds` niesie same identyfikatory, więc bez tej komendy wykaz nie zna ani nazwy, ani rodzaju, ani punktu dostępu. Odpowiednik `agent.plugin.list` dla drugiego z dwóch bytów | `agentId:string` (wym) | `connectors:AgentConnector[]` (wym) |
| `agent.connector.remove` | Odłącza konektor od eksperta; zakres wykorzystania zapisany w uprawnieniach zostaje na wypadek ponownego podłączenia tego samego rozszerzenia | `agentId:string` (wym)<br>`connectorId:string` (wym) | `agent:Agent` (wym) |
| `agent.create` | Zakłada eksperta jako komponent własny | `name:string` (wym)<br>`description:string` (opc)<br>`systemPrompt:string` (opc)<br>`channelId:string` (opc)<br>`model:string` (opc)<br>`displayName:string` (opc)<br>`favicon:string` (opc)<br>`mode:IdentityMode` (opc)<br>`memoryLevels:MemoryLevel[]` (opc)<br>`visibility:AgentVisibility` (opc) | `agent:Agent` (wym) |
| `agent.delete` | Usuwa eksperta z biblioteki | `agentId:string` (wym) | `agentId:string` (wym)<br>`deleted:bool` (wym) |
| `agent.isolation.get` | Odczytuje osiem zakresów izolacji technicznej zapisanych przy ekspercie jako jego wartość wyjściowa | `agentId:string` (wym) | `agentId:string` (wym)<br>`switches:IsolationTechnicalSwitch[]` (wym) |
| `agent.isolation.set` | Zapisuje osiem zakresów izolacji technicznej jako wartość wyjściową właściwą temu ekspertowi. Wartość obowiązuje wszędzie, gdzie ekspert działa, dopóki nie nadpisze jej reguła zapisana na poziomie zasięgu bardziej szczegółowym | `agentId:string` (wym)<br>`switches:IsolationTechnicalSwitch[]` (wym) | `switches:IsolationTechnicalSwitch[]` (wym) |
| `agent.layer.remove` | Usuwa treść warstwy promptu eksperta; brak warstwy znaczy prompt bez niej, nie błąd | `agentId:string` (wym)<br>`layer:IdentityLayer` (wym) | `agent:Agent` (wym) |
| `agent.layer.set` | Zapisuje treść jednej warstwy instrukcji kontraktowych eksperta. Warstwa dopisuje się do promptu systemowego, nie zastępuje go — trybu podania się tu nie wybiera | `agentId:string` (wym)<br>`layer:IdentityLayer` (wym)<br>`content:string` (wym)<br>`enabled:bool` (opc) | `agent:Agent` (wym) |
| `agent.list` | Zwraca ekspertów biblioteki | `query:string` (opc)<br>`enabledOnly:bool` (opc)<br>`limit:int` (opc)<br>`projectId:string` (opc) | `agents:Agent[]` (wym)<br>`total:int` (opc) |
| `agent.model.set` | Ustala model bazowy eksperta i parametry jego wywołania | `agentId:string` (wym)<br>`channelId:string` (wym)<br>`model:string` (opc)<br>`transport:ProviderTransport` (opc)<br>`parameters:json` (opc)<br>`effort:ReasoningEffort` (opc) | `agent:Agent` (wym) |
| `agent.modules.set` | Zapisuje moduły zastosowania eksperta — te, w których pojawia się jako wykonawca doraźny. Lista pusta `[]` znaczy brak ograniczenia, czyli dostępność wszędzie, i jest to stan wyjściowy; pominięcie pola znaczy „bez zmiany”. Ta sama zasada co przy `memoryLevels`, gdzie zbiór pusty też niesie znaczenie własne | `agentId:string` (wym)<br>`moduleCodes:string[]` (wym) | `agent:Agent` (wym) |
| `agent.permission.remove` | Zdejmuje wpis uprawnienia eksperta, przywracając stan „brak ustawienia = wartość domyślna”. `agent.permission.set` wyłącznie ustawia wartość wpisu, więc po pierwszym zawężeniu wiersz zakresu zostaje w wykazie na zawsze, choć jego wartość wróciła do przyznanej | `agentId:string` (wym)<br>`group:AgentPermissionGroup` (opc)<br>`scope:string` (opc) | `permissions:AgentPermission[]` (wym)<br>`removed:int` (wym) |
| `agent.permission.set` | Przyznaje albo odbiera ekspertowi uprawnienie w jednej z czterech grup zakresu | `agentId:string` (wym)<br>`group:AgentPermissionGroup` (wym)<br>`granted:bool` (wym)<br>`scope:string` (opc) | `permissions:AgentPermission[]` (wym) |
| `agent.plugin.add` | Przypisuje ekspertowi wtyczkę; wtyczka jest bytem odrębnym od konektora | `agentId:string` (wym)<br>`name:string` (wym)<br>`source:string` (opc)<br>`version:string` (opc) | `plugin:AgentPlugin` (wym) |
| `agent.plugin.list` | Oddaje wtyczki eksperta wraz z ich definicją; `Agent.pluginIds` niesie same identyfikatory, więc bez tej komendy wykaz nie zna ani nazwy, ani wersji | `agentId:string` (wym) | `plugins:AgentPlugin[]` (wym) |
| `agent.plugin.remove` | Odłącza wtyczkę od eksperta | `agentId:string` (wym)<br>`pluginId:string` (wym) | `agent:Agent` (wym) |
| `agent.policy.get` | Zwraca politykę efektywną eksperta: wynikowy zestaw ustawień po uwzględnieniu czterech grup zakresu. Przejrzystość, nie bramka — komenda niczego nie zapisuje i niczego nie rozstrzyga | `agentId:string` (wym)<br>`windowId:string` (opc) | `policy:AgentPolicy` (wym) |
| `agent.restore` | Przywraca eksperta z archiwum wraz z zapamiętanym stanem czynności | `agentId:string` (wym) | `agentId:string` (wym)<br>`restored:bool` (wym)<br>`agent:Agent` (opc) |
| `agent.skill.add` | Przypisuje ekspertowi umiejętność | `agentId:string` (wym)<br>`skillId:string` (wym) | `agent:Agent` (wym) |
| `agent.skill.remove` | Zdejmuje umiejętność z definicji eksperta; brak przypisania znaczy definicję bez niej, nie błąd | `agentId:string` (wym)<br>`skillId:string` (wym) | `agent:Agent` (wym) |
| `agent.subagent.set` | Zapisuje dostępność Subagent Network dla eksperta oraz górną liczbę jednoczesnych podagentów. Granica piętnastu jest parametrem technicznym platformy, nie decyzją produktową — ta sama, którą zna `subagent.spawn` | `agentId:string` (wym)<br>`enabled:bool` (wym)<br>`limit:int` (opc) | `agent:Agent` (wym) |
| `agent.update` | Zmienia tożsamość eksperta i zakłada kolejną wersję | `agentId:string` (wym)<br>`name:string` (opc)<br>`description:string` (opc)<br>`systemPrompt:string` (opc)<br>`enabled:bool` (opc)<br>`displayName:string` (opc)<br>`favicon:string` (opc)<br>`mode:IdentityMode` (opc)<br>`memoryLevels:MemoryLevel[]` (opc)<br>`visibility:AgentVisibility` (opc) | `agent:Agent` (wym) |
| `agent.version.get` | Oddaje tożsamość utrwaloną w jednej wersji eksperta. `AgentVersion` niesie etykietę, opis zmiany, sprawcę i sumę kontrolną, ale nie niesie treści — bez tej komendy podglądu wersji nie ma z czego złożyć, a przywrócenie jest jedynym sposobem zobaczenia, co w wersji stało | `agentId:string` (wym)<br>`versionId:string` (wym) | `version:AgentVersion` (wym)<br>`snapshot:AgentVersionSnapshot` (wym) |
| `agent.version.list` | Zwraca historię wersji tożsamości eksperta | `agentId:string` (wym)<br>`limit:int` (opc) | `agentId:string` (wym)<br>`versions:AgentVersion[]` (wym)<br>`total:int` (wym) |
| `agent.version.restore` | Przywraca wcześniejszą wersję eksperta jako wersję kolejną; historia późniejsza zostaje | `agentId:string` (wym)<br>`versionId:string` (wym) | `agent:Agent` (wym)<br>`version:AgentVersion` (wym) |

**Zdarzenia obszaru `agent` — 1:**

| Zdarzenie | Przeznaczenie | Pola ładunku |
|---|---|---|
| `agent.changed` | Zmiana eksperta zapisanego jako komponent własny | `change:ChangeKind` (wym)<br>`agent:Agent` (wym) |

Razem w wykazie: **30 komend** z 1 obszaru kontraktu.
...
|---|---|
| Siedem okien operacyjnych (Chat Window, Execution Loop Window, Agent Builder, Model Configuration, Skills Manager, Connectors Manager, Permissions Center) otwiera się z punktów wejścia rozdz. 1.3 | Otwarcie modułu ze strefy 2 strony głównej i z bocznej nawigacji TalkIn, WorkSpace, CodeStudio |
| Żaden przycisk akcji modułu nie występuje w stanie zablokowanym | Przegląd wszystkich kontrolek CTA katalogów elementów rozdz. 5–9 pod kątem obecności `disabled`/`not-allowed` |
| Zapis eksperta z nazwą pustą kończy się sukcesem, nie odmową | Kliknięcie „Zapisz eksperta” z pustym polem nazwy w widoku Edytor |
| Wszystkie 30 komend obszaru `agent` mają pokrycie w interfejsie modułu albo jawne wskazanie miejsca wywołania | Zestawienie rozdz. 14 z katalogami elementów rozdz. 5–9 |
| Permissions Center nie blokuje żadnej akcji — wyłącznie konfiguruje zakres | Test: wykonanie akcji przy zawężonym zakresie kończy się komunikatem, nie odmową wykonania |
| Definicja eksperta zapisana w module Agents zawiera siedem komponentów z rozdz. 3 | Porównanie pól formularza Agent Builder z listą pól komendy `agent.create` (rozdz. 14) |
| Stany kontrolek z rozdz. 4.2 są zaimplementowane dla każdego elementu interaktywnego katalogów rozdz. 5–9 | Przegląd katalogów elementów pod kątem kompletu stanów: domyślny, wskazanie kursorem, fokus, aktywny, ładowanie, ostrzeżenie, błąd |
| Historia wersji eksperta jest dostępna i przywracalna | Wykonanie `agent.version.list`, a następnie `agent.version.restore` na wersji wcześniejszej niż bieżąca |

---

## 16. Załącznik — szablon definicji eksperta jako formularz interfejsu
...
EKSPERT                                                          okno / zakładka
  nazwa:                 [ .dn-pole-kontrolka, nagłówek edytora ]         Agent Builder
  przeznaczenie:         [ .dn-pole-kontrolka małe ]                   Agent Builder
                                                                  → zakładka Tożsamość

  model_bazowy:          [ siatka kart .dn-karta--klikalna ] Model Configuration
  kanał_modelu:          [ .dn-zakladki--pigulki: API|CLI|SSH|HTTP ]
                                                                  Model Configuration

  instrukcje_systemowe:  [ .dn-pole-kontrolka duże ]                   Agent Builder
                                                                  → zakładka Tożsamość

  umiejętności:          [ .dn-tabela / .dn-listwa-pozycja +      Skills Manager
                            .dn-przelacznik per wiersz ]
  rozszerzenia:          [ .dn-tabela / .dn-listwa-pozycja +      Connectors Manager
                            .dn-przelacznik per wiersz; plakietka
                            rodzaju: Wtyczka|Konektor|Serwer MCP ]

  pamięć:                [ .dn-przelacznik ×5: globalna|projekt|sesja|  Agent Builder
  ...
  zakres_możliwości:                                              Permissions Center
    ...
    izolacja_techniczna: [ .dn-przelacznik ×8 — identyczne z oknem
                            konfiguracji punktów izolacji ]
    multitaskingai:
      subagent_network:  [ .dn-przelacznik (dostępność) +
                            .dn-suwak liczbowy (1–15) ]
...
*Koniec dokumentu. Moduł Agents — Specyfikacja docelowa, wersja 2.0, 2026-08-20.*
