...
| | |
|---|---|
| **Tytuł** | Moduł Assistant — pełnozakresowa dokumentacja projektowa |
| **Przeznaczenie dokumentu** | Źródło wykonawcze dla Designera (co, gdzie, w jakiej formie) i Dewelopera (co zbudować) — pełny zakres funkcji, narzędzi, okien operacyjnych i punktów sterowania modułu |
| **Środowisko wykorzystania operacyjnego** | TalkIn |
...
| **Stos techniczny odniesienia** | Rdzeń serwera: Go (orkiestracja, procesy sesji, pętla narzędziowa, harmonogram). Warstwa głosu: STT/TTS/fraza wybudzająca (biblioteki nazwane w rozdz. 7.B). Pamięć semantyczna: embeddingi + magazyn wektorowy. Wywołania narzędzi: JSON Schema + Model Context Protocol (MCP) |
| **Źródła** | Koncepcja platformy (rozdz. 2.3, 4, 5.2, 9.10, 10.2, 12, Załącznik A) · Specyfikacja modułów (rozdz. 4.10) · Specyfikacja okien operacyjnych (rozdz. 5, 6.10, 10) · System wizualny (rozdz. 6, 8, 10) · Model danych (rozdz. 6.3, 8, 17, 18.3) · Izolacja i zależności (rozdz. 1–6) |
...
|---|---|
| Rozmowa ogólna | Konwersacja wieloturowa głosem i tekstem, streaming, wątkowanie i rozgałęzianie, persony, załączniki, tryby odpowiedzi |
| Głos | Rozpoznawanie mowy (STT), synteza mowy (TTS), fraza wybudzająca, detekcja mowy (VAD), redukcja szumu, wielojęzyczność, dyktowanie |
| Pamięć i konteksty | Pamięć krótko- i długoterminowa, pamięć semantyczna (wektorowa), fakty o użytkowniku, konteksty przełączane, wstrzykiwanie wiedzy profilu |
| Szybkie akcje | Wywoływacz poleceń (launcher), paleta poleceń, makra głosowe i tekstowe, rozwijanie skrótów tekstowych, menedżer schowka, przechwytywanie notatek |
| Wywołania narzędzi | Katalog narzędzi (function calling), Model Context Protocol (MCP), umiejętności (skills), sterowanie modułami platformy i aplikacjami zewnętrznymi |
...
|---|---|
| Budowa i długotrwałe wykonywanie bezobsługowych procesów cyklicznych | Moduł **Automations** (Assistant inicjuje i nadzoruje pojedyncze zlecenia, nie projektuje pełnych pipeline'ów) |
| Definiowanie i zarządzanie autonomicznymi agentami wielozadaniowymi | Moduł **Agents** (Profil asystenta korzysta z uprawnień analogicznych do Centrum uprawnień, lecz nie jest fabryką agentów) |
...
| Lista zadań pętli | Lista pozycji z ikoną stanu | Prezentacja dekompozycji zlecenia | Średnia lista `.dn-karta--pozycja` | oczekujące · w realizacji · zakończone · wymagające uwagi | Kliknięcie pozycji rozwija komunikaty i wynik zadania | Sekcja Koordynator → Wykonawca | 1 | Widoczna bez interakcji po otwarciu okna pętli |
...
| Wynik kontroli jakości | Plakietka przy zadaniu | Informacja o ocenie wyniku zadania | Mała plakietka `.dn-plakietka--stan` | zgodna · niezgodna (ponowienie) | Kliknięcie pokazuje kryteria i uzasadnienie decyzji o ponowieniu | Przy pozycji zadania | 2 | Plakietka stanu przy zadaniu; kliknięcie rozwija kryteria i uzasadnienie ponowienia |
...
- Detekcja mowy (VAD) kończąca nagranie po ciszy.
...
| Wskaźnik pewności rozpoznania | Mała plakietka przy transkrypcji | Sygnalizacja niepewności rozpoznanej frazy | Mała plakietka `.dn-plakietka--ostrz` | ukryta (wysoka pewność) · widoczna (niska pewność) | Kliknięcie pozwala poprawić tekst ręcznie przed wysłaniem | Obok transkrypcji na żywo | 2 | Ujawnia się samoczynnie wyłącznie przy niskiej pewności rozpoznania |
...
| Pozycja zlecenia | Karta `.dn-karta--pozycja` z paskiem postępu | Reprezentuje jedno zlecenie wieloetapowe | Średnia karta z kropką stanu `.dn-plakietka--stan` | w toku · zakończone (✓) · nieudane (⚠) · wstrzymane | Rozwija listę kroków i dostępne akcje sterujące | Lista główna panelu | 1 | Widoczna bez interakcji po otwarciu panelu |
...
| Alert nieudanego kroku | Blok `.dn-alert--blad` | Informacja o przyczynie niepowodzenia | Mały blok z kreską lewą czerwoną | ukryty · widoczny | Przycisk „Ponów” uruchamia krok ponownie; „Szczegóły” pokazuje pełny log | Karta zlecenia nieudanego | 2 | Ujawnia się samoczynnie wyłącznie przy kroku nieudanym |
...
| Element | Co to jest | Do czego służy | Forma i waga | Stany | Zachowanie po interakcji | Występowanie | Warstwa | Sposób wywołania |
|---|---|---|---|---|---|---|---|---|
| Pole wyszukiwania | Pole `.dn-input` z ikoną (szukaj) | Odnalezienie wcześniejszego działania w historii | Małe pole, nagłówek panelu | puste · z wynikami | Filtruje listę na żywo, pełnotekstowo i semantycznie | Nagłówek panelu | 1 | Widoczne bez interakcji w nagłówku panelu |
...
| Pozycja działania | Wiersz z godziną, treścią polecenia i wynikiem | Reprezentuje jedno zrealizowane zlecenie | Mały wiersz `.dn-karta--pozycja` | domyślny · ważne (przypięte, wyróżnienie złote) · nieudane (oznaczenie ostrzegawcze) | Kliknięcie rozwija szczegóły i dostępne akcje | Lista główna panelu | 1 | Widoczna bez interakcji na liście głównej |
...
| Wpis pamięci | Wiersz z treścią faktu i polityką retencji | Reprezentuje jeden zapamiętany fakt | Mały wiersz `.dn-karta--pozycja` | trwały · z TTL · wrażliwy (oznaczenie) | Edycja zmienia treść, usunięcie kasuje fakt trwale, obie operacje trafiają do audytu | Lista faktów | 1 | Widoczny bez interakcji na liście faktów |
| Wyszukiwarka semantyczna | Pole `.dn-input` z ikoną (szukaj) | Odnajdywanie zapamiętanych treści po znaczeniu | Małe pole | puste · z wynikami | Zwraca listę fragmentów rozmów i dokumentów wraz ze wskaźnikiem podobieństwa | Zakładka pamięci semantycznej | 1 | Widoczna bez interakcji w zakładce pamięci semantycznej |
...
- Wywoływacz poleceń (launcher) i paleta poleceń (`Ctrl/Cmd + K`) wraz z konfiguracją siatki.
...
- Katalog umiejętności (skills) profilu — nazwanych, wielokrokowych zdolności skomponowanych z narzędzi i instrukcji.
...
| Pozycja narzędzia lub serwera MCP | Wiersz z nazwą, transportem i polityką potwierdzeń | Zarządzanie dostępnością narzędzia dla profilu | Średni wiersz `.dn-karta--pozycja` | włączone · wyłączone · wymagające potwierdzenia | Przełącznik zmienia dostępność, „Zakres” otwiera granice uprawnień i limity | Zakładka narzędzi i MCP | 3 | Zakładka narzędzi i MCP; element zbiorczy `[ Zakres ▼ ]` rozwija granice uprawnień i limity (warstwa 4) |
...
| Uprawnienia profilu do sterowania innymi modułami | Zgodnie z zakresem zdefiniowanym przy tworzeniu profilu (analogicznie do Centrum uprawnień modułu Agents) | Rozszerzenie lub zawężenie zakresu modułów, narzędzi i serwerów MCP dostępnych profilowi | Konfiguracja Profilu asystenta, strefa 2 strony głównej |
...
| **Assistant → Terminal** | Uruchomienie kodu lub zapytania obliczeniowego na potrzeby odpowiedzi | jednokierunkowe | Sandbox procesu sesji |
...
### 7.A. Rozmowa i konwersacja (rdzeń)
...
| **Streaming odpowiedzi z równoległą syntezą** | Odpowiedź pojawia się na żywo jako tekst i jednocześnie jest odczytywana głosem, z możliwością przerwania | Strumień tokenów; TTS (7.B); synchronizacja tekst↔mowa |
...
### 7.B. Głos — rozpoznawanie, synteza, wybudzanie
...
### 7.C. Dyktowanie i produktywność mowy

| Funkcja | Co robi | Zależności |
|---|---|---|
| **Dyktowanie do dowolnego pola** | Mowa zamieniana na tekst wstawiany do aktywnego okna lub modułu platformy | STT (7.B); wstawianie do fokusu; komendy formatujące |
...
### 7.E. Szybkie akcje, wywoływacz i schowek

| Funkcja | Co robi | Zależności |
|---|---|---|
| **Wywoływacz poleceń (launcher)** | Globalny pasek i skrót otwierający asystenta z dowolnego miejsca — polecenie wpisane lub wypowiedziane i wykonane | Skrót globalny klienta; paleta poleceń `.dn-paleta` (`Ctrl/Cmd + K`); indeks akcji |
...
| **Kalkulacje i konwersje w locie** | Obliczenia, przeliczniki jednostek i walut oraz działania na datach bez opuszczania paska poleceń | Ewaluator wyrażeń; kursy przez narzędzie (7.F) |
...
### 7.F. Wywołania narzędzi i integracje (function calling / MCP)
...
| **Umiejętności (skills) profilu** | Nazwane, wielokrokowe zdolności skomponowane z narzędzi i instrukcji, włączane per profil | Definicja skill (`JSON`/`YAML`); zakres uprawnień profilu |
...
| **Wykonawca kodu i narzędzia obliczeniowe** | Uruchomienie fragmentu kodu lub zapytania do danych na potrzeby jednej odpowiedzi | Integracja z modułem **Terminal**; sandbox procesu sesji |
...
### 7.G. Sterowanie zadaniami wieloetapowymi
...
### 7.H. Proaktywność, przypomnienia i rutyny
...
| **Brief dzienny** | Zestawienie kalendarza, zadań i wiadomości na start dnia, głosem lub tekstem | Konektory (7.F); szablon briefu; harmonogram |
...
### 7.I. Personalizacja profili asystenta
...
| **Zakres uprawnień profilu** | Które moduły i narzędzia profil wywołuje — zakres rozszerzalny i zawężalny, jak Centrum uprawnień modułu Agents | Zakres uprawnień; macierz izolacji; klucze jawne |
...
### 7.J. Wielojęzyczność, dostępność i tryby
...
### 7.K. Prywatność, bezpieczeństwo i higiena
...
### 7.L. Zależności techniczne — zestawienie
...
| **Zakres uprawnień profilu** | Które moduły i aplikacje profil steruje głosowo — rozszerzenie i zawężenie | Uprawnienia (jak Centrum uprawnień Agents) |
...
*Koniec dokumentu. Moduł Assistant — dokumentacja projektowa, wersja 2.0, 2026-08-06.*
