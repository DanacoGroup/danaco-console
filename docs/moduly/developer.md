...
| | |
|---|---|
| **Tytuł** | Moduł Developer — pełnozakresowa dokumentacja projektowa |
| **Przeznaczenie dokumentu** | Źródło wykonawcze dla Designera (co, gdzie, w jakiej formie) i Dewelopera (co zbudować); pełny katalog funkcji, narzędzi, okien operacyjnych i punktów sterowania modułu |
...
| **Źródła** | Koncepcja platformy (rozdz. 2, 3, 4, 7.3, 9.12, 12, Załącznik A) · Specyfikacja modułów 1.1 (rozdz. 4.11–4.14) · Specyfikacja okien operacyjnych 1.1 (rozdz. 2.4, 3, 5, 6.12, 10) · System wizualny (rozdz. 8, 10) · Model danych (rozdz. 8, 15, 17) · Model konfiguracji (rozdz. 3–6) · Izolacja i zależności (rozdz. 1–6, Załącznik A) · Rozszerzenia (rozdz. 1, 8) · Integracja modeli · Specyfikacja agentów |
| **Zasada nadrzędna** | Pełna kompozycyjność i pełna konfigurowalność (Koncepcja, rozdz. 14); zero blokad w interfejsie; klucze konfiguracji jawne; izolacja techniczna procesu i zakres uprawnień pozostają w gestii Operatora, nie są wymogiem; domyślne zachowanie modułu = wykonanie |
...
|---|---|
| Deweloperzy backendu i frontendu | Pisanie, refaktoryzacja i przegląd kodu z bezpośrednim wsparciem AI |
...
| Złożenie kompletnego produktu z wielu komponentów (frontend, backend, infrastruktura) | **Apps** — Developer jest jednym z komponentów procesu Product Builder |
...
- Wyjaśnienie komunikatu błędu lub niepowodzenia testu widocznego w Build Output oraz stanu zatrzymania debuggera, z sugestią poprawki.
...
- Cytowanie fragmentów kodu w rozmowie blokiem `.dn-kod`, krojem mono, z zachowaniem podświetlania składni języka.
...
| Blok kodu w odpowiedzi | `.dn-kod`, krój mono, podświetlenie składni | Prezentacja wygenerowanego lub zmodyfikowanego kodu | Średni lub duży blok w treści odpowiedzi | strumieniowanie · kompletny | Przycisk kopiowania w rogu bloku przy najechaniu | Treść odpowiedzi Wykonawcy | 1 | Widoczny w strumieniu odpowiedzi Wykonawcy |
| Przycisk „Wstaw do edytora” | Przycisk `--zarys`, mały | Przeniesienie fragmentu kodu do Code Editor | Mały przycisk tekstowy w pasku akcji bloku | domyślny · najechanie | Otwiera podgląd wstawienia (różnica przed wstawieniem) w miejscu kursora lub zaznaczenia | Pod blokiem kodu odpowiedzi | 2 | Pasek akcji bloku kodu; widoczny wyłącznie przy bloku zawierającym kod |
| Przycisk „Uruchom testy” | Przycisk `--zloty`, mały | Wykonanie wygenerowanego testu w warstwie wykonawczej Terminal | Mały przycisk CTA, akcent złoty | domyślny · ładowanie | Przekazuje polecenie do Terminala (gdy powiązanie skonfigurowane), wynik trafia do Build Output | Pod blokiem kodu odpowiedzi zawierającym test | 2 | Pasek akcji bloku kodu; widoczny wyłącznie przy bloku zawierającym test |
...
- Wyniki kontroli jakości przypisane do zadań: diagnostyka lintera, wynik zestawu testów, pokrycie kodu, wynik przeglądu różnicy, wynik skanu zależności i sekretów; decyzja o ponowieniu zadania podejmowana na podstawie tych wyników.
...
- Wielokrotne karty otwartych plików, z podziałem widoku (split editor) na kolejne kolumny.
...
- Paleta poleceń (Command Palette) — klawiaturowy dostęp do wszystkich funkcji edytora.
...
- Napraw — sugestia poprawki na podstawie zgłoszonego błędu lub ostrzeżenia lintera.
...
| Pole Grep w plikach | Pole wyszukiwania globalnego | Wyszukiwanie wzorca w całym repozytorium, z zamianą masową | Pole `.dn-input` rozwijane w panel wyników | zwinięte · rozwinięte · wyniki listowane | Enter uruchamia wyszukiwanie; kliknięcie wyniku otwiera plik w miejscu trafienia | Pasek narzędzi edytora | 2 | Ikona wyszukiwania w pasku narzędzi albo skrót klawiszowy; pole zwija się po otwarciu wyniku. Zamiana masowa w całym repozytorium należy do warstwy 4 |
| Paleta poleceń | Modal wyszukiwarki poleceń | Klawiaturowy dostęp do wszystkich funkcji edytora | Wąski modal wyśrodkowany, pole `.dn-input` w nagłówku | zamknięta · otwarta (`Ctrl/Cmd + K`) | Wpisanie frazy filtruje listę poleceń; Enter wykonuje wybrane | Nakładka nad edytorem | 4 | Wyszukiwarka funkcji wywoływana skrótem `Ctrl/Cmd + K`; brak reprezentacji wizualnej w stanie spoczynku |
| Pasek statusu edytora | Wiersz informacyjny | Język pliku, kodowanie, znaki końca linii, wcięcia, liczba błędów lintera, gałąź Git | Mały, niski pasek, tekst pomocniczy | brak błędów · błędy/ostrzeżenia (plakietka z liczbą) | Kliknięcie liczby błędów otwiera listę problemów pliku | Stopka kolumny edytora | 1 | Widoczny bez interakcji |
| Wskaźnik lintera w linii | Podkreślenie faliste pod kodem | Sygnalizuje błąd lub ostrzeżenie statycznej analizy | Bardzo mały, wewnątrz linii kodu | brak · ostrzeżenie (żółte) · błąd (czerwone) | Najechanie pokazuje treść komunikatu w `.dn-tooltip`, z podpowiedzią poprawki | W treści kodu | 1 | Widoczny w treści kodu; treść komunikatu w `.dn-tooltip` przy najechaniu |
...
| Pole szybkiego otwarcia | Pole wyszukiwania nazwy pliku | Odnalezienie i otwarcie pliku bez przeglądania drzewa | Pole `.dn-input`, nagłówek kolumny | puste · z wynikami listowanymi poniżej | Strzałki nawigują wyniki, Enter otwiera wybrany plik | Nagłówek kolumny | 2 | Ikona lupy w nagłówku kolumny albo skrót klawiszowy; pole zwija się po otwarciu pliku |
...
| Pole wiadomości commitu | Pole tekstowe jednowierszowe z rozwinięciem | Treść opisu zatwierdzanych zmian | Średnie pole `.dn-input`/`.dn-textarea` | puste · wypełnione · błąd (pusta wysyłka) | Enter lub przycisk zatwierdza commit | Środek kolumny | 1 | Widoczne bez interakcji w obrębie otwartego Git Panel |
...
| Przycisk „Zatwierdź zmiany” | Główny CTA kolumny | Wykonanie commitu przygotowanych zmian | Duży przycisk `--zloty`, pełna szerokość kolumny | aktywny (domyślny) · aktywny z ostrzeżeniem (brak przygotowanych zmian lub pustej wiadomości) · ładowanie | Tworzy commit, czyści sekcję przygotowanych zmian, dodaje wpis do historii; przy braku przygotowanych zmian lub pustej wiadomości przycisk pozostaje aktywny i wyświetla ostrzeżenie inline zamiast blokady | Sekcja commitu | 1 | Widoczny bez interakcji w obrębie otwartego Git Panel |
| Pozycja historii commitów | Wiersz `.dn-karta--pozycja` z kropką i skrótem | Reprezentuje jeden commit w historii | Mały wiersz z awatarem autora, skrótem i czasem względnym | domyślny · najechanie · rozwinięty (szczegóły) | Kliknięcie pokazuje pełną listę zmienionych plików tego commitu | Sekcja historii | 2 | Element zbiorczy `Historia ▼` w stopce kolumny; rozwinięcie prezentuje listę commitów |
| Przycisk „Wyślij wymuszony” | Przycisk `--blad`, mały, w menu zaawansowanym | Nadpisanie zdalnej gałęzi historią lokalną | Mały przycisk tekstowy w rozwijanym menu operacji zdalnych | domyślny · ładowanie | Domyślnie wykonuje wysłanie natychmiast, sygnalizując inline ostrzeżenie o nieodwracalności dla współpracowników; modal potwierdzenia przed wysłaniem włącza ustawienie `developer.git.potwierdz_force_push` | Menu operacji zdalnych repozytorium | 4 | Wyłącznie polecenie języka naturalnego, wyszukiwarka funkcji albo menu operacji zdalnych w trybie administracyjnym; użytkownik podstawowy nie widzi tego elementu |
...
| Plakietka statusu | `.dn-plakietka--stan` z kropką | Bieżący stan procesu budowania | Mała pigułka ze stanem | w toku (żółta, pulsująca) · sukces (zielona) · błąd (czerwona) · przerwane (szara) | — (informacyjna, aktualizowana na żywo) | Nagłówkowa część kolumny | 1 | Widoczna bez interakcji jako wskaźnik stanu wykonania |
...
| Przycisk „Wyjaśnij niepowodzenie” | Mały przycisk tekstowy | Wysłanie komunikatu błędu do Chat Window z prośbą o wyjaśnienie i poprawkę | Mały przycisk `--zarys` | domyślny | Otwiera Chat Window z wypełnionym kontekstem błędu | Pod linią błędu w logu | 2 | Pojawia się pod linią błędu przy najechaniu; znika po opuszczeniu wiersza |
...
| Pasek zapytania | Selektor metody i pole adresu | Definicja zapytania HTTP | Średni rząd: mały selektor i szerokie pole `.dn-input` | domyślny · fokus · wysyłanie | Enter lub „Wyślij” wykonuje zapytanie i wypełnia sekcję odpowiedzi | API Client | 1 | Widoczny bez interakcji w obrębie zakładki API Client |
...
──────────────────────────────────        ────────────────────────────────────────────
 ...
 repozytorium (np. usługą backendową)
        │
...
Karta sesji A · Developer        Karta sesji B · Developer        Karta sesji C · Developer
  ...
  (kontekst odrębny)                (kontekst odrębny)                (kontekst odrębny)
           ...
           współdzielona pamięć projektu obowiązuje po skonfigurowaniu
                        z okna konfiguracji punktów izolacji — np. przy
...
```
        ...
        └───────────│  Commit        │──► Build Output (budowanie uruchamiane
                    ...
                    └────────────────┘     `developer.build.auto_po_commicie`
                                            jest włączone)
```
...
Każda pozycja: nazwa funkcji, opis działania oraz zależności techniczne (biblioteki, formaty, protokoły, integracje). Biblioteki backendu podano w wariancie Go (rdzeń platformy), frontend edytora w wariancie webowym.

### 7.1. Rdzeń edytora kodu (Code Editor)

| Funkcja | Co robi | Zależności |
|---|---|---|
| **Silnik edytora wieloplikowy** | Karty, podział widoku (split) na kolejne kolumny, wielokursorowość, zaznaczanie kolumnowe, zwijanie bloków, minimapa | Monaco Editor lub CodeMirror 6 (frontend); model dokumentu przez WebSocket |
...
| **Serwer języka (LSP)** | Autouzupełnianie sygnatur, podgląd typu przy najechaniu, przejdź do definicji i implementacji, znajdź wystąpienia, diagnostyka na żywo, akcje kodu | Protokół **LSP**; serwery: `gopls`, `typescript-language-server`, `pyright`, `rust-analyzer`, `clangd`, `jdtls`; most LSP↔WebSocket |
...
| **Adnotacja autora (blame) w linii** | Kto i kiedy zmienił linię, przejście do commitu | `go-git` log per linia; powiązanie z Git Panel |
| **Margines zmian (gutter diff)** | Znaczniki dodano/zmieniono/usunięto względem HEAD, cofnięcie zmiany linii | różnica katalog roboczy↔HEAD: `sergi/go-diff` / `hexops/gotextdiff` |
...
| Funkcja | Co robi | Zależności |
|---|---|---|
| **Status i przygotowanie zmian** | Unstaged/staged/untracked, stage i unstage pliku, zaznaczenia lub wszystkiego, częściowe przygotowanie hunków | `go-git` lub `git2go` (libgit2) dla operacji na poziomie hunków |
...
### 7.11. Zestawienie zależności bibliotecznych (backend Go)
...
Wszystkie punkty są jawne, opatrzone objaśnieniem `[?]`, przechowywane jako `ustawienie` z kluczem (rozdz. 5.3 Modelu konfiguracji). Klucze są jawne, wartości podlegają zasadzie „brak ustawienia = wartość domyślna”. Konfiguracja steruje widocznością i wartościami domyślnymi, nie wyłącza działania modułu; niewłączona integracja ukrywa zakładkę zamiast prezentować zablokowany element interfejsu.

### 8.1. Edytor i jakość kodu

| Klucz | Co Operator personalizuje | Wartość domyślna |
|---|---|---|
| `developer.editor.motyw` | Motyw kolorystyczny kodu | z motywu platformy |
| `developer.editor.wciecia` | Spacje lub tabulacje i szerokość wcięcia | 2 spacje / z `.editorconfig` |
| `developer.editor.zawijanie` | Zawijanie wierszy | wyłączone |
| `developer.editor.formatuj_przy_zapisie` | Automatyczne formatowanie przy zapisie | włączone |
| `developer.lint.serwery` | Aktywne lintery per język | z konfiguracji repozytorium |
| `developer.lsp.serwery` | Mapowanie język → serwer LSP | automatyczne wykrycie |
| `developer.editor.snippety` | Zestaw snippetów (zasięg projekt lub globalny) | wbudowane |

### 8.2. Git i wersjonowanie

| Klucz | Co Operator personalizuje | Wartość domyślna |
|---|---|---|
| `developer.git.tozsamosc` | Nazwa i adres e-mail commitującego | z profilu Operatora |
| `developer.git.podpisywanie` | Podpis GPG/SSH commitów | wyłączone |
| `developer.git.potwierdz_usuniecie` | Modal potwierdzenia przed usunięciem pliku | wyłączony (usunięcie od razu z „Cofnij”) |
| `developer.git.potwierdz_force_push` | Modal potwierdzenia przed wysłaniem wymuszonym | wyłączony (ostrzeżenie inline) |
| `developer.git.commit_ai_domyslnie` | Automatyczne generowanie opisu commitu | na żądanie |
...
| Klucz | Co Operator personalizuje | Wartość domyślna |
|---|---|---|
| `developer.build.profil` | Profil domyślny (deweloperski lub produkcyjny) | deweloperski |
| `developer.build.watch` | Tryb obserwacji zmian i przeładowanie | wyłączony |
| `developer.test.prog_pokrycia` | Próg pokrycia prezentowany jako ostrzeżenie | 0% (bez ostrzeżenia) |
| `developer.build.auto_po_commicie` | Automatyczne budowanie po commicie | wyłączone |
| `developer.debug.adaptery` | Mapowanie język → adapter DAP | automatyczne wykrycie |

### 8.4. Integracje deweloperskie — widoczność zakładek Dev Tools

| Klucz | Co Operator personalizuje | Wartość domyślna |
|---|---|---|
| `developer.narzedzia.api_client` | Widoczność klienta API | ukryty |
| `developer.narzedzia.data_console` | Widoczność konsoli bazodanowej i połączeń | ukryta |
| `developer.narzedzia.containers` | Widoczność panelu kontenerów | ukryty |
| `developer.narzedzia.security` | Skany SCA, SAST i sekretów oraz polityka | ukryte |
| `developer.data.polaczenia` | Zdefiniowane połączenia baz danych (poświadczenia przez izolację) | brak |

### 8.5. AI i model

| Klucz | Co Operator personalizuje | Wartość domyślna |
|---|---|---|
| `developer.ai.kanal_modelu` | Kanał modelu dla operacji kontekstowych | domyślny kanał sesji |
| `developer.ai.tryb_agentowy` | Dopuszczenie zadań wielokrokowych prowadzonych w pętli wykonawczej | włączony |
| `developer.ai.auto_kontekst_pliki` | Automatyczne włączanie otwartych plików do kontekstu | włączone |
| `developer.ai.diff_przed_wstawieniem` | Podgląd różnicy przed wstawieniem sugestii | włączony |
...
*Koniec dokumentu. Moduł Developer — dokumentacja projektowa, wersja 2.0, 2026-08-06.*
