# Danaco Console — Moduł Diagnostics
...
| | |
|---|---|
| **Tytuł** | Moduł Diagnostics |
| **Klasa dokumentu** | Specyfikacja docelowa |
| **Odbiorcy** | projektant (co, gdzie, w jakiej formie) · deweloper (co zbudować) |
| **Przeznaczenie** | Ustala interfejs modułu Diagnostics: komplet okien operacyjnych, katalog funkcji i narzędzi, przepływy pracy, stany oraz punkty sterowania modułu dostępnego wyłącznie w CodeStudio |
| **Zakres** | okna operacyjne modułu, katalog funkcji i narzędzi diagnostycznych, katalog elementów interfejsu, komendy kontraktu obszaru `diagnostics` |
| **Poza zakresem** | redakcja danych wrażliwych i macierz izolacji jako mechanizm rdzenia — [Izolacja i zależności](../architektura/izolacja-i-zaleznosci.md) |
| **Dokument nadrzędny** | [Specyfikacja modułów](../specyfikacje/specyfikacja-modulow.md) |
| **Dokumenty powiązane** | [Moduł Developer](developer.md) · [Moduł Terminal](terminal.md) · [Koncepcja platformy](../architektura/koncepcja-platformy.md) · [Specyfikacja okien operacyjnych](../specyfikacje/specyfikacja-okien-operacyjnych.md) · [System wizualny](../interfejs-uzytkownika/system-wizualny.md) · [Model danych](../architektura/model-danych.md) · [Model konfiguracji](../architektura/model-konfiguracji.md) · [Integracja modeli](../architektura/integracja-modeli.md) · [Izolacja i zależności](../architektura/izolacja-i-zaleznosci.md) |
| **Prototypy odniesienia** | `design/05-okna/moduly/diagnostics.html` |
| **Źródła normatywne** | `budowa/shared/contract.json` (obszar `diagnostics`) · `design/zasoby/zetony/zetony.css` · `design/zasoby/css/komponenty.css`, `rama.css`, `prototyp.css` |
...
2. [Katalog funkcji i narzędzi](#2-katalog-funkcji-i-narzędzi)
   - [2.1 Agregacja i przeszukiwanie logów (Logs Viewer)](#21-agregacja-i-przeszukiwanie-logów-logs-viewer)
   - [2.2 Rejestr i grupowanie błędów (Errors Panel)](#22-rejestr-i-grupowanie-błędów-errors-panel)
   - [2.3 Zagregowany obraz stanu i pulpit (Diagnostics Center)](#23-zagregowany-obraz-stanu-i-pulpit-diagnostics-center)
   - [2.4 Prowenancja wywołań AI (Provenance Explorer)](#24-prowenancja-wywołań-ai-provenance-explorer)
   - [2.5 Zużycie kont, kosztów i budżetów (Usage & Cost)](#25-zużycie-kont-kosztów-i-budżetów-usage--cost)
   - [2.6 Metryki, wydajność i profilowanie (Metrics & Performance)](#26-metryki-wydajność-i-profilowanie-metrics--performance)
   - [2.7 Kontrola stanu i dostępność (Health & Uptime)](#27-kontrola-stanu-i-dostępność-health--uptime)
   - [2.8 Alerty i powiadomienia (Alerts)](#28-alerty-i-powiadomienia-alerts)
   - [2.9 Rekomendacje i operacje kontekstowe (Recommendations Panel)](#29-rekomendacje-i-operacje-kontekstowe-recommendations-panel)
   - [2.10 Raporty, migawki i wymiana danych](#210-raporty-migawki-i-wymiana-danych)
   - [2.11 Zestawienie zależności bibliotecznych (warstwa serwerowa, Go)](#211-zestawienie-zależności-bibliotecznych-warstwa-serwerowa-go)
...
4. [Specyfikacja okien operacyjnych](#4-specyfikacja-okien-operacyjnych)
   ...
   - [4.3 Diagnostics Center](#43-diagnostics-center)
   - [4.4 Logs Viewer](#44-logs-viewer)
   - [4.5 Errors Panel](#45-errors-panel)
   - [4.6 Recommendations Panel](#46-recommendations-panel)
   - [4.7 Observability Tools (kontener zakładek, funkcje eksperckie)](#47-observability-tools-kontener-zakładek-funkcje-eksperckie)
5. [Przepływy pracy](#5-przepływy-pracy)
   - [5.1 Przepływ podstawowy — od objawu do rekomendacji](#51-przepływ-podstawowy--od-objawu-do-rekomendacji)
   - [5.2 Przepływ pętli wykonawczej — Koordynator ↔ Wykonawca nad zadaniem diagnostycznym](#52-przepływ-pętli-wykonawczej--koordynator--wykonawca-nad-zadaniem-diagnostycznym)
   - [5.3 Przepływ zastosowania poprawki w module Developer](#53-przepływ-zastosowania-poprawki-w-module-developer)
   - [5.4 Przepływ odtworzenia objawu w module Terminal](#54-przepływ-odtworzenia-objawu-w-module-terminal)
   - [5.5 Przepływ prowenancji i kosztu wywołania modelu](#55-przepływ-prowenancji-i-kosztu-wywołania-modelu)
   - [5.6 Przepływ nadzoru nad procesem nienadzorowanym w MultitaskingAI](#56-przepływ-nadzoru-nad-procesem-nienadzorowanym-w-multitaskingai)
6. [Punkty sterowania z okna konfiguracji](#6-punkty-sterowania-z-okna-konfiguracji)
   - [6.1 Logi](#61-logi)
   - [6.2 Błędy](#62-błędy)
   - [6.3 Prowenancja wywołań AI](#63-prowenancja-wywołań-ai)
   - [6.4 Zużycie i budżety](#64-zużycie-i-budżety)
   - [6.5 Metryki i wydajność](#65-metryki-i-wydajność)
   - [6.6 Kontrola stanu, alerty i widoczność zakładek Observability Tools](#66-kontrola-stanu-alerty-i-widoczność-zakładek-observability-tools)
   - [6.7 Pętla wykonawcza (Execution Loop Window)](#67-pętla-wykonawcza-execution-loop-window)
   - [6.8 Izolacja i zależności (powiązania jawne)](#68-izolacja-i-zależności-powiązania-jawne)
7. [Stany, dane i powiązania](#7-stany-dane-i-powiązania)
   - [7.1 Model stanów rekomendacji](#71-model-stanów-rekomendacji)
   - [7.2 Model danych wykorzystywany przez moduł](#72-model-danych-wykorzystywany-przez-moduł)
   - [7.3 Izolacja i konfigurowalność — punkty właściwe modułowi Diagnostics](#73-izolacja-i-konfigurowalność--punkty-właściwe-modułowi-diagnostics)
...
11. [Załącznik — pełny wykaz komend kontraktu modułu Diagnostics](#załącznik--pełny-wykaz-komend-kontraktu-modułu-diagnostics)
...
| Powtarzające się błędy analizowane od nowa za każdym razem | Grupowanie błędów po odcisku z licznikiem wystąpień i historią częstotliwości |
...
| Obszar | Zakres |
|---|---|
| Logi | Agregacja strumienia logów na żywo i archiwalnych, wyszukiwanie pełnotekstowe i regex (wyrażenie regularne), filtry, deduplikacja, korelacja po identyfikatorze śledzenia, parsowanie strukturalne (JSON/logfmt), archiwum i retencja |
| Błędy | Rejestr błędów, grupowanie po odcisku, stos wywołań ze źródłem, wykrywanie regresji, przypisanie do wdrożenia i commitu, cykl życia statusu |
| Prowenancja wywołań AI | Pełny zapis każdego wywołania `kanal_modelu`: prompt, odpowiedź, tokeny, koszt, opóźnienie, użyte narzędzia, trafienia cache (pamięć podręczna), łańcuch wywołań agenta i podagentów, drzewo śledzenia (trace) |
...
| **Parsowanie strukturalne** | Rozbicie wpisu JSON/logfmt na pola, filtrowanie i sortowanie po polu w postaci `status=500` | `encoding/json`, parser logfmt; schemat pola z próbki |
...
| **Deduplikacja z licznikiem powtórzeń** | Grupowanie identycznych wpisów, licznik ×N w oknie czasu | odcisk treści (hash — skrót kryptograficzny, suma kontrolna — liczony na treści znormalizowanej); okno agregacji |
...
| Funkcja | Co robi | Zależności |
|---|---|---|
| **Tabela błędów z odciskiem** | Grupowanie podobnych błędów w jeden wpis z licznikiem wystąpień | normalizacja stosu i hash odcisku; agregacja po kluczu |
...
| **Cykl życia statusu** | Nowy → w analizie → rozwiązany → ignorowany, przygaszanie rozwiązanych | model błędu; przejścia bez okien nakładkowych w roli bramki |
| **Priorytet niezależny od poziomu logu** | Ręczne nadanie: krytyczny, wysoki, średni, niski | plakietka priorytetu; sortowanie |
| **Wykres częstotliwości w czasie** | Trend liczby wystąpień — narasta, stały, wygasa | szereg czasowy zdarzeń; render sparkline (mikrowykres przebiegu w jednym wierszu) i słupków po stronie klienta |
...
| **Pulpity konfigurowalne (kafle pulpitu)** | Układanie własnych kafli (metryka, log, błąd, koszt) w siatce | definicja pulpitu w `ustawienie`; siatka kafli po stronie klienta |
...
| **Łańcuch agenta i podagentów** | Powiązanie wywołań w ramach jednego zadania agentowego lub roli MultitaskingAI | korelacja `zadanie` i roli; zgodność ze [Specyfikacją agentów](../specyfikacje/specyfikacja-agentow.md) |
...
| **Monitoring syntetyczny** | Odtwarzalny scenariusz wywołania — testowy prompt do modelu — jako sonda | scenariusz jako `zadanie`; wykonanie przez warstwę Terminal lub HTTP |
...
| **Kanały powiadomień** | Dostarczenie alertu: powiadomienie w aplikacji, e-mail, komunikator, webhook | rozszerzenia i integracje ([Model konfiguracji](../architektura/model-konfiguracji.md), rozdz. 5.7); format webhook |
| **Always On Display** | Alert krytyczny wypychany do globalnej funkcji nadzoru nad pracą nienadzorowaną | powiązanie z Always On Display ([Koncepcja platformy](../architektura/koncepcja-platformy.md), rozdz. 12.2) |
...
| **Podgląd poprawki** | Fragment kodu, polecenie powłoki lub zmiana ustawienia w bloku `.dn-karta` (typografia `--dn-ff-mono`) | render różnicy; `hexops/gotextdiff` |
...
| **Współdzielenie raportu z zespołem** | Przekazanie raportu jako artefaktu do modułu Library | `powiazanie_komponentu`; artefakt pliku |

### 2.11. Zestawienie zależności bibliotecznych (warstwa serwerowa, Go)
...
| Kanały alertów | rozszerzenia i integracje ([Model konfiguracji](../architektura/model-konfiguracji.md), rozdz. 5.7) | webhook generyczny | e-mail i komunikatory jako integracje, nie rdzeń |
...
| 2 | Execution Loop Window | Komunikacja (kanał Koordynator ↔ Wykonawca) | Kolumna sąsiadująca z Chat Window, pełna wysokość obszaru roboczego | 2 | Zawsze na żądanie — wstążka narzędziowa, ikona szybkiego dostępu w prawym górnym rogu okna centralnego albo komponent Chat Window; nigdy stały element modułu | Prowadzenie pętli wykonawczej nad zadaniami diagnostycznymi: dekompozycja zlecenia, kolejka zadań, kontrola jakości, sterowanie przebiegiem |
...
Moduł stosuje regułę stopniowego ujawniania funkcjonalności: jeżeli funkcja nie jest potrzebna do realizacji aktualnego zadania, nie jest widoczna. W stanie spoczynku interfejs prezentuje wyłącznie elementy warstwy 1 — Chat Window, Diagnostics Center, pasek kontekstu oraz zwinięte wyzwalacze `▼`, `⋮` i `☰`.

**Reguła drugiego okna komunikacji.** Drugie okno komunikacji — w tym module Execution Loop Window — uruchamiane jest zawsze na żądanie i nigdy nie jest stałym elementem otwieranego modułu. Wyjątkiem są moduły okrągłego stołu (Roundtable) oraz środowisko MultitaskingAI, gdzie drugie okno komunikacji jest elementem stałym. Otwarcie następuje z komponentu stojącego w jednym z trzech miejsc: we wstążce narzędziowej, w ikonie szybkiego dostępu w prawym górnym rogu okna centralnego aplikacji albo w komponencie pierwszego okna komunikacji.

| Warstwa | Zawartość w module Diagnostics | Sposób dostępu |
|---|---|---|
| 1 | Chat Window, Diagnostics Center, pasek kontekstu, wskaźniki stanu wykonania i plakietka stanu ogólnego | Widoczne bez interakcji |
| 2 | Otwarcie Execution Loop Window, wybór kanału modelu i wykonawcy, zakres czasu, filtry poziomu i źródła, otwarcie Logs Viewer, Errors Panel i Recommendations Panel | Znacznik kontekstowy, ikona, przełącznik; po użyciu element zwija się samoczynnie. Execution Loop Window otwiera się na żądanie ze wstążki narzędziowej, z ikony szybkiego dostępu w prawym górnym rogu okna centralnego albo z komponentu Chat Window |
...
- Cytowanie logów, komunikatów błędów i fragmentów śladów blokiem `.dn-karta` w typografii `--dn-ff-mono`.
...
| Blok cytowanego logu, błędu lub śladu | `.dn-karta`, typografia `--dn-ff-mono` | Prezentacja przywołanego fragmentu logu, stosu wywołań lub span śladu | Średni blok w treści wiadomości | 1 | Widoczny w treści odpowiedzi | domyślny | Kliknięcie „Otwórz w Centrum” przenosi do materiału źródłowego | Treść wiadomości |
| Przycisk „Poproś o poprawkę” | Przycisk `.dn-btn--sygnal`, mały | Skierowanie zapytania o konkretną rekomendację naprawczą | Mały przycisk CTA, akcent sygnałowy | 1 | Widoczny pod odpowiedzią diagnostyczną | domyślny · ładowanie | Generuje wpis w Recommendations Panel powiązany z tym wątkiem rozmowy | Pod odpowiedzią Wykonawcy |
...
| Przycisk załącznika | Ikona (spinacz) | Dołączenie pliku — zrzutu awaryjnego albo dziennika — do wiadomości | Mała ikona | 2 | Kliknięcie ikony lub przeciągnięcie pliku | domyślny · aktywny (plik dołączony) | Otwiera okno wyboru pliku lub przyjmuje przeciągnięcie | Lewa strona pola poleceń |
...
| Warstwa | 2 — zawsze na żądanie; drugie okno komunikacji nigdy nie jest stałym elementem modułu (rozdz. 3.1) |
| Sposób wywołania | Wstążka narzędziowa, ikona szybkiego dostępu w prawym górnym rogu okna centralnego aplikacji albo komponent pierwszego okna komunikacji (Chat Window) |
...
| Element | Co to jest | Do czego służy | Forma i waga | Warstwa | Sposób wywołania | Stany | Zachowanie po interakcji | Występowanie |
|---|---|---|---|---|---|---|---|---|
| Plakietka stanu ogólnego | `.dn-plakietka--sukces` / `.dn-plakietka--ostrzezenie` / `.dn-plakietka--blad` | Jednozdaniowe podsumowanie kondycji systemu | Średnia pigułka z ikoną i etykietą | 1 | Widoczna bez interakcji | dobry stan · wymaga uwagi · krytyczny | Informacyjna, prowadzi wzrok do dalszej analizy | Nagłówek okna |
...
- Grupowanie podobnych błędów po odcisku z licznikiem wystąpień, zamiast powielania identycznych wpisów.
...
- Zmiana statusu błędu — oznaczenie jako rozwiązany, ignorowany lub w analizie, bez okien nakładkowych w roli bramki.
...
- Podgląd proponowanej poprawki — fragment kodu, polecenie powłoki lub zmiana konfiguracji — w bloku `.dn-karta` (typografia `--dn-ff-mono`).
...
| Blok podglądu poprawki | `.dn-karta`, typografia `--dn-ff-mono` | Prezentacja proponowanej zmiany przed zastosowaniem | Średni blok wewnątrz karty rekomendacji | 1 | Widoczny bez interakcji | domyślny | Kliknięcie rozwija pełną treść, gdy poprawka jest dłuższa niż podgląd | Ciało karty rekomendacji |
| Przycisk „Zastosuj poprawkę” | Przycisk `.dn-btn--sygnal`, mały | Przekazanie poprawki do modułu Developer w celu przeglądu i wdrożenia | Mały przycisk CTA, akcent sygnałowy — sygnalizuje wagę akcji, nie blokuje jej wykonania | 1 | Widoczny na karcie rekomendacji | domyślny · ładowanie | Otwiera Code Editor modułu Developer z gotową zmianą przygotowaną do przeglądu i zatwierdzenia | Stopka karty rekomendacji |
...
Wszystkie punkty są jawne, opatrzone objaśnieniem `[?]`, przechowywane jako `ustawienie` z kluczem ([Model danych](../architektura/model-danych.md#171-ustawienie-ustawienie), rozdz. 17.1). Wartości podlegają zasadzie „brak ustawienia = wartość domyślna”. Zero blokad — konfiguracja steruje **widocznością, zakresem i progami sygnalizacji**, nigdy nie wyłącza działania platformy siłą i nie zamienia progu w bramkę.
...
| Ikona `tarcza` | Kontekst bezpieczeństwa — wcielenie Security Auditor | Errors Panel, Recommendations Panel |
...
Moduł Diagnostics dzieli obszar roboczy na kolumny sąsiadujące poziomo (rozdz. 3). Poniższe progi, wspólne całej platformie, rozstrzygają zachowanie układu tych kolumn wraz ze zmianą szerokości okna aplikacji.

| Żeton | Próg szerokości | Zachowanie układu w module Diagnostics |
|---|---|---|
...
| `--dn-bp-w2` | 960 px | Tablet — boczna nawigacja modułów zwija się do samych ikon; kolumny modułu Diagnostics zachowują układ, lecz kolumny boczne (rozszerzenia warstwy 2–3) otwierają się jako nakładka zamiast stałej kolumny |
| `--dn-bp-w3` | 1280 px | Biurko — pełny kokpit; wszystkie okna operacyjne modułu Diagnostics wymienione w rozdz. 3 dostępne jednocześnie w układzie kolumnowym opisanym w tym rozdziale |
...
Poniżej progu `--dn-bp-w1` układ kolumnowy modułu Diagnostics nie jest dostępny — zachowanie na urządzeniach mobilnych ustala [Mobile](../funkcje-globalne/mobile.md).

### 10.2. Kryteria odbioru

| Warunek sprawdzalny | Sposób sprawdzenia |
|---|---|
| Moduł jest osiągalny wyłącznie z bocznej nawigacji CodeStudio | Próba odnalezienia Diagnostics w bocznej nawigacji TalkIn i WorkSpace — brak pozycji |
| Żaden przycisk akcji modułu nie występuje w stanie zablokowanym | Przegląd kontrolek CTA katalogu elementów rozdz. 4 pod kątem obecności `disabled`/`not-allowed` |
| Poprawka wypracowana przez Wykonawcę otwiera się domyślnie do przeglądu Operatora przed zastosowaniem | Kliknięcie „Zastosuj poprawkę” na karcie rekomendacji |
| Wszystkie 9 komend obszaru `diagnostics` mają pokrycie w interfejsie modułu albo jawne wskazanie miejsca wywołania | Zestawienie Załącznika z katalogiem elementów rozdz. 4 |
| Redakcja danych wrażliwych w treści śladu jest ustawieniem konfiguracyjnym, nie zachowaniem na sztywno | Sprawdzenie klucza `diagnostics.prowenancja.redakcja` w oknie konfiguracji |
| Plakietka stanu ogólnego odzwierciedla rzeczywistą kondycję systemu | Wywołanie stanu krytycznego — przekroczenie budżetu kosztu — i odczyt plakietki w nagłówku okna |
...
## Załącznik — pełny wykaz komend kontraktu modułu Diagnostics
...
|---|---|---|
| `diagnostics.analysis.changed` | Zmiana analizy diagnostycznej; odswieza Recommendations Panel | `change:ChangeKind` (wym)<br>`analysis:DiagnosticAnalysis` (wym) |

Razem w wykazie: **9 komend** z jednego obszaru kontraktu.

---

*Koniec dokumentu. Moduł Diagnostics — Specyfikacja docelowa, wersja 2.0, 2026-08-20.*
