...
| **Przeznaczenie** | Ustala zakres funkcjonalny, komplet okien operacyjnych i zachowanie modułu Library — centralnego repozytorium wiedzy i plików platformy — jako źródło wykonawcze dla dewelopera i projektanta. |
| **Zakres** | komplet okien operacyjnych modułu Library (Chat Window, Execution Loop Window, Library Explorer, Tags & Collections, File Preview, Versioning Panel, Metadata & Archive Panel), katalog funkcji, komendy obszaru `library`, żetony i komponenty widoku, stany kontrolek, przebiegi pracy i scenariusze użycia |
| **Poza zakresem** | edycja treści dokumentu — [Moduł Studio](studio.md); repozytorium kodu i kontrola wersji — [Moduł Developer](developer.md), okno Git Panel w środowisku CodeStudio; tworzenie i obróbka grafiki — [Moduł Design](design.md); orkiestracja procesów i harmonogramy — [Moduł Automations](automations.md); pozyskiwanie źródeł z sieci — [Moduł Browser](browser.md) i [Moduł Research](research.md); trwała pamięć kontekstowa modeli — zakres „Pamięć” okna konfiguracji (rozdz. 1.2) |
| **Dokument nadrzędny** | [Specyfikacja modułów](../specyfikacje/specyfikacja-modulow.md) |
| **Dokumenty powiązane** | [Model danych](../architektura/model-danych.md) · [Katalog komponentów](../interfejs-uzytkownika/katalog-komponentow.md) · [Moduł Studio](studio.md) · [Moduł Research](research.md) · [Moduł WorkSpace](workspace.md) |
| **Prototypy odniesienia** | `design/05-okna/moduly/library.html` |
| **Źródła normatywne** | `budowa/shared/contract.json` (obszar `library`) · `design/zasoby/zetony/zetony.css` · `design/zasoby/css/komponenty.css` · `design/05-okna/moduly/library.html` |
| **Zasada nadrzędna** | Pełna kompozycyjność i pełna konfigurowalność; zero blokad w interfejsie; klucze i powiązania jawne; każda zdolność sterowana z okna konfiguracji; domyślne zachowanie modułu = wykonanie. Jedyny wyjątek od zasady „zero blokad”: czynność nieodwracalna — trwałe usunięcie zasobu wraz z wersjami i treścią — wymaga jawnego potwierdzenia i bez niego jest odmawiana |
...
3. [Specyfikacja okien operacyjnych](#3-specyfikacja-okien-operacyjnych)
   ...
   - [3.3 Library Explorer](#33-library-explorer)
   - [3.4 Tags & Collections](#34-tags--collections)
   - [3.5 File Preview](#35-file-preview)
   - [3.6 Versioning Panel](#36-versioning-panel)
   - [3.7 Metadata & Archive Panel](#37-metadata--archive-panel)
4. [Przepływy pracy](#4-przepływy-pracy)
   - [4.1 Przepływ podstawowy — odbiór, katalogowanie i podgląd](#41-przepływ-podstawowy--odbiór-katalogowanie-i-podgląd)
   - [4.2 Przepływ pętli wykonawczej — zlecenie katalogowania](#42-przepływ-pętli-wykonawczej--zlecenie-katalogowania)
   - [4.3 Przepływ rozszerzony — porządkowanie zbiorcze z sugestiami AI](#43-przepływ-rozszerzony--porządkowanie-zbiorcze-z-sugestiami-ai)
   - [4.4 Przepływ rozszerzony — śledzenie wersji między modułami](#44-przepływ-rozszerzony--śledzenie-wersji-między-modułami)
   - [4.5 Przepływ rozszerzony — utrwalenie archiwalne](#45-przepływ-rozszerzony--utrwalenie-archiwalne)
   - [4.6 Przepływ rozszerzony — odpowiednik projektowy (Project Library)](#46-przepływ-rozszerzony--odpowiednik-projektowy-project-library)
5. [Stany, dane i powiązania](#5-stany-dane-i-powiązania)
   - [5.1 Model stanów repozytorium](#51-model-stanów-repozytorium)
   - [5.2 Model danych wykorzystywany przez moduł](#52-model-danych-wykorzystywany-przez-moduł)
   - [5.3 Izolacja i konfigurowalność — punkty właściwe modułowi Library](#53-izolacja-i-konfigurowalność--punkty-właściwe-modułowi-library)
...
7. [Katalog funkcji i narzędzi](#7-katalog-funkcji-i-narzędzi)
   - [7.1 Napływ, import i akwizycja zasobów](#71-napływ-import-i-akwizycja-zasobów)
   - [7.2 Katalogowanie, metadane i klasyfikacja](#72-katalogowanie-metadane-i-klasyfikacja)
   - [7.3 Kolekcje, etykiety i taksonomie](#73-kolekcje-etykiety-i-taksonomie)
   - [7.4 Wyszukiwanie: pełnotekstowe, semantyczne, fasetowe](#74-wyszukiwanie-pełnotekstowe-semantyczne-fasetowe)
   - [7.5 Podgląd, renderowanie i porównanie](#75-podgląd-renderowanie-i-porównanie)
   - [7.6 Wersjonowanie i śledzenie zmian](#76-wersjonowanie-i-śledzenie-zmian)
   - [7.7 Deduplikacja, higiena i normalizacja](#77-deduplikacja-higiena-i-normalizacja)
   - [7.8 Archiwizacja i długoterminowe przechowywanie](#78-archiwizacja-i-długoterminowe-przechowywanie)
   - [7.9 Multimedia — obraz, audio, wideo](#79-multimedia--obraz-audio-wideo)
   - [7.10 Bibliografia i menedżer źródeł](#710-bibliografia-i-menedżer-źródeł)
   - [7.11 Udostępnianie, eksport i publikacja](#711-udostępnianie-eksport-i-publikacja)
   - [7.12 Operacje AI na repozytorium](#712-operacje-ai-na-repozytorium)
   - [7.13 Automatyzacje, reguły i integracje](#713-automatyzacje-reguły-i-integracje)
   - [7.14 Analityka, raporty i audyt repozytorium](#714-analityka-raporty-i-audyt-repozytorium)
   - [7.15 Zależności techniczne o charakterze wiążącym](#715-zależności-techniczne-o-charakterze-wiążącym)
...
11. [Załącznik — pełny wykaz komend kontraktu modułu Library](#załącznik--pełny-wykaz-komend-kontraktu-modułu-library)
   - [Obszar `library` — 48 komend](#obszar-library--48-komend)
...
- **Zarządzanie zasobami cyfrowymi (DAM, Digital Asset Management)** — biblioteka zasobów z metadanymi, miniaturami i galerią mediów.
- **Zarządzanie dokumentami (DMS, Document Management System)** — repozytorium dokumentów z wersjonowaniem, historią i regułami napływu.
...
| 4 — funkcje eksperckie | Zakładki Archiwum i Higiena panelu Metadata & Archive Panel (PDF/A; BagIt — format pakowania zbiorów do przekazu archiwalnego, specyfikacja RFC 8493; PREMIS/METS — PREservation Metadata: Implementation Strategies oraz Metadata Encoding and Transmission Standard, standardy Biblioteki Kongresu do metadanych archiwizacji; polityki retencji; migawka repozytorium; weryfikacja integralności), dziennik audytu, definicje pól niestandardowych i schematu metadanych, reguły kolekcji inteligentnych i reguły napływu, wsadowa klasyfikacja i operacje wsadowe, eksport tezaurusa w zapisie SKOS/RDF (Simple Knowledge Organization System oraz Resource Description Framework — standardy World Wide Web Consortium do opisu słowników pojęciowych), eksport paczki migracyjnej, API repozytorium i webhooki, macierz izolacji dostępu do plików | Polecenie języka naturalnego w Chat Window, pasek `/operacje`, skrót klawiszowy, wyszukiwarka funkcji, tryb administracyjny albo konfiguracja roli. Użytkownik podstawowy nie widzi tych elementów |
...
| Odnośnik cytowania źródła | Odsyłacz do pliku i fragmentu | Wskazanie podstawy odpowiedzi Wykonawcy | Bardzo mały odsyłacz `--dn-fs-xs` | 1 | Widoczne bez interakcji w treści odpowiedzi | domyślny · wskazanie kursorem (podgląd fragmentu) | Kliknięcie otwiera File Preview na wskazanym fragmencie | W treści odpowiedzi |
...
- Bieżące zlecenie repozytoryjne i jego dekompozycja na zadania — dla zlecenia „skataloguj 240 plików importu” → ekstrakcja typu, sumy kontrolne, ekstrakcja tekstu, indeksacja, auto-metadane, sugestie etykiet).
...
- Wskaźniki przebiegu pętli: liczba zadań, postęp, czas trwania, liczba ponowień, zadania zablokowane zależnością — brakiem warstwy rozpoznania tekstu przed indeksacją.
...
| Pozycja zadania | Wiersz z nazwą zadania i znacznikiem stanu | Reprezentacja pojedynczego zadania pętli | Średni wiersz `.dn-karta--klikalna` | 1 | Widoczne bez interakcji | oczekujące · w toku · zakończone · ponowione · zablokowane zależnością | Kliknięcie rozwija komunikaty sterujące i wynik zadania (warstwa 3) | Kolejka zadań |
...
| Blok kontroli jakości | Wyróżniony blok z wynikiem sprawdzenia | Prezentacja oceny wyniku i decyzji o ponowieniu | Mały blok `.dn-plakietka--informacja` | 3 | Rozwinięcie wskaźnika przebiegu (`▼`) albo kliknięcie pozycji zadania | zwinięty (domyślny) · wynik pozytywny · wynik z ponowieniem | Kliknięcie otwiera zadanie, którego dotyczy ocena | Rozwinięcie pod kolejką zadań |
| Komunikaty sterujące Koordynator ↔ Wykonawca | Pełna treść poleceń i odpowiedzi zadania | Wgląd w wymianę sterującą pętli | Lista `--dn-fs-xs` w rozwinięciu zadania | 3 | Kliknięcie pozycji zadania w kolejce | zwinięty (domyślny) · rozwinięty | Rozwinięcie zwija się po ponownym kliknięciu | Rozwinięcie pozycji zadania |
...
*Operacje na plikach.*
- Otwarcie pliku we właściwym module źródłowym — dokument tekstowy otwiera się w Studio Editor.
...
| Element | Co to jest | Do czego służy | Forma i waga | Warstwa | Sposób wywołania | Stany | Zachowanie po interakcji | Występowanie |
|---|---|---|---|---|---|---|---|---|
| Pole wyszukiwania | Pole `.dn-pole` z ikoną (szukaj) | Wyszukiwanie pełnotekstowe w treści repozytorium | Średnie pole, góra kolumny | 1 | Widoczne bez interakcji; `Ctrl/Cmd + F` ustawia w nim fokus | puste · z wynikami | Enter uruchamia wyszukiwanie na żywo | Górna krawędź okna |
...
| Karta pliku (widok siatki) | Miniatura + nazwa + metadane | Reprezentacja pojedynczego artefaktu | Średnia karta `.dn-karta` w układzie siatki | 1 | Widoczne bez interakcji | domyślny · zaznaczony (obrys sygnałowy) · wskazanie kursorem · ładowanie miniatury | Pojedyncze kliknięcie zaznacza, podwójne otwiera File Preview lub moduł źródłowy | Ciało okna, widok siatki |
| Plakietka modułu źródłowego | Mała etykieta pod nazwą pliku | Wskazanie modułu, w którym artefakt powstał | Bardzo mała plakietka `.dn-plakietka--rola` | 1 | Widoczne bez interakcji na karcie pliku | domyślny | Kliknięcie filtruje widok do tego modułu źródłowego | Karta pliku |
| Wskaźnik liczby wersji | Mały tekst pomocniczy | Informacja o liczbie zapisanych wersji pliku | Bardzo mały tekst `--dn-fs-xs` | 1 | Widoczne bez interakcji na karcie pliku | jedna wersja · wiele wersji | Kliknięcie otwiera Versioning Panel dla tego pliku | Karta pliku |
...
- Tworzenie kolekcji swobodnych (foldery tematyczne niezależne od struktury plików) oraz kolekcji inteligentnych (reguła automatycznego przypisania w postaci „wszystkie pliki z modułu Research otagowane «rynek X»”).
...
| Karta sugestii tagowania AI | Mały blok z proponowaną etykietą | Przyspieszenie katalogowania nowych plików | Mały blok `.dn-plakietka--informacja` z dwoma przyciskami | 3 | Pojawia się w panelu wyłącznie w chwili wystąpienia sugestii Wykonawcy | ukryty (brak sugestii) · widoczny | „Zastosuj” przypisuje etykietę, „Odrzuć” usuwa sugestię | Stopka panelu |
...
| Przycisk „✕ Zamknij” | Ikona zamknięcia | Zamknięcie podglądu, powrót do Library Explorer | Mała ikona (zamknij) | 2 | Widoczna po otwarciu File Preview; klawisz `Esc` działa równoważnie | domyślny · wskazanie kursorem | Zamyka kolumnę, zachowuje pozycję w Library Explorer | Prawy róg nagłówka |
...
| Przycisk „Otwórz w [moduł źródłowy]” | Przycisk `--sygnal` | Przejście do pełnej edycji we właściwym module | Średni przycisk CTA | 2 | Widoczny po otwarciu File Preview jako jedyna akcja pierwszoplanowa | domyślny | Otwiera plik w Studio Editor / Design Board / Code Editor, zależnie od typu | Stopka kolumny |
...
| Element | Co to jest | Do czego służy | Forma i waga | Warstwa | Sposób wywołania | Stany | Zachowanie po interakcji | Występowanie |
|---|---|---|---|---|---|---|---|---|
| Pozycja wersji | Wiersz karty `.dn-karta--klikalna` | Reprezentuje jedną zapisaną wersję pliku | Średni wiersz z kropką stanu, znacznikiem czasu, autorem i modułem pochodzenia | 2 | Widoczna po otwarciu panelu | bieżąca (wypełniona kropka, wyróżnienie sygnałowe) · archiwalna · najechanie | Kliknięcie otwiera wersję w File Preview | Lista główna panelu |
| Plakietka modułu pochodzenia | Mała etykieta | Wskazanie, w którym module powstała dana wersja | Bardzo mała plakietka `.dn-plakietka--rola` | 2 | Widoczna przy pozycji wersji po otwarciu panelu | domyślny | — (informacyjny) | Przy pozycji wersji |
| Powiadomienie o nowej wersji | Toast lub subtelny pasek | Informacja, że plik zmienił się w innym module podczas jego przeglądania | Mały pasek `.dn-plakietka--informacja` | 2 | Pojawia się samoczynnie w chwili powstania nowej wersji, kanałem zdarzeń | ukryty · widoczny | Kliknięcie odświeża panel do najnowszej wersji | Góra panelu, pojawia się na żywo |
...
| Element | Co to jest | Do czego służy | Forma i waga | Warstwa | Sposób wywołania | Stany | Zachowanie po interakcji | Występowanie |
|---|---|---|---|---|---|---|---|---|
| Formularz Dublin Core | Zestaw pól tekstowych | Opis zasobu w standardowym schemacie metadanych | Średni formularz `.dn-pole` | 3 | Otwarcie panelu z menu kebab (⋮) karty pliku, pozycja „Metadane i archiwum” | domyślny · edytowany · uzupełniony przez AI (wyróżnienie) | Zapis aktualizuje encję `artefakt` i indeks | Zakładka Metadane |
...
| Blok metadanych technicznych | Lista właściwości odczytanych z pliku | Prezentacja EXIF/IPTC/XMP/ID3 i sumy kontrolnej | Wąska lista `--dn-fs-xs` | 3 | Rozwinięcie sekcji wyzwalaczem `▼` w nagłówku „TECHNICZNE” | zwinięty (domyślny) · rozwinięty · brak danych technicznych | — (informacyjny) | Zakładka Metadane |
...
| **Nasłuch zewnętrzny i API repozytorium** | Zewnętrzny dostęp do zasobów przez API z jawnym kluczem | Endpoint REST; token z okna konfiguracji |
...
| **API repozytorium i nasłuchy zewnętrzne** | Włączenie punktu końcowego REST, token jawny, zakres uprawnień | 5.8 Integracje | Wyłączone |
...
Moduł Library dzieli obszar roboczy na kolumny sąsiadujące poziomo (rozdz. 2). Poniższe progi, wspólne całej platformie, rozstrzygają zachowanie układu tych kolumn wraz ze zmianą szerokości okna aplikacji.

| Żeton | Próg szerokości | Zachowanie układu w module Library |
|---|---|---|
...
| `--dn-bp-w2` | 960 px | Tablet — boczna nawigacja modułów zwija się do samych ikon; kolumny modułu Library zachowują układ, lecz kolumny boczne (rozszerzenia warstwy 2–3) otwierają się jako nakładka zamiast stałej kolumny |
| `--dn-bp-w3` | 1280 px | Biurko — pełny kokpit; wszystkie okna operacyjne modułu Library wymienione w rozdz. 2 dostępne jednocześnie w układzie kolumnowym opisanym w tym rozdziale |
...
Poniżej progu `--dn-bp-w1` układ kolumnowy modułu Library nie jest dostępny — zachowanie na urządzeniach mobilnych ustala [Mobile](../funkcje-globalne/mobile.md).

### 10.2. Kryteria odbioru

Warunki sprawdzalne, których łączne spełnienie oznacza gotowość modułu Library do odbioru.

| Warunek sprawdzalny | Sposób sprawdzenia |
|---|---|
| Wszystkie 7 okien operacyjnych z rozdz. 2 otwiera się dokładnie sposobem wywołania opisanym w tabeli okien i w katalogu elementów interfejsu rozdz. 3 | Przegląd manualny wg tabeli rozdz. 2 — każde okno wywołane, sprawdzone wejście i zamknięcie |
| Każda z 48 komend obszaru `library` z Załącznika ma pokrycie w co najmniej jednym przebiegu pracy albo scenariuszu użycia (rozdz. 4, 6) | Zestawienie nazw komend z treścią rozdz. 4 i 6 |
| Żaden opisany element interfejsu nie odwołuje się do klasy `.dn-*` ani żetonu `--dn-*` nieobecnego w `komponenty.css`, `rama.css`, `prototyp.css` lub `zetony.css` | `grep` nazwy klas i żetonów użytych w dokumencie względem arkuszy `design/zasoby/` |
| Warstwy widoczności 1–4 (rozdz. 2.1) nie pozostawiają funkcji nieosiągalnej — każda ukryta funkcja ma jawny sposób wywołania | Przegląd tabeli warstw rozdz. 2.1, weryfikacja kolumny „Sposób wywołania” |
| Zasada „zero blokad” nie jest naruszona żadnym opisem stanu „zablokowany” lub „nieaktywny” bez alternatywy komunikatu | Wyszukanie słów „zablokowan”, „wyłączon”, „nieaktywn” w treści i weryfikacja kontekstu |
| Czynność nieodwracalna jest jedynym wyjątkiem od zasady „zero blokad”: trwałe usunięcie zasobu wraz z wersjami i treścią wymaga jawnego potwierdzenia, a jego brak skutkuje odmową wykonania | Sprawdzenie, że komenda `library.file.delete` niesie wymagane pole `confirm:bool` (rozdz. 9), oraz że opis komendy wskazuje odmowę przy braku potwierdzenia |


---

## Załącznik — pełny wykaz komend kontraktu modułu Library
...
### Obszar `library` — 48 komend

| Komenda | Przeznaczenie | Pola żądania | Pola wyniku |
|---|---|---|---|
| `library.audit.list` | Zwraca wpisy dziennika audytu repozytorium: kto, co i kiedy. Dziennik jest przyrostowy — wpisów nie da się zmienić ani usunąć tą rodziną komend | `fileId:string` (opc)<br>`actions:LibraryAuditAction[]` (opc)<br>`actor:string` (opc)<br>`since:int64` (opc)<br>`until:int64` (opc)<br>`limit:int` (opc)<br>`offset:int` (opc) | `entries:LibraryAuditEntry[]` (wym)<br>`total:int` (wym) |
| `library.classify.run` | Zleca wsadową klasyfikację zasobów: model nadaje rodzaj treści, słowa kluczowe i propozycje etykiet dla całego wskazanego zbioru. Wynik wraca sugestiami do przyjęcia, nie zapisem — domyślnym zachowaniem modułu jest sugestia z akceptacją Operatora | `fileIds:string[]` (opc)<br>`collectionId:string` (opc)<br>`untaggedOnly:bool` (opc)<br>`modelChannelId:string` (opc)<br>`apply:bool` (opc) | `suggestions:LibrarySuggestion[]` (wym)<br>`processedCount:int` (wym)<br>`appliedCount:int` (wym) |
| `library.collection.assign` | Przypisuje zasób do kolekcji (Tags & Collections) | `collectionId:string` (wym)<br>`fileIds:string[]` (wym) | `collectionId:string` (wym)<br>`assignedCount:int` (wym) |
| `library.collection.create` | Tworzy kolekcję zasobów (Tags & Collections) | `name:string` (wym)<br>`description:string` (opc) | `collectionId:string` (wym)<br>`name:string` (wym) |
| `library.collection.list` | Zwraca kolekcje repozytorium wraz z hierarchią, regułą i licznikiem zasobów. Bez tej komendy klient zna wyłącznie identyfikatory kolekcji wyczytane z plików, a nazwę kolekcji tylko w chwili jej założenia | `parentId:string` (opc)<br>`query:string` (opc)<br>`limit:int` (opc) | `collections:LibraryCollection[]` (wym)<br>`total:int` (wym) |
| `library.diff.compare` | Porównuje dwie treści repozytorium: dwa zasoby albo dwie wersje jednego zasobu. Dokumenty binarne porównuje po tekście z nich wydobytym i mówi o tym w odpowiedzi | `leftFileId:string` (wym)<br>`rightFileId:string` (opc)<br>`leftVersionId:string` (opc)<br>`rightVersionId:string` (opc)<br>`contextLines:int` (opc) | `diff:LibraryDiff` (wym) |
| `library.duplicate.merge` | Łączy grupę duplikatów w jeden zasób, zachowując etykiety, kolekcje i historię wersji zasobów wchłanianych. Zasoby wchłonięte trafiają do archiwum, nie znikają | `targetFileId:string` (wym)<br>`sourceFileIds:string[]` (wym)<br>`keepVersions:bool` (opc) | `file:LibraryFile` (wym)<br>`mergedCount:int` (wym)<br>`carriedTags:string[]` (wym)<br>`carriedVersions:int` (wym) |
| `library.duplicate.scan` | Wyszukuje duplikaty dokładne po sumie kontrolnej oraz niemal-duplikaty po podobieństwie treści i obrazu. Rozpoznanie przybliżone żąda porównania całego zbioru, więc jest czynnością rdzenia, nie okna | `collectionId:string` (opc)<br>`kinds:LibraryDuplicateKind[]` (opc)<br>`threshold:int` (opc)<br>`limit:int` (opc) | `groups:LibraryDuplicateGroup[]` (wym)<br>`total:int` (wym)<br>`skippedWithoutChecksum:int` (wym) |
| `library.file.archive` | Przenosi zasoby do archiwum. Czynność jest odwracalna komendą `library.file.restore` — to jest kosz repozytorium, nie usunięcie | `fileIds:string[]` (wym)<br>`reason:string` (opc) | `archivedCount:int` (wym)<br>`files:LibraryFile[]` (wym) |
| `library.file.delete` | Usuwa zasoby trwale wraz z ich wersjami i treścią. Jedyna droga utraty danych repozytorium; bez potwierdzenia jest odmowa | `fileIds:string[]` (wym)<br>`confirm:bool` (wym) | `deletedCount:int` (wym)<br>`deletedVersions:int` (wym) |
| `library.file.list` | Zwraca pliki repozytorium; wyszukiwanie obejmuje treść i znaczenie | `query:string` (opc)<br>`tags:string[]` (opc)<br>`collectionId:string` (opc)<br>`projectId:string` (opc)<br>`limit:int` (opc)<br>`offset:int` (opc) | `files:LibraryFile[]` (wym)<br>`total:int` (opc) |
| `library.file.move` | Przenosi zasoby w strukturze repozytorium pod wskazaną ścieżkę. Przeniesienie nie rusza treści ani historii wersji | `fileIds:string[]` (wym)<br>`path:string` (wym) | `movedCount:int` (wym)<br>`files:LibraryFile[]` (wym) |
| `library.file.preview` | Zwraca podgląd zawartości pliku bez opuszczania modułu | `fileId:string` (wym)<br>`page:int` (opc)<br>`maxChars:int` (opc) | `preview:LibraryPreview` (wym) |
| `library.file.restore` | Przywraca zasoby z archiwum do wykazu czynnego | `fileIds:string[]` (wym) | `restoredCount:int` (wym)<br>`files:LibraryFile[]` (wym) |
| `library.file.search` | Wyszukuje zasoby biblioteki (Library Explorer) | `query:string` (wym)<br>`limit:int` (opc) | `files:LibraryFile[]` (wym)<br>`total:int` (wym) |
| `library.file.upload` | Wgrywa plik do repozytorium wiedzy | `name:string` (wym)<br>`mimeType:string` (opc)<br>`contentBase64:string` (opc)<br>`sourcePath:string` (opc)<br>`projectId:string` (opc)<br>`sourceModuleId:string` (opc)<br>`tags:string[]` (opc) | `file:LibraryFile` (wym) |
| `library.fixity.check` | Sprawdza integralność zasobów: przelicza sumę kontrolną z bajtów i porównuje ją z zapisaną. Klient nie ma czym tego zrobić, bo nie sięga po bajty zasobu | `fileIds:string[]` (opc)<br>`collectionId:string` (opc)<br>`mismatchedOnly:bool` (opc) | `results:LibraryFixityResult[]` (wym)<br>`checkedCount:int` (wym)<br>`mismatchedCount:int` (wym)<br>`missingCount:int` (wym) |
| `library.metadata.get` | Zwraca opis zasobu w schemacie Dublin Core wraz z polami niestandardowymi oraz — na żądanie — metadane osadzone w pliku (EXIF, IPTC, XMP, ID3). Metadane osadzone czyta się z bajtów, więc ich odczyt jest kosztowny i wchodzi wyłącznie na wyraźne żądanie | `fileId:string` (wym)<br>`includeTechnical:bool` (opc) | `metadata:LibraryMetadata` (wym) |
| `library.metadata.set` | Zapisuje opis zasobu w schemacie Dublin Core wraz z polami niestandardowymi. Dziś formularz opisu nie ma dokąd pójść: `library.tag.set` zmienia wyłącznie etykiety i kolekcje. Zapis rozgłasza `library.file.changed` | `fileId:string` (wym)<br>`metadata:LibraryMetadata` (wym)<br>`replace:bool` (opc) | `metadata:LibraryMetadata` (wym)<br>`file:LibraryFile` (wym) |
| `library.name.normalize` | Ujednolica nazwy zasobów według reguły nazewnictwa: schemat, transliteracja znaków diakrytycznych, zamiana znaków niedozwolonych. Przebieg próbny pokazuje wynik bez zapisu | `fileIds:string[]` (opc)<br>`collectionId:string` (opc)<br>`pattern:string` (opc)<br>`transliterate:bool` (opc)<br>`dryRun:bool` (opc) | `results:LibraryNormalizeResult[]` (wym)<br>`changedCount:int` (wym) |
| `library.package.export` | Składa paczkę migracyjną wskazanego zbioru albo migawkę całego repozytorium: archiwum wraz z indeksem, opisem zasobów i manifestem sum kontrolnych | `kind:LibraryPackageKind` (wym)<br>`collectionId:string` (opc)<br>`fileIds:string[]` (opc)<br>`includeVersions:bool` (opc)<br>`format:string` (opc) | `fileId:string` (wym)<br>`entries:int` (wym)<br>`sizeBytes:int64` (wym)<br>`manifestChecksum:string` (wym) |
| `library.preservation.run` | Utrwala zasoby archiwalnie: normalizuje do PDF/A, pakuje w BagIt z manifestem sum kontrolnych albo zapisuje metadane utrwalenia w profilu PREMIS/METS. Wynik jest sprawdzany, a zapis sprawdzenia wchodzi do odpowiedzi | `fileIds:string[]` (opc)<br>`collectionId:string` (opc)<br>`kind:LibraryPreservationKind` (wym)<br>`profile:string` (opc)<br>`replaceOriginal:bool` (opc) | `results:LibraryPreservationResult[]` (wym)<br>`validCount:int` (wym)<br>`invalidCount:int` (wym) |
| `library.retention.list` | Zwraca polityki przechowywania oraz zasoby zbliżające się do końca okresu — raport retencji | `scope:ConfigScope` (opc)<br>`dueWithinDays:int` (opc)<br>`limit:int` (opc) | `policies:LibraryRetentionPolicy[]` (wym)<br>`due:LibraryRetentionDue[]` (opc) |
| `library.retention.set` | Zakłada albo zmienia politykę przechowywania zasobów na wskazanym poziomie zasięgu. Twarde usunięcie nie zachodzi samoczynnie: polityka najwyżej zgłasza zasób do usunięcia | `policy:LibraryRetentionPolicy` (wym)<br>`remove:bool` (opc) | `policy:LibraryRetentionPolicy` (wym)<br>`affectedFiles:int` (wym) |
| `library.rule.list` | Zwraca reguły repozytorium | `kind:LibraryRuleKind` (opc)<br>`enabledOnly:bool` (opc) | `rules:LibraryRule[]` (wym)<br>`total:int` (wym) |
| `library.rule.remove` | Usuwa regułę. Zasoby przypisane przez regułę kolekcji inteligentnej zostają w kolekcji, ale przestają być przeliczane — inaczej usunięcie reguły opróżniałoby kolekcję po cichu | `ruleId:string` (wym)<br>`detachFiles:bool` (opc) | `removed:bool` (wym)<br>`detachedFiles:int` (wym) |
| `library.rule.set` | Zakłada albo zmienia regułę repozytorium: kolekcję inteligentną, regułę napływu albo folder obserwowany. Zapis uruchamia przeliczenie | `rule:LibraryRule` (wym)<br>`runNow:bool` (opc) | `rule:LibraryRule` (wym)<br>`matchedFiles:int` (opc) |
| `library.schema.get` | Zwraca definicje pól niestandardowych schematu metadanych, zawężone do rodzaju treści albo kolekcji, gdy żądanie podaje to zawężenie | `mimeType:string` (opc)<br>`collectionId:string` (opc) | `fields:LibraryFieldDefinition[]` (wym) |
| `library.schema.set` | Zakłada albo zmienia definicję pola niestandardowego schematu metadanych. Pole o kodzie już istniejącym jest zmieniane, nie dublowane | `field:LibraryFieldDefinition` (wym)<br>`remove:bool` (opc) | `field:LibraryFieldDefinition` (wym)<br>`affectedFiles:int` (wym) |
| `library.share.create` | Wystawia odnośnik do zasobu albo kolekcji wraz z tokenem dostępu. Token jest jawny zgodnie z zasadą jawności kluczy platformy | `scope:LibraryShareScope` (wym)<br>`targetId:string` (wym)<br>`expiresInSeconds:int` (opc) | `share:LibraryShare` (wym)<br>`url:string` (wym) |
| `library.share.list` | Zwraca udostepnienia repozytorium | `targetId:string` (opc)<br>`activeOnly:bool` (opc) | `shares:LibraryShare[]` (wym)<br>`total:int` (wym) |
| `library.share.revoke` | Odwołuje udostępnienie. Odnośnik przestaje działać, wpis zostaje w dzienniku audytu | `shareId:string` (wym) | `revoked:bool` (wym) |
| `library.stats.get` | Zwraca pulpit stanu repozytorium liczony po całym zbiorze: liczbę zasobów, rozmiar, rozkład rodzajów treści i modułów wytwórcy, użycie etykiet i kolekcji, liczbę zasobów osieroconych i duplikatów | `collectionId:string` (opc)<br>`projectId:string` (opc)<br>`topN:int` (opc) | `stats:LibraryStats` (wym) |
| `library.suggestion.apply` | Przyjmuje albo odrzuca sugestię porządkującą. Przyjęcie wykonuje czynność, którą sugestia opisuje; odrzucenie zdejmuje ją z wykazu | `suggestionIds:string[]` (wym)<br>`accept:bool` (wym) | `appliedCount:int` (wym)<br>`rejectedCount:int` (wym)<br>`files:LibraryFile[]` (opc) |
| `library.suggestion.list` | Zwraca sugestie porządkujące oczekujące na decyzję Operatora | `fileId:string` (opc)<br>`kinds:LibrarySuggestionKind[]` (opc)<br>`limit:int` (opc) | `suggestions:LibrarySuggestion[]` (wym)<br>`total:int` (wym) |
| `library.tag.list` | Zwraca słownik etykiet repozytorium wraz z licznikiem użycia. Bez tej komendy klient składa słownik z etykiet plików odczytanej strony wykazu, więc pokazuje próbkę zamiast słownika | `query:string` (opc)<br>`unusedOnly:bool` (opc)<br>`limit:int` (opc) | `tags:LibraryTag[]` (wym)<br>`total:int` (wym) |
| `library.tag.merge` | Łączy etykiety duplikujące się w jedną. Zasoby noszące etykietę źródłową dostają docelową, a źródłowa znika ze słownika | `sourceNames:string[]` (wym)<br>`targetName:string` (wym) | `tag:LibraryTag` (wym)<br>`affectedFiles:int` (wym)<br>`mergedNames:string[]` (wym) |
| `library.tag.remove` | Usuwa etykietę ze słownika i ze wszystkich zasobów. Usunięcie etykiety używanej żąda potwierdzenia, bo zdejmuje ją z zasobów, których żądanie nie wymienia | `name:string` (wym)<br>`confirm:bool` (opc) | `removed:bool` (wym)<br>`affectedFiles:int` (wym) |
| `library.tag.set` | Nadaje pliku etykiety i przypisuje go do kolekcji | `fileId:string` (wym)<br>`tags:string[]` (opc)<br>`collectionIds:string[]` (opc) | `file:LibraryFile` (wym) |
| `library.tag.update` | Zmienia nazwę albo barwę etykiety w całym repozytorium. Zmiana nazwy przechodzi na wszystkie zasoby noszące etykietę — inaczej powstałaby druga etykieta o tym samym znaczeniu | `name:string` (wym)<br>`newName:string` (opc)<br>`color:string` (opc) | `tag:LibraryTag` (wym)<br>`affectedFiles:int` (wym) |
| `library.thesaurus.export` | Wywozi tezaurus etykiet wraz z relacjami w zapisie SKOS/RDF. Wywóz po stronie rdzenia obejmuje relacje, których klient nie ma skąd wziąć | `format:string` (opc)<br>`includeCollections:bool` (opc) | `content:string` (wym)<br>`format:string` (wym)<br>`conceptCount:int` (wym)<br>`relationCount:int` (wym) |
| `library.thesaurus.relate` | Ustanawia albo zdejmuje relację między dwiema etykietami w modelu SKOS. Bez relacji tezaurus repozytorium jest płaskim wykazem pojęć | `sourceName:string` (wym)<br>`targetName:string` (wym)<br>`relation:LibraryThesaurusRelation` (wym)<br>`remove:bool` (opc) | `relationSet:bool` (wym) |
| `library.version.add` | Dokłada nową wersję istniejącego pliku repozytorium | `fileId:string` (wym)<br>`contentBase64:string` (opc)<br>`sourcePath:string` (opc)<br>`label:string` (opc)<br>`author:string` (opc) | `version:LibraryVersion` (wym)<br>`file:LibraryFile` (wym) |
| `library.version.list` | Zwraca wersje dokumentu narastające przy każdej zmianie w dowolnym module | `fileId:string` (wym)<br>`limit:int` (opc) | `versions:LibraryVersion[]` (wym) |
| `library.version.restore` | Przywraca wcześniejszą wersję zasobu biblioteki (Versioning Panel) | `fileId:string` (wym)<br>`versionId:string` (wym) | `file:LibraryFile` (wym) |
| `library.webhook.list` | Zwraca nasłuchy zewnętrzne repozytorium | `enabledOnly:bool` (opc) | `webhooks:LibraryWebhook[]` (wym)<br>`total:int` (wym) |
| `library.webhook.remove` | Usuwa nasłuch zewnętrzny | `webhookId:string` (wym) | `removed:bool` (wym) |
| `library.webhook.set` | Zakłada albo zmienia nasłuch zewnętrzny na zdarzenia repozytorium | `webhook:LibraryWebhook` (wym) | `webhook:LibraryWebhook` (wym) |

**Zdarzenia obszaru `library` — 1:**

| Zdarzenie | Przeznaczenie | Pola |
|---|---|---|
| `library.file.changed` | Zmiana pliku repozytorium, w tym artefaktu przyjetego z innego modulu | `change:ChangeKind`, `file:LibraryFile` |


Razem w wykazie: **48 komend** z jednego obszaru kontraktu.

---

*Koniec dokumentu. Moduł Library — Specyfikacja docelowa, wersja 2.0, 2026-08-20.*
