...
| | |
|---|---|
| **Tytuł** | Moduł Terminal — pełnozakresowa dokumentacja projektowa |
...
| **Źródła** | Koncepcja platformy (rozdz. 2, 3, 4, 7.3, 9.11, 12, Załącznik A) · Specyfikacja modułów 1.1 (rozdz. 4.11) · Specyfikacja okien operacyjnych 1.1 (rozdz. 2.4, 3, 5, 6.11, 10) · System wizualny (rozdz. 8, 10) · Model danych (rozdz. 8, 15, 17) · Izolacja i zależności (rozdz. 1–6, Załącznik A) · Model konfiguracji (rozdz. 5, 6) |
| **Zasada nadrzędna** | Pełna kompozycyjność i pełna konfigurowalność (Koncepcja, rozdz. 14); zero blokad; klucze i parametry jawne; izolacja techniczna procesu jest jawną decyzją Operatora, nie wymogiem; domyślne zachowanie modułu = wykonanie |
...
|---|---|
| Deweloperzy backendu i frontendu | Uruchamianie poleceń budowania, instalacji zależności, serwerów deweloperskich |
...
| Zdalne wykonanie | SSH, SFTP, przekierowanie portów, agent SSH i ProxyJump, sesje na wielu hostach jednocześnie (broadcast), `docker exec`, `kubectl exec` |
...
| Wizualne budowanie przepływów automatyzacji między aplikacjami (nody, konektory) | Moduł **Automations** | Terminal automatyzuje polecenia powłoki, nie integracje między usługami zewnętrznymi |
...
| 5 | Process Monitor | Monitor procesu | Kolumna boczna, otwierana jako rozszerzenie boczne, na żywo | Obserwacja i kontrola uruchomionych procesów, drzewo procesów, metryki zasobów | 2 | Znacznik `[Procesy ▾]` w pasku kontekstu albo polecenie „pokaż procesy”; kolumna zwija się po zamknięciu |
...
| 7 | Task & Schedule | Monitor / edytor zadań | Prawa kolumna, dominująca — zakładka obszaru roboczego | Runner zadań, harmonogram, wyzwalacze, potoki, historia przebiegów | 2 | Zakładka obszaru roboczego wywoływana z elementu zbiorczego `Operacje ▾` |
...
- Generowanie polecenia powłoki na podstawie opisu w języku naturalnym (np. „znajdź procesy nasłuchujące na porcie 3000”).
...
- Cytowanie poleceń i wyniku w treści komunikacji blokiem `.dn-kod`, krojem mono.
- Załączanie plików (np. pliku logu) do wiadomości jako kontekstu zapytania.
...
|---|---|---|---|---|---|---|---|---|
| Wskaźnik kontekstu karty | Etykieta tekstowa w nagłówku | Pokazuje, której karty Terminal Tabs dotyczą bieżące polecenia | Mały tekst `--dn-tekst-3` z ikoną powłoki | domyślny · aktualizowany przy zmianie karty aktywnej | Kliknięcie otwiera listę kart do zmiany fokusu | Nagłówek Chat Window | 1 | Widoczny bez interakcji; kliknięcie rozwija listę kart (warstwa 2), która zwija się po wyborze |
...
| Blok proponowanego polecenia | `.dn-kod`, krój mono, tło drugorzędne | Prezentacja wygenerowanego polecenia powłoki | Średni blok w treści odpowiedzi | domyślny · zaznaczony do edycji | Kliknięcie w treść pozwala edytować polecenie przed wstawieniem | Treść odpowiedzi Wykonawcy | 1 | Widoczny bez interakcji jako element strumienia odpowiedzi |
| Przycisk „Wstaw do terminala” | Przycisk `--zarys`, mały | Przeniesienie polecenia do aktywnej karty bez uruchomienia | Mały przycisk tekstowy w pasku akcji | domyślny · najechanie | Wkleja polecenie w wierszu poleceń aktywnej karty, kursor gotowy do Enter | Pod blokiem proponowanego polecenia | 2 | Ujawnia się pod blokiem proponowanego polecenia i znika wraz z zamknięciem bloku |
| Przycisk „Uruchom teraz” | Przycisk `--zloty`, mały | Wykonanie polecenia bezpośrednio przez Wykonawcę | Mały przycisk CTA, wyróżniony akcentem złotym — sygnalizuje wagę akcji, nie blokuje jej wykonania | domyślny · ładowanie (polecenie w toku) | Uruchamia polecenie w aktywnej karcie; wynik trafia do Output Console, wpis w Process Monitor oznaczony jako „inicjator: Wykonawca” | Pod blokiem proponowanego polecenia | 2 | Ujawnia się pod blokiem proponowanego polecenia i znika wraz z zamknięciem bloku |
...
| Przycisk załącznika | Ikona (spinacz) | Dołączenie pliku (np. logu) do wiadomości | Mała ikona | domyślny · aktywny (plik dołączony) | Otwiera okno wyboru pliku lub akceptuje przeciągnięcie | Lewa strona pola poleceń | 2 | Ikona 📎 przy polu poleceń |
...
- Wyniki kontroli jakości uruchomienia: kod wyjścia, rozpoznane wzorce błędów, wynik lintera skryptu, porównanie z wynikiem poprzedniego przebiegu.
...
- Pasek broadcast — jedno wpisywane polecenie trafia równolegle do zaznaczonych paneli i hostów.
...
| Ikona zamknięcia karty | Mały „×” na karcie | Zamknięcie sesji powłoki | Mała ikona (zamknij), widoczna przy najechaniu na kartę | domyślny · potwierdzenie wymagane (proces aktywny) | Zamyka kartę natychmiast; gdy proces działa, otwiera `.dn-modal` z przyciskiem `--blad` sygnalizującym nieodwracalność, nie blokującym akcji | Prawa strona etykiety karty | 2 | Ujawnia się przy najechaniu na kartę |
...
| Przełącznik broadcast | Przycisk przełączający z licznikiem paneli | Kierowanie wpisu do wielu paneli i hostów jednocześnie | Mały przycisk tekstowy z plakietką liczby celów | wyłączony · włączony (obramowanie ostrzegawcze) | Wpis z klawiatury trafia do wszystkich zaznaczonych paneli | Pasek narzędzi karty | 4 | Polecenie języka naturalnego „rozgłaszaj polecenie na zaznaczone panele”, skrót klawiszowy albo wyszukiwarka funkcji `Ctrl/Cmd + K` |
...
| Pole Grep | Pole wyszukiwania z przełącznikami | Wyszukiwanie wzorca w zgromadzonym wyjściu | Pole `.dn-input` + ikony‑przełączniki (regex, wielkość liter) | puste · z wynikami · brak trafień | Enter uruchamia wyszukiwanie; podświetla i zlicza trafienia | Nagłówek okna | 2 | Ikona 🔍 w nagłówku albo skrót wyszukiwania; pole zwija się po wyczyszczeniu zapytania |
...
| Waga wizualna | Kolumna boczna, otwierana jako rozszerzenie boczne, stale widoczna, aktualizowana na żywo |
| Izolacja domyślna | Rejestr obejmuje procesy karty sesji bieżącej; widoczność procesów innych kart konfigurowalna |
| Warstwa widoczności | Warstwa 2 — otwierany znacznikiem `[Procesy ▾]` paska kontekstu albo poleceniem „pokaż procesy”; kolumna zwija się po zamknięciu |
...
- Drzewo procesów — hierarchia proces nadrzędny/potomny (np. proces menedżera pakietów uruchamiający proces środowiska wykonawczego).
...
```
 ═══════════════════════════════════════════════════════════════════════════
  Chat Window  │ Obszar roboczy modułu   │ Process Monitor (kolumna boczna)
...
| Panel szczegółów procesu | Rozwijany blok pod wierszem | Pełne polecenie, katalog roboczy, zmienne środowiskowe, drzewo potomków | Średni panel, tekst mono dla poleceń | zwinięty · rozwinięty | Rozwija się po kliknięciu wiersza; kod poleceń w `.dn-kod` | Pod wierszem procesu | 3 | Rozwinięcie kontekstowe wiersza — kliknięcie wiersza; zwija się ponownym kliknięciem |
...
| Warstwa widoczności | Warstwa 2 — zakładka obszaru roboczego wywoływana z elementu zbiorczego `Operacje ▾` |
...
- Zestaw plików konfiguracyjnych powłoki (dotfiles) wersjonowany i przenośny między hostami, zintegrowany z repozytorium przez moduł Developer.
...
| Obszar edytora | Edytor kodu z podświetlaniem składni | Redagowanie treści skryptu | Średni obszar tekstu mono | edycja · zapisany · z uwagami lintera | Zapis tworzy nową wersję skryptu w bibliotece | Środek kolumny | 1 | Widoczny bez interakcji po wybraniu skryptu |
...
| Wskaźnik lintera | Etykieta z liczbą uwag | Prezentuje wynik statycznej analizy skryptu | Mała etykieta z ikoną | bez uwag · uwagi · błędy składni | Kliknięcie rozwija listę uwag z przejściem do linii | Pod obszarem edytora | 1 | Widoczny bez interakcji pod obszarem edytora; lista uwag rozwija się jako panel warstwy 3 |
| Przycisk „Uruchom” | Przycisk `--zloty`, mały | Wykonanie skryptu w karcie powłoki | Mały przycisk CTA | domyślny · ładowanie | Uruchamia skrypt, wynik trafia do Output Console, wpis do Process Monitor | Stopka edytora | 1 | Widoczny bez interakcji w stopce edytora |
...
──────────────                                    ──────────────────────────
 ...
 polecenia (np. nieobsłużony wyjątek)
        │
        ▼
...
| `ustawienie` | Parametry modułu podlegające zasadzie „brak ustawienia = wartość domyślna” — np. limit buforowanych linii Output Console, próg powiadomienia o długo trwającym procesie, zasięg skanowania manifestów zadań |
...
| Dostęp sieciowy | Wyłączony (współdzielony) | Połączenia wychodzące poleceń uruchamianych w kartach (np. `curl`, `npm install`) | Ograniczenie sesji wykonującej skrypty z materiału niezaufanego |
...
Zgodnie z zasadą nadrzędną platformy żaden z powyższych zakresów nie jest wymuszony — moduł jest w pełni funkcjonalny bez jakiejkolwiek konfiguracji izolacji, a każdy zakres jest odwracalną, jawną decyzją Operatora podejmowaną z panelu macierzy izolacji (Koncepcja platformy, rozdz. 6.4).
...
Każda pozycja obejmuje nazwę funkcji, opis działania oraz zależności techniczne (biblioteki, formaty, integracje). Zależności backendowe realizuje rdzeń serwera w Go; warstwa terminala w kliencie oparta jest o `xterm.js`.
...
| Broadcast wpisu do wielu paneli | Jedno wpisywane polecenie trafia równolegle do zaznaczonych paneli i hostów | Multipleksacja strumieni wejścia w rdzeniu |
...
| Dotfiles i synchronizacja profilu | Wersjonowany zestaw plików konfiguracyjnych powłoki, przenośny między hostami | Repozytorium dotfiles; integracja z Git (moduł Developer) |
...
| Kalkulator poleceń i zmienne | Zmienne robocze i wyrażenia dostępne w powłoce (np. wynik poprzedniego polecenia) | Podstawienia w warstwie rdzenia; „ostatni wynik” jako zmienna |
...
| Bezpieczeństwo działania | Potwierdzenia wagi | Zakres akcji (zakończenie procesu z niezapisanym stanem, zamknięcie karty z procesem, przerwanie pętli) pokazujących modal sygnalizujący wagę — sygnał, nie blokada |
...
*Koniec dokumentu. Moduł Terminal — dokumentacja projektowa, wersja 2.0, 2026-08-06.*
