# Danaco Console — Moduł WorkSpace
...
| | |
|---|---|
| **Tytuł** | Moduł WorkSpace |
| **Klasa dokumentu** | Specyfikacja docelowa |
| **Odbiorcy** | deweloper · projektant · Operator |
| **Przeznaczenie** | Ustala zakres funkcjonalny, komplet okien operacyjnych i zachowanie modułu WorkSpace — jednostki organizacji pracy projektowej platformy — jako źródło wykonawcze dla dewelopera i projektanta. |
| **Zakres** | komplet okien operacyjnych modułu WorkSpace (Chat Window, Execution Loop Window, Project Dashboard, Instructions Panel, Context Memory, Project Library, Agent Manager), komponent własny (Projekt) strefy 2 strony głównej, katalog funkcji, komendy obszaru `workspace`, żetony i komponenty widoku, stany kontrolek, przebiegi pracy i scenariusze użycia |
| **Poza zakresem** | repozytorium centralne wieloprojektowe — [Moduł Library](library.md); zaawansowana edycja treści dokumentów — [Moduł Studio](studio.md); budowa i harmonogramowanie automatyk — [Moduł Automations](automations.md); tworzenie i globalna konfiguracja agentów — [Moduł Agents](agents.md); kod źródłowy i kontrola wersji — [Moduł Developer](developer.md) i [Moduł Terminal](terminal.md); grafika i edycja obrazów — [Moduł Design](design.md); tłumaczenia i badania wieloźródłowe — [Moduł Translate](translate.md) i [Moduł Research](research.md) (rozdz. 1.5) |
| **Dokument nadrzędny** | [Specyfikacja modułów](../specyfikacje/specyfikacja-modulow.md) |
| **Dokumenty powiązane** | [Moduł Library](library.md) · [Moduł Agents](agents.md) · [Moduł Automations](automations.md) · [Moduł Studio](studio.md) · [Moduł Design](design.md) · [Moduł Developer](developer.md) · [Moduł Terminal](terminal.md) · [Moduł Translate](translate.md) · [Moduł Research](research.md) · [Moduł Browser](browser.md) · [Model danych](../architektura/model-danych.md) · [Katalog komponentów](../interfejs-uzytkownika/katalog-komponentow.md) · [Model konfiguracji](../architektura/model-konfiguracji.md) |
| **Prototypy odniesienia** | `design/05-okna/moduly/workspace.html` |
| **Źródła normatywne** | `budowa/shared/contract.json` (obszar `workspace`) · `design/zasoby/zetony/zetony.css` · `design/zasoby/css/komponenty.css` · `design/05-okna/moduly/workspace.html` |
| **Zasada nadrzędna** | Zero blokad w interfejsie; klucze i zależności jawne; pełna konfigurowalność z okna konfiguracji; izolacja projektu jest ustawieniem Operatora, nie wymogiem |
...
1. [Przeznaczenie i kontekst](#1-przeznaczenie-i-kontekst)
   ...
   - [1.3 Po co](#13-po-co)
   - [1.4 Obszar pokrycia modułu](#14-obszar-pokrycia-modułu)
   - [1.5 Granica tematyczna modułu](#15-granica-tematyczna-modułu)
   - [1.6 Charakter pracy](#16-charakter-pracy)
...
2. [Komplet okien operacyjnych modułu — przegląd](#2-komplet-okien-operacyjnych-modułu--przegląd)
   ...
   - [2.2 Kanoniczne rozmieszczenie okien modułu](#22-kanoniczne-rozmieszczenie-okien-modułu)
3. [Specyfikacja okien operacyjnych — pełny arsenał narzędzi](#3-specyfikacja-okien-operacyjnych--pełny-arsenał-narzędzi)
   ...
   - [3.3 Project Dashboard](#33-project-dashboard)
   - [3.4 Instructions Panel](#34-instructions-panel)
   - [3.5 Context Memory](#35-context-memory)
   - [3.6 Project Library](#36-project-library)
   - [3.7 Agent Manager](#37-agent-manager)
4. [Katalog elementów interfejsu](#4-katalog-elementów-interfejsu)
5. [Przepływy pracy w module](#5-przepływy-pracy-w-module)
   - [5.1 Założenie nowego projektu i pierwsza konfiguracja](#51-założenie-nowego-projektu-i-pierwsza-konfiguracja)
   - [5.2 Zlecenie realizowane w pętli wykonawczej](#52-zlecenie-realizowane-w-pętli-wykonawczej)
   - [5.3 Codzienna praca równoległa nad wieloma projektami](#53-codzienna-praca-równoległa-nad-wieloma-projektami)
   - [5.4 Wykorzystanie agenta jako Wykonawcy zadania w projekcie](#54-wykorzystanie-agenta-jako-wykonawcy-zadania-w-projekcie)
   - [5.5 Świadome połączenie kontekstu dwóch projektów](#55-świadome-połączenie-kontekstu-dwóch-projektów)
6. [Stany, dane i powiązania z innymi modułami](#6-stany-dane-i-powiązania-z-innymi-modułami)
   ...
   - [6.2 Stany projektu jako komponentu własnego](#62-stany-projektu-jako-komponentu-własnego)
   - [6.3 Powiązania konfigurowalne z innymi modułami](#63-powiązania-konfigurowalne-z-innymi-modułami)
7. [Scenariusze użycia](#7-scenariusze-użycia)
   - [7.1 Kancelaria prowadząca równolegle kilka spraw klienckich](#71-kancelaria-prowadząca-równolegle-kilka-spraw-klienckich)
   - [7.2 Zespół produktowy łączący świadomie dwa powiązane projekty](#72-zespół-produktowy-łączący-świadomie-dwa-powiązane-projekty)
   - [7.3 Automatyzacja cyklicznego raportu projektowego](#73-automatyzacja-cyklicznego-raportu-projektowego)
   - [7.4 Migracja pracy z narzędzi zewnętrznych do jednego projektu](#74-migracja-pracy-z-narzędzi-zewnętrznych-do-jednego-projektu)
8. [Katalog funkcji i narzędzi](#8-katalog-funkcji-i-narzędzi)
   - [Grupa A — Projekty i przestrzeń robocza](#grupa-a--projekty-i-przestrzeń-robocza)
   - [Grupa B — Zadania, planowanie i harmonogram](#grupa-b--zadania-planowanie-i-harmonogram)
   - [Grupa C — Notatki, wiki i baza wiedzy](#grupa-c--notatki-wiki-i-baza-wiedzy)
   - [Grupa D — Pliki, repozytorium i zarządzanie dokumentami](#grupa-d--pliki-repozytorium-i-zarządzanie-dokumentami)
   - [Grupa E — Pamięć kontekstowa i wiedza dla Wykonawcy](#grupa-e--pamięć-kontekstowa-i-wiedza-dla-wykonawcy)
   - [Grupa F — Instrukcje, polecenia i szablony](#grupa-f--instrukcje-polecenia-i-szablony)
   - [Grupa G — Wykonawcy AI (Agent Manager)](#grupa-g--wykonawcy-ai-agent-manager)
   - [Grupa H — Wyszukiwanie, nawigacja i paleta poleceń](#grupa-h--wyszukiwanie-nawigacja-i-paleta-poleceń)
   - [Grupa I — Współpraca, komentarze i aktywność](#grupa-i--współpraca-komentarze-i-aktywność)
   - [Grupa J — Import, eksport, migracja i integracje](#grupa-j--import-eksport-migracja-i-integracje)
   - [Grupa K — Automatyzacja projektu i analityka](#grupa-k--automatyzacja-projektu-i-analityka)
9. [Punkty sterowania z okna konfiguracji](#9-punkty-sterowania-z-okna-konfiguracji)
...
11. [Załącznik — pełny wykaz komend kontraktu modułu WorkSpace](#załącznik--pełny-wykaz-komend-kontraktu-modułu-workspace)
...
Moduł jest jednym z elementów warstwy modułów, nie jej odpowiednikiem — nazwa modułu odnosi się wyłącznie do tej jednej, konkretnej, izolowanej przestrzeni projektowej.
...
| Project Dashboard | Ogólny widok stanu i postępu projektu, centrum planowania | Widok wiodący w obszarze roboczym, punkt wejścia do projektu | **Tak** | 1 | Widoczny bez interakcji — otwiera się po wybraniu projektu |
...
| Blok kodu w odpowiedzi | Podświetlanie składni, kopiowanie, przekazanie do modułu docelowego wskazanego w menu bloku | 1 | Widoczny bez interakcji w treści odpowiedzi; akcje bloku w menu kebab (`⋮`) |
...
| Pole | Treść |
|---|---|
| Cel | Ogólny widok stanu i postępu projektu wraz z centrum planowania — punkt wejścia otwierany po wybraniu projektu |
...
| Kafle nawigacyjne | Skróty do pozostałych okien modułu, z licznikiem pozycji w postaci „12 wpisów pamięci” albo „4 agentów” | 1 | Widoczne bez interakcji — siatka kafli okien modułu |
| Centrum planowania | Zakładki Lista / Kanban / Gantt / Kalendarz nad wspólnym zbiorem zadań projektu | 1 | Widoczny bez interakcji — pasek zakładek nad zbiorem zadań |
...
| Pulpit metryk | Kafle: zadania według statusu, obłożenie wykonawców, tempo prac, zbliżające się terminy, wykres wypalania iteracji | 2 | Zakładka `Metryki` w pasku zakładek dolnej części widoku |
...
| Operacje na projekcie | Duplikuj projekt, zarchiwizuj, eksportuj podsumowanie, usuń — akcja wykonuje się od razu, dla usunięcia dostępny mechanizm „Cofnij” po wykonaniu; okno nakładkowe potwierdzenia to ustawienie konfiguracyjne Operatora | 3 | Menu kebab (`⋮`) przy nagłówku projektu |
...
| Biblioteka szablonów instrukcji | Gotowe wzorce instrukcji do zastosowania i dalszej edycji — „redaktor techniczny”, „analityk danych” | 3 | Lista rozwijana `Szablony ▾` |
| Zmienne i odwołania dynamiczne | Wstawianie odwołań dynamicznych `{{nazwa_projektu}}` i `{{data}}` rozwijanych w czasie wykonania | 3 | Sekwencja `{{` w edytorze otwiera listę zmiennych |
...
| Kategoryzacja i tagi | Przypisanie własnych etykiet do wpisów — „decyzja”, „dane liczbowe”, „kontakt” | 3 | Menu kebab (`⋮`) karty wpisu |
...
| Udostępnienie do modułu zewnętrznego | Wysłanie pliku jako załącznika do sesji innego modułu — Design, Studio, Research | 3 | Menu kebab (`⋮`) pozycji → `Wyślij do modułu` |
...
|---|---|---|---|---|---|---|---|---|
| Kolumna Chat Window | Stała kolumna komunikacji, układ pionowy modułu | Główne okno komunikacji Użytkownik ↔ Wykonawca; sterowanie procesami modułu | Lewa kolumna, stała, pełna wysokość obszaru roboczego; regulowana szerokość | domyślny, zwężony, zwinięty do paska ikon | Regulacja szerokości uchwytem; zwinięcie nie przerywa strumienia | Wszystkie okna modułu | 1 | Widoczna bez interakcji |
| Kolumna Execution Loop Window | Kolumna pętli wykonawczej, rozszerzenie sąsiadujące | Prezentacja komunikacji Koordynator ↔ Wykonawca i sterowanie przebiegiem | Kolumna sąsiadująca z Chat Window, otwierana, pełna wysokość | zamknięta, otwarta, aktywny przebieg (obrys akcentowy), przeszkoda (obrys ostrzegawczy) | Otwarcie odsłania kolejkę zadań; sterowanie przebiegiem działa natychmiast | Wszystkie okna modułu | 2 | Kliknięcie zwiniętego wyzwalacza `Execution Loop ▸`; kolumna otwiera się samoczynnie z chwilą przyjęcia zlecenia |
| Karta zadania pętli | `.dn-karta--klikalna` z ikoną stanu | Prezentacja jednego zadania dekomponowanego ze zlecenia | Wiersz drzewa zadań | oczekujące, w realizacji, w kontroli, ukończone, ponawiane, przerwane | Kliknięcie rozwija komunikaty sterujące i wynik kontroli jakości | Execution Loop Window | 1 | Widoczna bez interakcji w drzewie zadań; komunikaty sterujące i wynik kontroli w warstwie 3 — kliknięcie karty |
| Kafel nawigacyjny okna | Karta `.dn-karta--klikalna` z licznikiem | Skrót do pozostałych okien modułu | Średni panel, siatka 4 kolumn w obszarze roboczym | domyślny, hover (tło), aktywny (obrys sygnałowy), ładowanie (szkielet) | Otwiera docelowe okno w tej samej karcie sesji | Project Dashboard | 1 | Widoczny bez interakcji |
| Plakietka statusu projektu | `.dn-kropka` z kropką koloru | Sygnalizacja stanu projektu (aktywny/wstrzymany/zarchiwizowany) | Mała pigułka, obok nagłówka | sukces (aktywny), ostrzeżenie (wstrzymany), neutralny (zarchiwizowany) | Kliknięcie otwiera menu zmiany statusu | Project Dashboard | 1 | Widoczna bez interakcji; menu zmiany statusu w warstwie 3 — kliknięcie plakietki |
| Przycisk „+ Dodaj wpis” | `.dn-btn--sygnal`, rozmiar `sm` | Jedyny, najważniejszy CTA okna Context Memory | Mały przycisk wypełniony, cień sygnałowy | domyślny, hover (uniesienie), aktywny (wciśnięcie) | Zawsze klikalny — otwiera formularz nowego wpisu inline; próba zapisania pustej treści sygnalizowana ostrzeżeniem, nie blokadą | Context Memory | 1 | Widoczny bez interakcji — jedyny stale widoczny przycisk wiodący okna |
| Karta wpisu pamięci | `.dn-karta--klikalna` | Prezentacja pojedynczego ustalenia | Wiersz listy, rozdzielony kreską | domyślny, przypięty (ikona pinezki), sugestia oczekująca (obrys akcentowy) | Kliknięcie rozwija treść i akcje (edytuj/usuń/przypnij) | Context Memory | 1 | Widoczna bez interakcji; akcje wpisu w warstwie 3 — menu kebab (`⋮`) |
...
| Ikona `[?]` objaśnienia kontekstowego | `.dn-tooltip`, wyzwalacz ikonowy 14 px | Wyjaśnienie działania ustawienia i jego wpływu na aplikację | Bardzo mała ikonka | domyślny, wskazanie kursorem albo fokus (dymek widoczny) | Wyświetla dymek z treścią objaśnienia | Wszystkie okna z ustawieniami | 2 | Ikona wyzwalacza; dymek pojawia się po wskazaniu kursorem lub uzyskaniu fokusu i znika po odsunięciu |
...
| Plakietka „domyślny wykonawca” | `.dn-plakietka--sygnal` | Oznaczenie agenta sugerowanego domyślnie | Bardzo mała pigułka | aktywna (widoczna) / nieaktywna (ukryta) | Kliknięcie „Ustaw jako domyślny” w menu agenta przełącza oznaczenie | Agent Manager | 1 | Widoczna bez interakcji przy agencie domyślnym; przełączenie w warstwie 3 — menu kebab (`⋮`) |
| Pasek postępu projektu | Pasek wypełnienia poziomego | Wizualizacja procentowego ukończenia zadań projektu | Cienki pasek pełnej szerokości panelu | 0–100%, kolor sygnałowy wypełnienia | Aktualizuje się automatycznie po zmianie statusu zadania | Project Dashboard | 1 | Widoczny bez interakcji |
| Pole wgrywania plików | Strefa przeciągnij-i-upuść z obrysem przerywanym | Wgranie nowego pliku do Project Library | Duży obszar w pustym stanie eksploratora | pusty (zachęta), wskazanie kursorem z plikiem (podświetlenie), wgrywanie (pasek postępu) | Upuszczenie pliku rozpoczyna wgrywanie i dodaje pozycję do listy | Project Library | 1 | Widoczne bez interakcji — obszar eksploratora przyjmuje upuszczone pliki |
...
| Okno nakładkowe potwierdzenia | `.dn-modal` z nakładką `.dn-modal` | Dodatkowe sygnalizowanie wagi akcji nieodwracalnej — usunięcia projektu; ustawienie konfiguracyjne Operatora, nie bramka domyślna | Duży panel wyśrodkowany | otwarty / zamknięty | Domyślnie akcja wykonuje się od razu, z dostępnym „Cofnij” po wykonaniu; gdy Operator włączy to ustawienie, okno nakładkowe poprzedza wykonanie | Project Dashboard (usuń, zarchiwizuj) | 4 | Ustawienie konfiguracyjne Operatora w oknie konfiguracji — element niewidoczny dla użytkownika podstawowego |
| Pole wyszukiwania | `.dn-pole` z ikoną lupy | Filtrowanie listy (plików, wpisów pamięci, agentów, zadań) | Pole tekstowe, pełna szerokość kolumny | pusty, wpisywanie, z wynikami, brak wyników | Filtruje listę na żywo podczas wpisywania | Context Memory, Project Library, Agent Manager, Project Dashboard | 2 | Ikona lupy w pasku okna; pole zwija się po wyczyszczeniu zapytania |
| Paleta poleceń | `.dn-modal`, wywołanie `Ctrl/Cmd + K` | Przejście do dowolnego okna, elementu lub akcji z klawiatury | Nakładka wyśrodkowana z polem i listą trafień | zamknięta, otwarta, wyniki rozmyte, brak wyników | Wybór pozycji wykonuje akcję lub otwiera element | Wszystkie okna modułu | 4 | Skrót klawiszowy `Ctrl/Cmd + K` — wyszukiwarka funkcji obejmująca również operacje eksperckie |
| Wskaźnik zajętości (pamięć/pojemność) | Mały pasek lub pierścień procentowy | Orientacyjna informacja o wykorzystaniu limitu | Bardzo mały element, róg panelu | poniżej progu / blisko limitu (kolor ostrzegawczy) | Wskazanie kursorem pokazuje wartość liczbową w dymku | Context Memory, Project Library | 1 | Widoczny bez interakcji — wskaźnik w rogu panelu; wartość liczbowa w dymku po wskazaniu kursorem |
...
2. Dla każdego projektu definiuje odrębne instrukcje systemowe w Instructions Panel — rejestr terminologii właściwy danej sprawie.
...
|---|---|---|---|
| 1 | Kreator projektu | Zakłada projekt z nazwą, opisem, ikoną, kolorem, celem i profilem izolacji; projekt jest operacyjny od razu, przed uzupełnieniem konfiguracji | SQLite (encja `project`), `google/uuid`; komunikat `project.create` ([Model konfiguracji](../architektura/model-konfiguracji.md), rozdz. 5.13) |
| 2 | Szablony projektów | Zakłada projekt z gotowego wzorca (zestaw zadań, folderów, instrukcji, tagów) — „sprawa kliencka”, „raport cykliczny”, „badanie” | JSON szablonu w SQLite; `text/template` do rozwinięcia zmiennych |
| 3 | Duplikacja projektu | Klonuje projekt w całości lub wybiórczo (struktura bez treści / z treścią) | Transakcja SQLite; kopiowanie plików treści |
| 4 | Archiwizacja i wznowienie | Przenosi projekt w stan „zarchiwizowany” (podgląd, bez pracy) i z powrotem, bez utraty danych; „Cofnij” po akcji | Pole stanu w SQLite; brak twardego usuwania |
| 5 | Miękkie usuwanie z „Cofnij” | Usuwa projekt z natychmiastowym mechanizmem odtworzenia; okno nakładkowe potwierdzenia to ustawienie konfiguracyjne Operatora, nie bramka | Kolejka „soft-delete” w SQLite z TTL; `robfig/cron` do czyszczenia |
| 6 | Karty sesji projektu | Prowadzi wiele równoległych kart sesji jednego projektu; przełączanie bez mieszania historii i pamięci | Izolacja per karta ([Model konfiguracji](../architektura/model-konfiguracji.md), rozdz. 5.12); stan trwały ([Architektura techniczna](../architektura/architektura.md), rozdz. 3) |
| 7 | Profil izolacji projektu | Przypisuje projektowi profil rozdzielenia kontekstu, pamięci i historii (od „zamknięty” po „współdzielony”) | Okno konfiguracji punktów izolacji ([Model konfiguracji](../architektura/model-konfiguracji.md), rozdz. 6) |
| 8 | Eksport podsumowania projektu | Generuje jednoplikowe podsumowanie stanu (zadania, pliki, ustalenia, oś czasu) do Markdown/PDF | `yuin/goldmark` (MD→HTML), `go-fitz`/maroto (PDF); trafia do Project Library |

### Grupa B — Zadania, planowanie i harmonogram

| # | Nazwa | Co robi | Zależności |
|---|---|---|---|
| 1 | Widok listy zadań | Lista zadań z polami: tytuł, status, wykonawca, priorytet, termin, etykiety, podzadania | SQLite (encja `task`); WebSocket na żywo |
| 2 | Tablica kanban | Kolumny według statusu, przeciąganie kart, limity zadań w toku, tory według wykonawcy | Model kolumn w JSON; kolejność przez pole rank |
| 3 | Oś czasu / Gantt | Wykres słupkowy zadań w czasie, przeciąganie ram, zależności „koniec-początek” | Obliczenia ścieżki na rdzeniu Go; render po stronie klienta |
| 4 | Kalendarz projektu | Zadania i kamienie milowe w siatce miesiąc/tydzień/dzień; eksport i import iCal | `arran4/golang-ical` (format `.ics`) |
| 5 | Zależności zadań | Definiuje relacje poprzednik/następnik, blokady, automatyczne przesunięcia terminów | Graf zależności w SQLite; walidacja cykli w Go |
| 6 | Podzadania i listy kontrolne | Rozbicie zadania na kroki z paskiem postępu; szablony list kontrolnych | Encja `subtask`; agregacja postępu |
| 7 | Priorytety i macierz ważności | Etykiety pilności i ważności, widok macierzy 2×2 | Pola priorytetu; widok filtrowany |
| 8 | Iteracje | Grupuje zadania w okresy z celem iteracji, prędkością i przeglądem | Encja `sprint`; liczniki agregujące |
| 9 | Kamienie milowe i cele | Definiuje punkty kontrolne i cele mierzalne powiązane z zadaniami | Encja `milestone`; procent ukończenia |
| 10 | Przypomnienia i terminy | Powiadomienia o zbliżających się i przekroczonych terminach; kanałem WebSocket i push | `robfig/cron`; kolejka powiadomień; komunikat `notify` |
| 11 | Zadania cykliczne | Definiuje powtarzalność (dzienna, tygodniowa, reguła RRULE) | `arran4/golang-ical` (RRULE); scheduler Go |
| 12 | Przypisanie wykonawcy (agent/użytkownik) | Wskazuje agenta z Agent Managera lub użytkownika jako odpowiedzialnego; zlecenie realizacji wprost z karty zadania | Powiązanie z Agent Manager; Chat Window jako kanał zlecenia, Execution Loop Window jako kanał nadzoru |
| 13 | Śledzenie czasu | Rejestruje czas pracy nad zadaniem (ręcznie lub stoperem); raport sumaryczny | Encja `time_entry`; agregacja w Go |
| 14 | Automatyzacje tablicy | Reguły „gdy status = X, wykonaj Y” (przenieś, przypisz, oznacz), lokalnie lub przez powiązanie z Automations | Silnik reguł Go; powiązanie z modułem Automations |

### Grupa C — Notatki, wiki i baza wiedzy

| # | Nazwa | Co robi | Zależności |
|---|---|---|---|
| 1 | Edytor notatek Markdown | Pełny edytor WYSIWYG/Markdown: nagłówki, listy, tabele, bloki kodu, cytaty, listy kontrolne | `yuin/goldmark` (+ rozszerzenia GFM); zapis jako plik treści |
| 2 | Dokumenty projektowe (strony) | Hierarchia stron zagnieżdżonych z drzewem nawigacji; strona jako notatka długa | Drzewo w SQLite; pliki treści `.md` |
| 3 | Odnośniki wsteczne | Automatyczne wiązanie stron przez `[[nazwa]]`; panel „co linkuje tutaj” | Parser wikilinków w Go; graf w SQLite |
| 4 | Graf wiedzy | Wizualizuje sieć powiązań notatek, zadań i plików projektu | Graf z odnośników wstecznych; render po stronie klienta |
| 5 | Tablica wizualna | Nieskończone płótno z kartkami, strzałkami, grupami; osadzanie zadań i plików | Model sceny JSON w SQLite; render na kliencie |
| 6 | Mapy myśli | Rozgałęziona struktura węzłów wokół tematu; konwersja do listy zadań | Struktura drzewa JSON; eksport do zadań („Widok listy zadań”) |
| 7 | Bloki osadzane | Wstawia do notatki żywy widok zadania, pliku, wpisu pamięci lub grafiki | System referencji `@`; render żywy przez WebSocket |
| 8 | Szablony notatek | Wielokrotne wzorce (protokół spotkania, notatka decyzyjna, brief) z polami | `text/template`; biblioteka szablonów w SQLite |
| 9 | Adnotacje i wyróżnienia | Zaznaczanie fragmentów, komentarze marginesowe, kolory wyróżnień | Zakresy tekstu w SQLite; warstwa nakładki na kliencie |
| 10 | Bloki kodu z podświetlaniem | Notatki techniczne z kolorowaniem składni i kopiowaniem; przekazanie do Developer | `alecthomas/chroma`; powiązanie z Developer |
| 11 | Osadzanie tabel i arkuszy | Lekka tabela danych z sortowaniem i filtrem wewnątrz notatki | Model tabeli JSON; eksport CSV `encoding/csv` |
| 12 | Zakładki i wycinki webowe | Zapis odnośnika z metadanymi, miniaturą i notatką; odbiór wycinków z modułu Browser | `mimetype`; powiązanie z Browser (Notes/Sources Panel) |
| 13 | Dziennik / notatki dzienne | Nota per dzień z agregacją zdarzeń projektu | Generacja z osi czasu; `robfig/cron` |
| 14 | Spis treści i nawigacja notatki | Automatyczny spis nagłówków z przewijaniem; zwijanie sekcji | Parser nagłówków `goldmark` (AST) |

### Grupa D — Pliki, repozytorium i zarządzanie dokumentami

| # | Nazwa | Co robi | Zależności |
|---|---|---|---|
| 1 | Eksplorator plików (siatka/lista) | Przegląda pliki projektu w folderach, dwa widoki, sortowanie, kolumny metadanych | SQLite (metadane) + pliki treści; `spf13/afero` (abstrakcja FS) |
| 2 | Przeciągnij i upuść / wgrywanie | Wgrywa pliki z urządzenia, wiele naraz, pasek postępu, wznawianie | Chunkowany transfer WebSocket; zapis pliku treści |
| 3 | Foldery i struktura | Tworzy zagnieżdżone foldery, przenosi, zmienia nazwy | Drzewo folderów w SQLite |
| 4 | Tagi i kolekcje | Nadaje etykiety wielokrotne i grupuje pliki w kolekcje tematyczne niezależne od folderów | Relacja tag↔plik w SQLite |
| 5 | Podgląd plików w oknie | Renderuje bez pobierania: obrazy, PDF, DOCX, TXT/MD, CSV, audio, wideo | `go-fitz` (PDF→obraz), `mimetype`, `disintegration/imaging` (obrazy) |
| 6 | Ekstrakcja tekstu (PDF/DOCX/TXT) | Wydobywa treść tekstową dokumentu do indeksu i podglądu | PDF: `ledongthuc/pdf` (czysty Go) albo `gen2brain/go-fitz` (MuPDF, CGo); DOCX: `nguyenthenguyen/docx` albo `unidoc/unioffice` |
| 7 | OCR obrazów i skanów | Rozpoznaje tekst z obrazów i skanów PDF, dokłada do indeksu wyszukiwania | `otiai10/gosseract` (Tesseract), języki `pol`+`eng` |
| 8 | Wyszukiwanie pełnotekstowe | Przeszukuje nazwy i treść wszystkich materiałów projektu, z podświetleniem trafień | SQLite FTS5 jako mechanizm domyślny (spójny z jednym plikiem danych); `blevesearch/bleve` przy bogatszych zapytaniach i fasetach; indeks inkrementalny |
| 9 | Wersjonowanie dokumentów | Historia wersji pliku (ręcznych i generowanych przez Wykonawcę), porównanie, przywrócenie | Snapshoty treści; `sergi/go-diff` |
| 10 | Porównanie wersji (diff) | Pokazuje różnice między dwiema wersjami dokumentu tekstowego linia po linii | `sergi/go-diff` (diffmatchpatch) albo `hexops/gotextdiff` (format unified) |
| 11 | Deduplikacja i wykrywanie duplikatów | Wykrywa identyczne pliki po skrócie treści, przedstawia scalenie | `crypto/sha256`; indeks skrótów w SQLite |
| 12 | Akcje zbiorcze | Zaznacza wiele plików: pobierz, przenieś, usuń, taguj, spakuj do ZIP | `archive/zip` (stdlib); transakcja SQLite |
| 13 | Odbiór artefaktów | Wyniki pracy Wykonawcy z Chat Window, Execution Loop Window i powiązanych modułów trafiają automatycznie do wskazanego folderu | Reguła kierowania w konfiguracji; komunikat `artifact.created` |
| 14 | Udostępnienie do modułu zewnętrznego | Wysyła plik jako załącznik do sesji modułów Studio, Design i Research | Powiązania konfigurowalne ([Specyfikacja modułów](../specyfikacje/specyfikacja-modulow.md), rozdz. 4.2) |
| 15 | Synchronizacja z Library | Wybiórcza synchronizacja plików projektu z repozytorium centralnym Library | Powiązanie Project Library ↔ Library; kolejka sync |
| 16 | Wskaźnik pojemności i limity | Pokazuje wykorzystanie limitu pamięci masowej projektu; miękkie ostrzeżenia, nie blokada | Agregacja rozmiarów w SQLite; próg z konfiguracji |

### Grupa E — Pamięć kontekstowa i wiedza dla Wykonawcy

| # | Nazwa | Co robi | Zależności |
|---|---|---|---|
| 1 | Lista wpisów pamięci | Karty ustaleń z treścią, znacznikiem czasu, źródłem (sesja/użytkownik/agent) | Encja `memory_entry` w SQLite |
| 2 | Ręczne dodawanie ustaleń | Formularz nowego faktu lub decyzji z kategorią i zasięgiem | SQLite; pola kategorii |
| 3 | Sugestie Wykonawcy z rozmowy | Wykonawca przedstawia zapis nowego ustalenia z toczącej się rozmowy; akceptacja, edycja albo odrzucenie | Analiza wątku w rdzeniu; stan „sugestia oczekująca” |
| 4 | Wyszukiwanie semantyczne | Znajduje wpisy znaczeniowo bliskie zapytaniu, nie tylko po słowach kluczowych | Adapter osadzeń ([Integracja modeli](../architektura/integracja-modeli.md)) + `asg017/sqlite-vec` (rozszerzenie SQLite, spójne z jednym plikiem danych) albo `philippgille/chromem-go` (baza wektorowa w czystym Go) |
| 5 | Kategoryzacja i tagi wpisów | Etykiety własne („decyzja”, „liczba”, „kontakt”), filtr po tagu, dacie i źródle | Relacja tag↔wpis w SQLite |
| 6 | Przypinanie priorytetowych | Utrzymuje kluczowe ustalenia na początku listy i zawsze w kontekście Wykonawcy | Pole „pinned”; kolejność |
| 7 | Scalanie i porządkowanie wpisów | Łączy powiązane wpisy w jeden spójny zapis; usuwa sprzeczności | Operacja merge; historia zmian wpisu |
| 8 | Poziom zasięgu wpisu | Podnosi wpis z poziomu projektu do środowiska lub poziomu globalnego (świadoma decyzja) | Warstwowość pamięci ([Model konfiguracji](../architektura/model-konfiguracji.md), rozdz. 5.10 i 6.2) |
| 9 | Wskaźnik zajętości pamięci i eksport | Pokazuje wykorzystanie pojemności; eksportuje pamięć do dokumentu w Project Library | Agregacja; `goldmark` do eksportu MD |

### Grupa F — Instrukcje, polecenia i szablony

| # | Nazwa | Co robi | Zależności |
|---|---|---|---|
| 1 | Edytor instrukcji systemowych | Pełnoekranowa edycja instrukcji projektu: Markdown, numeracja linii, zawijanie | `goldmark`; zapis na warstwie projektu ([Model konfiguracji](../architektura/model-konfiguracji.md), rozdz. 5.6) |
| 2 | Biblioteka szablonów instrukcji | Gotowe wzorce ról („redaktor techniczny”, „analityk”) do zastosowania i edycji | Szablony w SQLite |
| 3 | Zmienne i odwołania dynamiczne | Odwołania dynamiczne `{{nazwa_projektu}}` i `{{data}}` rozwijane w czasie wykonania | `text/template` |
| 4 | Wersjonowanie instrukcji | Lista wersji z autorem i datą, porównanie (diff), przywrócenie | Snapshoty + `sergi/go-diff` |
| 5 | Podgląd instrukcji efektywnej | Pokazuje wynikową instrukcję po dziedziczeniu z warstw szerszych (środowisko, globalna) | Rozwinięcie warstw ([Model konfiguracji](../architektura/model-konfiguracji.md), rozdz. 4) |
| 6 | Test instrukcji w Chat | Kieruje zapytanie próbne z bieżącą wersją bez zapisu do historii właściwej | Kanał testowy Chat Window; brak zapisu |
| 7 | Licznik długości i tokenów | Liczba znaków i przybliżona liczba tokenów instrukcji | `pkoukk/tiktoken-go` |
| 8 | Menedżer snippetów i poleceń | Zapisane, wielokrotne polecenia i fragmenty tekstu wstawiane skrótem | SQLite; wywołanie z kompozytora Chat Window |
| 9 | Nadpisania per agent | Wskazuje, czy agent używa instrukcji projektu, czy własnych (z Agent Builder) | Powiązanie Agent Manager ↔ Instructions Panel |

### Grupa G — Wykonawcy AI (Agent Manager)

| # | Nazwa | Co robi | Zależności |
|---|---|---|---|
| 1 | Lista i przypisanie agentów | Pokazuje agentów z modułu Agents; przypisuje ich do projektu (przeciągnięcie lub przycisk) | Powiązanie z modułem Agents |
| 2 | Zakres uprawnień w projekcie | Ustala, do których okien (Chat Window, Execution Loop Window, Project Library, Context Memory) agent ma dostęp; klucze jawne | Uprawnienia jawne, bez ukrytych bram |
| 3 | Domyślny wykonawca | Oznacza jednego agenta jako sugerowanego w Chat Window i w pętli wykonawczej | Pole „default executor” |
| 4 | Dziennik działań agenta | Chronologiczny zapis czynności agenta w kontekście projektu | Encja `agent_activity` |
| 5 | Skrót do Agent Builder | Tworzy nowego agenta bez opuszczania projektu | Głębokie powiązanie z modułem Agents |
| 6 | Skrót do Permissions Center | Przegląd i zmiana uprawnień platformowych agenta | Powiązanie z Permissions Center |
| 7 | Zasięg przypisania | Wskazuje, czy przypisanie obowiązuje tylko ten projekt, czy także inne (decyzja użytkownika) | Warstwowość zasięgu ([Model konfiguracji](../architektura/model-konfiguracji.md), rozdz. 6.2) |

### Grupa H — Wyszukiwanie, nawigacja i paleta poleceń

| # | Nazwa | Co robi | Zależności |
|---|---|---|---|
| 1 | Wyszukiwanie globalne projektu | Jedno pole przeszukujące zadania, notatki, pliki, wpisy pamięci i instrukcje | `bleve` / FTS5 nad wszystkimi encjami |
| 2 | Paleta poleceń (`Ctrl/Cmd + K`) | Szybkie przejście do dowolnego okna, elementu lub akcji z klawiatury | Indeks akcji; dopasowanie rozmyte `sahilm/fuzzy` |
| 3 | Wyszukiwanie rozmyte | Toleruje literówki i częściowe dopasowania nazw | `sahilm/fuzzy` |
| 4 | Filtry zapisane i widoki własne | Zapisuje kombinacje filtrów jako nazwany widok — „moje pilne zadania” | Definicje widoków w SQLite |
| 5 | Ostatnio używane i ulubione | Lista ostatnio otwartych i przypiętych elementów projektu | Rejestr dostępu; pole „favorite” |
| 6 | Nawigacja skrótami klawiszowymi | Kompletny zestaw skrótów (przełączanie okien, tworzenie zadań i notatek, wyszukiwanie) | Mapa skrótów konfigurowalna |
| 7 | Powiązania krzyżowe elementów | Łączy zadanie z notatką, plikiem lub wpisem pamięci referencją `@` | Tabela referencji uniwersalnych |

### Grupa I — Współpraca, komentarze i aktywność

| # | Nazwa | Co robi | Zależności |
|---|---|---|---|
| 1 | Oś czasu aktywności | Chronologiczny zapis zdarzeń projektu (zmiany instrukcji, wpisy pamięci, pliki, zadania, przebiegi pętli wykonawczej) | Log zdarzeń; WebSocket na żywo |
| 2 | Komentarze do elementów | Wątki komentarzy przy zadaniu, notatce lub pliku; wzmianki `@` | Encja `comment`; powiązania `@` |
| 3 | Wzmianki kontekstowe `@` | Odwołania do pliku, wpisu pamięci, zadania lub agenta wprost w treści polecenia i komentarza | Parser `@`; render plakietek |
| 4 | Notatka właściciela projektu | Wolne pole opisowe: cel, uwagi, przypomnienia widoczne na dashboardzie | Pole tekstowe projektu |
| 5 | Eksport rozmowy do biblioteki | Zapisuje wątek Chat Window jako dokument (MD/PDF) do Project Library | `goldmark` + PDF; zapis pliku |
| 6 | Oznaczanie ustaleń projektu | Oznacza wiadomość czatu jako ustalenie i przypina do Context Memory | Powiązanie Chat Window ↔ Context Memory |
| 7 | Powiadomienia projektu | Zbiorczy strumień powiadomień (terminy, wzmianki, zakończone zlecenia pętli wykonawczej) | Kolejka `notify`; WebSocket + push |

### Grupa J — Import, eksport, migracja i integracje

| # | Nazwa | Co robi | Zależności |
|---|---|---|---|
| 1 | Import z Markdown/folderu | Wciąga zbiór plików `.md` z zachowaniem struktury folderów jako notatki | Rekurencyjny odczyt; `goldmark` |
| 2 | Import eksportu MD/CSV | Wczytuje eksport w formacie Markdown wraz z bazami CSV do stron i tabel | Parser MD + `encoding/csv` |
| 3 | Import tablic z JSON | Odtwarza tablice, listy i karty z eksportu JSON | Parser JSON; mapowanie na zadania |
| 4 | Import kalendarza (iCal) | Wciąga wydarzenia `.ics` jako zadania z terminem | `arran4/golang-ical` |
| 5 | Eksport projektu (paczka) | Pakuje cały projekt (pliki + JSON struktury) do archiwum przenośnego | `archive/zip`; manifest JSON |
| 6 | Eksport do PDF/DOCX | Renderuje notatkę lub podsumowanie do PDF albo DOCX | maroto / `go-fitz` (PDF), `unioffice` (DOCX) |
| 7 | Eksport tabel do CSV/XLSX | Zapisuje tabele zadań i danych do arkusza | `encoding/csv`, `xuri/excelize` (XLSX) |
| 8 | Wiązanie projektu z Automations | Spina projekt z gotową automatyką — cyklicznym raportem do Project Library | Powiązanie z [Modułem Automations](automations.md) ([Specyfikacja modułów](../specyfikacje/specyfikacja-modulow.md), rozdz. 4.2) |
| 9 | Udostępnienie przez WebDAV | Udostępnia repozytorium projektu przez WebDAV klientom zewnętrznym; sterowane ustawieniem konfiguracyjnym | `golang.org/x/net/webdav` |
| 10 | Obserwacja zmian plików | Wykrywa zmiany w powiązanym folderze lokalnym i aktualizuje bibliotekę | `fsnotify/fsnotify` |

### Grupa K — Automatyzacja projektu i analityka

| # | Nazwa | Co robi | Zależności |
|---|---|---|---|
| 1 | Pasek postępu projektu | Procent ukończenia na podstawie zamkniętych zadań i kamieni milowych | Agregacja w Go; render klienta |
| 2 | Pulpit metryk projektu | Kafle: zadania według statusu, obłożenie wykonawców, tempo, zbliżające się terminy | Zapytania agregujące SQLite |
| 3 | Wykres wypalania | Wizualizuje postęp iteracji w czasie | Szereg czasowy z logu zadań |
| 4 | Raport obciążenia wykonawców | Rozkład zadań i czasu na agentów i użytkowników | Agregacja `time_entry` + przypisań |
| 5 | Reguły automatyczne projektu | „Gdy zdarzenie X → akcja Y” lokalnie, bez modułu Automations | Silnik reguł Go; log wykonania |
| 6 | Cykliczne migawki stanu | Zapisuje okresową migawkę projektu do audytu i porównań | `robfig/cron`; snapshot do Project Library |
| 7 | Cele i wskaźniki | Śledzi mierzalne cele projektu z procentem realizacji | Encja `goal`; agregacja |
| 8 | Objaśnienia kontekstowe `[?]` | Przy każdym ustawieniu dymek wyjaśniający działanie i wpływ na aplikację | Mechanizm `.dn-tooltip`; treść jako część definicji ustawienia |
...
| Zakres (według [Modelu konfiguracji](../architektura/model-konfiguracji.md)) | Co Operator personalizuje | Domyślna |
|---|---|---|
...
| Aplikacja / UX | Zachowanie „Cofnij” wobec okna nakładkowego potwierdzenia usuwania; skróty klawiszowe; szerokości kolumn obszaru roboczego; domyślny widok centrum planowania (Lista/Kanban/Gantt/Kalendarz); kolumny kanban i stany zadań; folder odbioru artefaktów | „Cofnij” / Lista / stany domyślne |
...
Moduł WorkSpace dzieli obszar roboczy na kolumny sąsiadujące poziomo (rozdz. 2). Poniższe progi, wspólne całej platformie, rozstrzygają zachowanie układu tych kolumn wraz ze zmianą szerokości okna aplikacji.

| Żeton | Próg szerokości | Zachowanie układu w module WorkSpace |
|---|---|---|
...
| `--dn-bp-w2` | 960 px | Tablet — boczna nawigacja modułów zwija się do samych ikon; kolumny modułu WorkSpace zachowują układ, lecz kolumny boczne (rozszerzenia warstwy 2–3) otwierają się jako nakładka zamiast stałej kolumny |
| `--dn-bp-w3` | 1280 px | Biurko — pełny kokpit; wszystkie okna operacyjne modułu WorkSpace wymienione w rozdz. 2 dostępne jednocześnie w układzie kolumnowym opisanym w tym rozdziale |
...
Poniżej progu `--dn-bp-w1` układ kolumnowy modułu WorkSpace nie jest dostępny — zachowanie na urządzeniach mobilnych ustala [Mobile](../funkcje-globalne/mobile.md).

### 10.2. Kryteria odbioru

Warunki sprawdzalne, których łączne spełnienie oznacza gotowość modułu WorkSpace do odbioru.

| Warunek sprawdzalny | Sposób sprawdzenia |
|---|---|
| Wszystkie okna operacyjne z rozdz. 3 (Chat Window, Execution Loop Window, Project Dashboard, Instructions Panel, Context Memory, Project Library, Agent Manager) otwierają się dokładnie sposobem wywołania opisanym w tej specyfikacji | Przegląd manualny wg rozdz. 3 — każde okno wywołane, sprawdzone wejście i zamknięcie |
| Każda z 59 komend obszaru `workspace` z Załącznika ma pokrycie w co najmniej jednym przebiegu pracy albo scenariuszu użycia (rozdz. 5, 7) | Zestawienie nazw komend z treścią rozdz. 5 i 7 |
| Izolacja kontekstu, pamięci i historii między dwoma projektami jest w pełni konfigurowalna z okna konfiguracji i nie jest wymuszona jako warunek startu (rozdz. 1.3) | Przegląd opisu profilu izolacji w rozdz. 1.3 i 6.3 |
...
| Komponent własny „Projekt” strefy 2 strony głównej i okno modułowe WorkSpace w trzech środowiskach pozostają dwiema drogami wejścia do tej samej, spójnej przestrzeni projektu (rozdz. 1.7) | Przegląd opisu podwójnej drogi wejścia w rozdz. 1.7 |


---

## Załącznik — pełny wykaz komend kontraktu modułu WorkSpace
...
| `workspace.agent.access.set` | Ustala, do których okien projektu przypisany agent ma dostęp, i zasięg przypisania; klucze okien są jawne | `projectId:string` (wym)<br>`agentId:string` (wym)<br>`windowAccess:string[]` (wym)<br>`scope:ConfigScope` (opc) | `assignment:WorkspaceAgentAssignment` (wym) |
| `workspace.agent.assign` | Przypisuje agenta jako wykonawcę zadań w projekcie | `projectId:string` (wym)<br>`agentId:string` (wym)<br>`role:string` (opc)<br>`defaultExecutor:bool` (opc)<br>`scope:ConfigScope` (opc)<br>`scopeId:string` (opc) | `assignment:WorkspaceAgentAssignment` (wym) |
| `workspace.agent.assignment.list` | Zwraca przypisania agentów do projektu wraz z rolą, wskazaniem domyślnego wykonawcy, zakresem dostępu do okien i zasięgiem przypisania | `projectId:string` (wym) | `assignments:WorkspaceAgentAssignment[]` (wym) |
| `workspace.agent.unassign` | Odłącza agenta od projektu. Nie usuwa agenta z biblioteki modułu Agents — znosi wyłącznie jego przypisanie do tego projektu | `projectId:string` (wym)<br>`agentId:string` (wym) | `unassigned:bool` (wym) |
| `workspace.board.get` | Zwraca tablicę kanban projektu — kolumny wraz z kartami w kolejności | `projectId:string` (wym)<br>`includeDone:bool` (opc) | `board:WorkspaceBoard` (wym) |
...
| `workspace.instructions.set` | Zapisuje instrukcje systemowe projektu | `projectId:string` (wym)<br>`content:string` (wym)<br>`scope:ConfigScope` (opc)<br>`scopeId:string` (opc) | `instructions:WorkspaceInstructions` (wym) |
...
| `workspace.library.version.compare` | Porównuje dwie wersje dokumentu projektu linia po linii. Brak drugiej wersji porównuje wskazaną wersję z treścią bieżącą pliku | `projectId:string` (wym)<br>`fileId:string` (wym)<br>`versionId:string` (wym)<br>`otherVersionId:string` (opc) | `lines:WorkspaceDiffLine[]` (wym)<br>`addedCount:int` (wym)<br>`removedCount:int` (wym)<br>`binary:bool` (opc) |
...
| `workspace.note.backlink.list` | Zwraca odnośniki wsteczne strony — panel „co linkuje tutaj” | `noteId:string` (wym)<br>`limit:int` (opc)<br>`offset:int` (opc) | `backlinks:WorkspaceBacklink[]` (wym)<br>`total:int` (wym) |
...
| `workspace.project.duplicate` | Klonuje projekt: instrukcje, wpisy pamięci, przypisania agentów i zadania trafiają do projektu nowego pod nową nazwą | `projectId:string` (wym)<br>`name:string` (opc) | `project:WorkspaceProject` (wym)<br>`copiedTaskCount:int` (wym)<br>`copiedMemoryEntryCount:int` (wym) |
| `workspace.project.export` | Składa jednoplikowe podsumowanie stanu projektu w Markdown — zadania, pliki, ustalenia, agentów i oś czasu — i odkłada je jako plik biblioteki projektu | `projectId:string` (wym) | `fileName:string` (wym)<br>`content:string` (wym)<br>`file:LibraryFile` (opc) |
| `workspace.project.list` | Zwraca projekty przestrzeni roboczej — wykaz do wyboru projektu w Project Dashboard i bocznej nawigacji | `query:string` (opc)<br>`includeArchived:bool` (opc)<br>`includeDeleted:bool` (opc)<br>`limit:int` (opc) | `projects:WorkspaceProject[]` (wym)<br>`total:int` (wym) |
...
|---|---|---|
| `workspace.project.changed` | Zmiana projektu przestrzeni roboczej | `change:ChangeKind`, `project:WorkspaceProject` |
...
*Koniec dokumentu. Moduł WorkSpace — Specyfikacja docelowa, wersja 2.0, 2026-08-20.*
