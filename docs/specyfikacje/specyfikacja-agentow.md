...
1. [Agent w strukturze platformy](#1-agent-w-strukturze-platformy)
2. [Dwa kanały komunikacji operacyjnej i pętla wykonawcza](#2-dwa-kanały-komunikacji-operacyjnej-i-pętla-wykonawcza)
3. [Warstwy widoczności w zarządzaniu agentami](#3-warstwy-widoczności-w-zarządzaniu-agentami)
4. [Okna operacyjne modułu Agents i cykl życia agenta](#4-okna-operacyjne-modułu-agents-i-cykl-życia-agenta)
5. [Tworzenie agenta — komponenty definicji](#5-tworzenie-agenta--komponenty-definicji)
6. [Centrum uprawnień (Permissions Center)](#6-centrum-uprawnień-permissions-center)
7. [Agent jako komponent własny ponad środowiskami](#7-agent-jako-komponent-własny-ponad-środowiskami)
8. [Przypisanie agenta do ról w środowisku MultitaskingAI](#8-przypisanie-agenta-do-ról-w-środowisku-multitaskingai)
9. [Przypisanie agenta do projektów w module Workspace](#9-przypisanie-agenta-do-projektów-w-module-workspace)
10. [Szablon definicji agenta](#10-szablon-definicji-agenta)
11. [Model danych i kontrakty komunikacji agenta](#11-model-danych-i-kontrakty-komunikacji-agenta)
12. [Konfigurowalność i zasada braku twardych blokad](#12-konfigurowalność-i-zasada-braku-twardych-blokad)
13. [Scenariusze użycia](#13-scenariusze-użycia)
14. [Słowniczek pojęć](#14-słowniczek-pojęć)
- [Załącznik A. Przykład wypełnionej definicji agenta](#załącznik-a-przykład-wypełnionej-definicji-agenta)
- [Załącznik B. Macierz elementów definicji agenta](#załącznik-b-macierz-elementów-definicji-agenta)
...
|---|---|---|
| Koncepcja platformy | 9.15 | Moduł Agents — cel, okna operacyjne, charakterystyka, powiązania |
| Koncepcja platformy | 2.3, 5.2 | Agent jako komponent własny; strefa 2 strony głównej |
| Koncepcja platformy | 4 | Konfigurowalność zależności i punktów izolacji |
| Koncepcja platformy | 8 | Macierz dostępności modułów |
| Koncepcja platformy | 9.2 | Moduł Workspace — Agent Manager, przypisanie do projektu |
| Koncepcja platformy | 11 | Środowisko MultitaskingAI — role wykonawcze |
| Architektura | 8 | Integracja modeli; kanały API, CLI, SSH, HTTP |
| Architektura | 9 | Izolacja — osiem zakresów izolacji technicznej |
| Architektura | 10 | Konfiguracja — struktura warstwowa |
| Architektura | 12 | Rozszerzenia — źródła Danaco Plugin i Personal |
| Architektura | 13 | Uwierzytelnianie |
| Model danych | 8 | Modele i agenci — encja Agent, encja Rola w MultitaskingAI |
| Kontrakty komunikacji | 5.8 | Komunikaty warstwy komunikacji dla modeli i agentów |
...
Ta cecha odróżnia agenta od pozostałych komponentów własnych, których zastosowanie operacyjne jest ograniczone do jednego kontekstu pracy. Zależność tę potwierdza dokument Modelu danych: agent jest komponentem platformowym ponad wszystkimi środowiskami (rozdz. 8.2), a w rozdziale 13 tego dokumentu stwierdzono wprost — agent jest niezależny od środowiska i przypisywany jako Wykonawca.
...
|---|---|---|---|
| Agents (9.15) | TAK | TAK | TAK |
| Workspace (9.2) | TAK | TAK | TAK |
| Roundtable (9.8) | TAK | TAK | TAK |
...
| 5. Wykonanie | Wykonawca | Realizacja zadania z wykorzystaniem skilli, pluginów, konektorów i pamięci zapisanych w definicji agenta, w granicach uprawnień z Permissions Center | Rezultat zadania |
...
|---|---|
| Przydział względem kompetencji | Zadanie trafia do Wykonawcy, którego definicja — skille, konektory, kanał modelu i uprawnienia — odpowiada charakterowi zadania |
...
| 3 — rozwinięcia kontekstowe | Operacje na zapisanej definicji agenta: duplikowanie, eksport, przypisanie do projektu, przypisanie do roli, ustawienia szybkie pamięci | Menu kebab (⋮), menu hamburger (☰), menu kontekstowe, panel popover, lista rozwijana |
...
| Skills Manager | Dobór i konfiguracja skilli agenta (rozdz. 5.4 niniejszego dokumentu). | Obszar roboczy modułu | 3 |
| Connectors Manager | Podłączanie pluginów i konektorów zewnętrznych (rozdz. 5.5 niniejszego dokumentu). | Obszar roboczy modułu | 3 |
...
Makiety tych okien, pełne katalogi ich elementów wraz z warstwą widoczności i sposobem wywołania każdego elementu oraz stany szczegółowe każdego okna zawiera `moduly/agents.md`, rozdz. 2 i 5–9.
...
```
 ...
 (model bazowy,(skille)               (pluginy,       (uprawnienia)
...
STREFA 2 STRONY GŁÓWNEJ (rozdz. 5.2)          OKNO MODUŁOWE AGENTS (środowisko)
  ...
  (model bazowy,       (skille)        Manager        Center
   kanał modelu)                       (pluginy,      (uprawnienia)
                                        konektory)
        │                │               │               │
        └────────────────┴───────┬───────┴───────────────┘
                                  ▼
...
Rozdział 11.15 Koncepcji platformy wymienia siedem elementów definiowanych przy tworzeniu agenta w Agent Builder: model bazowy, tożsamość, instrukcje systemowe, skille, pluginy, konektory oraz uprawnienia; Załącznik D.1 Koncepcji platformy dodaje pamięć jako pole swoiste rodzaju „agent”. Uprawnienia, ze względu na zakres, otrzymują odrębny rozdział 4.
...
| Moduły zastosowania | Środowiska i moduły wykorzystania operacyjnego |
| Zasięg widoczności | Globalny lub projektowy |

Dwóch agentów może korzystać z tego samego modelu i kanału, różniąc się tożsamością, instrukcjami systemowymi, skillami, konektorami, pamięcią i uprawnieniami — a więc pełnić w praktyce zupełnie odmienne funkcje operacyjne.
...
Skille dobierane w Skills Manager rozszerzają zakres działania agenta o wyspecjalizowane procedury lub wiedzę proceduralną. Są, obok pluginów i konektorów, jedną z trzech kategorii rozszerzeń, których wspólny kontrakt integracji opisuje rozdział 14 Architektury.

| Kategoria rozszerzenia | Okno konfiguracji | Co wnosi do agenta | Wspólny kontrakt integracji |
|---|---|---|---|
| Skille | Skills Manager | Wyspecjalizowane procedury lub wiedza proceduralna | rozdz. 14 Architektury |
| Pluginy | Connectors Manager | Rozszerzenia funkcjonalne podłączane do definicji agenta | rozdz. 14 Architektury |
| Konektory | Connectors Manager | Integracje zewnętrzne podłączane do definicji agenta | rozdz. 14 Architektury |

### 5.5. Pluginy i konektory

Connectors Manager podłącza do definicji agenta pluginy i konektory zewnętrzne. Zgodnie z rozdziałem 14 Architektury rozszerzenia dzielą się na dwa źródła.
...
Rdzeń serwera nie rozróżnia źródła rozszerzenia — obowiązuje wspólny kontrakt integracji (rozdz. 14 Architektury) — więc agent korzysta z pluginu lub konektora Personal dokładnie tak samo, jak z rozszerzenia Danaco Plugin. Podłączenie rozszerzenia do definicji agenta i decyzja o zakresie jego wykorzystania są rozstrzygane w dwóch różnych oknach.

| Pytanie | Rozstrzyga |
|---|---|
| Czy rozszerzenie jest podłączone do definicji agenta | Connectors Manager (skille — Skills Manager) |
...
Konfiguracja pamięci agenta korzysta z ogólnego mechanizmu pamięci opisanego w rozdziale 6 Modelu danych oraz w rozdziale 2.4 i rozdziale 6 Koncepcji platformy: pamięć działa na konfigurowalnych poziomach, podlega pełnej konfiguracji z okna konfiguracji i można ją wyłączyć w całości lub przypisać dowolnie.
...
|---|---|---|---|
| Model bazowy i kanał | Model Configuration | rozdz. 9.15 | rozdz. 8 |
| Tożsamość | Agent Builder | rozdz. 9.15, Załącznik D.1 | — |
| Instrukcje systemowe | Agent Builder | rozdz. 9.15 | — |
| Skille | Skills Manager | rozdz. 9.15 | rozdz. 12 |
| Pluginy i konektory | Connectors Manager | rozdz. 9.15 | rozdz. 12 |
| Pamięć | Agent Builder | rozdz. 9.15, rozdz. 2.4, rozdz. 4 | rozdz. 9, rozdz. 10 |
| Uprawnienia | Permissions Center | rozdz. 9.15, rozdz. 4.4–4.6 | rozdz. 9, rozdz. 13 |
...
|---|---|---|
| Dostęp do rozszerzeń | Które z podłączonych w Connectors Manager pluginów i konektorów agent może rzeczywiście wykorzystać oraz w jakim zakresie (odczyt, zapis, wywołanie akcji). | Connectors Manager (rozdz. 3.5), rozdz. 14 Architektury |
| Dostęp do modułów i zasobów | W których modułach platformy agent może działać jako Wykonawca oraz do jakich zasobów modułowych (np. zapis plików w Library, wykonywanie poleceń w Terminal) ma dostęp. | Macierz dostępności modułów (rozdz. 10 Koncepcji platformy) jako ramy działania agenta |
...
| MultitaskingAI | Przypisywany do trwałej roli w zespole modeli | Trwały, per rola | 5.2, 6 |
...
Rolę pełni agent zapisany jako komponent własny albo model bazowy wskazany bezpośrednio, bez pośrednictwa agenta (rozdz. 9.4 Koncepcji platformy). Wybór między tymi dwiema możliwościami jest ustawieniem konfiguracyjnym sekcji Role panelu orkiestracji.
...
3. Przypisanie agenta niesie ze sobą całą jego definicję:
      tożsamość · instrukcje systemowe · skille · konektory · pamięć · uprawnienia
      (z Centrum uprawnień)
...
Rozdział 5 Modelu danych opisuje encję Projekt jako jednostkę organizacji pracy obejmującą, między innymi, przypisany profil izolacji, instrukcje systemowe i zakres pamięci. Rozdział 13 Modelu danych potwierdza, że agent jest niezależny od środowiska i przypisywany jako Wykonawca. Koncepcja platformy przewiduje dwa mechanizmy tego przypisania.
...
| Skille | Wyspecjalizowane procedury lub wiedza proceduralna | Skills Manager | lista skilli |
| Pluginy i konektory | Podłączone rozszerzenia zewnętrzne | Connectors Manager | lista rozszerzeń; źródło: Danaco Plugin \| Personal (rozdz. 14 Architektury) |
| Pamięć | Poziomy pamięci, z których agent korzysta domyślnie | Agent Builder | globalna \| projekt \| sesja \| środowisko; wyłączona (rozdz. 6 Modelu danych) |
...
| Powiązania | Przypisania do projektów i ról | Agent Manager (9.2), sekcja Role panelu orkiestracji (11.8) | lista przypisań |
...
AGENT
  ...
  skille:                [ <skill>, … ]                # Skills Manager
  rozszerzenia:                                        # Connectors Manager
    - nazwa:             <plugin lub konektor>
      źródło:            Danaco Plugin | Personal

  pamięć:
    poziomy:             [ globalna | projekt | sesja | środowisko ]   # lub: wyłączona
...
Rozdział 8.2 Modelu danych definiuje encję Agent jako definicję agenta AI, komponent platformowy ponad wszystkimi środowiskami. Zestaw atrybutów odpowiada siedmiu komponentom definicji z rozdziału 3 oraz Centrum uprawnień z rozdziału 4.
...
| skille | Skills Manager (rozdz. 5.4) |
| pluginy | Connectors Manager (rozdz. 5.5) |
...
Rozdział 8.3 Modelu danych definiuje encję Rola w MultitaskingAI jako przypisanie roli w środowisku wielomodelowym.
...
|---|---|---|
| `agent.changed` | Zdarzenie rozgłaszane po każdej zmianie definicji agenta; niesie stan wystarczający do odświeżenia widoku klienta bez dodatkowego zapytania. | rozdz. 6 |
| `extension.list` | Wykaz rozszerzeń podłączonych w Connectors Manager. | rozdz. 5.9 |
| `extension.install` | Instalacja rozszerzenia. | rozdz. 5.9 |
| `extension.toggle` | Włączenie lub wyłączenie rozszerzenia. | rozdz. 5.9 |
| `isolation.profile.set` | Zapis profilu izolacji. | rozdz. 5.10 |
| `isolation.profile.assign` | Przypisanie profilu izolacji roli (rozdz. 8.5 niniejszego dokumentu). | rozdz. 5.10 |
...
|---|---|
| Pełna kompozycyjność | Agent może zostać złożony z dowolną kombinacją skilli, konektorów i uprawnień; może uczestniczyć w pełnej pętli Coordinator–Executor z Subagent Network (rozdz. 13.3 Koncepcji platformy). |
| Pełna konfigurowalność (zasada centralna) | Każdy element definicji agenta — model, tożsamość, instrukcje, skille, konektory, pamięć, uprawnienia — konfigurowany jest z poziomu okien modułu Agents i okna konfiguracji punktów izolacji; żaden element nie jest zaszyty na stałe. |
...
| Skills Manager | Okno operacyjne modułu Agents służące do doboru i konfiguracji skilli agenta. |
| Connectors Manager | Okno operacyjne modułu Agents służące do podłączania pluginów i konektorów, pochodzących ze źródła Danaco Plugin lub Personal. |
...
| Skille | wzorce architektury API, konwencje kodowania repozytorium |
| Pluginy i konektory | konektor repozytorium kodu (Danaco Plugin) |
...
|---|---|---|---|---|
| Model bazowy i kanał | 5.1 | 9.15 | 8 | 8.1, 8.2 |
| Tożsamość | 5.2 | 9.15, Załącznik D.1 | — | 8.2 |
| Instrukcje systemowe | 5.3 | 9.15 | — | 8.2 |
| Skille | 5.4 | 9.15 | 12 | 8.2, 9 |
| Pluginy i konektory | 5.5 | 9.15 | 12 | 8.2, 9 |
| Pamięć | 5.6 | 9.15, 2.4, 4 | 9, 10 | 6, 8.2 |
| Uprawnienia (Centrum uprawnień) | 6 | 9.15, 4.4–4.6 | 9, 13 | 8.2, 10 |
| Przypisanie do roli | 8 | 11.3, 11.8 | 8, 10 | 8.3, 13 |
| Przypisanie do projektu | 9 | 9.2, 9.15 | 10 | 5, 13 |

---

*Koniec dokumentu. Danaco Console — Specyfikacja agentów, wersja 2.0.*
