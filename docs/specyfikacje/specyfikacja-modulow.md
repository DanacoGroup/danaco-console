...
3a. [Warstwy widoczności funkcji modułu](#3a-warstwy-widoczności-funkcji-modułu)
4. [Specyfikacja modułów](#4-specyfikacja-modułów)
...
5. [Zbiorcze zestawienie powiązań międzymodułowych](#5-zbiorcze-zestawienie-powiązań-międzymodułowych)
6. [Zgodność z zasadami nadrzędnymi platformy](#6-zgodność-z-zasadami-nadrzędnymi-platformy)
...
| Workflow modułu | Typowy, domyślny przebieg pracy krok po kroku |
...
| Wyposażenie | Własny zestaw okien operacyjnych, dedykowane narzędzia, workflow oraz typ danych wykorzystywanych przez AI | Konkretna, zindywidualizowana instancja: automatyka, agent, projekt lub profil asystenta |
...
| Roundtable | Tak | Nie | — |
| Design | Tak | Nie | — |
| Assistant | Nie — okno modułowe realizowane operacyjnie w TalkIn, konfiguracja w strefie 2 | Tak | Profil asystenta |
| Terminal | Tak | Nie | — |
| Developer | Tak | Nie | — |
| Diagnostics | Tak | Nie | — |
| Apps | Tak | Nie | — |
...
| Assistant | Jedyne „TAK" — kolumna TalkIn | Konfigurowany jako komponent własny na stronie głównej, a wykorzystywany operacyjnie wyłącznie w środowisku TalkIn |
...
| 3 | Rozwinięcia kontekstowe | Zestawy akcji modułu, ustawienia szybkie, warianty operacji | Menu kebab (⋮), menu hamburger (☰), menu kontekstowe, panel popover, lista rozwijana |
...
Poniżej pełna specyfikacja operacyjna piętnastu modułów, w kolejności przyjętej w rozdziale 11 Koncepcji platformy. Każdy moduł otwiera karta modułu zestawiająca cel, okna komunikacji operacyjnej, okna robocze, funkcjonalności i powiązania, po której następują tabele szczegółowe: okien operacyjnych wraz z warstwami widoczności (rozdz. 3a), workflow, danych wykorzystywanych przez AI oraz powiązań konfigurowalnych. Chat Window oraz Execution Loop Window występują w każdym module jako elementy pierwszoplanowe. Środowiska, w których dany moduł jest dostępny, wskazuje macierz w rozdziale 3. Przegląd wszystkich piętnastu modułów przedstawia poniższa tabela.

| Moduł | Cel operacyjny (w skrócie) | Forma udostępnienia | Okna komunikacji operacyjnej |
|---|---|---|---|
| Studio | Zaawansowana praca z tekstem, dokumentami i treścią | Okno modułowe | Chat Window · Execution Loop Window |
| Workspace | Izolowane środowisko projektowe | Komponent własny (Projekt) oraz okno modułowe | Chat Window · Execution Loop Window |
| Automations | Budowanie i wykonywanie procesów automatycznych | Komponent własny (Automatyka), wyłącznie strefa 2 | Chat Window · Execution Loop Window |
| Browser | Współdzielone przeglądanie internetu | Okno modułowe | Chat Window · Execution Loop Window |
| Research | Realizacja badań, analiz i opracowań | Okno modułowe | Chat Window · Execution Loop Window |
| Library | Centralne repozytorium wiedzy i plików | Okno modułowe | Chat Window · Execution Loop Window |
| Translate | Wielojęzyczne tłumaczenia w trybie wielozadaniowym | Okno modułowe | Chat Window · Execution Loop Window |
| Roundtable | Współpraca wielu modeli AI nad wspólnym problemem | Okno modułowe | Chat Window · Execution Loop Window |
| Design | Projektowanie i tworzenie zasobów wizualnych | Okno modułowe | Chat Window · Execution Loop Window |
| Assistant | Naturalna komunikacja głosowa z AI | Komponent własny (Profil asystenta), operacyjnie w TalkIn | Chat Window · Execution Loop Window |
| Terminal | Praca z konsolami i środowiskami wykonawczymi | Okno modułowe | Chat Window · Execution Loop Window |
| Developer | Tworzenie i rozwój kodu | Okno modułowe | Chat Window · Execution Loop Window |
| Diagnostics | Analiza oraz usuwanie problemów technicznych | Okno modułowe | Chat Window · Execution Loop Window |
| Apps | Budowa kompletnych produktów cyfrowych | Okno modułowe | Chat Window · Execution Loop Window |
| Agents | Tworzenie i zarządzanie własnymi agentami AI | Komponent własny (Agent) oraz okno modułowe | Chat Window · Execution Loop Window |
...
ZBIEŻNOŚĆ KLASTRÓW
  Klaster treści      ─┐
  Klaster inżynierii  ─┼──►  środowisko MultitaskingAI (orkiestracja ról i kolejek, rozdz. 11)
...
[3] KLASTER ORKIESTRACJI                          zbieżność: MultitaskingAI
    ...
    Automations ───►  MultitaskingAI    harmonogramy i kolejki pętli pracy ciągłej (rozdz. 11.6)
...
|---|---|
| Pełna kompozycyjność | Workflow modułu opisany przy każdym module przedstawia typowy, domyślny przebieg pracy, a nie jedyną dopuszczalną sekwencję działań; użytkownik może komponować własne układy z dostępnych komponentów (prompt, zadanie, akcja, orkiestracja, subagenci) w każdym module, zgodnie z rozdziałem 7.5 dokumentu Model danych |
...
*Koniec dokumentu. Danaco Console — Specyfikacja modułów, wersja 2.0.*
