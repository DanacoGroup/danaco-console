...
| | |
|---|---|
| **Tytuł** | Specyfikacja okien operacyjnych |
| **Klasa dokumentu** | Specyfikacja docelowa |
| **Odbiorcy** | deweloper · projektant |
| **Przeznaczenie** | Ustala katalog wszystkich typów okien operacyjnych piętnastu modułów platformy oraz czterech ról środowiska MultitaskingAI — dla każdego okna cel, zawartość, zachowanie, warstwę widoczności, sposób wywołania i powiązane operacje |
| **Zakres** | katalog okien operacyjnych piętnastu modułów (rozdz. 6), okna komunikacji operacyjnej wspólne wszystkim modułom (rozdz. 5), warstwy widoczności (rozdz. 3.1), okna robocze czterech ról MultitaskingAI (rozdz. 7), okno konfiguracji punktów izolacji (rozdz. 8), macierz zbiorcza (rozdz. 9) |
| **Poza zakresem** | szczegóły projektu wizualnego okien — wymiary, tokeny kolorów i typografii, warstwa graficzna — przedmiot [Systemu wizualnego](../interfejs-uzytkownika/system-wizualny.md); model danych okien — [Model danych](../architektura/model-danych.md); kontrakty komunikacji klient–serwer — [Kontrakty komunikacji](../architektura/kontrakty-komunikacji.md) |
| **Dokument nadrzędny** | [Koncepcja platformy](../architektura/koncepcja-platformy.md) |
| **Dokumenty powiązane** | [Koncepcja platformy](../architektura/koncepcja-platformy.md) · [Architektura techniczna](../architektura/architektura.md) · [Specyfikacja modułów](specyfikacja-modulow.md) · [Specyfikacja agentów](specyfikacja-agentow.md) |
| **Prototypy odniesienia** | `design/05-okna/` — komplet 38 prototypów |
| **Źródła normatywne** | `design/05-okna/` (38 plików HTML, jeden na okno wiodące modułu albo rolę) · [Koncepcja platformy](../architektura/koncepcja-platformy.md) rozdz. 1–14, Załącznik A · [Architektura techniczna](../architektura/architektura.md) rozdz. 5, 7, 9, 11, 13, 16 |
| **Zasada nadrzędna** | Jeżeli funkcja nie jest potrzebna do realizacji aktualnego zadania, nie jest widoczna — interfejs ujawnia możliwości platformy stopniowo, zależnie od kontekstu, roli i wykonywanej czynności |

Dokument stanowi katalog wszystkich typów okien operacyjnych występujących w piętnastu modułach platformy oraz w środowisku MultitaskingAI. Dla każdego okna określono cel, zawartość, zachowanie, warstwę widoczności, sposób wywołania oraz powiązane operacje. Dokument porządkuje okna wymienione w Koncepcji platformy (rozdział 11, rozdział 13, Załącznik A) i w Architekturze (rozdział 13), stosując się do zasady nadrzędnej pełnej konfigurowalności i braku twardych blokad. Katalog obejmuje komplet 38 prototypów zgromadzonych w `design/05-okna/` — liczba potwierdzona poleceniem `find design/05-okna -name '*.html' | wc -l` i zgodna z liczbą okien wiodących wymienionych w rozdziałach 6 i 7.
...
1. [Wprowadzenie](#1-wprowadzenie)
   ...
   - [1.2 Zakres](#12-zakres)
   - [1.3 Źródła](#13-źródła)
2. [Pojęcie okna operacyjnego](#2-pojęcie-okna-operacyjnego)
   - [2.1 Definicja](#21-definicja)
   - [2.2 Uwaga terminologiczna: dwa znaczenia „okna operacyjnego”](#22-uwaga-terminologiczna-dwa-znaczenia-okna-operacyjnego)
   - [2.3 Miejsce w hierarchii platformy](#23-miejsce-w-hierarchii-platformy)
   - [2.4 Anatomia okna operacyjnego](#24-anatomia-okna-operacyjnego)
3. [Zasady wspólne dla okien operacyjnych](#3-zasady-wspólne-dla-okien-operacyjnych)
   - [3.1 Warstwy widoczności — stopniowe ujawnianie funkcjonalności](#31-warstwy-widoczności--stopniowe-ujawnianie-funkcjonalności)
...
5. [Okna komunikacji operacyjnej — Chat Window i Execution Loop Window](#5-okna-komunikacji-operacyjnej--chat-window-i-execution-loop-window)
   ...
   - [5.3 Role uczestników kanałów](#53-role-uczestników-kanałów)
   - [5.4 Rozmieszczenie okien komunikacji](#54-rozmieszczenie-okien-komunikacji)
...
7. [Okna robocze środowiska MultitaskingAI](#7-okna-robocze-środowiska-multitaskingai)
   - [7.1 Charakter okien środowiska MultitaskingAI](#71-charakter-okien-środowiska-multitaskingai)
   - [7.2 Okna czterech ról](#72-okna-czterech-ról)
   - [7.3 Pozostałe sekcje panelu orkiestracji](#73-pozostałe-sekcje-panelu-orkiestracji)
8. [Okno konfiguracji punktów izolacji](#8-okno-konfiguracji-punktów-izolacji)
   - [8.1 Miejsce okna w katalogu](#81-miejsce-okna-w-katalogu)
   - [8.2 Cel](#82-cel)
   - [8.3 Zawartość](#83-zawartość)
   - [8.4 Zachowanie](#84-zachowanie)
   - [8.5 Powiązane operacje](#85-powiązane-operacje)
...
| Obejmuje | Warstwy widoczności okien i elementów interfejsu wraz ze sposobem ich wywołania (rozdział 3.1) |
...
| Nie obejmuje | Szczegóły projektu wizualnego okien: wymiary, tokeny kolorów i typografii oraz warstwa graficzna — przedmiot [Systemu wizualnego](../interfejs-uzytkownika/system-wizualny.md) |
...
Makiety tekstowe zamieszczone w rozdziałach 5–8 przedstawiają **schematyczne, funkcjonalne rozmieszczenie okien w kolumnach** — ich sąsiedztwo i porządek od lewej do prawej — a nie warstwę graficzną. Makiety rysowane są w stanie spoczynku interfejsu, zgodnie z regułą warstw widoczności (rozdział 3.1): widoczne są wyłącznie elementy warstwy 1 oraz zwinięte wyzwalacze warstw 2–3.

### 1.3. Źródła

| Dokument | Wykorzystane części |
|---|---|
| Koncepcja platformy | Rozdziały 1–14 oraz Załączniki A–E; w szczególności rozdział 11 (Moduły), rozdział 13 (MultitaskingAI — rozwinięcie środowiska), Załącznik A (Macierz okien operacyjnych per moduł) |
| Architektura | Rozdział 13 (Konfiguracja), rozdział 12 (Izolacja i konfigurowalność zależności) |
...
Okno operacyjne jest najniższym poziomem trójstopniowej hierarchii organizacji pracy przyjętej przez platformę (Koncepcja platformy, Wprowadzenie i rozdział 2). Poniższy schemat przedstawia tę hierarchię wraz z rolą obu okien komunikacji operacyjnej na poziomie okna operacyjnego.
...
Poniższy schemat i tabela porządkują elementy wspólne każdego okna operacyjnego — wynikające bezpośrednio z zasad wspólnych z rozdziału 3 oraz z reguły warstw widoczności z rozdziału 3.1. Stanowią wzorzec anatomii, do którego odnoszą się szczegółowe katalogi okien w rozdziałach 5–8.
...
|---|---|---|
| Nagłówek okna | Identyfikacja okna, jego przynależności do modułu i warstwy widoczności | Rozdział 2.1, rozdział 3.1 |
...
| Wyzwalacze warstw 2–3 | Udostępnienie ustawień i akcji ukrytych w stanie spoczynku, osiągalnych jednym kliknięciem | Rozdział 3.1 |
...
| 2 | Widoczna na żądanie | Wybór modelu, wybór wykonawcy, wybór środowiska, wybór trybu pracy, poziom wysiłku, parametry przebiegu pracy, panele pomocnicze i monitory otwierane jako rozszerzenia boczne | Ikona, przycisk, przełącznik, znacznik kontekstowy; po użyciu element zwija się samoczynnie |
| 3 | Rozwinięcia kontekstowe | Zestawy akcji, ustawienia szybkie, warianty operacji | Menu kebab (⋮), menu hamburger (☰), menu kontekstowe, panel nakładkowy, lista rozwijana |
...
**Maszyna stanów okna warstwy 2–3.** Każde okno pomocnicze przechodzi przez ten sam ciąg stanów, niezależnie od modułu — jedynie wyzwalacz otwarcia różni się między oknem warstwy 2 a warstwą 3.

```
              wywołanie (znacznik / przycisk / menu)
                             │
                             ▼
   ┌────────────┐    otwarcie    ┌────────────┐   zamknięcie   ┌────────────┐
   │ ZWINIĘTE   │ ─────────────► │ OTWARTE    │ ─────────────► │ ZWINIĘTE   │
   │ (wyzwalacz │                │(rozszerze  │                │ (wyzwalacz │
   │  widoczny) │ ◄───────────── │nie boczne  │ ◄───────────── │  widoczny) │
   └────────────┘ brak interakcji└─────┬──────┘   klik poza    └────────────┘
                  poza oknem           │          obszarem okna
                                       │ wybór pozycji / zmiana ustawienia
                                       ▼
                              ┌──────────────────┐
                              │  ZASTOSOWANE     │  wynik zapisany w kontekście
                              │  (zwinięcie      │  sesji, karty lub definicji
                              │   samoczynne)    │  komponentu własnego
                              └──────────────────┘
```

Stan ZASTOSOWANE jest przejściowy — okno wraca automatycznie do stanu ZWINIĘTE natychmiast po zapisaniu wyboru, zgodnie z zasadą jednego kliknięcia; wyjątkiem są okna warstwy 1, które nie mają stanu ZWINIĘTE i pozostają OTWARTE przez cały czas pracy w module.

**Macierz uprawnień — dostęp ról do warstw widoczności.** Rolą w rozumieniu tej macierzy jest uczestnik wymiany zdefiniowany w rozdziale 5.3 (Użytkownik, Koordynator, Wykonawca), nie rola panelu orkiestracji z rozdziału 7.

| Warstwa | Użytkownik | Koordynator | Wykonawca |
|---|---|---|---|
| 1 — zawsze widoczna | Pełny dostęp — odczyt i sterowanie | Pełny dostęp — odczyt i zapis stanu pętli | Pełny dostęp — odczyt zlecenia, zapis wyniku |
| 2 — widoczna na żądanie | Pełny dostęp — wybór i zmiana ustawień | Odczyt ustawień wpływających na przydział zadań | Odczyt ustawień wpływających na wykonanie (model, poziom wysiłku) |
| 3 — rozwinięcia kontekstowe | Pełny dostęp — operacje na zapisanych zasobach | Brak dostępu bezpośredniego — operacje inicjowane wyłącznie poleceniem Użytkownika | Brak dostępu bezpośredniego — operacje inicjowane wyłącznie przydziałem zadania |
| 4 — funkcje eksperckie | Pełny dostęp warunkowany trybem administracyjnym | Odczyt konfiguracji ról i limitów zasobów niezbędny do przydziału zadań | Brak dostępu — konfiguracja uprawnień nie jest osiągalna z poziomu Wykonawcy |

Macierz odzwierciedla zasadę nadrzędną: stanem wyjściowym jest pełny dostęp operacyjny Użytkownika do wszystkich czterech warstw, a ograniczenia Koordynatora i Wykonawcy wynikają z podziału ról pętli wykonawczej (rozdział 2 Specyfikacji agentów), nie z osobnego mechanizmu uwierzytelniania.
...
| Automations | Polecenia wobec kolejki zadań automatyki | Nadzór nad przebiegami pracy, ponowienia i korekty zlecenia |
...
Regulacji podlega wyłącznie **szerokość** kolumn. Makiety rysowane są w stanie spoczynku: kolumny boczne warstw 2–3 nie są rozwinięte, a ich wyzwalacze zaznaczono jako `▼`, `⋮`, `☰` po prawej stronie kolumny dominującej. Znak `│` rozdziela kolumny makiety: nazwa okna stoi w pierwszym wierszu swojej kolumny, skrót jego funkcji w wierszach kolejnych tej samej kolumny; kolumna skrajnie lewa niesie boczną nawigację modułów, a nie okno operacyjne. Wiersz zamykający makietę nazywa wagę wizualną każdej kolumny zgodnie z powyższą typologią.
...
| Okno | Warstwa | Sposób wywołania | Cel | Zawartość | Zachowanie | Powiązane operacje |
|---|---|---|---|---|---|---|
| Workflow Builder | 1 | Widoczne bez interakcji — kolumna dominująca | Projektowanie procesu automatycznego | Kroki procesu, warunki, kolejność wykonania | Zmiany zapisywane jako definicja automatyki — komponentu własnego | Dodanie kroku, ustalenie warunku, zapis przebiegu pracy |
...
| Scheduler | 2 | Przycisk harmonogramu — rozszerzenie boczne | Ustalanie cykliczności uruchomień | Harmonogram: częstotliwość, godziny, zdarzenia wyzwalające | Harmonogram obowiązuje po powiązaniu z przebiegiem pracy | Ustalenie cykliczności, edycja harmonogramu |
| Orchestrator | 3 | Menu kebab (⋮) — rozszerzenie boczne | Definiowanie zależności między krokami procesu | Zależności i kolejność wykonania kroków przebiegu pracy | Zależności respektowane w toku wykonania procesu | Ustalenie zależności, zmiana kolejności kroków |
...
| Skills Manager | 2 | Znacznik umiejętności — rozszerzenie boczne | Dobór umiejętności agenta | Lista dostępnych umiejętności możliwych do przypisania agentowi | Przypisane umiejętności dostępne agentowi w pracy operacyjnej | Dodanie umiejętności, usunięcie umiejętności |
| Connectors Manager | 3 | Menu hamburger (☰) — rozszerzenie boczne | Podłączenie integracji zewnętrznych | Lista dostępnych konektorów i wtyczek | Podłączone konektory dostępne agentowi w toku pracy | Podłączenie konektora, konfiguracja integracji |
...
| Sekcje z własnymi oknami roboczymi | Jedna sekcja — Role — operuje czterema oknami przypisanymi bezpośrednio rolom zespołu (rozdział 7.2) |
| Sekcje korzystające z okien modułu Automations | Cztery sekcje — Kolejki, Orkiestracja, Harmonogram i automatyki, Monitor procesu — po skonfigurowaniu powiązań korzystają z okien skatalogowanych w rozdziale 6.3; szósta sekcja, Zespoły, operuje zapisanymi konfiguracjami okien ról (rozdział 7.3) |
...
|---|---|---|---|---|
| Zespoły | 2 | Pozycja panelu orkiestracji | Zapisane konfiguracje zespołu — gotowe zestawy ról, powiązań i kolejek; zapis, wczytanie, duplikowanie | Konfiguracje ról z okien opisanych w rozdziale 7.2 |
...
| Izolacja techniczna procesu sesji | Osiem zakresów zdefiniowanych w rozdziale 12 dokumentu Architektury: katalog roboczy sesji, środowisko procesu, katalog danych i konfiguracji modelu, dostęp sieciowy, zakres odczytu i zapisu plików, konto i token per sesja, model procesu, serwer wykonania |
...
|---|---|---|---|
| Selektor zasięgu (lewa) | 4 | Osiem poziomów zasięgu: globalny, środowisko, moduł, para modułów, projekt, karta sesji, rola, okno komunikacji ([Izolacja i zależności](../architektura/izolacja-i-zaleznosci.md) rozdział 6) | Wybór poziomu determinuje, do którego zakresu odnoszą się ustawienia w kolumnie macierzy izolacji |
...
```
 ...
 │ • okno komun.  │   dane/konfig.modelu[wł|wył] │ Polityka efektywna: │
...
| Zapis, wczytanie i przypisanie profilu izolacji | Przypisanie profilu do sesji, roli środowiska MultitaskingAI (rozdział 7.2) albo pojedynczego okna komunikacji — najwęższego z ośmiu poziomów zasięgu |
...
| Warstwa widoczności | Jedna z czterech warstw (1–4), do której należy każde okno, panel i pojedyncza funkcja; wyznacza, czy element jest widoczny bez interakcji, na żądanie, w rozwinięciu kontekstowym, czy jako funkcja ekspercka | Rozdział 3.1 |
| Sposób wywołania | Konkretny mechanizm ujawnienia elementu warstw 2–4: znacznik kontekstowy, przycisk, przełącznik, menu kebab (⋮), menu hamburger (☰), polecenie języka naturalnego, skrót klawiszowy | Rozdział 3.1 |
...
Opracowanie jest przyjmowane do budowy, gdy poniższe warunki są spełnione łącznie. Kolumna „Sposób sprawdzenia” wskazuje procedurę weryfikowalną, nie opinię recenzenta.

| Warunek sprawdzalny | Sposób sprawdzenia |
|---|---|
| Liczba prototypów odniesienia zgodna z katalogiem okien | `find design/05-okna -name '*.html' \| wc -l` zwraca 38, zgodnie z polem „Prototypy odniesienia” metryki |
| Każdy z piętnastu modułów ma katalog okien operacyjnych | Rozdział 6 zawiera podrozdziały 6.1–6.15, po jednym na moduł z macierzy rozdziału 10 dokumentu Koncepcja platformy |
| Każde okno operacyjne ma przypisaną dokładnie jedną warstwę widoczności | Kolumna „Warstwa” obecna w każdym wierszu tabel rozdziałów 5–8; wartość należy do zbioru {1, 2, 3, 4} |
| Każde okno warstwy 2–4 ma podany sposób wywołania | Kolumna „Sposób wywołania” obecna i niepusta w każdym wierszu tabel rozdziałów 5–8 |
| Chat Window i Execution Loop Window opisane jako wspólne wszystkim modułom | Rozdział 5 oraz wiersz „Okno wspólne” w Słowniczku (rozdział 10) wskazują piętnaście modułów |
| Cztery role środowiska MultitaskingAI mają przypisane okna robocze | Rozdział 7 zawiera po jednym zestawie okien dla Executor 1, Executor 2, Coordinator oraz Executor 3 / Validator |
| Okno konfiguracji punktów izolacji opisane jako okno globalne | Rozdział 8 oraz wiersz „Okno globalne” w Słowniczku (rozdział 10) |
| Macierz zbiorcza obejmuje wszystkie piętnaście modułów | Rozdział 9 zawiera siedemnaście wierszy — po jednym na każdy z piętnastu modułów oraz wiersz okien ról środowiska MultitaskingAI i wiersz okna globalnego — z liczbą okien warstw 1–4 |
| Nazwy okien identyczne z nazwami w Koncepcji platformy | Zerowy wynik porównania nazw własnych okien rozdziałów 5–8 z Załącznikiem A dokumentu Koncepcja platformy |
...
| Objętość dokumentu nie mniejsza niż 85 000 znaków | `python3 -c "print(len(open('docs/specyfikacje/specyfikacja-okien-operacyjnych.md', encoding='utf-8').read()))"` |

---

*Koniec dokumentu. Specyfikacja okien operacyjnych — Specyfikacja docelowa, wersja 2.0, 2026-08-20.*
