# Danaco Console — Środowisko MultitaskingAI
...
| **Poza zakresem** | wnętrze modułu Automations jako takie — [Moduł Automations](../moduly/automations.md); mechanizm magistrali kontekstu jako taki — [Przepływ okien](../interfejs-uzytkownika/przeplyw-okien.md) |
| **Dokument nadrzędny** | [Koncepcja platformy](../architektura/koncepcja-platformy.md) |
| **Dokumenty powiązane** | [Specyfikacja modułów](../specyfikacje/specyfikacja-modulow.md) · [Specyfikacja okien operacyjnych](../specyfikacje/specyfikacja-okien-operacyjnych.md) · [Strona główna i nawigacja](../interfejs-uzytkownika/strona-glowna-i-nawigacja.md) · [System wizualny](../interfejs-uzytkownika/system-wizualny.md) · [Model konfiguracji](../architektura/model-konfiguracji.md) · [Model danych](../architektura/model-danych.md) · [Architektura techniczna](../architektura/architektura.md) · [Integracja modeli](../architektura/integracja-modeli.md) · [Specyfikacja agentów](../specyfikacje/specyfikacja-agentow.md) · [Izolacja i zależności](../architektura/izolacja-i-zaleznosci.md) · [Rozszerzenia](../architektura/rozszerzenia.md) · [Przepływ okien](../interfejs-uzytkownika/przeplyw-okien.md) · [Elementy okien](../interfejs-uzytkownika/elementy-okien.md) · [Always On Display](../funkcje-globalne/always-on-display.md) |
| **Prototypy odniesienia** | `design/05-okna/srodowiska/multitaskingai.html` · `design/05-okna/srodowiska/multitaskingai-przedsionek.html` · `design/05-okna/srodowiska/mtai-okna-rol.html` · `design/05-okna/srodowiska/mtai-izolacja-i-zespoly.html` |
| **Źródła normatywne** | `budowa/shared/contract.json` (obszary `environment`, `role`, `orchestration`, `advisor`, `subagent`) · `design/zasoby/zetony/zetony.css` · `design/zasoby/css/komponenty.css`, `design/zasoby/rama.css`, `design/zasoby/prototyp.css` · `design/05-okna/srodowiska/` |
| **Zasada nadrzędna** | Danaco Console nie narzuca twardych blokad, bram bezpieczeństwa ani wymuszonych zgód; domyślne zachowanie systemu to wykonanie polecenia; izolacja techniczna, izolacja kontekstu i wszelkie ograniczenia uprawnień opisane w dokumencie są ustawieniami konfiguracyjnymi sterowanymi przez Operatora z okna konfiguracji, ze stanem wyjściowym „wyłączone / pełny dostęp” |
...
2. [Miejsce środowiska w platformie i architektura ogólna](#1-miejsce-środowiska-w-platformie-i-architektura-ogólna)
   - [1.1 Definicja i odróżnienie od pozostałych trzech środowisk](#11-definicja-i-odróżnienie-od-pozostałych-trzech-środowisk)
   - [1.2 Odróżnienie od modułu Roundtable](#12-odróżnienie-od-modułu-roundtable)
   - [1.3 Typowe zastosowania](#13-typowe-zastosowania)
   - [1.4 Położenie w hierarchii platformy](#14-położenie-w-hierarchii-platformy)
   - [1.5 Schemat warstw wewnętrznych środowiska](#15-schemat-warstw-wewnętrznych-środowiska)
   - [1.6 Warstwy widoczności w środowisku](#16-warstwy-widoczności-w-środowisku)
   - [1.7 Dwa kanały komunikacji operacyjnej środowiska](#17-dwa-kanały-komunikacji-operacyjnej-środowiska)
   - [1.8 Przebieg wejścia do środowiska](#18-przebieg-wejścia-do-środowiska)
3. [Warstwa centralna — Always On Display](#2-warstwa-centralna--always-on-display)
   - [2.1 Charakterystyka i umiejscowienie w środowisku](#21-charakterystyka-i-umiejscowienie-w-środowisku)
   - [2.2 Dwa tryby względem procesu MultitaskingAI](#22-dwa-tryby-względem-procesu-multitaskingai)
   - [2.3 Zdolności właściwe nadzorowi nad procesem środowiska](#23-zdolności-właściwe-nadzorowi-nad-procesem-środowiska)
   - [2.4 Współdziałanie z funkcją Mobile](#24-współdziałanie-z-funkcją-mobile)
   - [2.5 Zakres dostępu — pokrycie sekcji Monitor procesu](#25-zakres-dostępu--pokrycie-sekcji-monitor-procesu)
   - [2.6 Elementy interfejsu swoiste dla środowiska](#26-elementy-interfejsu-swoiste-dla-środowiska)
   - [2.7 Przedsionek środowiska — widok wejściowy](#27-przedsionek-środowiska--widok-wejściowy)
4. [Role zespołu środowiska MultitaskingAI](#3-role-zespołu-środowiska-multitaskingai)
   - [3.0 Tabela zbiorcza — cel, wejście, wyjście](#30-tabela-zbiorcza--cel-wejście-wyjście)
   - [3.1 Executor 1 — główny wykonawca](#31-executor-1--główny-wykonawca)
   - [3.2 Subagent Network](#32-subagent-network)
   - [3.3 Coordinator — koordynator](#33-coordinator--koordynator)
   - [3.4 Executor 2 — równoległy wykonawca](#34-executor-2--równoległy-wykonawca)
   - [3.5 Executor 3 / Validator — czwarty model](#35-executor-3--validator--czwarty-model)
   - [3.6 Zestawienie zbiorcze ról](#36-zestawienie-zbiorcze-ról)
5. [Silnik kolejek](#4-silnik-kolejek)
   - [4.1 Zasięgi kolejek](#41-zasięgi-kolejek)
   - [4.2 Jedenaście akcji silnika kolejek](#42-jedenaście-akcji-silnika-kolejek)
   - [4.3 Cykl życia zadania w kolejce](#43-cykl-życia-zadania-w-kolejce)
   - [4.4 Powiązanie z modułem Automations](#44-powiązanie-z-modułem-automations)
   - [4.5 Katalog elementów interfejsu sekcji Kolejki](#45-katalog-elementów-interfejsu-sekcji-kolejki)
   - [4.6 Makieta — sekcja Kolejki panelu orkiestracji](#46-makieta--sekcja-kolejki-panelu-orkiestracji)
6. [Orkiestracja i zależności](#5-orkiestracja-i-zależności)
   - [5.1 Funkcja orkiestracji](#51-funkcja-orkiestracji)
   - [5.2 Rodzaje zależności](#52-rodzaje-zależności)
   - [5.3 Odpowiedzialność za orkiestrację](#53-odpowiedzialność-za-orkiestrację)
   - [5.4 Katalog elementów interfejsu sekcji Orkiestracja](#54-katalog-elementów-interfejsu-sekcji-orkiestracja)
   - [5.5 Diagram — przykładowa mapa zależności](#55-diagram--przykładowa-mapa-zależności)
7. [Panel orkiestracji — boczna nawigacja środowiska](#6-panel-orkiestracji--boczna-nawigacja-środowiska)
   - [6.1 Zasada organizacji bocznej nawigacji](#61-zasada-organizacji-bocznej-nawigacji)
   - [6.2 Sześć sekcji panelu](#62-sześć-sekcji-panelu)
   - [6.3 Schemat układu panelu](#63-schemat-układu-panelu)
   - [6.4 Katalog elementów interfejsu panelu](#64-katalog-elementów-interfejsu-panelu)
   - [6.5 Konfigurowalność kolejności i widoczności](#65-konfigurowalność-kolejności-i-widoczności)
8. [Integracja z modułem Automations — praca ciągła 24/7/365](#7-integracja-z-modułem-automations--praca-ciągła-247365)
   - [7.1 Warunek uruchomienia](#71-warunek-uruchomienia)
   - [7.2 Mapowanie mechanizmów](#72-mapowanie-mechanizmów)
   - [7.3 Schemat spięcia](#73-schemat-spięcia)
   - [7.4 Efekt integracji — podział odpowiedzialności w pętli ciągłej](#74-efekt-integracji--podział-odpowiedzialności-w-pętli-ciągłej)
   - [7.5 Rola funkcji Mobile w pracy ciągłej](#75-rola-funkcji-mobile-w-pracy-ciągłej)
   - [7.6 Katalog elementów interfejsu integracji](#76-katalog-elementów-interfejsu-integracji)
9. [Hierarchia decyzji](#8-hierarchia-decyzji)
   - [8.1 Poziomy hierarchii](#81-poziomy-hierarchii)
   - [8.2 Rozstrzyganie konfliktów](#82-rozstrzyganie-konfliktów)
   - [8.3 Schemat hierarchii](#83-schemat-hierarchii)
   - [8.4 Konfigurowalność hierarchii](#84-konfigurowalność-hierarchii)
10. [Okna środowiska — makiety tekstowe](#9-okna-środowiska--makiety-tekstowe)
   - [9.1 Anatomia wspólna okna środowiska](#91-anatomia-wspólna-okna-środowiska)
   - [9.2 Tabela okien środowiska](#92-tabela-okien-środowiska)
   - [9.3 Makieta główna — pełny układ okna środowiska MultitaskingAI](#93-makieta-główna--pełny-układ-okna-środowiska-multitaskingai)
   - [9.4 Makieta — Chat Window](#94-makieta--chat-window)
   - [9.5 Makieta — Execution Loop Window](#95-makieta--execution-loop-window)
   - [9.6 Makieta — Executor Chat](#96-makieta--executor-chat)
   - [9.7 Makieta — panel Subagent Network jako rozszerzenie boczne](#97-makieta--panel-subagent-network-jako-rozszerzenie-boczne)
   - [9.8 Makieta — Coordinator Chat](#98-makieta--coordinator-chat)
   - [9.9 Makieta — Results Analyzer](#99-makieta--results-analyzer)
   - [9.10 Makieta — okno konfiguracji punktów izolacji, zasięg „Rola”](#910-makieta--okno-konfiguracji-punktów-izolacji-zasięg-rola)
11. [Katalog elementów interfejsu](#10-katalog-elementów-interfejsu)
   - [10.1 Elementy globalne środowiska](#101-elementy-globalne-środowiska)
   - [10.2 Elementy panelu orkiestracji](#102-elementy-panelu-orkiestracji)
   - [10.3 Elementy sekcji Zespoły](#103-elementy-sekcji-zespoły)
   - [10.4 Elementy okien ról — wspólne i swoiste](#104-elementy-okien-ról--wspólne-i-swoiste)
   - [10.5 Elementy Subagent Network](#105-elementy-subagent-network)
   - [10.6 Elementy okna Coordinator Chat — swoiste](#106-elementy-okna-coordinator-chat--swoiste)
   - [10.7 Elementy okna Results Analyzer — swoiste](#107-elementy-okna-results-analyzer--swoiste)
   - [10.8 Elementy silnika kolejek i sekcji Orkiestracja](#108-elementy-silnika-kolejek-i-sekcji-orkiestracja)
   - [10.9 Elementy sekcji Harmonogram i automatyki oraz Monitor procesu](#109-elementy-sekcji-harmonogram-i-automatyki-oraz-monitor-procesu)
   - [10.10 Elementy Chat Window i Execution Loop Window](#1010-elementy-chat-window-i-execution-loop-window)
12. [Diagramy przepływu — orkiestracja i kolejkowanie](#11-diagramy-przepływu--orkiestracja-i-kolejkowanie)
   - [11.1 Pełny przepływ orkiestracji, z adnotacją okien i sekcji panelu](#111-pełny-przepływ-orkiestracji-z-adnotacją-okien-i-sekcji-panelu)
   - [11.2 Przepływ silnika kolejek — cykl życia pojedynczego zadania](#112-przepływ-silnika-kolejek--cykl-życia-pojedynczego-zadania)
   - [11.3 Przepływ eskalacji przy niepowodzeniu walidacji](#113-przepływ-eskalacji-przy-niepowodzeniu-walidacji)
   - [11.4 Przepływ pracy ciągłej 24/7/365 z modułem Automations](#114-przepływ-pracy-ciągłej-247365-z-modułem-automations)
   - [11.5 Przepływ interwencji zdalnej (Mobile + Always On Display)](#115-przepływ-interwencji-zdalnej-mobile--always-on-display)
13. [Stany](#12-stany)
   - [12.1 Stany przebiegu procesu (pętli orkiestracji)](#121-stany-przebiegu-procesu-pętli-orkiestracji)
   - [12.2 Stany roli / okna roboczego](#122-stany-roli--okna-roboczego)
   - [12.3 Stany zadania w kolejce](#123-stany-zadania-w-kolejce)
   - [12.4 Diagram przejść stanów zadania w kolejce](#124-diagram-przejść-stanów-zadania-w-kolejce)
   - [12.5 Stany Always On Display w kontekście procesu](#125-stany-always-on-display-w-kontekście-procesu)
   - [12.6 Konwencja stanów elementów interfejsu](#126-konwencja-stanów-elementów-interfejsu)
14. [Konfigurowalność i punkty izolacji na poziomie roli](#13-konfigurowalność-i-punkty-izolacji-na-poziomie-roli)
   - [13.1 Poziom zasięgu „Rola” — pierwszeństwo najwyższe](#131-poziom-zasięgu-rola--pierwszeństwo-najwyższe)
   - [13.2 Struktura profilu izolacji przypisanego roli](#132-struktura-profilu-izolacji-przypisanego-roli)
   - [13.3 Atrybuty roli w modelu danych](#133-atrybuty-roli-w-modelu-danych)
   - [13.4 Stan wyjściowy — wykonanie jest domyślne](#134-stan-wyjściowy--wykonanie-jest-domyślne)
   - [13.5 Katalog elementów interfejsu okna izolacji](#135-katalog-elementów-interfejsu-okna-izolacji)
15. [Szablony konfiguracji zespołu](#14-szablony-konfiguracji-zespołu)
   - [14.1 Zespół „Budowa aplikacji”](#141-zespół-budowa-aplikacji)
   - [14.2 Zespół „Badanie i redakcja”](#142-zespół-badanie-i-redakcja)
   - [14.3 Zespół „Pętla ciągła 24/7”](#143-zespół-pętla-ciągła-247)
   - [14.4 Formularz pusty konfiguracji zespołu](#144-formularz-pusty-konfiguracji-zespołu)
16. [Scenariusze operacyjne](#15-scenariusze-operacyjne)
   - [15.1 Eskalacja przy niepowodzeniu walidacji](#151-eskalacja-przy-niepowodzeniu-walidacji)
   - [15.2 Rozstrzygnięcie konfliktu w trybie iteracyjnym](#152-rozstrzygnięcie-konfliktu-w-trybie-iteracyjnym)
   - [15.3 Praca ciągła z interwencją zdalną](#153-praca-ciągła-z-interwencją-zdalną)
   - [15.4 Zespół w pełni oprzyrządowany — od agentów do pętli ciągłej](#154-zespół-w-pełni-oprzyrządowany--od-agentów-do-pętli-ciągłej)
17. [Zgodność z zasadami nadrzędnymi platformy](#16-zgodność-z-zasadami-nadrzędnymi-platformy)
18. [Powłoka środowiska — nawigacja i przestrzeń robocza okien](#17-powłoka-środowiska--nawigacja-i-przestrzeń-robocza-okien)
   - [17.1 Nawigacja środowiska](#171-nawigacja-środowiska)
   - [17.2 Przestrzeń robocza okien ról](#172-przestrzeń-robocza-okien-ról)
19. [Karty sesji — procesy orkiestracji i trwałość stanu](#18-karty-sesji--procesy-orkiestracji-i-trwałość-stanu)
20. [Panel stanu procesu, skróty klawiszowe i paleta poleceń](#19-panel-stanu-procesu-skróty-klawiszowe-i-paleta-poleceń)
   - [19.1 Panel stanu procesu](#191-panel-stanu-procesu)
   - [19.2 Skróty klawiszowe i paleta poleceń](#192-skróty-klawiszowe-i-paleta-poleceń)
21. [Współdzielenie kontekstu między rolami, procesami i modułami](#20-współdzielenie-kontekstu-między-rolami-procesami-i-modułami)
22. [Funkcje wspólne środowiska](#21-funkcje-wspólne-środowiska)
23. [Punkty sterowania z okna konfiguracji](#22-punkty-sterowania-z-okna-konfiguracji)
24. [Słownik pojęć](#23-słownik-pojęć)
25. [Wykazy normatywne i kryteria odbioru](#24-wykazy-normatywne-i-kryteria-odbioru)
   - [24.1 Etykiety interfejsu z prototypów](#241-etykiety-interfejsu-z-prototypów)
   - [24.2 Komunikaty](#242-komunikaty)
   - [24.3 Żetony projektowe](#243-żetony-projektowe)
   - [24.4 Komponenty interfejsu](#244-komponenty-interfejsu)
   - [24.5 Skróty klawiszowe](#245-skróty-klawiszowe)
   - [24.6 Stany kontrolek](#246-stany-kontrolek)
   - [24.7 Punkty łamania](#247-punkty-łamania)
   - [24.8 Kryteria odbioru](#248-kryteria-odbioru)
26. [Załącznik A. Katalog ikon i komponentów systemu wizualnego wykorzystanych w środowisku](#załącznik-a-katalog-ikon-i-komponentów-systemu-wizualnego-wykorzystanych-w-środowisku)
   - [A.1. Ikony](#a1-ikony)
   - [A.2. Komponenty](#a2-komponenty)
   - [A.3. Punkty łamania](#a3-punkty-łamania)
27. [Załącznik B. Macierz zgodności ze źródłami](#załącznik-b-macierz-zgodności-ze-źródłami)
28. [Załącznik C. Pełny wykaz komend kontraktu środowiska MultitaskingAI](#załącznik-c-pełny-wykaz-komend-kontraktu-środowiska-multitaskingai)
   - [Obszar `environment` — 2 komendy](#obszar-environment--2-komendy)
   - [Obszar `role` — 4 komendy](#obszar-role--4-komendy)
   - [Obszar `orchestration` — 8 komend](#obszar-orchestration--8-komend)
   - [Obszar `advisor` — 1 komenda](#obszar-advisor--1-komenda)
   - [Obszar `subagent` — 4 komendy](#obszar-subagent--4-komendy)
   - [Komendy obszarów sąsiednich przywołane w korpusie — 4 komendy](#komendy-obszarów-sąsiednich-przywołane-w-korpusie--4-komendy)
...
|---|---|
| Wieloetapowe projekty w module Apps | Podział na Executor 1 i Executor 2 przy równoległej pracy nad warstwą serwerową i warstwą kliencką |
...
### 1.8. Przebieg wejścia do środowiska

Wejście do środowiska MultitaskingAI korzysta z tej samej komendy kontraktu `environment.enter`, którą posługują się wszystkie cztery środowiska platformy (obszar `environment`, Załącznik C). Powłoka wysyła żądanie po wybraniu karty środowiska na stronie głównej; rdzeń zwraca definicję środowiska, wykaz modułów i wykaz sesji karty, na podstawie których powłoka otwiera panel orkiestracji jako boczną nawigację i wznawia albo zakłada kartę sesji.
...
Legenda: `environment.enter` — komenda kontraktu obszaru `environment`; pola żądania `environmentId`, `clientId` (wymagane), `sessionId` (opcjonalne — puste zakłada kartę pustą); pola wyniku `environment`, `modules`, `sessions` (wymagane), `focusedSessionId` (opcjonalne). Pusty wykaz `sessions` otwiera środowisko bez karty aktywnej — panel orkiestracji jest widoczny, obszar roboczy ról pozostaje pusty do chwili przypisania ról poleceniem w Chat Window (rozdz. 3, `role.assign`).
...
Poniższa tabela obejmuje zdolności swoiste dla nadzoru nad zespołem ról, kolejkami i przebiegiem pętli. Zdolności wspólne całej platformie — proaktywne doradztwo, pomoc kontekstowa, komunikacja tekstowa i głosowa, pełny dostęp do środowisk, projektów, sesji i historii — opisuje [Always On Display](../funkcje-globalne/always-on-display.md) (rozdz. 1.3, 3–5).
...
|---|---|
| Always On Display (tryb operatora) | Obserwuje przebieg procesu i inicjuje interwencję — zatwierdzenie kroku albo wstrzymanie procesu |
...
Awatar funkcji, dymek kontekstowy sugestii, powierzchnia interakcji, plakietka powiadomień i przycisk mikrofonu są elementami wspólnymi całej platformy — ich formę, stany, warstwy widoczności i sposób wywołania podaje [Always On Display](../funkcje-globalne/always-on-display.md) (rozdz. 2.6, 8). Poniższa tabela obejmuje elementy występujące wyłącznie w środowisku MultitaskingAI.

| Element | Co to jest | Do czego służy | Forma i waga | Stany | Zachowanie po interakcji | Gdzie występuje | Warstwa | Sposób wywołania |
|---|---|---|---|---|---|---|---|---|
| Przełącznik trybu (obserwator / operator) | Segmentowana kontrolka dwustanowa | Wybór zakresu działania funkcji wobec procesu (rozdz. 2.2) | Przełącznik segmentowy / suwak (`.dn-suwak`) | Domyślny „obserwator” · zaznaczony „operator” | Zmiana trybu natychmiast rozszerza lub zawęża dostępne akcje interwencyjne; ustawienie „operator” pozostaje dostępne również przy braku aktywnego procesu — komunikat kontekstowy informuje wtedy, że nadzór obejmie najbliższe uruchomienie | Panel stanu procesu (rozdz. 19.1) oraz nagłówek sekcji Monitor procesu panelu orkiestracji (rozdz. 10.9) — dwa równoprawne miejsca tej samej kontrolki | 2 | Znacznik `AOD ▼` w panelu stanu procesu albo w nagłówku sekcji Monitor procesu |
...
| Przycisk zatwierdzenia kroku | Kontrolka trybu operatora | Zatwierdzenie etapu procesu oczekującego na decyzję | Przycisk (`.dn-btn--sygnal`) | Domyślny · ładowanie (przetwarzanie decyzji) · zatwierdzony | Kliknięcie w trybie obserwatora wyświetla komunikat kontekstowy ze wskazaniem przełączenia w tryb operatora, zamiast blokować przycisk; kliknięcie w trybie operatora zatwierdza krok i przekazuje proces do kolejnego kroku orkiestracji | Dymek funkcji, sekcja Monitor procesu | 2 | Przycisk akcji w dymku funkcji |

---

### 2.7. Przedsionek środowiska — widok wejściowy
...
sekcji sterowania tę samą rolę, którą Centrum dowodzenia pełni wobec środowisk:
...
|---|---|---|
| Szyna sesji („Twoje sesje”) | Wykaz sesji Operatora w tym środowisku, zgrupowany procesami zespołowymi, z filtrem zakresu (czynne · wszystkie · zakończone) i wskaźnikiem pracy w tle. W stopce — wejście do pełnej historii sesji | Kolumna przy lewej krawędzi, waga średnia |
| Płótno — nagłówek środowiska | Godło, nazwa, motto i zdanie o przedmiocie pracy środowiska | Waga główna |
| Płótno — kafle sekcji orkiestracji | Sześć kafli sekcji sterowania zespołem — Zespoły, Role, Kolejki, Orkiestracja, Harmonogram i automatyki, Monitor procesu — oraz kafle modułów Workspace i Agents, każdy z ikoną, nazwą, zdaniem o przeznaczeniu i miarą bieżącego obłożenia | Siatka kafli, waga pośrednia |
| Płótno — listwa działań środowiska | Trzy pozycje: **Nowy proces zespołowy**, **Konfiguracja środowiska**, **Ustawienia** — działania dotyczące środowiska jako całości | Listwa jednorzędowa, waga najniższa |

**Czego przedsionek nie zawiera.** Chat Window, Execution Loop Window,
panelu orkiestracji, pasa kart sesji ani żadnego okna operacyjnego. Wszystkie te
elementy należą do przestrzeni roboczej i pojawiają się dopiero po wyborze
sekcji. Moduł Workspace występuje w przedsionku **wyłącznie jako kafel**;
...
roboczą z tą sekcją jako wiodącym i zakłada pierwszą kartę sesji. Kliknięcie
...
**Prototyp odniesienia:** `design/05-okna/srodowiska/multitaskingai-przedsionek.html`.
...
| Executor 2 — równoległy wykonawca | Drugi niezależny model wykonawczy, procesy równoległe (warstwa serwerowa / warstwa kliencka, badania/raport, kod/dokumentacja) | Jak Executor 1; dodatkowo wyniki pośrednie Executora 1, zależnie od trybu współpracy | Jak Executor 1; wynik przekazywany zależnie od trybu współpracy |
...
| Wcielenia opisane w opracowaniu | Agent UI, Agent Backend, Agent API, Agent Security, Agent Database, Agent Testing — sześć wcieleń wymienionych w niniejszym rozdziale. Zbioru wcieleń nie zamyka platforma, lecz konfiguracja: każde kolejne wcielenie Operator ustanawia jako komponent własny rodzaju „agent” w module Agents |
...
Konkretne wcielenie roli jest w pełni zależne od charakteru realizowanego procesu i pozostaje przedmiotem konfiguracji. Pięć wcieleń wymienionych w tabeli powyżej to wcielenia opisane w niniejszym opracowaniu; zbioru nie zamyka platforma, lecz konfiguracja — każde kolejne wcielenie Operator ustanawia jako komponent własny rodzaju „agent” w module Agents. Zgodnie z hierarchią decyzji (rozdz. 8) udział tej roli w procesie jest sterowany ustawieniem konfiguracyjnym: prostszy zespół pomija tę rolę, a użytkownik może w każdej chwili ominąć jej ocenę i zainterweniować bezpośrednio — funkcja kontrolna nie jest bramą blokującą przepływ pracy, lecz konfigurowalnym etapem, który można włączyć, wyłączyć lub pominąć.
...
| Selektor zasięgu kolejki | Pole wyboru jednego z pięciu zasięgów | Określenie, do jakiego poziomu należy tworzona lub edytowana kolejka | Pole wyboru (`.dn-pole-kontrolka`) | Domyślny (globalna) · rozwinięty · wybrany | Zmiana zasięgu przypisuje kolejkę do wskazanego poziomu | Formularz nowej kolejki, wiersz tabeli kolejek | 2 | Lista rozwijana `Zasięg ▼` |
| Przycisk „Nowa kolejka” | Kontrolka dodania kolejki | Utworzenie nowej definicji kolejki | Przycisk (`.dn-btn--sygnal`), ikona `plus` | Domyślny · wskazanie kursorem · wciśnięty | Otwiera formularz definicji nowej kolejki | Nagłówek sekcji Kolejki | 1 | Widoczny bez interakcji |
...
| Pole priorytetu | Wartość liczbowa porządkująca kolejność obsługi | Ustalenie, które zadanie w kolejce obsłużyć w pierwszej kolejności | Pole liczbowe (`.dn-pole-kontrolka`) | Domyślny · edytowany · ostrzeżenie (wartość nietypowa) | Zmiana wartości przelicza kolejność wyświetlania zadań w tabeli; wartość nietypowa jest sygnalizowana ostrzeżeniem, które nie blokuje zapisu | Wiersz zadania w tabeli kolejek | 2 | Kliknięcie wartości w wierszu |
| Selektor obsługi błędów | Pole wyboru zachowania przy niepowodzeniu zadania | Ustalenie domyślnej reakcji kolejki na błąd: `retry`, `route` lub `pause` | Pole wyboru (`.dn-pole-kontrolka`) | Domyślny (`retry`) · rozwinięty · wybrany | Zmiana ustawia regułę stosowaną automatycznie przy kolejnym niepowodzeniu | Formularz definicji kolejki | 3 | Rozwinięcie formularza definicji kolejki |
...
Orkiestracja pozwala definiować zależności pomiędzy modelami, agentami, zadaniami, kolejkami, automatyzacjami oraz projektami. W warstwie komunikacji definicja zależności jest realizowana poleceniem `orchestration.dependency.set` (obszar `orchestration`, `budowa/shared/contract.json`); odczyt zależności niesie `orchestration.dependency.list`, usunięcie — `orchestration.dependency.remove`, a spójność całego układu sprawdza `orchestration.validate`. Warstwa orkiestracji nadaje sens funkcjonowaniu poszczególnych ról i kolejek jako spójnej całości — odpowiada za to, że zadanie Executora 2 nie rozpocznie się przed zakończeniem etapu przypisanego Executorowi 1, jeśli taka zależność została ustalona, oraz że wynik pracy trafi do właściwej kolejki lub roli w kolejnym kroku procesu.
...
| Element | Co to jest | Do czego służy | Forma i waga | Stany | Zachowanie po interakcji | Gdzie występuje | Warstwa | Sposób wywołania |
|---|---|---|---|---|---|---|---|---|
| Lista zależności | Zestawienie wszystkich zdefiniowanych zależności procesu | Przegląd reguł porządkujących przepływ pracy między rolami i kolejkami | Lista pozycji (`.dn-karta--klikalna`) | Domyślna · pusty stan · z zaznaczoną pozycją | Kliknięcie pozycji otwiera szczegóły reguły do edycji | Sekcja Orkiestracja panelu orkiestracji | 1 | Widoczna bez interakcji |
...
| Przycisk „Dodaj zależność” | Kontrolka dodania nowej reguły | Utworzenie nowej zależności sekwencyjnej, warunkowej lub równoległej | Przycisk (`.dn-btn--sygnal`), ikona `plus` | Domyślny · wskazanie kursorem · wciśnięty | Otwiera kreator zależności (wybór rodzaju z rozdz. 5.2) | Nagłówek sekcji Orkiestracja | 1 | Widoczny bez interakcji |
| Plakietka statusu zgodności | Wskaźnik, czy bieżący przebieg respektuje wszystkie zdefiniowane zależności | Szybka sygnalizacja konfliktu lub naruszenia reguły | Mała plakietka (`.dn-plakietka`) | Zgodny (sukces) · oczekujący (info) · naruszony (błąd) | Kliknięcie przy stanie „naruszony” otwiera szczegóły konfliktu | Lista zależności, widok grafu | 1 | Widoczna bez interakcji |
...
|---|---|---|---|---|---|---|
| Zespoły | Zapisane konfiguracje zespołu — szablony ról, powiązań i kolejek | Zapis nowego zespołu, wczytanie zapisanego zespołu, duplikowanie, usunięcie | Szablony konfiguracji zespołu | 14 | 1 | Pozycja panelu orkiestracji, widoczna bez interakcji; zestaw akcji zespołu w menu kebab (⋮) — warstwa 3 |
...
│  PANEL ORKIESTRACJI     │
├─────────────────────────┤
│ ▸ Zespoły               │  szablony ról, powiązań, kolejek
...
| Element | Co to jest | Do czego służy | Forma i waga | Stany | Zachowanie po interakcji | Gdzie występuje | Warstwa | Sposób wywołania |
|---|---|---|---|---|---|---|---|---|
| Pozycja nawigacyjna sekcji | Wiersz listy pionowej z ikoną i etykietą jednej z sześciu sekcji | Przełączenie widoku głównego obszaru roboczego na wskazaną sekcję | Pozycja listy średniej wagi (`.dn-karta--klikalna`) | Domyślna · wskazanie kursorem · aktywna (zaznaczona, tło `--dn-sygnal-tlo`) · z plakietką liczbową (liczba zadań w kolejce, aktywnych ról, naruszonych zależności albo oczekujących decyzji — zgodnie z rozdz. 17.1) | Kliknięcie ładuje zawartość sekcji w głównym obszarze roboczym | Panel orkiestracji, sześć wystąpień | 1 | Widoczna bez interakcji |
| Uchwyt przeciągania kolejności | Mały uchwyt przy pozycji sekcji | Zmiana kolejności wyświetlania sekcji w panelu | Mała ikonka | Domyślny (widoczny przy wskazaniu kursorem) · przeciągany | Przeciągnięcie zmienia kolejność zapisaną w konfiguracji panelu | Panel orkiestracji, tryb edycji układu | 3 | Menu kontekstowe panelu, tryb edycji układu |
...
| Przycisk „Przywróć domyślne” | Kontrolka resetu kolejności i widoczności | Powrót do zestawu i kolejności sześciu sekcji domyślnych | Przycisk tekstowy (`.dn-btn--duch`) | Domyślny · wskazanie kursorem | Przywraca kolejność i widoczność ustaloną jako zestaw domyślny | Okno konfiguracji panelu orkiestracji | 4 | Okno konfiguracji panelu orkiestracji |
...
| Lista wpiętych automatyk | Zestawienie automatyk z modułu Automations powiązanych z bieżącym procesem | Przegląd i zarządzanie automatykami zasilającymi pętlę ciągłą | Lista pozycji (`.dn-karta--klikalna`) | Domyślna · pusty stan · z pozycją aktywną | Kliknięcie pozycji otwiera szczegóły automatyki | Sekcja Harmonogram i automatyki | 1 | Widoczna bez interakcji |
...
| Powiadomienie push interwencji | Komunikat wysyłany na urządzenie mobilne | Poinformowanie użytkownika o zdarzeniu wymagającym decyzji — powtarzającym się niepowodzeniu walidacji | Dymek powiadomienia / powiadomienie systemowe | Wysłane · odczytane · z akcją szybkiej odpowiedzi | Dotknięcie otwiera widok Mobile z kontekstem zdarzenia | Urządzenie mobilne, inicjowane przez Always On Display w trybie operatora | 1 | Dostarczane na urządzenie mobilne |
...
| Element | Co to jest | Do czego służy | Forma i waga | Stany | Zachowanie po interakcji | Gdzie występuje | Warstwa | Sposób wywołania |
|---|---|---|---|---|---|---|---|---|
| Pasek kontekstu procesu | Listwa kontekstu nad kolumnami obszaru roboczego, złożona ze znaczników kontekstowych | Identyfikacja procesu i dostęp do selektorów kontekstu; identyfikację platformy oraz karty sesji niesie rama okna (belka tytułowa i pasmo kart sesji na wstążce) | Listwa kontekstu ze znacznikami (Pasek kontekstu ze znacznikami — [Katalog komponentów](../interfejs-uzytkownika/katalog-komponentow.md) rozdz. 15.9) `[MultitaskingAI] [Zespół] [Rola] [Model]` | Domyślny · ze znacznikiem aktywnym | Kliknięcie znacznika otwiera odpowiedni selektor; listwa nie zmienia stanu przy nawigacji wewnętrznej | Nad kolumnami obszaru roboczego każdej karty sesji | 1 | Widoczny bez interakcji |
...
| Przycisk „+” nowej karty sesji | Ikona dodania karty | Otwarcie nowego procesu orkiestracji równolegle do istniejących | Mała ikonka (`plus`) | Domyślny · wskazanie kursorem | Otwiera nową, pustą kartę sesji środowiska MultitaskingAI | Pasek kontekstu procesu, przy ostatniej karcie | 1 | Widoczny bez interakcji |
| Przycisk zamknięcia karty | Ikona „x” na karcie sesji | Zamknięcie procesu orkiestracji w danej karcie | Mała ikonka (`zamknij`) | Domyślny (widoczny przy wskazaniu kursorem) | Zamyka kartę; stan procesu po stronie serwera zachowany zgodnie z trwałością sesji | Każda karta sesji | 2 | Ujawniany wskazaniem kursorem na kartę |
...
| Pole wyszukiwania | Pole tekstowe z ikoną lupy | Szybkie odnalezienie roli, zadania lub zależności w bieżącym procesie | Pole (`.dn-pole-kontrolka`) z ikoną `szukaj` | Domyślne (puste) · z treścią · brak wyników | Filtruje widoczne listy w otwartej sekcji panelu orkiestracji | Pasek kontekstu procesu, panele list | 2 | Ikona `szukaj`, skrót klawiszowy |
...
| Element | Co to jest | Do czego służy | Forma i waga | Stany | Zachowanie po interakcji | Gdzie występuje | Warstwa | Sposób wywołania |
|---|---|---|---|---|---|---|---|---|
| Lista zapisanych zespołów | Zestawienie szablonów konfiguracji zespołu | Przegląd i wybór gotowej konfiguracji ról, powiązań i kolejek | Lista kart (`.dn-karta--klikalna`) | Domyślna · pusty stan · z zaznaczoną pozycją | Kliknięcie wczytuje podgląd konfiguracji zespołu | Sekcja Zespoły | 1 | Widoczna bez interakcji |
| Przycisk „Zapisz zespół” | Kontrolka zapisu bieżącej konfiguracji jako szablonu | Utrwalenie bieżącego układu ról, trybów współpracy i kolejek do ponownego użycia | Przycisk (`.dn-btn--sygnal`) | Domyślny · ładowanie (zapis w toku) · zapisany (potwierdzenie) | Otwiera pole nazwy szablonu, następnie dodaje go do listy | Nagłówek sekcji Zespoły | 1 | Widoczny bez interakcji |
| Przycisk „Wczytaj zespół” | Kontrolka zastosowania zapisanego szablonu | Ustawienie ról, trybów i kolejek zgodnie z wybranym szablonem | Przycisk (`.dn-btn--zarys`) | Domyślny · ładowanie | Nadpisuje bieżącą konfigurację ról wybranym szablonem od razu; okno nakładkowe potwierdzenia przed nadpisaniem jest ustawieniem konfiguracyjnym włączanym przez Operatora, domyślnie wyłączonym — po wykonaniu dostępne jest „Cofnij” | Wiersz listy zespołów | 2 | Akcja w wierszu listy zespołów |
| Przycisk „Duplikuj” | Ikona kopiowania szablonu | Utworzenie nowej, edytowalnej kopii istniejącego zespołu | Mała ikonka | Domyślny · wskazanie kursorem | Dodaje do listy kopię szablonu z przyrostkiem „(kopia)” | Wiersz listy zespołów, menu kontekstowe | 3 | Menu kebab (⋮) wiersza |
| Przycisk „Usuń” | Ikona kosza | Usunięcie zapisanego szablonu zespołu | Mała ikonka (`kosz`) | Domyślny · ładowanie · usunięty (z „Cofnij”) | Usuwa pozycję z listy od razu, z dostępnym „Cofnij” w dymku powiadomienia po wykonaniu; okno nakładkowe potwierdzenia przed usunięciem jest ustawieniem konfiguracyjnym włączanym przez Operatora, domyślnie wyłączonym | Wiersz listy zespołów, menu kontekstowe | 3 | Menu kebab (⋮) wiersza |
...
| Awatar roli | Element identyfikacji wizualnej przypisanego wykonawcy | Rozróżnienie, czy rolę pełni agent, czy model bazowy | Awatar (`.dn-awatar`), wariant kwadratowy dla agenta, kołowy dla modelu bazowego | Domyślny · ze wskaźnikiem statusu (`-stan`) | Wskazanie kursorem pokazuje nazwę i tożsamość przypisanego wykonawcy | Karta roli, nagłówek okna roboczego | 1 | Widoczny bez interakcji |
| Selektor przypisania wykonawcy | Pole wyboru agenta z modułu Agents lub modelu bazowego | Ustalenie, kto faktycznie pełni daną rolę | Pole wyboru (`.dn-pole-kontrolka`) | Domyślny (brak przypisania) · wybrany agent · wybrany model bazowy | Zmiana natychmiast podmienia definicję działającą w roli | Karta roli, nagłówek okna roboczego | 2 | Znacznik kontekstowy `Agent ▼` |
| Selektor kanału modelu | Pole wyboru jednego z czterech kanałów | Ustalenie sposobu połączenia z modelem dla danej roli, nadpisujące kanał domyślny agenta | Pole wyboru (`.dn-pole-kontrolka`) | Domyślny (dziedziczony z definicji agenta) · nadpisany | Zmiana kanału wymaga ponownego uruchomienia procesu roli | Nagłówek okna roboczego roli | 2 | Znacznik kontekstowy `Kanał ▼` |
| Plakietka statusu roli | Wskaźnik bieżącego stanu wykonania | Szybka orientacja, czy rola pracuje, oczekuje, czy zgłosiła błąd | Plakietka + kropka (`.dn-plakietka`, `.dn-kropka`) | Bezczynna · pracuje · oczekuje na zależność · błąd | Kliknięcie otwiera log ostatniej operacji roli | Karta roli, makieta główna, pasek stanu | 1 | Widoczna bez interakcji |
...
| Zestaw przycisków sterowania procesem | Siedem przycisków: Start, Stop, Pauza, Wznów, Przekaż, Powtórz, Waliduj | Bezpośrednie sterowanie przebiegiem procesu z poziomu okna Coordinatora | Zestaw przycisków ikonowych (`.dn-btn-ikona`) | Każdy przycisk niezależnie: dostępny · ładowanie | Wywołuje odpowiadającą akcję lub mechanizm (rozdz. 3.3); przycisk nieadekwatny do bieżącego stanu procesu — „Wznów” przy procesie już uruchomionym — pozostaje klikalny i pokazuje komunikat kontekstowy zamiast się blokować | Coordinator Chat, element zbiorczy `Sterowanie ▼` | 3 | Rozwinięcie `Sterowanie ▼` |
| Selektor trybu współpracy | Pole wyboru jednego z czterech trybów (rozdz. 3.4) | Ustalenie relacji między Executorem 1 a Executorem 2 dla bieżącego procesu | Pole wyboru (`.dn-pole-kontrolka`) | Domyślny (praca niezależna) · zmieniony | Zmiana trybu natychmiast wpływa na sposób kierowania zadań między wykonawcami | Coordinator Chat, karta roli Executor 2 | 2 | Znacznik kontekstowy `Tryb ▼` |
...
| Selektor wcielenia | Pole wyboru jednego z siedmiu wcieleń roli (rozdz. 3.5) | Ustalenie, w jakiej funkcji kontrolnej działa Executor 3 / Validator w bieżącym etapie | Pole wyboru (`.dn-pole-kontrolka`) | Domyślny (Validator) · zmieniony | Zmiana wcielenia dostosowuje zestaw kryteriów oceny prezentowanych w panelu | Nagłówek Results Analyzer | 2 | Znacznik kontekstowy `Wcielenie ▼` |
| Przycisk „Zatwierdź” | Kontrolka pozytywnej oceny etapu | Uznanie wyniku za zgodny i przekazanie procesu dalej | Przycisk (`.dn-btn--sygnal`), ikona `ptaszek` | Domyślny · ładowanie · zatwierdzony | Kieruje wynik do kolejnego etapu orkiestracji (`route`) | Results Analyzer | 1 | Widoczny bez interakcji |
| Przycisk „Odrzuć / zgłoś niezgodność” | Kontrolka negatywnej oceny etapu | Zwrot etapu do Coordinatora z uwagami, w celu powtórzenia | Przycisk (`.dn-btn--niebezpieczny`) | Domyślny · ładowanie | Wywołuje `retry` z dołączonym uzasadnieniem | Results Analyzer | 1 | Widoczny bez interakcji |
...
| Pole uzasadnienia oceny | Pole tekstowe towarzyszące decyzji | Zapisanie uwag przekazywanych do Coordinatora wraz z decyzją | Pole tekstowe (`.dn-pole-kontrolka`) | Domyślne (puste) · wypełnione | Treść dołączana jest do zdarzenia `retry` lub do historii etapu | Results Analyzer | 1 | Widoczne bez interakcji |
...
| Plakietka statusu przebiegu | Znacznik sukcesu, błędu lub trwania przebiegu | Szybka orientacja w wyniku każdego cyklu | Plakietka + kropka (`.dn-plakietka`) | Sukces · w toku · błąd · wstrzymany | Kliknięcie przy błędzie otwiera bezpośrednio przyczynę niepowodzenia | Wiersz tabeli przebiegów | 1 | Widoczna bez interakcji |
...
| Przełącznik trybu AOD w monitorze | Kontrolka tożsama z opisaną w rozdziale 2.6 | Zmiana trybu Always On Display bezpośrednio z poziomu monitora | Przełącznik segmentowy | Obserwator · operator | Jak w rozdziale 2.6 | Nagłówek sekcji Monitor procesu — jedno z dwóch równoprawnych, zamierzonych miejsc tej kontrolki; drugim jest panel stanu procesu (rozdz. 19.1), a opis wspólny prowadzi rozdz. 2.6 | 2 | Znacznik `AOD ▼` w nagłówku sekcji albo w panelu stanu procesu |
| Przycisk „Szczegóły przebiegu” | Odnośnik/przycisk otwierający pełny log cyklu | Głęboki wgląd w każdy krok danego przebiegu — pełna, nieograniczona widoczność procesu | Przycisk tekstowy / link | Domyślny · wskazanie kursorem | Otwiera pełny, chronologiczny log zdarzeń przebiegu, łącznie z każdą akcją silnika kolejek | Wiersz tabeli przebiegów | 3 | Menu kebab (⋮) wiersza tabeli przebiegów |
...
| Panel dekompozycji zlecenia | Lista zadań, na które Koordynator rozłożył bieżące zlecenie, z przypisaną rolą i stanem | Zrozumienie, z czego składa się bieżące zlecenie i kto je realizuje | Lista pozycji (`.dn-karta--klikalna`) | Domyślna · pusty stan · z zadaniem zaznaczonym | Kliknięcie zadania otwiera jego szczegóły w sekcji Kolejki | Execution Loop Window | 1 | Widoczny bez interakcji |
...
Powyższe stany opisują funkcję wyłącznie w odniesieniu do procesu środowiska. Stany ogólnoplatformowe funkcji — spoczynek, sugestia oczekująca, rozmowa, nasłuch, mowa, wyciszenie, tryby obecności — podaje [Always On Display](../funkcje-globalne/always-on-display.md), rozdz. 9.1–9.2.
...
| Wskazanie kursorem | Subtelna zmiana tła lub uniesienie dla elementów akcentowanych złotem | Przyciski, karty, pozycje list |
| Aktywny / wciśnięty | Przesunięcie w dół zgodne z regułą `.dn-btn:active` arkusza `design/zasoby/css/komponenty.css` albo zaznaczenie trwałe (zakładki, karty ról) | Przyciski, zakładki, pozycje nawigacyjne |
| Niegotowy (aktywny, z komunikatem) | Wygląd tożsamy ze stanem domyślnym, bez przyciemnienia i bez blokady kursora — element pozostaje w pełni klikalny | Akcje, których warunek nie jest jeszcze spełniony w bieżącym stanie procesu — jak `merge` przed zakończeniem wszystkich podagentów; kliknięcie wywołuje komunikat kontekstowy zamiast wykonania akcji lub wykonuje ją częściowo, z ostrzeżeniem |
...
| Fokus | Pierścień fokusu o szerokości `--dn-wym-fokus`, odsunięciu `--dn-wym-fokus-odsuniecie` i kolorze `--dn-fokus`, wyzwalany przez nawigację klawiaturą | Wszystkie kontrolki interaktywne, obowiązkowo |
...
Pełny katalog przedstawia rozdział 6.4 opracowania [Koncepcja platformy](../architektura/koncepcja-platformy.md) — „Okno konfiguracji punktów izolacji” — oraz makieta w rozdziale 9.10 niniejszego dokumentu; poniżej elementy swoiste zastosowaniu na poziomie „Rola”.

| Element | Co to jest | Do czego służy | Forma i waga | Stany | Zachowanie po interakcji | Gdzie występuje | Warstwa | Sposób wywołania |
|---|---|---|---|---|---|---|---|---|
| Pozycja „Rola” w selektorze zasięgu | Wpis listy poziomów zasięgu | Wybranie poziomu „Rola” jako kontekstu edytowanej macierzy izolacji | Pozycja listy (`.dn-karta--klikalna`) | Domyślna · zaznaczona | Po zaznaczeniu ujawnia dodatkowy selektor konkretnej roli (Executor 1/2/Coordinator/Validator) | Panel selektora zasięgu okna konfiguracji izolacji | 4 | Okno konfiguracji punktów izolacji |
| Selektor konkretnej roli | Pole wyboru, której z czterech ról dotyczy edytowany profil | Doprecyzowanie, do jakiej roli bieżącego procesu przypiąć profil | Pole wyboru (`.dn-pole-kontrolka`) | Widoczny wyłącznie przy zasięgu „Rola” | Zmiana przełącza podgląd aktualnej polityki efektywnej danej roli | Panel selektora zasięgu, przy pozycji „Rola” | 4 | Okno konfiguracji punktów izolacji, przy pozycji „Rola” |
| Ikona objaśnienia kontekstowego | Znak zapytania przy każdym przełączniku macierzy | Wyjaśnienie działania ustawienia i jego wpływu na aplikację | Mała ikonka (`info`) | Domyślna · rozwinięty (dymek podpowiedzi widoczny) | Wskazanie kursorem lub kliknięcie pokazuje tekst objaśnienia | Każdy wiersz macierzy izolacji | 2 | Wskazanie kursorem lub kliknięcie ikony `[?]` |
...
|---|---|---|---|---|---|
| Coordinator | Planowanie etapów, przypisanie pracy warstwie serwerowej i warstwie klienckiej | — | Nie | Domyślny (dziedziczony) | Tak |
| Executor 1 | Realizacja warstwy serwerowej | Praca niezależna | Tak — Agent Backend, Agent API, Agent Database | Domyślny (dziedziczony) | Tak |
| Executor 2 | Realizacja warstwy klienckiej (zasoby z modułu Design) | Praca niezależna | Tak — Agent UI | Domyślny (dziedziczony) | Tak |
...
| 5 | Operator definiuje zależności sekwencyjne i warunkowe w sekcji Orkiestracja, wiążąc etapy warstwy serwerowej, warstwy klienckiej i walidacji | Panel orkiestracji ▸ Orkiestracja |
...
|---|---|
| 1. Pełna kompozycyjność | Role, tryby współpracy, akcje silnika kolejek i zależności orkiestracji są komponentami swobodnie składanymi w dowolną konfigurację zespołu — trzy szablony z rozdziału 14 są gotowymi kompozycjami dostarczonymi wraz z opracowaniem, a nie granicą możliwych złożeń |
...
| Przełącznik środowisk | Wyjście z MultitaskingAI do TalkIn, WorkSpace, CodeStudio lub strony głównej; procesy MultitaskingAI działają dalej w tle | Płynne przechodzenie między środowiskami bez utraty stanu procesów | [Strona główna i nawigacja](../interfejs-uzytkownika/strona-glowna-i-nawigacja.md) (rozdz. 8.2–8.3); trwałość sesji | 2 | Znacznik kontekstowy `[MultitaskingAI]` w pasku kontekstu |
| Nawigacja klawiaturą po sekcjach | Klawisze 1–6 przełączają wprost do sekcji panelu; strzałki poruszają się po pozycjach list | Praca bez myszy przy sterowaniu procesem | Rozdz. 19.2 | 4 | Klawisze `1`–`6`, klawisze strzałek |
...
| Zapisywane układy kolumn | Zapis bieżącego rozmieszczenia, szerokości i zwinięć kolumn jako nazwany szablon; wczytanie jednym kliknięciem; szablony wspólne z szablonami zespołu | Powrót do sprawdzonego układu pracy dla danego typu procesu | Sekcja Zespoły (rozdz. 6.2, 14); [Model konfiguracji](../architektura/model-konfiguracji.md) | 3 | Rozwinięcie `Układ ▼ ▸ Szablony` |
...
| Okna oderwane | Okno roli lub sekcja wypinane są do osobnego okna systemowego — praca na wielu monitorach; okno oderwane zachowuje układ kolumnowy | Wykorzystanie drugiego ekranu do rozdzielenia wykonania od nadzoru | Klient Tauri ([Architektura techniczna](../architektura/architektura.md)); trwałość stanu sesji | 3 | Menu kebab (⋮) nagłówka okna |
...
| Migawka układu | Bieżące rozmieszczenie kolumn i statusów eksportowane jest jako obraz lub schemat tekstowy | Dokumentowanie konfiguracji zespołu i dzielenie się układem | Rozdz. 21; [System wizualny](../interfejs-uzytkownika/system-wizualny.md) | 3 | Menu kebab (⋮) paska kontekstu procesu |
...
| Grupowanie kart sesji | Powiązane procesy łączone są w nazwaną grupę — nazwa grupy pochodzi od Operatora, wzorem zapisu jest „Projekt X — warstwa serwerowa, warstwa kliencka, kontrola jakości” — ze wspólnym zwijaniem | Porządek przy dziesiątkach równoległych zespołów | Pasek kontekstu procesu (rozdz. 10.1) | 3 | Menu kebab (⋮) karty, przeciągnięcie karty na kartę |
| Migawka procesu | Pełny stan procesu — role, kolejki, zależności, historia — zapisywany jest jako punkt przywracania; powrót następuje jednym kliknięciem | Bezpieczne eksperymentowanie z konfiguracją zespołu i powrót do znanego stanu | Trwałość stanu po stronie serwera (rozdz. 9.1); [Model danych](../architektura/model-danych.md) | 3 | Menu kebab (⋮) karty, paleta poleceń |
...
| Zamknięcie z zachowaniem stanu | Zamknięcie karty nie kończy procesu po stronie serwera; dymek powiadomienia „Cofnij” przywraca widok; trwałe zakończenie procesu jest osobną, świadomą akcją | Zero utraty pracy przy przypadkowym zamknięciu | Trwałość sesji; komponent `.dn-toast` (Załącznik A.2) | 1 | Ikona zamknięcia karty |
...
| Licznik kosztu i zużycia | Bieżący koszt i zużycie tokenów przebiegu oraz sumarycznie, z rozbiciem na poszczególne role po wskazaniu kursorem | Kontrola budżetu procesu wieloagentowego (do 30 jednostek wykonawczych) | [Integracja modeli](../architektura/integracja-modeli.md); kanały modeli (rozdz. 3.1) | 2 | Wskazanie kursorem segmentu, rozwinięcie szczegółów |
...
| Konfiguracja segmentów panelu | Włączenie, wyłączenie i kolejność segmentów panelu stanu procesu | Dopasowanie panelu do tego, co w danym procesie istotne | Rozdz. 22; [Model konfiguracji](../architektura/model-konfiguracji.md) | 4 | Okno konfiguracji |
| Wskaźnik połączenia na żywo | Stan kanału WebSocket: na żywo, ponawianie, rozłączony — bez blokowania pracy | Zaufanie do aktualności statusów ról i kolejek | [Architektura techniczna](../architektura/architektura.md) (WebSocket/JSON) | 1 | Widoczny bez interakcji |

### 19.2. Skróty klawiszowe i paleta poleceń

**Reguła platformowa.** Żaden skrót klawiszowy środowiska nie jest ustalony na sztywno. Wszystkie skróty nadaje Operator samodzielnie w oknie konfiguracji i ustawień, w edytorze skrótów opisanym niżej w tej samej tabeli; poniższa tabela podaje **wartości domyślne** i wskazuje czynności, dla których wartości fabrycznej nie ma — Operator nadaje im kombinację przy pierwszym użyciu. Każdy skrót jest zmienialny i usuwalny, a przycisk „Przywróć domyślne” przywraca stan z tej tabeli. Kombinacje wpisane w kolumnie „Sposób wywołania” są zatem punktem wyjścia, nie zbiorem zamkniętym.
...
| Skróty sterowania procesem | Klawisze do operacji Start, Pauza, Wznów, Stop, Waliduj oraz `retry` ostatniego zadania | Szybkie sterowanie pętlą bez myszy | Akcje silnika kolejek (rozdz. 4.2); rozdz. 19.1 | 4 | Skrót nadawany przez Operatora w edytorze skrótów — po jednej czynności na operację Start, Pauza, Wznów, Stop, Waliduj i `retry`; bez wartości fabrycznej, pole przypisania puste do pierwszego nadania |
| Skróty akcji kolejek | Przypisania klawiszowe do jedenastu akcji silnika kolejek na zaznaczonym zadaniu | Sprawna praca w gęstej sekcji Kolejki | Zestaw przycisków akcji (rozdz. 4.5) | 4 | Skrót nadawany przez Operatora w edytorze skrótów — po jednej czynności na każdą z jedenastu akcji (`enqueue`, `dequeue`, `delay`, `retry`, `pause`, `resume`, `split`, `merge`, `route`, `branch`, `condition`), działającej na zadaniu zaznaczonym w sekcji Kolejki; bez wartości fabrycznej |
| Skróty przełączania okien | Klawisze do skoku między Chat Window, Execution Loop Window, Executor 1, Executor 2, Coordinator i Validatorem oraz do trybu skupienia | Płynne przechodzenie między torami pracy | Karty ról (rozdz. 10.4); rozdz. 17.2 | 4 | Skrót nadawany przez Operatora w edytorze skrótów — po jednej czynności na skok do Chat Window, Execution Loop Window, Executora 1, Executora 2, Coordinatora i Validatora oraz na tryb skupienia; bez wartości fabrycznej. Przechodzenie między kartami sesji zachowuje wartości domyślne powłoki (`Ctrl+Tab`, `Ctrl+Shift+Tab`, rozdz. 17.1) i pozostaje czynnością odrębną od skoku między oknami ról |
| Skróty warstwy centralnej | Wywołanie dymka Always On Display, mikrofonu i przełącznika trybu AOD | Natychmiastowy dostęp do nadzoru i komunikacji głosowej | Elementy AOD (rozdz. 2.6) | 4 | Domyślnie `Ctrl+Shift+A` — otwarcie powierzchni interakcji Always On Display; wartość widoczna w menu `Always On Display` prototypu `design/05-okna/srodowiska/multitaskingai.html` i przyjęta tu jako domyślna. Nasłuch głosowy i przełącznik trybu obserwator/operator (rozdz. 2.6, 10.9) są w tym samym menu pozycjami bez wartości fabrycznej; skrót nadaje im Operator w edytorze skrótów |
| Edytor skrótów | Okno konfiguracji, w którym każdy skrót powłoki jest podglądalny i przypisywalny na nowo; „Przywróć domyślne” pozostaje dostępne | Dopasowanie skrótów do nawyków Operatora | Rozdz. 22; [Model konfiguracji](../architektura/model-konfiguracji.md) | 4 | Okno konfiguracji |
| Ściągawka skrótów | Nakładka z listą wszystkich aktywnych skrótów, pogrupowaną tematycznie | Przypomnienie skrótów bez opuszczania środowiska | Edytor skrótów | 4 | Domyślnie `Ctrl+/` — wartość domyślna ramy okna dla czynności „Skróty klawiszowe” ([Rama okna](../interfejs-uzytkownika/rama-okna.md), rozdz. 22.4); środowisko jej nie przedefiniowuje, a Operator zmienia ją w edytorze skrótów |
...
| Przekazanie wyniku do modułu docelowego | Rezultat procesu kierowany jest do modułu tematycznego (Apps, Studio, Library) jako artefakt, jednym poleceniem z okna roli lub z Chat Window | Domknięcie drogi „proces → produkt” bez wychodzenia ze środowiska | Powiązania modułów ([Koncepcja platformy](../architektura/koncepcja-platformy.md)); akcja `route` (rozdz. 4.2) | 3 | Menu kebab (⋮) wyniku, polecenie w Chat Window |
...
| Odpięcie kontekstu jednym ruchem | Dowolne współdzielenie cofane jest natychmiast, z powrotem do stanu odrębnego; dymek powiadomienia zawiera „Cofnij” | Odwracalność każdej decyzji o współdzieleniu | Komponent `.dn-toast` | 2 | Kliknięcie źródła w pasku widoczności kontekstu |
...
| Centrum powiadomień | Mechanizm w postaci jednolitej dla całej platformy — [Katalog komponentów](../interfejs-uzytkownika/katalog-komponentow.md), rozdz. 11.6. Specyfika MultitaskingAI: zakres źródeł obejmuje wszystkie procesy orkiestracji równocześnie, a filtr źródła zawęża listę do wskazanego procesu, roli albo kolejki | Nic istotnego nie umyka przy wielu równoległych procesach | Plakietka powiadomień AOD (rozdz. 2.6); powiadomienia wypychane Mobile (rozdz. 7.6); encja `powiadomienie` — [Model danych](../architektura/model-danych.md), rozdz. 18.4 | 3 | Plakietka powiadomień z licznikiem w pasku kontekstu |
...
| Motyw i gęstość interfejsu | Motyw jasny lub ciemny oraz gęstość interfejsu: komfortowa albo kompaktowa — dla widoku wielokolumnowego | Czytelność przy wielu kolumnach naraz i oszczędność szerokości | Przełącznik motywu (rozdz. 10.1); [System wizualny](../interfejs-uzytkownika/system-wizualny.md) | 3 | Menu kebab (⋮) paska kontekstu, okno konfiguracji |
...
| Eksport i import konfiguracji środowiska | Cała konfiguracja powłoki — układy kolumn, skróty, segmenty panelu stanu, sekcje — zapisywana jest i wczytywana jako plik przenośny | Przeniesienie ustawień między stanowiskami i kopia zapasowa | [Model konfiguracji](../architektura/model-konfiguracji.md); rozdz. 17.2, 19, 22 | 4 | Okno konfiguracji |
...
| Objaśnienia kontekstowe | Przy każdym elemencie konfiguracji powłoki ikona `info` z wyjaśnieniem działania i wpływu ustawienia | Obniżenie progu wejścia do najbardziej złożonego środowiska platformy | Ikona objaśnienia (rozdz. 13.5); [System wizualny](../interfejs-uzytkownika/system-wizualny.md) | 2 | Wskazanie kursorem lub kliknięcie ikony `[?]` |
...
| Domyślny układ kolumn okien ról | Układ startowy nowego procesu (rozdz. 17.2) | środowisko / zespół | cztery kolumny albo układ z szablonu zespołu |
| Szablony układu kolumn | Nazwane rozmieszczenia kolumn (rozdz. 17.2) | zespół / projekt | brak — tworzone przez Operatora |
...
| Powiadomienia i ich waga | Które klasy zdarzeń zgłasza centrum powiadomień (rozdz. 21; [Katalog komponentów](../interfejs-uzytkownika/katalog-komponentow.md), rozdz. 11.6) | środowisko / proces | wszystkie klasy, waga normalna |
...
| Potwierdzenia akcji nieodwracalnych | Okno nakładkowe przed usunięciem zespołu, kolejki albo procesu | globalny / środowisko | wyłączone (akcja od razu + „Cofnij”) |
...
## 24. Wykazy normatywne i kryteria odbioru

Rozdział zamyka korpus dokumentu kompletem wykazów obowiązkowych wskazanych w [Standardzie redakcyjnym i językowym](../STANDARD-REDAKCYJNY-I-JEZYKOWY.md), rozdz. 4.2 — etykiety interfejsu, komunikaty, żetony, komponenty, skróty klawiszowe, stany kontrolek i punkty łamania — a kończy go zbiór kryteriów odbioru. Wartości pochodzą z prototypów `design/05-okna/srodowiska/multitaskingai.html`, `multitaskingai-przedsionek.html`, `mtai-okna-rol.html` i `mtai-izolacja-i-zespoly.html`, z arkuszy `design/zasoby/zetony/zetony.css` i `design/zasoby/css/komponenty.css` oraz z `budowa/shared/contract.json`. Wykaz komend kontraktu prowadzony jest osobno, w Załączniku C, i nie jest tu powtarzany.

### 24.1. Etykiety interfejsu z prototypów

Dosłowne brzmienia przejęte z prototypów odniesienia. Etykiety wymienione już w rozdziałach 2–14 nie są powtarzane; wykaz uzupełnia je o brzmienia formularzy, okien nakładkowych i pól poleceń.

| Element | Dosłowne brzmienie | Miejsce wystąpienia |
|---|---|---|
| Tytuł powłoki środowiska | „Danaco Console — Powłoka MultitaskingAI · panel orkiestracji” | Belka tytułowa, rozdz. 17.1 |
| Motto środowiska w szynie i na karcie | „Orkiestracja pracy ciągłej” | Szyna nawigacji, przedsionek (rozdz. 2.7) |
| Sekcje panelu orkiestracji | „Zespoły” · „Role” · „Kolejki” · „Orkiestracja” · „Harmonogram i automatyki” · „Monitor procesu” | Panel orkiestracji, rozdz. 6.2 |
| Tytuł okna konfiguracji zespołu | „Formularz konfiguracji zespołu” | Rozdz. 14.4 |
| Tytuł okna akcji kolejek | „Jedenaście akcji silnika kolejek” | Rozdz. 4.2, 4.5 |
| Tytuł okna konfiguracji panelu | „Konfiguracja panelu orkiestracji” | Rozdz. 6.5 |
| Tytuł okna izolacji | „Okno konfiguracji punktów izolacji — zasięg „Rola”” | Rozdz. 9.10, 13.5 |
| Tytuł okna szczegółów przebiegu | „Szczegóły przebiegu #131” | Monitor procesu, rozdz. 10.9 |
| Etykiety pól formularza zespołu | „Nazwa zespołu” · „Cel procesu” · „Executor 1 — przypisany agent/model” · „Executor 1 — zadanie” · „Executor 1 — Subagent Network (wcielenia)” · „Coordinator — przypisany agent/model” · „Executor 3 / Validator — wcielenie” · „Tryb współpracy Executor 1 / Executor 2” · „Zasięg i nazwa kolejki (kolejek)” · „Zależności orkiestracji” · „Powiązanie z Automations” · „Harmonogram (jeśli powiązanie = tak)” · „Tryb Always On Display” · „Profil izolacji — poziom „Rola”” | Rozdz. 14.4 |
| Etykiety pól okna roli | „Przypisanie” · „Kanał modelu” · „Wcielenie” · „Udział roli w procesie” · „Kolejka docelowa” · „Prompt dla” · „Treść budowanego promptu (edytowalna)” · „Pole uzasadnienia oceny — treść przekazywana do Coordinatora” | Rozdz. 10.4, 10.6, 10.7 |
| Etykiety pól okna izolacji | „Nazwa profilu izolacji” · „Rola, której dotyczy profil” · „Reguła czasowa” · „Cykliczność” · „Następny cykl” | Rozdz. 13.2, 13.5 |
| Podpowiedzi pól poleceń | „Wpisz polecenie dla wykonawcy…” · „Polecenie dla roli — poza kolejką…” | Executor Chat, Coordinator Chat (rozdz. 9.6, 9.8) |
| Podpowiedź pola wyszukiwania | „Szukaj w sesji, projekcie i środowisku…  Ctrl+K” | Pole wyszukiwania powłoki, rozdz. 10.1, 21 |
| Podpowiedź filtru macierzy izolacji | „Filtruj zakresy…” | Rozdz. 13.5 |
| Podpowiedzi wartości przykładowych | „np. Budowa aplikacji” · „np. realizacja produktu cyfrowego w module Apps” · „lokalna — Kolejka główna” · „co dobę 22:00” · „domyślny (dziedziczony)” | Rozdz. 14.4 |
| Znacznik trybu warstwy centralnej | „AOD · OBSERWATOR” | Pasek kontekstu, rozdz. 2.2, 19.1 |
| Nagłówki ról w makietach | „GŁÓWNY WYKONAWCA” · „KOORDYNATOR” · „CZWARTY MODEL” · „MECHANIZM” | Rozdz. 3, 9.6, 9.8, 9.9 |
| Wpis nowego procesu w przedsionku | „Nowy proces zespołowy” | Rozdz. 2.7 |
| Filtry szyny sesji przedsionka | „Czynne” · „Wszystkie” · „Zakończone” | Rozdz. 2.7 |
| Pozycje menu Always On Display | „Powierzchnia interakcji” · „Oczekujące sugestie” · „Historia sugestii” · „Nasłuch głosowy” · „Obserwacja procesu” · „Wyciszenie sugestii” · „Reguły wyzwalania” | Rozdz. 2.6, 19.2 |
| Etykiety stanów ról | „Pracuje” · „Bezczynny” · „Oczekuje na zależność” · „Oczekuje na ocenę” · „Błąd wykonania” | Rozdz. 12.2 |
| Etykiety stanów zgodności zależności | „Zgodny” · „Oczekujący” · „Naruszony” | Rozdz. 5.4, 19.1 |

### 24.2. Komunikaty

| Sytuacja | Dosłowna treść albo wzorzec | Rodzaj |
|---|---|---|
| Proces bez spięcia z modułem Automations | „Proces nie jest spięty z modułem Automations” — „Włącz przełącznik „Powiąż z Automations”, aby otworzyć panel wyboru automatyki i harmonogramu. Stan wyjściowy: niespięte.” | Stan pusty |
| Filtr zespołów bez trafień | „Brak dopasowanych zespołów” — „Zmień frazę wyszukiwania w pasku górnym.” | Stan pusty |
| Filtr macierzy izolacji bez trafień | „Brak zakresów pasujących do filtru” — „Wyczyść pole filtru, aby zobaczyć wszystkie jedenaście pozycji macierzy.” | Stan pusty |
| Reguła harmonogramu wewnętrznie sprzeczna | „Reguła sprzeczna zostaje zapisana z ostrzeżeniem zamiast blokady zapisu.” | Ostrzeżenie |
| Puste pole profilu izolacji roli | „Puste pole oznacza profil domyślny (dziedziczony) — brak aktywnej izolacji technicznej.” | Stan pusty |
| Rola pominięta w prostszym zespole | „Udział roli jest opcjonalny — prostszy zespół może ją pominąć.” | Objaśnienie kontekstowe |
| Limit podagentów Subagent Network | „Do 15 wyspecjalizowanych podagentów jednocześnie; limit ma charakter referencyjny.” | Objaśnienie kontekstowe |
| Nowy proces bez zespołu | Pusty stan (`.dn-pusty-stan`) z krokami startowymi: wczytanie zespołu, przypisanie ról, dodanie kolejki (rozdz. 21) | Stan pusty |
| Akcja niedostępna w bieżącym stanie procesu | Komunikat kontekstowy zamiast wykonania akcji — kontrolka pozostaje klikalna, zgodnie z konwencją stanu „niegotowy” (rozdz. 12.6) | Ostrzeżenie |
| Wykonanie akcji nieodwracalnej | Dymek powiadomienia (`.dn-toast`) z przyciskiem „Cofnij”; okno potwierdzenia wyłącznie po włączeniu ustawienia w oknie konfiguracji (rozdz. 22) | Potwierdzenie |
| Niepowodzenie połączenia z modułem Automations | Plakietka błędu w segmencie integracji panelu stanu procesu, treść z pola błędu wyniku komendy — praca środowiska trwa dalej (rozdz. 19.1) | Błąd |
| Naruszenie reguły orkiestracji | Plakietka „Naruszony” w segmencie zgodności zależności, kliknięcie otwiera szczegóły konfliktu (rozdz. 5.4, 19.1) | Ostrzeżenie |
| Utrata kanału zdarzeń na żywo | Wskaźnik połączenia w stanie „ponawianie” albo „rozłączony”, bez blokowania pracy (rozdz. 19.1) | Ostrzeżenie |

### 24.3. Żetony projektowe

| Miejsce zastosowania | Żeton `--dn-*` | Czego dotyczy |
|---|---|---|
| Kropka stanu roli i przebiegu | `--dn-kropka`, `--dn-wym-kropka`, `--dn-czas-tetno` | Wskaźnik pracy roli w karcie roli i w panelu stanu procesu (rozdz. 10.4, 19.1) |
| Ognisko wszystkich kontrolek | `--dn-fokus`, `--dn-fokus-cien`, `--dn-wym-fokus`, `--dn-wym-fokus-odsuniecie` | Pierścień ogniska klawiaturowego, obowiązkowy dla każdej kontrolki (rozdz. 12.6) |
| Akcja główna sterowania procesem | `--dn-sygnal`, `--dn-sygnal-wypelnienie`, `--dn-sygnal-wypelnienie-hover` | Przyciski Start, Wznów, „Wyślij”, „Scal wyniki” (rozdz. 3.3, 10.6) |
| Stan powodzenia walidacji | `--dn-sukces-tekst`, `--dn-sukces-tlo`, `--dn-sukces-obrys` | Plakietka pozytywnej oceny Results Analyzera (rozdz. 10.7) |
| Stan ostrzeżenia orkiestracji | `--dn-ostrzezenie-tekst`, `--dn-ostrzezenie-tlo`, `--dn-ostrzezenie-obrys` | Reguła sprzeczna, zależność oczekująca, akcja niegotowa (rozdz. 5.4, 12.6) |
| Stan błędu wykonania | `--dn-blad-tekst`, `--dn-blad-tlo`, `--dn-blad-obrys` | Plakietka „Błąd wykonania” roli i zadania (rozdz. 12.2, 12.3) |
| Stan informacyjny | `--dn-informacja-tekst`, `--dn-informacja-tlo`, `--dn-informacja-obrys` | Plakietki objaśnień kontekstowych i wskaźnik trybu AOD (rozdz. 2.6) |
| Odstępy kolumn, kart i wierszy | `--dn-od-2`, `--dn-od-3`, `--dn-od-4`, `--dn-od-6`, `--dn-odstep-panel`, `--dn-odstep-sekcji` | Odstęp wewnętrzny kart ról, wierszy tabeli kolejek, sekcji panelu orkiestracji |
| Promienie zaokrągleń | `--dn-r-sm`, `--dn-r-md`, `--dn-r-lg`, `--dn-r-xl`, `--dn-r-pill` | Pola i przyciski (`--dn-r-sm`), karty ról (`--dn-r-md`), okna nakładkowe (`--dn-r-lg`), karta środowiska w przedsionku (`--dn-r-xl`), plakietki (`--dn-r-pill`) |
| Typografia interfejsu kokpitu | `--dn-ff-bazowa`, `--dn-fs-sm`, `--dn-fs-base`, `--dn-lh-bazowy`, `--dn-fw-srednia` | Tekst kontrolek, etykiet pól i pozycji list w gęstości kompaktowej (rozdz. 21) |
| Typografia identyfikatorów technicznych | `--dn-ff-mono`, `--dn-ls-mono-wersaliki` | Nazwy komend, akcji kolejek, numery zadań i przebiegów |
| Czasy przejść | `--dn-czas-1`, `--dn-czas-2`, `--dn-czas-3`, `--dn-ease` | Mikroreakcje kontrolek, przejścia barw, otwarcie i zwinięcie kolumn oraz okien nakładkowych |
| Wymiary powierzchni powłoki | `--dn-wym-belka`, `--dn-wym-pasek`, `--dn-wym-stan`, `--dn-wym-boczna`, `--dn-wym-pas-kart`, `--dn-wym-pas-komunikacji` | Belka tytułowa, pasek edycji, pasek stanu, panel orkiestracji, pasmo kart sesji, kolumny okien komunikacji (rozdz. 17) |
| Wymiary kontrolek i awatarów ról | `--dn-wym-ikonowy`, `--dn-wym-ikona`, `--dn-wym-ikona-sm`, `--dn-wym-kontrolka`, `--dn-wym-awatar`, `--dn-wym-awatar-sm`, `--dn-wym-przelacznik-szer`, `--dn-wym-przelacznik-wys` | Przyciski akcji kolejek, awatary ról i podagentów, przełączniki macierzy izolacji |
| Powierzchnie i obrysy | `--dn-tlo`, `--dn-panel`, `--dn-powierzchnia`, `--dn-powierzchnia-2`, `--dn-obrys`, `--dn-obrys-subtelny`, `--dn-obrys-mocny`, `--dn-hover`, `--dn-wcisniecie` | Tło kokpitu, tła kolumn, karty, rozdzielenia i stany wskazania kursorem |
| Warstwy widoczności elementów | `--dn-z-podloga`, `--dn-z-boczna`, `--dn-z-pas-komunikacji`, `--dn-z-modal`, `--dn-z-nakladka`, `--dn-z-tooltip`, `--dn-z-powiadomienie`, `--dn-z-centrum-polecen`, `--dn-z-aod` | Kolejność nakładania: kolumny, panele, okna nakładkowe, paleta poleceń i warstwa centralna (rozdz. 1.6) |
| Wskaźniki postępu i ładowania | `--dn-wym-spinner`, `--dn-grad-sygnal` | Spinner akcji kolejek, pasek postępu podagentów |
| Powiadomienia | `--dn-wym-toast-min`, `--dn-wym-toast-max`, `--dn-cien-2`, `--dn-cien-lg` | Dymek powiadomienia z opcją „Cofnij”, uniesienie okien nakładkowych |

### 24.4. Komponenty interfejsu

Wykaz uzupełnia Załącznik A.2 o role, modyfikatory i stany wymagane rozdz. 4.2 standardu; klasy pochodzą z `design/zasoby/css/komponenty.css` i są w nim obecne bez wyjątku.

| Klasa `.dn-*` | Rola | Modyfikatory | Stany |
|---|---|---|---|
| `.dn-btn` | Przycisk czynności | `--sygnal` · `--zarys` · `--duch` · `--atrament` · `--niebezpieczny` · `--wybrany` · `--sm` · `--lg` | spoczynek · wskazanie kursorem · wciśnięcie · ognisko · ładowanie |
| `.dn-btn-ikona` | Przycisk ikonowy akcji kolejek | `--sm` · `--na-ramie` | spoczynek · wskazanie kursorem · wciśnięcie · ognisko · ładowanie |
| `.dn-pole` | Wrapper pola formularza wraz z etykietą, opisem i komunikatem | — | spoczynek · ostrzeżenie · błąd |
| `.dn-pole-kontrolka` | Kontrolka pola: tekst, wybór, liczba, obszar wieloliniowy | — | pusty · wypełniony · ognisko · błąd |
| `.dn-suwak` | Przełącznik dwustanowy macierzy izolacji i widoczności sekcji | — | wyłączony · włączony · ognisko |
| `.dn-przelacznik` | Przełącznik grupy wariantów (tryb współpracy, tryb AOD) | — | wybrany · niewybrany · ognisko |
| `.dn-karta` | Karta roli, podagenta, pozycji listy | `--klikalna` · `--wybrana` | spoczynek · wskazanie kursorem · wybrana · ognisko |
| `.dn-plakietka` | Plakietka statusu roli, zadania, zgodności | `--sukces` · `--ostrzezenie` · `--blad` · `--informacja` · `--rola` · `--sygnal` | spoczynek |
| `.dn-kropka` | Wskaźnik semantyczny stanu | `--sukces` · `--ostrzezenie` · `--blad` · `--sygnal` · `--neutralna` · `--tetno` | aktywny · bezczynny |
| `.dn-krok` | Etap przebiegu w Monitorze procesu i Execution Loop Window | `--pracuje` · `--poprawny` · `--wstrzymany` · `--bledy` | w toku · zakończony · wstrzymany · błąd |
| `.dn-kolejka` | Lista zadań kolejki | — | wypełniona · pusta |
| `.dn-tabela` | Tabela kolejek i przebiegów | — | spoczynek · sortowana · pusta |
| `.dn-zakladki` | Karty sesji procesów oraz zakładki wewnętrzne okien ról | `--pigulki` | aktywna · w tle · z pracą w toku |
| `.dn-wpis` | Wpis w oknie komunikacji | `--czlowiek` · `--inteligencja` · `--system` · `--pracuje` | zakończony · strumieniowany |
| `.dn-prompt` | Pole polecenia okna komunikacji | — | pusty · wypełniony · ognisko · wysyłanie |
| `.dn-awatar` | Awatar roli, podagenta i Always On Display | `--kwadrat` · `--inteligencja` · `--sm` · `--lg` | spoczynek · ze znacznikiem stanu (`.dn-awatar-stan`) |
| `.dn-postep` | Pasek postępu podagentów i przebiegu | — | określony · nieokreślony |
| `.dn-modal` | Okno nakładkowe konfiguracji, izolacji, szczegółów przebiegu | `--szeroki` | otwarte · zamykane |
| `.dn-toast` | Dymek powiadomienia z opcją „Cofnij” | `--sukces` · `--ostrzezenie` · `--blad` · `--informacja` | widoczny · wygaszany |
| `.dn-pusty-stan` | Stan pusty sekcji i list | — | spoczynek |
| `.dn-spinner` | Wskaźnik ładowania akcji | — | aktywny |
| `.dn-tooltip` | Objaśnienie kontekstowe elementu konfiguracji | — | ukryte · widoczne |
| `.dn-boczna` | Panel orkiestracji jako boczna nawigacja środowiska | — | rozwinięty · ikonowy |
| `.dn-szukaj` | Pole wyszukiwania powłoki środowiska | — | pusty · wypełniony · ognisko |
| `.dn-alert` | Pas komunikatu w oknie konfiguracji | `--info` · `--ostrzezenie` | widoczny |

### 24.5. Skróty klawiszowe

Wykaz podaje **wartości domyślne**; zgodnie z regułą rozdz. 19.2 każdy skrót nadaje i zmienia Operator w edytorze skrótów okna konfiguracji, a czynność bez wartości fabrycznej czeka na nadanie kombinacji. Kolizje odnotowuje kolumna „Kolizje”. Prototypy `design/05-okna/` są wzorcem wyglądu i układu, nie źródłem normatywnym przypisań klawiszowych — rozbieżność między kombinacją widoczną w prototypie a wartością domyślną z tej tabeli nie jest usterką; wiążąca jest tabela wraz z regułą rozdz. 19.2.

| Kombinacja | Działanie | Zasięg | Kolizje |
|---|---|---|---|
| `Ctrl+K` | Paleta poleceń środowiska (rozdz. 19.2) | globalny | — |
| `Ctrl+Shift+A` | Powierzchnia interakcji Always On Display (rozdz. 2.6) | globalny | — |
| `Ctrl+/` | Ściągawka skrótów aktywnych (rozdz. 19.2) | globalny | — |
| `Ctrl+Tab` | Następna karta sesji — proces orkiestracji (rozdz. 17.1, 18) | powłoka | czynność odrębna od skoku między oknami ról, który wartości fabrycznej nie ma |
| `Ctrl+Shift+Tab` | Poprzednia karta sesji (rozdz. 17.1, 18) | powłoka | jak wyżej |
| `Ctrl+Shift+F` | Tryb skupienia środowiska (rozdz. 21) | globalny | — |
| `Ctrl+Shift+B` | Szyna nawigacji — przełącznik widoczności | globalny | — |
| `Ctrl+B` | Szyna sesji — przełącznik widoczności | globalny | — |
| `Ctrl+E` | Przedsionek środowiska (rozdz. 2.7) | globalny | — |
| `Ctrl+Home` | Centrum dowodzenia | globalny | — |
| `Ctrl+F` | Znajdź w bieżącym widoku | widok | — |
| `Ctrl+,` | Okno konfiguracji i ustawień, w tym edytor skrótów (rozdz. 22) | globalny | — |
| bez wartości fabrycznej | Start, Pauza, Wznów, Stop, Waliduj, `retry` — sterowanie procesem (rozdz. 19.2) | środowisko | kombinację nadaje Operator; edytor odmawia zapisania kombinacji już zajętej i wskazuje czynność zajmującą |
| bez wartości fabrycznej | Jedenaście akcji silnika kolejek na zadaniu zaznaczonym (rozdz. 4.2) | sekcja Kolejki | jak wyżej |
| bez wartości fabrycznej | Skok do Chat Window, Execution Loop Window i okien czterech ról (rozdz. 17.2) | obszar roboczy | jak wyżej |
| bez wartości fabrycznej | Nasłuch głosowy i przełącznik trybu obserwator/operator warstwy centralnej (rozdz. 2.2) | globalny | jak wyżej |

Kombinacje `Ctrl+Tab`, `Ctrl+Shift+Tab`, `Ctrl+Shift+F`, `Ctrl+Shift+B`, `Ctrl+B`, `Ctrl+E`, `Ctrl+Home`, `Ctrl+F` i `Ctrl+,` należą do ramy okna i są wspólne dla całej platformy — [Rama okna](../interfejs-uzytkownika/rama-okna.md), rozdz. 22.4; środowisko ich nie przedefiniowuje.

### 24.6. Stany kontrolek

Dziewięć kolumn za standardem rozdz. 4.2, w brzmieniu polskim. Tam, gdzie stan nie występuje, komórka niesie adnotację „nie dotyczy” wraz z przyczyną. Kolumna „nieaktywny” niemal wszędzie brzmi „nie dotyczy”, ponieważ platforma nie stawia twardych blokad (zasada nadrzędna, rozdz. 16) — akcja niegotowa pozostaje klikalna i odpowiada komunikatem kontekstowym (rozdz. 12.6).

| Kontrolka | Spoczynek | Wskazanie kursorem | Wciśnięcie | Ognisko | Nieaktywny | Ładowanie | Pusty | Błąd |
|---|---|---|---|---|---|---|---|---|
| Pole polecenia okna roli (`.dn-prompt`, `.dn-pole-kontrolka`) | Obrys `--dn-obrys` | Obrys `--dn-sygnal-obrys` | nie dotyczy — pole nie jest przyciskiem | Pierścień `--dn-fokus` | nie dotyczy — zasada zero blokad | Ikona wysłania ustępuje wskaźnikowi strumieniowania | Podpowiedź „Wpisz polecenie dla wykonawcy…” | Obrys `--dn-blad-obrys`, komunikat pod polem (`.dn-pole-blad`) |
| Przycisk sterowania procesem (`.dn-btn--sygnal`) | Wypełnienie `--dn-sygnal-wypelnienie` | Wypełnienie `--dn-sygnal-wypelnienie-hover` | Przesunięcie w dół wg `.dn-btn:active` | Pierścień `--dn-fokus` | nie dotyczy — akcja niegotowa pozostaje klikalna i odpowiada komunikatem | Spinner (`.dn-spinner`) w miejscu etykiety | nie dotyczy — przycisk nie niesie zbioru | Plakietka `--dn-blad-tekst` przy przycisku po niepowodzeniu akcji |
| Przycisk ikonowy akcji kolejki (`.dn-btn-ikona`) | Ikona `--dn-tekst-2` | Tło `--dn-hover`, ikona `--dn-tekst` | Tło `--dn-wcisniecie` | Pierścień `--dn-fokus` | nie dotyczy — jak wyżej | Spinner w miejscu ikony | nie dotyczy | Dymek powiadomienia `.dn-toast--blad` |
| Karta roli (`.dn-karta--klikalna`) | Tło `--dn-powierzchnia`, obrys `--dn-obrys-subtelny` | Tło `--dn-hover`, cień `--dn-cien-1` | Zaznaczenie trwałe (`--wybrana`) | Pierścień `--dn-fokus` | nie dotyczy — cztery role zawsze osiągalne | Kropka `--dn-kropka--tetno`, plakietka „Pracuje” | „Brak przypisania” — rola bez agenta ani modelu | Plakietka „Błąd wykonania”, tło `--dn-blad-tlo` |
| Wiersz tabeli kolejki (`.dn-tabela`, `.dn-kolejka`) | Tło `--dn-panel` | Tło `--dn-hover` | Rozwinięcie szczegółów zadania | Pierścień `--dn-fokus` | nie dotyczy | Pasek postępu (`.dn-postep`) w wierszu | Pusty stan sekcji Kolejki z krokami startowymi | Plakietka błędu zadania, treść w szczegółach przebiegu |
| Przełącznik macierzy izolacji (`.dn-suwak`) | Położenie „wyłączone / pełny dostęp” | Podświetlenie toru | Przejście położenia | Pierścień `--dn-fokus` | nie dotyczy — każdy zakres jest przestawialny | Zapis profilu — spinner przy etykiecie | nie dotyczy — przełącznik ma zawsze jedno z dwóch położeń | Obrys `--dn-blad-obrys`, komunikat o niepowodzeniu zapisu profilu |
| Pozycja panelu orkiestracji (`.dn-boczna-pozycja`) | Tekst `--dn-tekst-2` | Tło `--dn-hover` | Przejście do sekcji | Pierścień `--dn-fokus` | nie dotyczy — sekcja ukryta jest zdejmowana z panelu, nie wygaszana | nie dotyczy — przejście jest lokalne | nie dotyczy | nie dotyczy — błąd zgłasza treść sekcji, nie pozycja |
| Karta sesji procesu (`.dn-karta-sesji`) | Karty w kolejności otwarcia | Ujawnienie znacznika zamknięcia | Przełączenie ogniska na kartę | Pierścień `--dn-fokus` | nie dotyczy | Kropka sygnałowa przy sesji z zadaniem w toku | Pasmo bez kart — puste, bez komunikatu | Plakietka błędu przy karcie procesu zakończonego niepowodzeniem |
| Pole przypisania skrótu (rozdz. 19.2) | Zapis kombinacji bieżącej | nie dotyczy — pole nie reaguje na wskazanie kursorem | Tekst „naciśnij kombinację…” po ustawieniu ogniska | Pierścień `--dn-fokus` | nie dotyczy | nie dotyczy | Brak kombinacji nadanej — pole puste z podpowiedzią | Kombinacja zajęta — komunikat wskazujący czynność zajmującą |

### 24.7. Punkty łamania

Progi z `design/zasoby/zetony/zetony.css`; pełny opis zachowań układu prowadzi Załącznik A.3 i nie jest tu powtarzany.

| Próg szerokości | Zachowanie układu |
|---|---|
| `--dn-bp-w1` — 640px | Widok mobilny środowiska przez funkcję globalną Mobile; Monitor procesu jako widok wiodący zamiast panelu orkiestracji |
| `--dn-bp-w2` — 960px | Panel orkiestracji zwija się do ikon; okna ról przechodzą z układu kolumnowego na zakładkowy, a Subagent Network i Results Analyzer — na okno nakładkowe |
| `--dn-bp-w3` — 1280px | Pełny kokpit: Chat Window, Execution Loop Window i co najmniej jedno okno roli widoczne jednocześnie |
| `--dn-bp-w4` — 1600px | Szerokie biurko: cztery okna ról obok obu okien komunikacji, bez zwijania kolumn |

### 24.8. Kryteria odbioru

| Warunek sprawdzalny | Sposób sprawdzenia |
|---|---|
| Wejście do środowiska otwiera przedsionek, nie przestrzeń roboczą procesu | Kliknięcie karty MultitaskingAI na stronie głównej; potwierdzenie, że żaden proces nie uruchamia się samoczynnie (rozdz. 1.8, 2.7) |
| Panel orkiestracji ma sześć sekcji w kolejności rozdz. 6.5 | Przegląd panelu w nowym procesie, bez zmian konfiguracji |
| Oba okna komunikacji są obecne stale, w kolumnach o pełnej wysokości | Otwarcie procesu; potwierdzenie, że Chat Window zajmuje lewą kolumnę, a Execution Loop Window kolumnę sąsiadującą (zasada układu, rozdz. 1.7) |
| Silnik kolejek udostępnia wszystkie jedenaście akcji z rozdz. 4.2 | Przegląd przycisków akcji w wierszu zadania i w oknie „Jedenaście akcji silnika kolejek” |
| Żadna kontrolka nie jest trwale nieaktywna | Przegląd kontrolek z rozdz. 24.4 pod kątem atrybutu `disabled`; zero wystąpień poza stanem „ładowanie” |
| Akcja niegotowa odpowiada komunikatem kontekstowym, nie blokadą | Kliknięcie `merge` przed zakończeniem pracy wszystkich podagentów (rozdz. 12.6) |
| Profil izolacji roli ma pierwszeństwo nad zasięgami szerszymi | Ustawienie sprzecznych wartości na poziomie środowiska i roli; odczyt polityki efektywnej w oknie izolacji (rozdz. 13.1) |
| Stan wyjściowy izolacji to „wyłączone / pełny dostęp” | Otwarcie okna izolacji w nowym procesie; wszystkie przełączniki macierzy w położeniu wyłączonym (rozdz. 13.4) |
| Spięcie z modułem Automations jest wyłączone domyślnie i sygnalizowane jawnie | Otwarcie nowego procesu; pusty stan „Proces nie jest spięty z modułem Automations” (rozdz. 7.1) |
| Każdy skrót jest zmienialny, a czynność bez wartości fabrycznej czeka na nadanie | Otwarcie edytora skrótów; próba przypisania kombinacji zajętej — edytor wskazuje czynność zajmującą (rozdz. 19.2) |
| Akcja nieodwracalna wykonuje się od razu, z możliwością cofnięcia | Usunięcie kolejki przy wyłączonym potwierdzeniu; obecność „Cofnij” w dymku powiadomienia (rozdz. 22) |
| Karty sesji zachowują stan procesów po powrocie z przedsionka | Powrót do przedsionka i ponowne wejście; przegląd szyny sesji — procesy czynne pozostają w toku (rozdz. 18) |
| Układ odpowiada czterem progom łamania | Zmiana szerokości okna kolejno przez 640px, 960px, 1280px i 1600px; obserwacja zachowań z rozdz. 24.7 |
| Każdy żeton i każda klasa przywołane w dokumencie istnieją w arkuszach źródłowych | Zestawienie nazw z rozdz. 24.3 i 24.4 z `design/zasoby/zetony/zetony.css` i `design/zasoby/css/komponenty.css` |
| Każda komenda przywołana w korpusie występuje w wykazie Załącznika C | Zestawienie nazw komend z korpusu z wykazem Załącznika C i z `budowa/shared/contract.json` |
...
| `gwiazdka` | Sekcja Zespoły (szablony konfiguracji zespołu) |
...
Wykaz pełny obejmuje osiemdziesiąt dwie ikony — liczba pochodzi z pola `liczba-ikon` pliku `design/zasoby/ikony/manifest.json`, który jest jedynym źródłem normatywnym katalogu ikon. Pliki źródłowe ikon leżą w katalogu `design/zasoby/ikony/svg/` — po jednym pliku SVG na ikonę, o nazwie zgodnej z identyfikatorem z manifestu; to ten katalog jest miejscem podglądu i pobrania grafiki.

### A.2. Komponenty

| Komponent | Klasa bazowa | Zastosowanie w środowisku |
|---|---|---|
| Przycisk | `.dn-btn` (`--sygnal`, `--zarys`, `--duch`, `--niebezpieczny`) | Sterowanie procesem, akcje kolejki, decyzje Results Analyzer |
| Przycisk ikonowy | `.dn-btn-ikona` | Jedenaście akcji silnika kolejek, sterowanie Coordinatora |
| Pole formularza | `.dn-pole` (wrapper), `.dn-pole-kontrolka` (kontrolka: wybór, liczba, tekst wieloliniowy) | Selektory przypisania, kanału, priorytetu, pola poleceń |
| Przełącznik | `.dn-suwak` | Macierz izolacji, widoczność sekcji panelu, powiązanie z Automations |
| Karta | `.dn-karta` (`--klikalna`, `--wybrana`) | Karty ról, karty podagentów, pozycje list |
| Plakietka | `.dn-plakietka` (`--sukces`, `--ostrzezenie`, `--blad`, `--informacja`, `--rola`, `--sygnal`) | Statusy ról, przebiegów, zgodności zależności |
...
| Tabela | `.dn-tabela` | Tabela kolejek, tabela przebiegów (Monitor procesu) |
| Awatar | `.dn-awatar` (`--kwadrat` dla agenta, kołowy dla modelu bazowego, `-stan`) | Awatar ról, awatar Always On Display |
| Okno nakładkowe | `.dn-modal` | Potwierdzenie akcji nieodwracalnej (usunięcie zespołu, kolejki) — ustawienie konfiguracyjne włączane przez Operatora, domyślnie wyłączone, akcja wykonuje się od razu |
| Dymek powiadomienia | `.dn-toast` | Powiadomienia interwencji (Mobile), potwierdzenia zapisu, opcja „Cofnij” po akcjach nieodwracalnych |
...
Wartości progów z `design/zasoby/zetony/zetony.css`, rozdział „Siatka i punkty łamania” — środowisko MultitaskingAI jest jedynym środowiskiem platformy definiującym zachowanie na wszystkich czterech progach, ponieważ jego układ pionowy kolumnowy (rozdz. „Zasada układu” we wstępie) jest najbardziej wrażliwy na dostępną szerokość.

| Żeton | Próg szerokości | Zachowanie układu w środowisku MultitaskingAI |
|---|---|---|
| `--dn-bp-w1` | 640px | Widok mobilny — środowisko udostępniane przez funkcję globalną Mobile ([Mobile](../funkcje-globalne/mobile.md)), z Monitorem procesu jako widokiem wiodącym zamiast pełnego panelu orkiestracji |
| `--dn-bp-w2` | 960px | Panel orkiestracji (boczna nawigacja sześciu sekcji, rozdz. 6) zwija się do samych ikon; okna ról przechodzą z układu kolumnowego na zakładkowy |
| `--dn-bp-w3` | 1280px | Pełny kokpit — Chat Window, Execution Loop Window i co najmniej jedno okno robocze roli widoczne jednocześnie |
| `--dn-bp-w4` | 1600px | Szerokie biurko — cztery okna robocze ról (rozdz. 9.2) mogą być otwarte równocześnie w kolumnach sąsiadujących z Chat Window i Execution Loop Window, bez zwijania żadnej z kolumn |

Poniżej `--dn-bp-w2` panel Subagent Network i Results Analyzer przechodzą z prezentacji kolumnowej na prezentację w oknie nakładkowym (`.dn-modal`) otwieranym z karty roli, zamiast stałej kolumny obszaru roboczego.

---

## Załącznik B. Macierz zgodności ze źródłami

| Rozdział niniejszego dokumentu | [Koncepcja platformy](../architektura/koncepcja-platformy.md) | [Architektura techniczna](../architektura/architektura.md) | [Model danych](../architektura/model-danych.md) | [Kontrakty komunikacji](../architektura/kontrakty-komunikacji.md) | [Specyfikacja okien operacyjnych](../specyfikacje/specyfikacja-okien-operacyjnych.md) | [Specyfikacja agentów](../specyfikacje/specyfikacja-agentow.md) | [System wizualny](../interfejs-uzytkownika/system-wizualny.md) | [Izolacja i zależności](../architektura/izolacja-i-zaleznosci.md) |
|---|---|---|---|---|---|---|---|---|
...
| 12. Stany | — | — | — | — | — | — | rozdz. 6 | — |
| 13. Izolacja na poziomie roli | rozdz. 6.4–6.6 | rozdz. 9, 10, 12 | rozdz. 13.2, 15.5, 17.1 | rozdz. 15 | rozdz. 8 | rozdz. 4 | rozdz. 12 | rozdz. 3, 4, 6, 7 |
...
| 20. Współdzielenie kontekstu | rozdz. 6.4–6.6 | rozdz. 12 | rozdz. 15.5 | rozdz. 15 | rozdz. 8 | rozdz. 4 | — | rozdz. 3, 4, 6, 7 |
...
## Załącznik C. Pełny wykaz komend kontraktu środowiska MultitaskingAI

Wykaz wygenerowany wprost z `budowa/shared/contract.json` — jedynego źródła nazw komend, pól ładunków i zdarzeń. Obejmuje pięć obszarów właściwych środowisku — `environment`, `role`, `orchestration`, `advisor`, `subagent` — oraz komendy obszarów sąsiednich przywołane w korpusie niniejszego opracowania. Każda pozycja jest wiążąca dla warstwy klienckiej i serwerowej; deweloper nie dodaje ani nie zmienia nazw poza tym wykazem.

### Obszar `environment` — 2 komendy

| Komenda | Pola żądania | Pola wyniku |
|---|---|---|
| `environment.list` | `includeModules:bool` (opc) | `environments:Environment[]` (wym) |
| `environment.enter` | `environmentId:string` (wym)<br>`clientId:string` (wym)<br>`sessionId:string` (opc) | `environment:Environment` (wym)<br>`modules:Module[]` (wym)<br>`sessions:Session[]` (wym)<br>`focusedSessionId:string` (opc) |

### Obszar `role` — 4 komendy

| Komenda | Pola żądania | Pola wyniku |
|---|---|---|
| `role.assign` | `windowId:string` (wym)<br>`role:WindowRole` (wym)<br>`coordinatorWindowId:string` (opc) | `windowId:string` (wym)<br>`role:WindowRole` (wym)<br>`coordinatorWindowId:string` (opc) |
| `role.update` | `windowId:string` (wym)<br>`role:WindowRole` (opc)<br>`persona:string` (opc)<br>`coordinatorWindowId:string` (opc) | `windowId:string` (wym)<br>`role:WindowRole` (wym)<br>`persona:string` (opc) |
| `role.list` | `coordinatorWindowId:string` (opc) | `assignments:WindowRoleAssignment[]` (wym)<br>`total:int` (wym) |
| `role.remove` | `windowId:string` (wym) | `windowId:string` (wym)<br>`removed:bool` (wym) |

**Zdarzenia obszaru `role` — 1:** `role.changed`.

### Obszar `orchestration` — 8 komend

| Komenda | Pola żądania | Pola wyniku |
|---|---|---|
| `orchestration.dependency.set` | `workflowId:string` (wym)<br>`dependency:AutomationDependency` (wym) | `dependencies:AutomationDependency[]` (wym)<br>`valid:bool` (wym)<br>`issues:string[]` (opc) |
| `orchestration.dependency.list` | `workflowId:string` (wym)<br>`stepId:string` (opc) | `dependencies:AutomationDependency[]` (wym) |
| `orchestration.dependency.remove` | `workflowId:string` (wym)<br>`fromStepId:string` (wym)<br>`toStepId:string` (wym) | `dependencies:AutomationDependency[]` (wym)<br>`removed:bool` (wym) |
| `orchestration.validate` | `workflowId:string` (wym) | `valid:bool` (wym)<br>`issues:string[]` (opc)<br>`criticalPathStepIds:string[]` (opc) |
| `orchestration.gate.set` | `workflowId:string` (wym)<br>`stepId:string` (wym)<br>`rule:OrchestrationGateRule` (wym)<br>`count:int` (opc) | `gates:OrchestrationGate[]` (wym)<br>`valid:bool` (wym)<br>`issues:string[]` (opc) |
| `orchestration.group.set` | `workflowId:string` (wym)<br>`groupId:string` (opc)<br>`name:string` (wym)<br>`stepIds:string[]` (wym)<br>`kind:AutomationDependencyKind` (wym) | `groups:OrchestrationGroup[]` (wym) |
| `orchestration.compensation.set` | `workflowId:string` (wym)<br>`stepId:string` (wym)<br>`compensationStepId:string` (wym) | `compensations:OrchestrationCompensation[]` (wym) |
| `orchestration.multitasking.link` | `workflowId:string` (wym)<br>`linked:bool` (wym)<br>`roleId:string` (opc) | `linked:bool` (wym)<br>`queueIds:string[]` (opc) |

**Zdarzenia obszaru `orchestration` — 1:** `orchestration.changed`.

### Obszar `advisor` — 1 komenda

| Komenda | Pola żądania | Pola wyniku |
|---|---|---|
| `advisor.consult` | `windowId:string` (wym)<br>`question:string` (wym)<br>`context:string` (opc)<br>`requestedAdvisor:string` (opc) | `advice:string` (wym)<br>`advisorChannel:string` (wym)<br>`advisorModel:string` (opc)<br>`selection:AdvisorSelection` (wym) |

**Zdarzenia obszaru `advisor` — 1:** `advisor.consulted`.

### Obszar `subagent` — 4 komendy

| Komenda | Pola żądania | Pola wyniku |
|---|---|---|
| `subagent.spawn` | `windowId:string` (wym)<br>`task:string` (wym)<br>`name:string` (opc)<br>`count:int` (opc)<br>`modelChannelId:string` (opc) | `subagents:Subagent[]` (wym) |
| `subagent.list` | `windowId:string` (opc)<br>`sessionId:string` (opc)<br>`status:SubagentStatus` (opc) | `subagents:Subagent[]` (wym) |
| `subagent.result.collect` | `windowId:string` (opc)<br>`subagentIds:string[]` (opc)<br>`waitForAll:bool` (opc) | `subagents:Subagent[]` (wym)<br>`complete:bool` (wym) |
| `subagent.stop` | `windowId:string` (opc)<br>`subagentIds:string[]` (opc) | `stopped:string[]` (wym)<br>`notRunning:string[]` (wym)<br>`subagents:Subagent[]` (wym) |

**Zdarzenia obszaru `subagent` — 1:** `subagent.changed`.

### Komendy obszarów sąsiednich przywołane w korpusie — 4 komendy

Środowisko posługuje się ponadto czterema komendami spoza pięciu obszarów własnych: silnik kolejek (rozdz. 4) wywołuje komendę obszaru `queue`, warstwa centralna (rozdz. 2.2) — dwie komendy obszaru `aod`, a przypisanie profilu izolacji do roli (rozdz. 13.3) — komendę obszaru `isolation`.

| Komenda | Pola żądania | Pola wyniku |
|---|---|---|
| `queue.action` | `queueId:string` (wym)<br>`action:QueueAction` (wym)<br>`itemId:string` (opc) | `queue:Queue` (wym) |
| `aod.observe.attach` | `processId:string` (wym)<br>`mode:AodProcessMode` (opc)<br>`deviceId:string` (opc) | `attachedProcessIds:string[]` (wym)<br>`mode:AodProcessMode` (wym) |
| `aod.observe.detach` | `processId:string` (wym)<br>`deviceId:string` (opc) | `attachedProcessIds:string[]` (wym) |
| `isolation.profile.assign` | `profileId:string` (wym)<br>`scope:ConfigScope` (wym)<br>`scopeId:string` (opc)<br>`layer:IsolationLayer` (opc) | `policy:IsolationPolicy` (wym) |

Żadna z czterech komend nie zwraca zdarzenia własnego.

Razem w wykazie: **23 komendy** z 8 obszarów kontraktu, 5 zdarzeń zwrotnych.

---

*Koniec dokumentu. Środowisko MultitaskingAI — Specyfikacja docelowa, wersja 2.0, 2026-08-20.*
