# Danaco Console — Środowisko TalkIn
...
| | |
|---|---|
| **Tytuł** | Środowisko TalkIn — przeznaczenie, moduły, komplet okien, specyfikacja okien, przepływy pracy, stany i powiązania, punkty sterowania, scenariusze użycia |
...
| **Zakres** | przeznaczenie i kontekst środowiska, dziewięć modułów, przedsionek wejściowy, komplet okien z warstwami widoczności, specyfikacja okien, przepływy pracy, stany i powiązania, punkty sterowania z okna konfiguracji, scenariusze użycia, wykazy normatywne, skróty klawiszowe, kryteria odbioru |
| **Poza zakresem** | wnętrze poszczególnych modułów udostępnianych w środowisku — opisują je opracowania katalogu `moduly/`; mechanizm rozpoznania mowy jako taki — [Architektura techniczna](../architektura/architektura.md) |
| **Dokument nadrzędny** | [Koncepcja platformy](../architektura/koncepcja-platformy.md) |
| **Dokumenty powiązane** | [Strona główna i nawigacja](../interfejs-uzytkownika/strona-glowna-i-nawigacja.md) · [Specyfikacja okien operacyjnych](../specyfikacje/specyfikacja-okien-operacyjnych.md) · [Specyfikacja modułów](../specyfikacje/specyfikacja-modulow.md) · [Model konfiguracji](../architektura/model-konfiguracji.md) · [System wizualny](../interfejs-uzytkownika/system-wizualny.md) · [Izolacja i zależności](../architektura/izolacja-i-zaleznosci.md) · [Elementy okien](../interfejs-uzytkownika/elementy-okien.md) · [Przepływ okien](../interfejs-uzytkownika/przeplyw-okien.md) |
| **Prototypy odniesienia** | `design/05-okna/srodowiska/talkin.html` · `design/05-okna/srodowiska/talkin-przedsionek.html` |
| **Źródła normatywne** | `budowa/shared/contract.json` (obszary `environment`, `message`, `stream`, `speech`) · `design/zasoby/zetony/zetony.css` · `design/zasoby/css/komponenty.css`, `design/zasoby/rama.css`, `design/zasoby/prototyp.css` · `design/05-okna/srodowiska/` |
...
2. [Przeznaczenie i kontekst środowiska](#1-przeznaczenie-i-kontekst-środowiska)
   - [1.1 Przedmiot pracy środowiska](#11-przedmiot-pracy-środowiska)
   - [1.2 Miejsce środowiska w platformie](#12-miejsce-środowiska-w-platformie)
   - [1.3 Granica powłoki środowiska i modułów](#13-granica-powłoki-środowiska-i-modułów)
   - [1.4 Układ obszaru roboczego](#14-układ-obszaru-roboczego)
   - [1.5 Warstwy widoczności w środowisku](#15-warstwy-widoczności-w-środowisku)
3. [Moduły dostępne w środowisku](#2-moduły-dostępne-w-środowisku)
   - [2.1 Zestaw modułów](#21-zestaw-modułów)
   - [2.2 Boczna nawigacja modułów](#22-boczna-nawigacja-modułów)
   - [2.3 Otwieranie modułu w kolumnie obszaru roboczego](#23-otwieranie-modułu-w-kolumnie-obszaru-roboczego)
...
4. [Komplet okien środowiska](#3-komplet-okien-środowiska)
   - [3.1 Okna komunikacji operacyjnej](#31-okna-komunikacji-operacyjnej)
   - [3.2 Okna operacyjne modułów](#32-okna-operacyjne-modułów)
   - [3.3 Okna pomocnicze powłoki](#33-okna-pomocnicze-powłoki)
   - [3.4 Nakładki](#34-nakładki)
   - [3.5 Elementy stałe powłoki](#35-elementy-stałe-powłoki)
5. [Specyfikacja okien](#4-specyfikacja-okien)
   - [4.1 Anatomia wspólna okna](#41-anatomia-wspólna-okna)
   ...
   - [4.4 Obszar roboczy modułu](#44-obszar-roboczy-modułu)
   - [4.5 Kolumna stanu środowiska](#45-kolumna-stanu-środowiska)
   - [4.6 Pasek kart sesji](#46-pasek-kart-sesji)
   - [4.7 Rama okna środowiska](#47-rama-okna-środowiska)
   - [4.8 Magistrala kontekstu](#48-magistrala-kontekstu)
6. [Przepływy pracy](#5-przepływy-pracy)
   - [5.1 Przepływ podstawowy — polecenie w Chat Window](#51-przepływ-podstawowy--polecenie-w-chat-window)
   - [5.2 Przepływ treści między modułami](#52-przepływ-treści-między-modułami)
   - [5.3 Przepływ pracy w karcie sesji](#53-przepływ-pracy-w-karcie-sesji)
   - [5.4 Przepływ nawigacji między modułami](#54-przepływ-nawigacji-między-modułami)
   - [5.5 Przepływ nadzoru nad pętlą wykonawczą](#55-przepływ-nadzoru-nad-pętlą-wykonawczą)
7. [Stany i powiązania](#6-stany-i-powiązania)
   - [6.1 Stany karty sesji](#61-stany-karty-sesji)
   - [6.2 Stany połączenia](#62-stany-połączenia)
   - [6.3 Stany pętli wykonawczej](#63-stany-pętli-wykonawczej)
   - [6.4 Powiązania między oknami](#64-powiązania-między-oknami)
   - [6.5 Powiązania kontekstu między modułami](#65-powiązania-kontekstu-między-modułami)
   - [6.6 Stany elementów wspólnych](#66-stany-elementów-wspólnych)
8. [Punkty sterowania z okna konfiguracji](#7-punkty-sterowania-z-okna-konfiguracji)
   - [7.1 Zakresy sterowania](#71-zakresy-sterowania)
   - [7.2 Profile personalizacji środowiska](#72-profile-personalizacji-środowiska)
   - [7.3 Ustawienia szybkie sesji](#73-ustawienia-szybkie-sesji)
9. [Scenariusze użycia](#8-scenariusze-użycia)
   - [8.1 Redakcja raportu ze zgromadzonych źródeł](#81-redakcja-raportu-ze-zgromadzonych-źródeł)
   - [8.2 Przekład dokumentu z kontrolą terminologii](#82-przekład-dokumentu-z-kontrolą-terminologii)
   - [8.3 Debata wielomodelowa nad wariantem treści](#83-debata-wielomodelowa-nad-wariantem-treści)
   - [8.4 Wznowienie złożonej pracy po przerwie](#84-wznowienie-złożonej-pracy-po-przerwie)
   - [8.5 Nadzór nad pracą poza stanowiskiem](#85-nadzór-nad-pracą-poza-stanowiskiem)
...
11. [Załącznik A. Skróty klawiszowe środowiska](#załącznik-a-skróty-klawiszowe-środowiska)
   - [A.1. Sterowanie powłoką](#a1-sterowanie-powłoką)
   - [A.2. Moduły i nawigacja](#a2-moduły-i-nawigacja)
   - [A.3. Karty sesji](#a3-karty-sesji)
   - [A.4. Układ kolumn](#a4-układ-kolumn)
   - [A.5. Okna komunikacji operacyjnej](#a5-okna-komunikacji-operacyjnej)
   - [A.6. Warstwa spajająca treść](#a6-warstwa-spajająca-treść)
12. [Załącznik B. Pełny wykaz komend kontraktu środowiska TalkIn](#załącznik-b-pełny-wykaz-komend-kontraktu-środowiska-talkin)
   - [Obszar `environment` — 2 komendy](#obszar-environment--2-komendy)
   - [Obszar `message` — 3 komendy](#obszar-message--3-komendy)
   - [Obszar `stream` — 0 komend, 1 zdarzenie](#obszar-stream--0-komend-1-zdarzenie)
   - [Obszar `speech` — 8 komend](#obszar-speech--8-komend)
...
Boczna nawigacja jest kolumną na lewej krawędzi środowiska, złożoną z pozycji `.dn-karta--klikalna` pogrupowanych w cztery sekcje zwijalne. Nagłówek kolumny zawiera nazwę środowiska działającą jako przełącznik środowisk.
...
| Podgląd modułu | Popover z nazwami okien modułu i ostatnim stanem | Decyzja o przełączeniu bez utraty bieżącego widoku | 2 | Wskazanie kursorem pozycji modułu |
| Pole filtrowania modułów | Pole `.dn-pole-kontrolka` w nagłówku kolumny | Dojście do modułu z klawiatury | 2 | Skrót klawiszowy lub kliknięcie ikony `szukaj` |
...
|---|---|---|
| Szyna sesji („Twoje sesje”) | Wykaz sesji Operatora w tym środowisku, zgrupowany projektami treściowymi, z filtrem zakresu (czynne · wszystkie · zakończone) i wskaźnikiem pracy w tle. W stopce — wejście do pełnej historii sesji | Kolumna przy lewej krawędzi, waga średnia |
...
| Płótno — listwa działań środowiska | Trzy pozycje: **Nowy projekt treściowy**, **Konfiguracja środowiska**, **Ustawienia** — działania dotyczące środowiska jako całości | Listwa jednorzędowa, waga najniższa |
...
**Prototyp odniesienia:** `design/05-okna/srodowiska/talkin-przedsionek.html`.
...
| Centrum powiadomień | Rejestr zdarzeń modułów środowiska; mechanizm w postaci jednolitej dla całej platformy — [Katalog komponentów](../interfejs-uzytkownika/katalog-komponentow.md), rozdz. 11.6. Specyfika TalkIn: wyzwalacz umieszczony w kolumnie stanu środowiska, zakres źródeł obejmuje wszystkie moduły bieżącej karty | Kolumna boczna, otwierana jako rozszerzenie boczne | 3 | Plakietka powiadomień z licznikiem w kolumnie stanu |
...
| Tryb odczytu | Widok treści aktywnego okna operacyjnego w postaci czytelnej, bez obramowań, pasków narzędzi i kontrolek okna | 3 | Pozycja w menu środowiska (☰) albo skrót klawiszowy |
...
| Okno punktów izolacji | Osiem poziomów zasięgu izolacji — od globalnego po okno komunikacji, najwęższy z nich ([Izolacja i zależności](../architektura/izolacja-i-zaleznosci.md) rozdz. 6) — dla karty, okna i relacji między modułami | 4 | Kliknięcie znacznika zasięgu izolacji w kolumnie stanu albo okno konfiguracji |

### 3.5. Elementy stałe powłoki

| Element | Zawartość | Waga wizualna | Warstwa | Sposób wywołania |
|---|---|---|---|---|
| Rama okna | Szyna nawigacji (środowiska, moduły, szybki wybór, menu aplikacji), belka tytułowa (znak i tytuł widoku), wstążka pozioma (czynności, wyszukiwanie z filtrem, pasmo kart sesji) oraz pasek stanu | Cztery pasy obejmujące okno środowiska; specyfikacja: [Rama okna aplikacji](../interfejs-uzytkownika/rama-okna.md) | 1 | Widoczna bez interakcji |
| Pasmo kart sesji | Karty sesji, przycisk „+”, selektor presetu układu; nawigacja wstecz/dalej stoi w rodzinie „nawigacja historii” wstążki | Na wstążce, za polem wyszukiwania | 1 | Widoczne bez interakcji |
...
| Pole polecenia | Pole `.dn-pole-kontrolka` | Wydanie polecenia w języku naturalnym | Puste · wypełnione · wysyłanie | 1 | Widoczne |
...
| Kebab okna `⋮` | Zbiorcze menu okna | Zakres kontekstu Chat Window, historia i retencja, eksport rozmowy | Domyślny · rozwinięty | 3 | Kliknięcie ikony ⋮ w nagłówku |
...
| Kebab okna `⋮` | Zbiorcze menu okna | Historia przebiegów, eksport przebiegu, przypisanie Wykonawcy do zadań | Domyślny · rozwinięty | 3 | Kliknięcie ikony ⋮ w nagłówku |
...
| Plakietka powiadomień | Liczbę zdarzeń nowych z modułów środowiska | Otwiera centrum powiadomień jako rozszerzenie boczne ([Katalog komponentów](../interfejs-uzytkownika/katalog-komponentow.md), rozdz. 11.6) | 1 | Widoczna |
...
| Przycisk „+” | Ikona dodania karty | Otwarcie nowej przestrzeni pracy | Domyślny · wskazanie kursorem | 1 | Widoczny przy ostatniej karcie |
...
| Menu karty | Lista akcji karty: zmiana nazwy, przypnij, duplikuj, dodaj do grupy, przekaż na Mobile, zamknij | Zarządzanie pojedynczą kartą | Domyślne · rozwinięte | 3 | Prawy przycisk myszy na karcie albo kebab (⋮) karty |
...
### 4.7. Rama okna środowiska

Rama okna jest wspólna dla całej platformy — pełną specyfikacją jest
[Rama okna aplikacji](../interfejs-uzytkownika/rama-okna.md). Poniżej ujęto wyłącznie to, czym rama
napełnia się w środowisku TalkIn; sam układ czterech pasów (szyna nawigacji,
belka tytułowa, wstążka pozioma, pasek stanu) nie różni się między środowiskami.

| Element ramy | Czym napełnia go TalkIn | Warstwa | Sposób wywołania |
|---|---|---|---|
| Szyna nawigacji — środowiska i moduły | Dziewięć modułów TalkIn pod rozwiniętym środowiskiem; szybki wybór ulubionych komponentów | 1 | Widoczna |
| Szyna nawigacji — głowa (menu aplikacji) | Menu aplikacji z sekcjami Plik, Edycja, Widok, Przejdź, Narzędzia, Dla programistów, Pomoc | 3 | Kliknięcie hamburgera |
| Belka tytułowa | Znak i nazwa Danaco Console oraz tytuł bieżącego widoku (praca w toku i jej miejsce) | 1 | Widoczna |
| Wstążka — wyszukiwanie | Pole wyszukiwania z filtrem zakresu środowisk: karty, moduły, zbiory Library, wyniki Research, historia rozmów | 1 | Widoczne; `Alt + D` |
| Wstążka — pasmo kart sesji | Karty otwartych sesji środowiska; sposób wyświetlania i grupowania ustala okno „Dostosuj” | 1 | Widoczne |
| Wstążka — czynności | Historia widoku, Centrum dowodzenia, odświeżenie, zrzut, schowek, kontekst i praca w tle, rozwinięcie „Więcej” | 1 | Widoczne |
| Ustawienia szybkie sesji | Model i kanał, motyw, retencja historii karty, zasięg izolacji karty, kontekst wiedzy, profil personalizacji — podzbiór zakresów okna konfiguracji | 2 | Menu „Ustawienia i konfiguracja” w stopce szyny |
| Przełącznik gęstości interfejsu | Zwarta (48 px) albo swobodna (56 px) — okno „Dostosuj”, zakładka Wygląd i zachowanie | 3 | Okno „Dostosuj” |
| Pasek stanu | Operator, położenie (środowisko · moduł), sesje czynne, motyw, obciążenie zasobów | 1 | Widoczny |

### 4.8. Magistrala kontekstu

Mechanizm magistrali kontekstu — przeznaczenie, sposób przekazania artefaktu, zasięg, warstwę widoczności i sposób wywołania oraz relacje do modułu Library, punktów izolacji i encji artefaktu — opisuje [Przepływ okien](../interfejs-uzytkownika/przeplyw-okien.md) (rozdz. 6a). Specyfika środowiska TalkIn: wyzwalaczem magistrali jest licznik `◈` w kolumnie stanu, a zasięg domyślny obejmuje wszystkie moduły środowiska, zgodnie z konwersacyjnym charakterem pracy — zasób odłożony w jednym module jest dostępny w pozostałych bez przełączania karty.
...
```
  Nowa karta ──► wybór modułu ──► układ kolumn ──► praca nad treścią
       ...
       │              │                │                   │
       │              │                ▼                   ▼
       │              │         szablon układu      oś czasu treści
       │              │            zapisany                │
       │              ▼                                    ▼
       │  zapamiętany układ modułu                referencje krzyżowe
       ▼
...
| Rozłączony | `⇄ Rozłączony` | Stan procesów sesji trwa po stronie serwera; okna odtwarzają się po wznowieniu połączenia |
...
Model konfiguracji platformy obejmuje warstwy: **globalny → środowisko → projekt → sesja**; izolacja ma osobne okno o ośmiu poziomach zasięgu, od globalnego po okno komunikacji ([Izolacja i zależności](../architektura/izolacja-i-zaleznosci.md) rozdz. 6). Brak jawnego ustawienia oznacza wartość domyślną i zachowanie kanoniczne. Wszystkie poniższe zakresy są sterowane przez Operatora bez blokad w interfejsie.
...
| Układ obszaru roboczego | Kanoniczny układ kolumn nowej karty, liczba kolumn modułowych, zapamiętane położenie paska Chat Window i paska kolumny Execution Loop Window (ustawiane przesunięciem paska, nie suwakiem) | Środowisko / projekt |
...
| Izolacja powłoki | Poziom zasięgu karty, okna komunikacji i relacji między modułami — co współdzielone, co odrębne; stan wyjściowy „pełny dostęp” | Okno punktów izolacji (osiem poziomów) |
...
Ustawienia szybkie sesji — dostępne z menu „Ustawienia i konfiguracja” w stopce szyny nawigacji — obejmują podzbiór zakresów z tabeli 7.1, sterowany bez otwierania pełnego okna konfiguracji: model i kanał, motyw, retencja historii karty, zasięg izolacji karty, aktywny kontekst wiedzy, profil personalizacji, gęstość interfejsu. Panel zawiera odnośnik do pełnego okna konfiguracji.
...
Środowisko TalkIn korzysta z komend czterech obszarów kontraktu: `environment` (2 komendy), `message` (3 komendy), `stream` (0 komend, 1 zdarzenie), `speech` (8 komend) — razem 13 komend. Pełny wykaz nazw komend, pól żądania i pól wyniku znajduje się w [Załączniku B. Pełnym wykazie komend kontraktu środowiska TalkIn](#załącznik-b-pełny-wykaz-komend-kontraktu-środowiska-talkin), na końcu niniejszego dokumentu.

| Obszar | Liczba komend | Reprezentatywne komendy | Zastosowanie w środowisku |
|---|---|---|---|
| `environment` | 2 | `environment.list`, `environment.enter` | Wejście do środowiska z przedsionka (rozdz. 2.4) |
| `message` | 3 | `message.send`, `message.stop`, `message.list` | Chat Window — kanał Użytkownik ↔ Wykonawca (rozdz. 4.2) |
| `stream` | 0 (1 zdarzenie: `stream.chunk`) | — | Strumieniowanie odpowiedzi w Chat Window i w oknach modułów redakcyjnych |
| `speech` | 8 | `speech.availability.get`, `speech.transcribe`, `speech.listen.start` | Profil Assistant — dialog głosowy (moduł Assistant, rozdz. 2.1) |

### 9.2. Żetony projektowe

| Miejsce zastosowania | Żeton `--dn-*` | Czego dotyczy |
|---|---|---|
| Kropka sygnału procesu aktywnego | `--dn-kropka`, `--dn-czas-tetno` | Wskaźnik pracy w tle przy karcie sesji w szynie sesji (rozdz. 2.4) |
| Fokus kontrolek | `--dn-fokus` | Obrys fokusu klawiaturowego pól, przycisków i pozycji list |
| Przycisk główny akcji | `--dn-sygnal`, `--dn-sygnal-wypelnienie`, `--dn-sygnal-wypelnienie-hover` | „Wyślij”, „Powtórz ostatnią sesję”, akcje pierwszoplanowe przedsionka |
| Stan powodzenia dyktowania | `--dn-sukces-tekst`, `--dn-sukces-tlo`, `--dn-sukces-obrys` | Potwierdzenie zakończonej transkrypcji w profilu Assistant |
| Stan błędu połączenia | `--dn-blad-tekst`, `--dn-blad-tlo`, `--dn-blad-obrys` | Sygnalizacja awarii silnika mowy albo braku jego składnika w instalacji (`speech.availability.get`, pole `available:false`) |
| Odstępy paneli i list | `--dn-od-2`, `--dn-od-4`, `--dn-od-6` | Odstęp wewnętrzny kart modułów, wierszy szyny sesji, pól formularza |
| Promień zaokrąglenia | `--dn-r-sm`, `--dn-r-md`, `--dn-r-xl` | Pola i przyciski (`--dn-r-sm`), karty sesji (`--dn-r-md`), karta środowiska w przedsionku (`--dn-r-xl`) |
| Typografia interfejsu | `--dn-ff-bazowa`, `--dn-fs-base`, `--dn-lh-bazowy` | Tekst kontrolek, etykiet i list w gęstości zwartej |
| Typografia treści redagowanej | `--dn-ff-bazowa`, `--dn-fs-md`, `--dn-lh-luzny` | Studio Editor — treść ciągła w odróżnieniu od gęstości interfejsu |
| Czas przejścia warstw | `--dn-czas-3` | Otwarcie/zamknięcie panelu pomocniczego, nakładki, palety poleceń |
...
| `.dn-pole-kontrolka` | Kontrolka pola (tekst, wybór, wieloliniowa) | — | puste · wypełnione · fokus · nasłuch (dyktowanie) · błąd |
...
| `.dn-awatar` | Awatar Operatora / Always On Display | `-stan` | domyślny |
| `.dn-zakladki` | Pas kart sesji | — | aktywna · w tle · z pracą w toku |
| `.dn-modal` | Okno nakładkowe | — | otwarte · zamykane |
...
Wykaz dosłownych brzmień z `design/05-okna/srodowiska/talkin.html` i `talkin-przedsionek.html`, nieopisanych już w rozdziałach 1–8.

| Element | Dosłowne brzmienie | Miejsce wystąpienia |
|---|---|---|
| Nagłówek środowiska w przedsionku | „TalkIn”, „Wiedza, komunikacja i praca z treścią” | Rozdz. 2.4 |
| Siatka modułów przedsionka | „Wiedza, komunikacja i praca z treścią — dziewięć modułów.” | Rozdz. 2.4 |
| Opis kafla Studio | „Redakcja i tworzenie treści długiej — dokumenty, raporty, opracowania.” | Rozdz. 2.1 |
| Opis kafla Library | „Repozytorium dokumentów, plików i zbiorów wiedzy z wyszukiwaniem pełnotekstowym.” | Rozdz. 2.1 |
| Opis kafla Browser | „Praca ze stronami i źródłami sieciowymi bez opuszczania stanowiska.” | Rozdz. 2.1 |
| Opis kafla Research | „Badanie zagadnienia: zbieranie źródeł, ustalenia i raport końcowy.” | Rozdz. 2.1 |
| Opis kafla Translate | „Przekład i praca dwujęzyczna nad treścią wraz z glosariuszem.” | Rozdz. 2.1 |
| Opis kafla Roundtable | „Narada wielu modeli nad jednym zagadnieniem i wypracowanie stanowiska.” | Rozdz. 2.1 |
| Opis kafla Assistant | „Dialog głosowy i tekstowy nad bieżącą treścią stanowiska.” | Rozdz. 2.1 |
| Szyna sesji przedsionka | „Twoje sesje”, filtry „Czynne” · „Wszystkie” · „Zakończone” | Rozdz. 2.4 |
| Listwa działań przedsionka | „Powtórz ostatnią sesję”, „Nowy projekt”, „Otwórz projekt istniejący”, „Utwórz z szablonu projektu” | Rozdz. 2.4 |
| Ukrycie pozycji nawigacji | „Moduł zdjęty z szybkiego wyboru pozostaje dostępny w przedsionku środowiska.” | Rozdz. 2.2 |

### 9.5. Komunikaty

| Sytuacja | Dosłowna treść albo wzorzec | Rodzaj |
|---|---|---|
| Awaria silnika mowy albo brak jego składnika w instalacji | Komunikat kontekstowy budowany z pola `reason` wyniku `speech.availability.get`, ze wskazaniem naprawy instalacji: „Silnik mowy nie odpowiada — sprawdź kompletność instalacji”. Nie jest to przewidziany tryb pracy środowiska, lecz stan awaryjny | Błąd |
| Transkrypcja zakończona | Podstawienie tekstu `transcript` w pole polecenia; licznik `characters` i `durationMs` w podpowiedzi | Potwierdzenie |
| Fraza wybudzająca wykryta | Zdarzenie `speech.wake.detected` uruchamia nasłuch; wskaźnik mikrofonu zmienia stan na „nasłuch”. Sytuacja powstaje wyłącznie przy wybudzaniu włączonym przez Operatora — domyślnie funkcja jest wyłączona | Stan |
| Wybudzanie głosem wyłączone albo bez modelu frazy | Mikrofon uruchamia się kliknięciem; komunikat wskazuje włączenie wybudzania w ustawieniach. Nie jest to awaria — model frazy nie należy do składników wymaganych instalacji, odmiennie od silnika mowy | Stan pusty |
| Częściowa transkrypcja w toku | Zdarzenie `speech.listen.partial` aktualizuje podgląd tekstu na bieżąco; tekst oznaczony jako nietrwały do chwili `speech.transcribe` | Stan pośredni |
...
| Karta oznaczona do monitorowania z Mobile | „Przekaż na Mobile” — potwierdzenie w dymku powiadomienia | Potwierdzenie |
| Brak wyników wyszukiwania w module | Stan pusty panelu z podpowiedzią zmiany filtru albo zakresu | Stan pusty |

### 9.6. Punkty łamania

Wartości progów z `design/zasoby/zetony/zetony.css`, rozdział „Siatka i punkty łamania”.

| Żeton | Próg szerokości | Zachowanie układu w środowisku TalkIn |
|---|---|---|
...
| `--dn-bp-w3` | 1280px | Pełny układ — Chat Window, obszar roboczy modułu i panel pomocniczy widoczne jednocześnie |
...
Poniżej `--dn-bp-w2` panele pomocnicze (rozdz. 3.3) przechodzą z prezentacji jako rozszerzenie boczne na prezentację w nakładce (`.dn-modal`), zgodnie z ogólną zasadą nakładek rozdz. 3.4.

### 9.7. Stany kontrolek

| Kontrolka | Spoczynek | Wskazanie kursorem | Wciśnięcie | Ognisko | Nieaktywny | Ładowanie | Pusty | Błąd |
|---|---|---|---|---|---|---|---|---|
| Pole polecenia Chat Window (`.dn-pole-kontrolka`) | Obrys `--dn-obrys` | Obrys `--dn-sygnal-obrys` | — | Obrys `--dn-fokus` | nie występuje (zero blokad) | Ikona wysłania zastąpiona wskaźnikiem strumieniowania | Tekst podpowiedzi „Napisz polecenie…” | Obrys `--dn-blad-obrys`, komunikat pod polem |
| Przycisk „Wyślij” (`.dn-btn--sygnal`) | Wypełnienie `--dn-sygnal-wypelnienie` | Wypełnienie `--dn-sygnal-wypelnienie-hover` | Przyciemnienie o jeden krok | Obrys `--dn-fokus` | nie występuje (zero blokad) | Spinner (`.dn-spinner`) w miejscu ikony | — | — |
| Mikrofon profilu Assistant | Ikona spoczynkowa | Podświetlenie subtelne | Rozpoczęcie nagrania | Obrys `--dn-fokus` | nie występuje (zero blokad) — silnik mowy jest składnikiem wymaganym instalacji, więc stan nieaktywny nie powstaje w normalnej pracy | Pierścień pulsujący (`--dn-czas-tetno`) w trakcie nasłuchu | — | Plakietka `--dn-blad-tekst` z treścią pola `reason` — awaria silnika albo brak składnika instalacji (rozdz. 9.5) |
| Karta sesji (`.dn-karta--klikalna`) | Tło `--dn-tlo` | Tło `--dn-powierzchnia`, uniesienie cienia | Przyciśnięcie | Obrys `--dn-fokus` | nie występuje | Kropka `--dn-kropka` z tętnem | — | Plakietka `--dn-blad-tlo` przy karcie z niepowodzeniem |
...
|---|---|
| Wejście do środowiska otwiera przedsionek, nie przestrzeń roboczą | Kliknięcie karty TalkIn na stronie głównej; obserwacja, że żaden moduł nie jest otwarty automatycznie |
| Wszystkie dziewięć modułów jest osiągalnych z bocznej nawigacji | Przegląd bocznej nawigacji względem wykazu rozdz. 2.1 |
...
| Tor głosowy zgłasza awarię silnika mowy jako błąd, nie jako przewidziany tryb pracy | Wywołanie `speech.availability.get` przy uszkodzonej instalacji silnika; sprawdzenie, że mikrofon pozostaje widoczny i klikalny, a komunikat wskazuje naprawę instalacji zgodnie z rozdz. 9.5 |
| Brak modelu frazy wybudzającej nie jest zgłaszany jako awaria — odmiennie od braku silnika mowy | Wywołanie `speech.wake.get` bez zainstalowanego modelu frazy; sprawdzenie, że wynik nosi `available:false` z rodzajem „stan pusty”, aplikacja działa normalnie, mikrofon uruchamia się kliknięciem, a komunikat wskazuje włączenie wybudzania w ustawieniach zamiast naprawy instalacji |
| Wybudzanie głosem jest domyślnie wyłączone | Odczyt nastawy `speech.wake.get` na świeżej instalacji; sprawdzenie, że nasłuch ciągły nie startuje samoczynnie przed włączeniem funkcji przez Operatora |
| Żaden przycisk nie jest trwale nieaktywny | Przegląd kontrolek wykazanych w rozdz. 9.3 pod kątem stanu `disabled`; zero wystąpień poza stanem „ładowanie” |
...
## Załącznik B. Pełny wykaz komend kontraktu środowiska TalkIn
...
### Obszar `message` — 3 komendy

| Komenda | Przeznaczenie | Pola żądania | Pola wyniku |
|---|---|---|---|
| `message.send` | Wysyła wiadomość do okna komunikacji | `windowId:string` (wym)<br>`content:string` (wym)<br>`attachments:string[]` (opc)<br>`stream:bool` (opc) | `message:Message` (wym) |
| `message.stop` | Zatrzymuje bieżącą odpowiedź; przycisk zatrzymania zawsze czynny | `windowId:string` (wym)<br>`messageId:string` (opc) | `messageId:string` (wym)<br>`stopped:bool` (wym) |
| `message.list` | Zwraca wiadomości okna komunikacji | `windowId:string` (wym)<br>`limit:int` (opc)<br>`before:string` (opc) | `messages:Message[]` (wym)<br>`hasMore:bool` (wym) |

**Zdarzenia obszaru `message` — 1:**

| Zdarzenie | Przeznaczenie | Pola |
|---|---|---|
| `message.changed` | Zmiana wiadomości | — |

### Obszar `stream` — 0 komend, 1 zdarzenie

Obszar `stream` nie niesie komend: kontrakt opisuje w nim wyłącznie zdarzenie strumienia
odpowiedzi, którym rdzeń dosyła kolejne fragmenty wypowiedzi do okna komunikacji
(rozdz. 4.2) i do okien modułów redakcyjnych.

**Zdarzenia obszaru `stream` — 1:**

| Zdarzenie | Przeznaczenie | Pola |
|---|---|---|
| `stream.chunk` | Fragment strumienia odpowiedzi; numer fragmentu i znacznik końca stoją w kopercie (`seq`, `done`) | `windowId:string` (wym)<br>`messageId:string` (wym)<br>`kind:ChunkKind` (wym)<br>`text:string` (opc)<br>`data:json` (opc) |

### Obszar `speech` — 8 komend

| Komenda | Przeznaczenie | Pola żądania | Pola wyniku |
|---|---|---|---|
| `speech.availability.get` | Sprawdzenie gotowości silnika mowy. Silnik jest składnikiem wymaganym instalacji, więc odpowiedź `available:false` oznacza awarię albo brak składnika, nie przewidziany tryb pracy — klient pozostawia mikrofon widoczny i klikalny, z komunikatem wskazującym naprawę instalacji | — | `available:bool` (wym)<br>`python:string` (opc)<br>`engine:string` (opc)<br>`model:string` (opc)<br>`reason:string` (opc)<br>`wakeWordAvailable:bool` (opc)<br>`listenAvailable:bool` (opc)<br>`uploadAvailable:bool` (opc)<br>`synthesisAvailable:bool` (opc)<br>`synthesisReason:string` (opc) |
| `speech.transcribe` | Zamienia nagranie na tekst silnikiem lokalnym. Dźwięk nie opuszcza maszyny, na której stoi silnik | `audioRef:string` (wym)<br>`language:string` (opc)<br>`model:string` (opc) | `processed:bool` (wym)<br>`transcript:string` (wym)<br>`characters:int` (wym)<br>`durationMs:int` (wym)<br>`model:string` (wym)<br>`language:string` (opc)<br>`confidence:int` (opc) |
| `speech.audio.upload` | Przyjmuje nagranie w bajtach i oddaje odnośnik, którym posługują się `speech.transcribe` i `assistant.voice.command`. Jest to brak platformowy, nie brak jednego okna: `speech.transcribe` bierze `audioRef`, czyli ścieżkę pliku na maszynie silnika, a nagranie z mikrofonu istnieje wyłącznie jako bajty w pamięci karty. Ten sam brak wstrzymuje dyktowanie w oknie komunikacji (`budowa/client/src/okno-komunikacji/dyktowanie/dostarczenie-nagrania.ts`) i mikrofon Voice Console — jedna komenda otwiera obie drogi naraz. Dźwięk nie opuszcza maszyny rdzenia: bajty idą do magazynu nagrań, nie do sieci | `audio:string` (wym)<br>`contentType:string` (wym)<br>`sessionId:string` (opc)<br>`windowId:string` (opc)<br>`retain:bool` (opc) | `audioRef:string` (wym)<br>`sizeBytes:int` (wym)<br>`durationMs:int` (opc)<br>`expiresAt:int64` (opc) |
| `speech.audio.fetch` | Oddaje bajty nagrania albo odpowiedzi syntezowanej wskazanej odnośnikiem. Domyka parę do `speech.audio.upload` i do `translate.speech.synthesize`: dziś rdzeń oddaje odnośniki (`AssistantActivityEntry.audioRef`, `AssistantVoiceCommandResponse.speechRef`), lecz nie ma komendy, która by po nie sięgnęła — odsłuchu nie ma czym wykonać | `audioRef:string` (wym)<br>`rangeStartMs:int` (opc)<br>`rangeEndMs:int` (opc) | `audio:string` (wym)<br>`contentType:string` (wym)<br>`sizeBytes:int` (wym)<br>`durationMs:int` (opc) |
| `speech.wake.get` | Zwraca nastawę frazy wybudzającej i trybu nasłuchu wraz z uczciwym stanem wykonalności. Wybudzanie głosem jest domyślnie wyłączone i włącza je Operator w ustawieniach. Brak modelu frazy jest odpowiedzią, nie awarią — model frazy jest bytem odrębnym od silnika mowy i **nie** należy do składników wymaganych instalacji; przy jego braku aplikacja działa normalnie, a mikrofon uruchamia się kliknięciem. Odwrotnie niż przy `speech.availability.get`, gdzie `available:false` oznacza awarię — obu miejsc nie wolno wyrównywać do siebie | `scope:ConfigScope` (opc)<br>`scopeId:string` (opc) | `config:WakeWordConfig` (wym)<br>`available:bool` (wym)<br>`reason:string` (opc) |
| `speech.wake.set` | Zapisuje frazę wybudzającą, tryb nasłuchu i próg wykrycia mowy oraz włącza wybudzanie głosem, które domyślnie jest wyłączone. Pola pominięte zostają bez zmian | `scope:ConfigScope` (opc)<br>`scopeId:string` (opc)<br>`phrase:string` (opc)<br>`mode:ListenMode` (opc)<br>`vadThreshold:int` (opc)<br>`noiseSuppression:bool` (opc) | `config:WakeWordConfig` (wym) |
| `speech.listen.start` | Uruchamia nasłuch ciągły rdzenia na wskazanym oknie. Rdzeń słucha strumienia i ogłasza zdarzeniami: częściową transkrypcję oraz wykrycie frazy wybudzającej. Nasłuch nie jest bramką — Operator zatrzymuje go komendą `speech.listen.stop`, a rdzeń nie zatrzymuje go sam | `windowId:string` (wym)<br>`sessionId:string` (opc)<br>`mode:ListenMode` (opc)<br>`language:string` (opc) | `listening:bool` (wym)<br>`listenerId:string` (wym)<br>`reason:string` (opc) |
| `speech.listen.stop` | Zatrzymuje nasłuch ciągły. Zatrzymanie nasłuchu, którego nie ma, nie jest błędem | `listenerId:string` (opc)<br>`windowId:string` (opc) | `stopped:bool` (wym) |

**Zdarzenia obszaru `speech` — 2:**

| Zdarzenie | Przeznaczenie | Pola |
|---|---|---|
| `speech.wake.detected` | Rdzeń usłyszał frazę wybudzającą w nasłuchu ciągłym | — |
| `speech.listen.partial` | Częściowa transkrypcja z nasłuchu ciągłego. Tekst jest nietrwały i bywa poprawiany kolejnym zdarzeniem — trwały zapis daje dopiero `speech.transcribe` | — |


Razem w wykazie: **13 komend** z czterech obszarów kontraktu oraz cztery zdarzenia zwrotne.

---

*Koniec dokumentu. Środowisko TalkIn — Specyfikacja docelowa, wersja 2.0, 2026-08-20.*
